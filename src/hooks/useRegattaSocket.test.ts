...
describe('useRegattaSocket', () => {
    ...
    it('should initialize and subscribe to channel', () => {
        const { result } = renderHook(() => useRegattaSocket('match-123', 'user-456', 'Tester'));

        expect(result.current.isConnected).toBe(true);
        expect(createClient().channel).toHaveBeenCalledWith('regatta:match-123', expect.any(Object));
    });
    ...
    it('should cleanup on unmount', () => {
        const { unmount } = renderHook(() => useRegattaSocket('match-123', 'user-456', 'Tester'));
        unmount();
