...
export const useSmartNotifications = () => {
	...
	const scheduleStreakReminder = useCallback(
		async (userId: string) => {
			...
			try {
				...
				const hours = attempts.map((a: any) =>
				...
				const modeHour =
					hours
						.sort(
							(a: number, b: number) =>
								hours.filter((v: number) => v === a).length -
								hours.filter((v: number) => v === b).length,
						)
	...
	// --- Exam Alert ---
	const scheduleExamReminders = useCallback(
		async (userId: string) => {
			if (!Capacitor.isNativePlatform()) return;
			...
			try {
				...
				if (attendance) {
					for (const record of attendance) {
						...
						const isExam = session.observaciones
							?.toLowerCase()
							.includes("examen");
						...
						if (isExam) {
							...
							const reminderDate = new Date(
								examDate.getTime() - 24 * 60 * 60 * 1000,
							); // 24h before
							...
							if (reminderDate > new Date()) {
								...
								const notifId =
									(Math.abs(hashCode(record.sesion_id)) % 100000) + 2000;
								...
								console.log(
									`Exam reminder scheduled for ${reminderDate.toISOString()}`,
								);
							}
						}
					}
	...
	const listenToModuleCompletion = useCallback(
		(userId: string) => {
