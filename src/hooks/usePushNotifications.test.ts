...
describe('usePushNotifications', () => {
    ...
    it('should check permissions on native platform', async () => {
        ...
        vi.mocked(PushNotifications.checkPermissions).mockResolvedValue({ receive: 'granted' } as any);
    ...
    it('should handle error when checking permissions', async () => {
        ...
        expect(consoleSpy).toHaveBeenCalledWith('Error checking push permissions:', mockError);
        consoleSpy.mockRestore();
    });

    it('should request permissions and register if granted', async () => {
        vi.mocked(Capacitor.isNativePlatform).mockReturnValue(true);
        vi.mocked(PushNotifications.checkPermissions).mockResolvedValue({ receive: 'prompt' } as any);
    ...
    it('should handle error when requesting permissions', async () => {
        ...
        vi.mocked(PushNotifications.requestPermissions).mockRejectedValue(mockError);
        ...
        expect(consoleSpy).toHaveBeenCalledWith('Error requesting push permissions:', mockError);
        consoleSpy.mockRestore();
    });
    ...
    it('should handle registration success and save token to Supabase', async () => {
        ...
        vi.mocked(PushNotifications.addListener).mockImplementation(((event: string, handler: any) => {
    ...
    it('should handle Supabase upsert error', async () => {
        ...
        expect(consoleSpy).toHaveBeenCalledWith('Error saving FCM token:', mockError);
        consoleSpy.mockRestore();
    });
    ...
    it('should handle registration error event', async () => {
        ...
        expect(result.current.error).toBe(mockError);
        expect(consoleSpy).toHaveBeenCalledWith('Push Registration Error:', mockError);
        consoleSpy.mockRestore();
    });
});
