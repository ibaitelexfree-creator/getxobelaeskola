...
describe('useAcademyData', () => {
    ...
    it('should fetch all data in parallel and map correctly', async () => {
        ...
        const mockProgress = { progreso: [{ tipo_entidad: 'nivel', nivel_id: 'L1', estado: 'en_progreso' }], is_staff: false };
        const mockEnrollments = { enrollments: ['C1'] };
        const mockUnlock = { niveles: { 'L1': 'available' } };

        (global.fetch as any).mockImplementation((url: string) => {
            if (url.includes('/api/levels')) return Promise.resolve({ ok: true, json: async () => mockLevels });
            if (url.includes('/api/courses')) return Promise.resolve({ ok: true, json: async () => mockCourses });
            if (url.includes('/api/progress')) return Promise.resolve({ ok: true, json: async () => mockProgress });
            if (url.includes('/api/enrollments')) return Promise.resolve({ ok: true, json: async () => mockEnrollments });
            if (url.includes('/api/unlock-status')) return Promise.resolve({ ok: true, json: async () => mockUnlock });
    ...
    it('should handle critical fetch error (levels)', async () => {
        (global.fetch as any).mockImplementation((url: string) => {
            if (url.includes('/api/levels')) return Promise.resolve({ ok: false, status: 500, json: async () => ({ error: 'Fatal' }) });
    ...
    describe('getEstadoNivel', () => {
        it('should return correct state based on unlockStatus', async () => {
            ...
            (global.fetch as any).mockImplementation((url: string) => {
                if (url.includes('/api/levels')) return Promise.resolve({ ok: true, json: async () => ({ niveles: [{ id: 'L1' }] }) });
                if (url.includes('/api/unlock-status')) return Promise.resolve({ ok: true, json: async () => ({ niveles: { 'L1': 'available' } }) });
                return Promise.resolve({ ok: true, json: async () => ({}) });
            });
        ...
        it('should return locked if not enrolled in any course of the level', async () => {
            (global.fetch as any).mockImplementation((url: string) => {
                if (url.includes('/api/levels')) return Promise.resolve({ ok: true, json: async () => ({ niveles: [{ id: 'L1' }] }) });
                if (url.includes('/api/courses')) return Promise.resolve({ ok: true, json: async () => ({ cursos: [{ id: 'C1', nivel_formacion_id: 'L1' }] }) });
                if (url.includes('/api/enrollments')) return Promise.resolve({ ok: true, json: async () => ({ enrollments: [] }) });
                return Promise.resolve({ ok: true, json: async () => ({}) });
            });
