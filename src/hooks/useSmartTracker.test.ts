...
describe("useSmartTracker", () => {
	...
	it("should handle speed exceeding car threshold", async () => {
		...
		(Geolocation.watchPosition as any).mockImplementation(
		...
		expect(result.current.statusMessage).toBe(
			"Detección de Vehículo (Coche) - Abortando",
		);
	});
	...
	it("should detect geofence return to school", async () => {
		...
		expect(result.current.statusMessage).toBe(
			"Regreso a Escuela - Travesía Completada",
		);
	...
	it("should add points every 5 seconds", async () => {
		...
		act(() => {
			watchCallback(
				{ coords: { latitude: 0, longitude: 0, speed: 1 }, timestamp: now },
				null,
			);
		});
		...
		act(() => {
			watchCallback(
				{
					coords: { latitude: 0.001, longitude: 0.001, speed: 1 },
					timestamp: now + 2000,
				},
				null,
			);
		...
		act(() => {
			watchCallback(
				{
					coords: { latitude: 0.002, longitude: 0.002, speed: 1 },
					timestamp: now + 6000,
				},
				null,
			);
	...
	it("should handle error when watchPosition fails to start", async () => {
		(Geolocation.watchPosition as any).mockRejectedValue(
			new Error("Failed to start watch"),
		);
