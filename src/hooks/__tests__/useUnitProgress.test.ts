...
describe('useUnitProgress hook', () => {
    ...
    it('should calculate puedeCompletar correctly based on sections and time', async () => {
        ...
        expect(result.current.mensajeRequisito).toBe('Debes leer todas las secciones antes de completar.');
        ...
        expect(result.current.puedeCompletar).toBe(false);
        expect(result.current.mensajeRequisito).toBe('Debes dedicar al menos 5 minutos a la lectura.');
    ...
    it('should log error if fetch throws exception', async () => {
        ...
        expect(errorSpy).toHaveBeenCalledWith('Error registering read:', expect.any(Error));
