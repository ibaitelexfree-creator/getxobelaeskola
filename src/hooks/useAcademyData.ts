...
export function useAcademyData() {
		...
		async function fetchData() {
			...
			try {
				...
				if (!resNiveles.ok) {
					...
					console.error(
						"CRITICAL ERROR: Failed to fetch levels",
						resNiveles.status,
						errData,
					);
					throw new Error(
						errData.error || `Failed to fetch levels: ${resNiveles.status}`,
					);
				}
	...
	const getEstadoNivel = useCallback(
		(
			nivel: Nivel,
			...
			if (unlockStatus?.niveles?.[nivel.id]) {
				...
				if (status === "completado" || status === "distincion")
					return "completado";
				if (status === "en_progreso") return "en_progreso";
				if (status === "no_iniciado" || status === "available")
					return "disponible";
