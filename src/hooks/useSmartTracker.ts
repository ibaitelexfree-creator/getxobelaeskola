...
export function useSmartTracker() {
	...
	useEffect(() => {
		currentSpeedThresholdRef.current =
			windSpeed > 18 ? HIGH_WIND_CAR_THRESHOLD : BASE_CAR_SPEED_THRESHOLD;
	}, [windSpeed]);
	 ...
	 */
	const checkVehicleSpeed = useCallback(
		(speed: number | null) => {
	...
	const checkGeofence = useCallback(
		(latitude: number, longitude: number) => {
			const distToSchool = getDistance(
				latitude,
				longitude,
				SCHOOL_COORDS.lat,
				SCHOOL_COORDS.lng,
	 ...
	 */
	const handleTrackingPoint = useCallback(
		(point: LocationPoint, isAuto: boolean) => {
