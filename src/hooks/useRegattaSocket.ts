...
import { OpponentState, RegattaMatch } from '@/types/regatta';
...
export function useRegattaSocket(matchId: string | null, userId: string | null, username: string | null): UseRegattaSocketResult {
    const supabase = createClient();
    const [opponents, setOpponents] = useState<OpponentState[]>([]);
    const [matchStatus, setMatchStatus] = useState<RegattaMatch['status']>('waiting');
    ...
    useEffect(() => {
        ...
        channel
            .on('broadcast', { event: 'pos' }, ({ payload }: { payload: OpponentState }) => {
