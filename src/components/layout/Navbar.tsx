			...
			try {
				...
				if (authUser) {
					const res = await fetch(
						apiUrl(`/api/profile?user_id=${authUser.id}`),
					);
					if (res.ok) {
	...
	const navItems: NavItem[] = [
		{
			...
			dropdown: [
				{
					href: "courses?cat=infantiles",
					label: "kids",
					icon: <School className="w-4 h-4" />,
				},
				{
					href: "courses?cat=adultos",
					label: "adults",
					icon: <Users className="w-4 h-4" />,
				},
				{
					href: "courses?cat=windsurf",
					label: "windsurf_courses",
		...
		},
		{
			...
			dropdown: [
				{
					href: "rental?category=veleros",
					label: "sailboats",
					icon: <Sailboat className="w-4 h-4" />,
				},
				{
					href: "rental?category=windsurf",
					label: "windsurf",
					icon: <Wind className="w-4 h-4" />,
				},
				{
					href: "rental?category=kayak",
					label: "kayak_paddle",
		...
		<>
			<nav className="fixed top-0 left-0 w-full z-[9999] px-4 md:px-12 py-4 md:py-8 flex justify-between items-center bg-nautical-deep/70 backdrop-blur-2xl border-b border-white/5 transition-all duration-500 hover:bg-nautical-deep/90 min-h-[70px]">
				...
				>
					...
					<div className="flex flex-col">
						...
						<span className="text-[9px] md:text-[10px] uppercase tracking-[0.5em] text-white/40 font-bold mt-1.5 transition-premium group-hover:text-white/70">
							Escuela Náutica
						</span>
					</div>
				</Link>
				...
				<div className="hidden xl:flex gap-8 items-center text-[10px] uppercase tracking-[0.4em] font-black h-full">
					{navItems.map((item) => (
						<div
							key={item.label}
							className="relative group/main h-full flex items-center"
						>
							<Link
								...
								<div className="absolute top-full left-1/2 -translate-x-1/2 pt-4 opacity-0 pointer-events-none translate-y-2 group-hover/main:opacity-100 group-hover/main:pointer-events-auto group-hover/main:translate-y-0 transition-all duration-300 z-[10000]">
									<div className="w-56 py-3 bg-nautical-deep/95 backdrop-blur-2xl border border-white/10 rounded-xl shadow-2xl shadow-black/40 overflow-hidden">
										...
										{item.dropdown.map((sub) => (
											...
											>
												<span className="text-accent/60 group-hover/sub:text-accent transition-colors">
													{sub.icon}
												</span>
												{t(sub.label)}
											</Link>
				...
				<div className="flex gap-4 items-center relative z-[110]">
					...
					{loading ? (
						...
						<Link
							...
							prefetch={false}
							className="hidden xl:block text-[10px] uppercase tracking-[0.4em] font-black border border-white/20 px-8 py-3 rounded-full bg-white/5 hover:bg-white hover:text-nautical-black transition-premium"
						>
							{t("login")}
						</Link>
					)}
			...
			>
				...
				<div className="flex flex-col h-full pt-32 pb-12 px-8 overflow-y-auto">
							...
							<div key={item.label}>
								<div className="flex items-center justify-between">
									...
									>
										...
										<span
											className={`text-3xl font-display italic transition-all duration-500 ${isMenuOpen ? "translate-x-0 opacity-100" : "-translate-x-10 opacity-0"}`}
										>
											{t(item.label)}
										</span>
									</Link>
									...
									{item.dropdown && (
										...
										>
											<ChevronDown
												className={`w-5 h-5 transition-transform duration-300 ${mobileExpanded === item.label ? "rotate-180 text-accent" : ""}`}
											/>
										</button>
									)}
								</div>
					...
					<div
						className={`space-y-10 transition-all duration-700 delay-500 ${isMenuOpen ? "translate-y-0 opacity-100" : "translate-y-10 opacity-0"}`}
					>
						{user ? (
						...
						<div className="flex flex-col gap-4 pb-12">
							<span className="text-[10px] uppercase tracking-[0.4em] font-black text-white/30 ml-4">
								{t("language_selector")}
							</span>
