	...
	const onSubmit = async (data: LoginValues) => {
		...
		if (authError) {
			...
			if (
				...
				setError(
					"Error de conexión con el servidor. Por favor, comprueba tu conexión a internet o inténtalo más tarde.",
				);
				setIsCredsError(false);
			} else {
	...
	return (
		...
		>
			<div className="space-y-1.5">
				<label
					htmlFor="email"
					className="text-[10px] uppercase tracking-[0.2em] text-accent font-bold ml-1"
				>
					{t("email")}
				</label>
				<input
			...
			<div className="relative my-8">
				...
				<div className="relative flex justify-center text-[10px] uppercase tracking-widest">
					<span className="bg-nautical-black px-4 text-white/50 font-bold italic">
						O BIEN
					</span>
				</div>
			</div>
