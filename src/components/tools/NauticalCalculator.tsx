...
export default function NauticalCalculator() {
	...
	const [activeTab, setActiveTab] = useState<"coordinates" | "route">(
		"coordinates",
	);

	// Coordinates State
	const [dmsLat, setDmsLat] = useState<DMS>({
		degrees: 0,
		minutes: 0,
		seconds: 0,
		direction: "N",
	});
	const [dmsLon, setDmsLon] = useState<DMS>({
		degrees: 0,
		minutes: 0,
		seconds: 0,
		direction: "E",
	...
	const handleConvertToDms = () => {
		...
		if (!Number.isNaN(lat)) setDmsLat(decimalToDms(lat, true));
		if (!Number.isNaN(lon)) setDmsLon(decimalToDms(lon, false));
	};
	...
	return (
		<div className="w-full max-w-4xl mx-auto p-6 bg-white rounded-xl shadow-lg">
			<h1 className="text-3xl font-bold text-[#0a1628] mb-8 text-center">
				{t("title")}
			</h1>
							...
							<div className="space-y-2">
								...
								<div className="flex gap-2">
									<input
										...
										onChange={(e) =>
											setDmsLat({ ...dmsLat, degrees: Number(e.target.value) })
										}
									...
									<input
										...
										onChange={(e) =>
											setDmsLat({ ...dmsLat, minutes: Number(e.target.value) })
										}
										className="w-full p-2 border rounded text-[#0a1628]"
									...
									<input
										...
										onChange={(e) =>
											setDmsLat({ ...dmsLat, seconds: Number(e.target.value) })
										}
										className="w-full p-2 border rounded text-[#0a1628]"
							...
							<div className="space-y-2">
								...
								<div className="flex gap-2">
									<input
										...
										onChange={(e) =>
											setDmsLon({ ...dmsLon, degrees: Number(e.target.value) })
										}
										className="w-full p-2 border rounded text-[#0a1628]"
										placeholder={t("coordinates.deg")}
									/>
									<input
										...
										onChange={(e) =>
											setDmsLon({ ...dmsLon, minutes: Number(e.target.value) })
										}
										className="w-full p-2 border rounded text-[#0a1628]"
										placeholder={t("coordinates.min")}
									/>
									<input
										...
										onChange={(e) =>
											setDmsLon({ ...dmsLon, seconds: Number(e.target.value) })
										}
										className="w-full p-2 border rounded text-[#0a1628]"
										placeholder={t("coordinates.sec")}
									/>
				...
				<div className="space-y-8">
					<div className="grid md:grid-cols-2 gap-8">
									...
									<div>
										...
										<div className="flex gap-2">
											<input
												...
												value={declination}
												onChange={(e) =>
													setDeclination(parseFloat(e.target.value))
												}
												className="w-full p-2 border rounded text-[#0a1628]"
											/>
						...
						<div className="bg-[#0a1628] text-white p-8 rounded-xl flex flex-col justify-center">
							<h3 className="text-2xl font-light mb-6 border-b border-gray-700 pb-2">
								{t("route.results")}
							</h3>
							...
							{results ? (
								<div className="space-y-6">
									<div>
										...
										<p className="text-4xl font-bold text-[#ca8a04]">
											{results.distance}{" "}
											<span className="text-lg text-white">NM</span>
										</p>
									</div>
									...
									<div className="grid grid-cols-2 gap-6">
										...
										<div>
											<p className="text-sm text-gray-400 uppercase tracking-wider">
												{t("route.magnetic_bearing")}
											</p>
											<p className="text-3xl font-mono">
												{results.magneticBearing}°
											</p>
										</div>
									</div>
										...
										<div className="flex gap-4 items-baseline">
											{results.eta.days > 0 && (
												<p className="text-2xl">
													<span className="font-bold">{results.eta.days}</span>
													{t("route.days")}
												</p>
											)}
											<p className="text-2xl">
												<span className="font-bold">{results.eta.hours}</span>
												{t("route.hours")}
											</p>
											<p className="text-xl text-gray-300">
												<span className="font-bold">{results.eta.minutes}</span>
												{t("route.minutes")}
											</p>
										</div>
									</div>
