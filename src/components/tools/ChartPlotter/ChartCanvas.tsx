...
export default function ChartCanvas({ width = 800, height = 600 }: ChartCanvasProps) {
    const canvasRef = useRef<HTMLCanvasElement>(null);
    const containerRef = useRef<HTMLDivElement>(null);
    const [viewport, setViewport] = useState<Viewport>({ x: 0, y: 0, scale: 1 });
    const [isDragging, setIsDragging] = useState(false);
    const [lastMouse, setLastMouse] = useState<Point>({ x: 0, y: 0 });
    const [activeTool, setActiveTool] = useState<'pan' | 'ruler' | 'dividers'>('pan');

    // Tool State: Parallel Ruler
    const [rulerState, setRulerState] = useState({ x: 400, y: 300, angle: 0, active: false });

    // Tool State: Dividers
    const [dividersState, setDividersState] = useState({ p1: { x: 300, y: 300 }, p2: { x: 500, y: 300 }, active: false });
    ...
    const handleWheel = (e: React.WheelEvent) => {
        ...
        const newScale = delta > 0 ? viewport.scale * zoomFactor : viewport.scale / zoomFactor;
    ...
    useEffect(() => {
        ...
        ctx.beginPath();
        ctx.moveTo(0, 0); ctx.lineTo(500, 0); ctx.lineTo(600, 300); ctx.lineTo(400, 600); ctx.lineTo(0, 800);
        ctx.fill();
        ctx.stroke();
    ...
    return (
        <div className="relative border border-slate-300 rounded overflow-hidden bg-slate-100 block w-full h-[600px]" ref={containerRef}>
            ...
            <div className="absolute bottom-4 left-4 z-10 bg-white/90 px-3 py-1 rounded text-2xs font-mono text-slate-600">
                Scale: {viewport.scale.toFixed(2)}x | Pos: {Math.round(viewport.x)},{Math.round(viewport.y)}
            </div>
            ...
            <svg className="absolute inset-0 pointer-events-none w-full h-full">
                {activeTool === 'ruler' && (
                    <ParallelRuler
                        ...
                        onUpdate={(newState) => setRulerState({ ...rulerState, ...newState })}
                    />
                )}
                {activeTool === 'dividers' && (
                    <DividersTool
