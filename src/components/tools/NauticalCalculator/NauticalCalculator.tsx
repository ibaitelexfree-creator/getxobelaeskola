...
export default function NauticalCalculator() {
  const [activeTab, setActiveTab] = useState<'converter' | 'route' | 'time' | 'compass'>('route');

  // --- Converter State ---
  const [dmsInput, setDmsInput] = useState<DMS>({ degrees: 0, minutes: 0, seconds: 0, direction: 'N' });
  const [decimalInput, setDecimalInput] = useState<number>(0);
  const [converterMode, setConverterMode] = useState<'dms-to-dec' | 'dec-to-dms'>('dms-to-dec');
  const [converterResult, setConverterResult] = useState<string | null>(null);

  // --- Route State ---
  const [startPoint, setStartPoint] = useState<Coordinate>({ lat: 43.35, lon: -3.05 }); // Bilbao aprox
  const [endPoint, setEndPoint] = useState<Coordinate>({ lat: 43.40, lon: -2.95 });
  const [routeResult, setRouteResult] = useState<{ distance: number; bearing: number } | null>(null);
  ...
  const [compassMode, setCompassMode] = useState<'true-to-mag' | 'mag-to-true'>('true-to-mag');
  ...
  useEffect(() => {
    ...
    if (converterMode === 'dms-to-dec') {
      ...
      setConverterResult(`${dms.degrees}° ${dms.minutes}' ${dms.seconds}" ${decimalInput >= 0 ? '+' : '-'}`);
    }
      ...
      <div className="p-6 min-h-[400px]">
          ...
          <div className="space-y-8 animate-fade-in">
            <div className="grid md:grid-cols-2 gap-8">
              ...
              <div className="space-y-4 p-6 bg-white/5 rounded-lg border border-white/5">
                ...
                <div className="space-y-4">
                  <div>
                    <label className="text-xs uppercase text-white/50 block mb-1">Latitud (Decimal)</label>
                    <input
                      type="number"
                  ...
                  <div>
                    <label className="text-xs uppercase text-white/50 block mb-1">Longitud (Decimal)</label>
                    <input
                      type="number"
                      value={startPoint.lon}
                      onChange={(e) => setStartPoint({...startPoint, lon: parseFloat(e.target.value) || 0})}
                      className="w-full bg-black/20 border border-white/10 rounded px-3 py-2 text-sea-foam focus:border-accent outline-none transition-colors"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      type="number"
                      value={endPoint.lon}
                      onChange={(e) => setEndPoint({...endPoint, lon: parseFloat(e.target.value) || 0})}
                      className="w-full bg-black/20 border border-white/10 rounded px-3 py-2 text-sea-foam focus:border-accent outline-none transition-colors"
                    />
                  </div>
                </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-accent/10 border border-accent/20 p-6 rounded-lg text-center">
                <p className="text-xs uppercase tracking-widest text-accent mb-2">Distancia</p>
                <p className="text-4xl font-display font-bold text-white">
                  {routeResult?.distance} <span className="text-lg text-white/40 font-sans">NM</span>
                </p>
              </div>
              <div className="bg-white/5 border border-white/10 p-6 rounded-lg text-center">
                <p className="text-xs uppercase tracking-widest text-sea-foam mb-2">Rumbo Inicial</p>
                <p className="text-4xl font-display font-bold text-white">
                  {routeResult?.bearing}°
                </p>
              </div>
        ...
        {activeTab === 'time' && (
          <div className="space-y-8 animate-fade-in max-w-2xl mx-auto">
             <div className="grid md:grid-cols-2 gap-8">
              <div className="space-y-4">
                <label className="text-xs uppercase text-white/50 block mb-1">Distancia (Millas)</label>
                <input
                  type="number"
                  value={timeDistance}
              ...
              <div className="space-y-4">
                <label className="text-xs uppercase text-white/50 block mb-1">Velocidad (Nudos)</label>
                <input
                  type="number"
                  value={timeSpeed}
            ...
            <div className="bg-white/5 border border-white/10 p-8 rounded-lg text-center mt-8">
              <p className="text-xs uppercase tracking-widest text-sea-foam mb-2">Tiempo Estimado</p>
          ...
          <div className="space-y-8 animate-fade-in max-w-2xl mx-auto">
            ...
            <div className="grid md:grid-cols-2 gap-8">
              <div className="space-y-4">
                <label className="text-xs uppercase text-white/50 block mb-1">
                  {compassMode === 'true-to-mag' ? 'Rumbo Verdadero (Rv)' : 'Rumbo Magnético (Rm)'}
                </label>
                <input
                  type="number"
                  value={compassHeading}
                  onChange={(e) => setCompassHeading(parseFloat(e.target.value) || 0)}
                  className="w-full bg-black/20 border border-white/10 rounded p-4 text-xl text-sea-foam focus:border-accent outline-none transition-colors"
                />
              </div>
              <div className="space-y-4">
                ...
                <input
                  ...
                  onChange={(e) => setCompassVariation(parseFloat(e.target.value) || 0)}
                  className="w-full bg-black/20 border border-white/10 rounded p-4 text-xl text-sea-foam focus:border-accent outline-none transition-colors"
                />
              </div>
            </div>
            ...
            <div className="bg-white/5 border border-white/10 p-8 rounded-lg text-center mt-8 relative overflow-hidden">
              ...
              <p className="text-xs uppercase tracking-widest text-sea-foam mb-2">
                {compassMode === 'true-to-mag' ? 'Rumbo Magnético (Rm)' : 'Rumbo Verdadero (Rv)'}
              </p>
        ...
        {activeTab === 'converter' && (
          <div className="space-y-8 animate-fade-in max-w-2xl mx-auto">
            ...
            {converterMode === 'dms-to-dec' ? (
              <div className="grid grid-cols-4 gap-4">
                <div className="col-span-1">
                   <label className="text-xs uppercase text-white/50 block mb-1">Grados</label>
                   <input
                      type="number"
                      value={dmsInput.degrees}
                      onChange={(e) => handleDmsChange('degrees', parseFloat(e.target.value) || 0)}
                      className="w-full bg-black/20 border border-white/10 rounded p-2 text-center"
                    />
                </div>
                <div className="col-span-1">
                   <label className="text-xs uppercase text-white/50 block mb-1">Minutos</label>
                   <input
                      type="number"
                      value={dmsInput.minutes}
                      onChange={(e) => handleDmsChange('minutes', parseFloat(e.target.value) || 0)}
                      className="w-full bg-black/20 border border-white/10 rounded p-2 text-center"
                    />
                </div>
                <div className="col-span-1">
                   <label className="text-xs uppercase text-white/50 block mb-1">Segundos</label>
                   <input
                      type="number"
                ...
                <div className="col-span-1">
                   <label className="text-xs uppercase text-white/50 block mb-1">Dir</label>
                   <select
                      value={dmsInput.direction}
                      onChange={(e) => handleDmsChange('direction', e.target.value)}
              ...
              <div>
                <label className="text-xs uppercase text-white/50 block mb-1">Coordenada Decimal</label>
                <input
                  type="number"
                  value={decimalInput}
                  onChange={(e) => setDecimalInput(parseFloat(e.target.value) || 0)}
                  className="w-full bg-black/20 border border-white/10 rounded p-4 text-xl text-sea-foam focus:border-accent outline-none transition-colors"
                />
              </div>
            )}
            ...
            <div className="bg-white/5 border border-white/10 p-6 rounded-lg text-center mt-6">
              <p className="text-xs uppercase tracking-widest text-sea-foam mb-2">Resultado</p>
