							...
							<div className="w-1/3 min-w-[120px]">
								...
								{activeTab === "distance" && (
									<select
										value={distInput}
										onChange={(e) =>
											setDistInput(e.target.value as DistanceUnit)
										}
										className="w-full h-full bg-nautical-black border border-white/10 rounded-xl px-3 text-sm text-white focus:outline-none focus:border-accent/50"
									>
								...
								{activeTab === "temperature" && (
									<select
										value={tempInput}
										onChange={(e) =>
											setTempInput(e.target.value as TemperatureUnit)
										}
										className="w-full h-full bg-nautical-black border border-white/10 rounded-xl px-3 text-sm text-white focus:outline-none focus:border-accent/50"
									>
								...
								{activeTab === "pressure" && (
									<select
										value={pressInput}
										onChange={(e) =>
											setPressInput(e.target.value as PressureUnit)
										}
										className="w-full h-full bg-nautical-black border border-white/10 rounded-xl px-3 text-sm text-white focus:outline-none focus:border-accent/50"
									>
					...
					<div className="space-y-4">
						...
						<div className="flex gap-4">
							<div className="flex-1 bg-nautical-black/50 border border-white/10 rounded-xl px-4 py-3 flex items-center">
								<span
									className={`text-2xl font-mono ${result !== null ? "text-accent" : "text-white/10"}`}
								>
									{result !== null
							...
							<div className="w-1/3 min-w-[120px]">
								{activeTab === "speed" && (
									<select
										value={speedOutput}
										onChange={(e) =>
											setSpeedOutput(e.target.value as SpeedUnit)
										}
										className="w-full h-full bg-nautical-black border border-white/10 rounded-xl px-3 text-sm text-white focus:outline-none focus:border-accent/50"
									>
										<option value="knots">{t("knots")}</option>
								...
								{activeTab === "distance" && (
									<select
										value={distOutput}
										onChange={(e) =>
											setDistOutput(e.target.value as DistanceUnit)
										}
										className="w-full h-full bg-nautical-black border border-white/10 rounded-xl px-3 text-sm text-white focus:outline-none focus:border-accent/50"
									>
										<option value="nautical_miles">
								...
								{activeTab === "temperature" && (
									<select
										value={tempOutput}
										onChange={(e) =>
											setTempOutput(e.target.value as TemperatureUnit)
										}
										className="w-full h-full bg-nautical-black border border-white/10 rounded-xl px-3 text-sm text-white focus:outline-none focus:border-accent/50"
									>
										<option value="celsius">{t("celsius")}</option>
								...
								{activeTab === "pressure" && (
									<select
										value={pressOutput}
										onChange={(e) =>
											setPressOutput(e.target.value as PressureUnit)
										}
										className="w-full h-full bg-nautical-black border border-white/10 rounded-xl px-3 text-sm text-white focus:outline-none focus:border-accent/50"
									>
										<option value="hpa">{t("hpa")}</option>
					...
					<div className="flex-1 overflow-y-auto space-y-3 custom-scrollbar">
						{history.length === 0 ? (
							...
							history.map((item) => (
								<div
									key={item.id}
									className="bg-nautical-black/40 rounded-lg p-3 border border-white/5 hover:border-white/10 transition-colors group"
								>
									<div className="flex justify-between items-start mb-1">
										<span className="text-[10px] uppercase text-white/30 font-bold tracking-wider">
											{t(item.type)}
										</span>
									...
									<div className="flex items-center justify-between font-mono text-sm">
										<div className="text-white/70">
											<span className="font-bold text-white">
												{item.inputValue.toLocaleString()}
											</span>
										...
										<div className="text-accent text-right">
											<span className="font-bold">
												{item.outputValue.toLocaleString(undefined, {
													maximumFractionDigits: 4,
												})}
											</span>
