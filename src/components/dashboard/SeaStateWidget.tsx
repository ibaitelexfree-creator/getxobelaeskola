		...
		>
			...
			<header className="flex justify-between items-start mb-8 relative z-10">
				...
				>
					<RefreshCw
						size={16}
						className={`text-white/20 group-hover/refresh:text-blue-400 transition-colors ${isRefreshing ? "animate-spin" : ""}`}
					/>
				</button>
			</header>
			...
			<div className="grid grid-cols-2 md:grid-cols-4 gap-8 relative z-10">
				...
				<div className="space-y-2">
					...
					<div className="flex items-baseline gap-1">
						<span className="text-4xl font-display text-white italic tracking-tighter">
							{data.waveHeight}
						</span>
					...
					<div className="h-1 w-full bg-white/5 rounded-full overflow-hidden">
						<motion.div
							initial={{ width: 0 }}
							animate={{
								width: `${Math.min(100, (data.waveHeight / 5) * 100)}%`,
							}}
				...
				<div className="space-y-2">
					...
					<div className="h-1 w-full bg-white/5 rounded-full overflow-hidden">
						<motion.div
							initial={{ width: 0 }}
							animate={{
								width: `${Math.min(100, ((data.waterTemp - 10) / 15) * 100)}%`,
							}}
