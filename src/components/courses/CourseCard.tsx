...
export default function CourseCard({ course, locale }: CourseCardProps) {
	...
	const name =
		...
		"Curso sin nombre";
	const description =
		(locale === "es" ? course.descripcion_es : course.descripcion_eu) ||
		course.descripcion_es ||
		"";
			...
			<div className="p-8 md:p-10 relative z-10 space-y-6">
				<div className="space-y-4">
					<div className="flex justify-between items-baseline border-b border-white/10 pb-4 gap-2">
						<span className="text-technical">Premium Academy</span>
						<span className="text-2xl font-display text-white italic flex-shrink-0">
							{course.precio}
							<span className="text-brass-gold text-lg ml-1">€</span>
						</span>
					</div>
				...
				>
					...
					<span className="text-xl translate-x-0 group-hover/link:translate-x-4 transition-transform duration-700 opacity-0 group-hover/link:opacity-100 italic font-light text-accent">
						→
					</span>
				</Link>
			</div>
