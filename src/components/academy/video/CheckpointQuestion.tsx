...
export default function CheckpointQuestion({
	...
	return (
		<div className="bg-nautical-black/90 backdrop-blur-md p-8 rounded-lg border border-white/20 max-w-2xl w-full mx-auto shadow-2xl animate-fade-in text-white">
			<h3 className="text-xl md:text-2xl font-display italic mb-6 text-center">
				{question}
			</h3>
			...
			<div className="space-y-4 mb-8">
				{options.map((option, index) => {
					let optionClass =
						"w-full p-4 rounded-md border text-left transition-all duration-300 ";
					...
					if (submitted) {
						...
						if (selectedOption === index) {
							optionClass +=
								"bg-accent/20 border-accent text-accent shadow-lg shadow-accent/10 scale-[1.02]";
						} else {
							optionClass +=
						...
						>
							<div className="flex items-center gap-4">
								<div
                                    ...
                                    ${
																			submitted && index === correctOptionIndex
																				? "border-green-500 bg-green-500 text-nautical-black"
																				: submitted && index === selectedOption
																					? "border-red-500 bg-red-500 text-white"
																					: selectedOption === index
																						? "border-accent bg-accent text-nautical-black"
