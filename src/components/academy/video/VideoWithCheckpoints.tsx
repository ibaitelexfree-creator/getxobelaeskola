...
export default function VideoWithCheckpoints({
	...
	const checkCheckpoints = (time: number) => {
		...
		const checkpoint = checkpoints.find(
			(cp) =>
				Math.abs(cp.time - time) < 1 && !completedCheckpoints.includes(cp.time),
		);
	...
	return (
		<div
			className="relative w-full aspect-video bg-black rounded-lg overflow-hidden shadow-2xl border border-white/10 group"
			ref={containerRef}
		>
			{videoType === "youtube" ? (
