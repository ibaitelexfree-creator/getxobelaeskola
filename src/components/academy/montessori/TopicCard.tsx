...
import { MontessoriTopic } from './types';
import { CheckCircle, Lock, Anchor, Ship, Lightbulb, Flag, Radio } from 'lucide-react';
...
const CategoryIcon: React.FC<{ category: string, className?: string }> = ({ category, className }) => {
    switch (category) {
        case 'knots': return <Anchor className={className} />;
        case 'lights': return <Lightbulb className={className} />;
        case 'flags': return <Flag className={className} />;
        case 'radio': return <Radio className={className} />;
        default: return <Ship className={className} />;
    }
};

export const TopicCard: React.FC<TopicCardProps> = ({ topic, status, onClick }) => {
    ...
    return (
        ...
        >
            <div className="flex items-start justify-between mb-4">
                <div className={`p-3 rounded-xl ${isMastered ? 'bg-emerald-500/20 text-emerald-400' : 'bg-accent/10 text-accent'}`}>
