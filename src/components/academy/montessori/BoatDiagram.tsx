...
export default function BoatDiagram({ highlightId, onPartClick, className = '' }: BoatDiagramProps) {
    const isActive = (id: string) => highlightId === id;

    // Base styles for parts
    const baseStyle = "fill-transparent stroke-white/20 stroke-2 transition-all duration-300 cursor-pointer hover:stroke-accent/50";
    const activeStyle = "fill-accent/10 stroke-accent stroke-[3] filter drop-shadow-[0_0_8px_rgba(255,255,255,0.5)]";
    const textStyle = "text-xs fill-white/40 pointer-events-none select-none uppercase tracking-widest";

    const getPartClass = (id: string) => `${baseStyle} ${isActive(id) ? activeStyle : ''}`;
    ...
    return (
        ...
        >
            ...
            <line x1="50" y1="350" x2="450" y2="350" className="stroke-blue-400/20 stroke-1" strokeDasharray="5,5" />

            {/* GROUP: ESTRUCTURA */}
            <g id="structure">
                {/* BABOR / ESTRIBOR Click Areas (Invisible halves) */}
                <rect x="0" y="0" width="250" height="500" className={`fill-transparent cursor-pointer ${isActive('babor') ? 'fill-red-500/10' : ''}`} onClick={() => onPartClick?.('babor')} />
                <rect x="250" y="0" width="250" height="500" className={`fill-transparent cursor-pointer ${isActive('estribor') ? 'fill-green-500/10' : ''}`} onClick={() => onPartClick?.('estribor')} />
                ...
                <circle cx="110" cy="320" r="30" className={`fill-transparent cursor-pointer ${isActive('popa') ? 'stroke-accent stroke-2' : 'stroke-transparent'}`} onClick={() => onPartClick?.('popa')} />

                {/* PROA */}
                <circle cx="390" cy="320" r="30" className={`fill-transparent cursor-pointer ${isActive('proa') ? 'stroke-accent stroke-2' : 'stroke-transparent'}`} onClick={() => onPartClick?.('proa')} />

                {/* BABOR/ESTRIBOR Labels */}
                {isActive('babor') && <text x="125" y="150" textAnchor="middle" className="fill-red-400/80 text-4xl font-black opacity-50 select-none pointer-events-none">BABOR</text>}
                {isActive('estribor') && <text x="375" y="150" textAnchor="middle" className="fill-green-400/80 text-4xl font-black opacity-50 select-none pointer-events-none">ESTRIBOR</text>}
            </g>
            ...
            <text x="50" y="480" className={textStyle}>ESQUEMA BÁSICO VELERO</text>
