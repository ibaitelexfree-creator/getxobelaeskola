...
export default function ModuleQA({ moduleId, locale }: ModuleQAProps) {
	...
	useEffect(() => {
		...
		async function fetchData() {
			...
			try {
				...
				const mergedQuestions: ModuleQuestion[] = (questionsData || []).map(
					(q: any) => {
						const qVotes = userVotes.find(
							(v) => v.item_id === q.id && v.item_type === "question",
						);
						const qAnswers = answersData
							.filter((a) => a.question_id === q.id)
							.map((a) => ({
								...a,
								user_vote: userVotes.find(
									(v) => v.item_id === a.id && v.item_type === "answer",
								)?.vote_type,
							}));
	...
	const handleAskQuestion = async (e: React.FormEvent) => {
		...
		try {
			...
			addNotification({
				...
				message: "Tu duda ha sido publicada.",
			});
		} catch (error) {
			console.error(error);
			addNotification({
				type: "error",
				title: "Error",
				message: "No se pudo enviar la pregunta.",
			});
		} finally {
	...
	const handleVote = async (
		...
		try {
			...
			if (newVoteType === 0) {
				await supabase.from("module_qa_votes").delete().match({
					user_id: currentUser.id,
					item_id: itemId,
					item_type: itemType,
				});
	...
	const handleMarkCorrect = async (answerId: string, questionId: string) => {
		...
		try {
			...
			addNotification({
				...
				message: willBeAccepted
					? "Has marcado esta respuesta como correcta."
					: "La respuesta ya no está marcada como correcta.",
			});
		} catch (error) {
		...
		<section className="max-w-4xl mx-auto mt-20 mb-32 px-4 md:px-0">
			<header className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10 border-b border-white/10 pb-8">
				<div>
					...
					<p className="text-white/60 text-sm mt-2 font-light max-w-xl">
						Pregunta tus dudas y comparte conocimientos con otros alumnos. Las
						mejores respuestas pueden ser validadas por los instructores.
					</p>
				</div>
			</header>
			...
			{currentUser && (
				<div className="glass-panel p-6 mb-10 border border-white/10 bg-white/[0.02]">
					<h3 className="text-lg font-display italic text-white mb-4">
						Nueva Pregunta
					</h3>
			...
			<div className="space-y-6">
				{questions.length === 0 ? (
					<div className="glass-panel p-16 text-center border-dashed border-2 border-white/10 opacity-60">
						...
						<h3 className="text-2xl text-white font-display italic mb-3">
					...
					questions.map((question) => (
						<div
							key={question.id}
							className="glass-panel p-6 lg:p-8 border-l-4 border-l-accent/50 hover:border-l-accent transition-all duration-300"
						>
							{/* Question Header */}
							...
							{expandedQuestions.has(question.id) && (
								<div className="mt-8 pl-4 lg:pl-14 space-y-8 animate-fade-in relative">
									...
									{question.answers?.map((answer) => (
										<div
											key={answer.id}
											className={`relative p-5 rounded-lg border ${answer.is_accepted ? "bg-accent/5 border-accent/40 shadow-[0_0_30px_-10px_rgba(var(--accent-rgb),0.2)]" : "bg-nautical-black/40 border-white/5"}`}
										>
											{answer.is_accepted && (
												...
												<div className="flex-1">
													<div className="flex items-center gap-2 mb-3">
														...
														{answer.user?.rol === "instructor" && (
															<span className="text-[9px] bg-accent text-nautical-black px-1.5 py-0.5 rounded font-black uppercase tracking-wider">
																Instructor
															</span>
														)}
													...
													<div className="flex items-center justify-between mt-4 pt-3 border-t border-white/5">
														...
														{(currentUser?.rol === "instructor" ||
															...
															<button
																onClick={() =>
																	handleMarkCorrect(answer.id, question.id)
																}
										...
										<div className="flex gap-3">
											{currentUser?.avatar_url ? (
												<img
													...
													alt="Me"
													className="w-8 h-8 rounded-full object-cover opacity-50 grayscale"
												/>
											) : (
