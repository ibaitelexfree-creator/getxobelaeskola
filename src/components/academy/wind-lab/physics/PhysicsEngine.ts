...
export class PhysicsEngine {
	...
	public static calculatePhysics(state: WindLabState): DerivedPhysics {
		...
		const boatSpeed = PhysicsEngine.clamp(
			state.boatSpeed || 0,
			0,
			PhysicsEngine.MAX_BOAT_SPEED,
		);
		const trueWindSpeed = PhysicsEngine.clamp(
			state.trueWindSpeed || 0,
			0,
			PhysicsEngine.MAX_WIND_SPEED,
		);
		const trueWindDirection = PhysicsEngine.normalizeAngle(
			state.trueWindDirection || 0,
		);
		const heading = PhysicsEngine.normalizeAngle(state.boatHeading || 0);
		const mainAngle = PhysicsEngine.clamp(
			Math.abs(state.sailAngle || 0),
			0,
			90,
		);
		const jibAngle = PhysicsEngine.clamp(Math.abs(state.jibAngle || 0), 0, 90);
		...
		const mainPhysics = PhysicsEngine.computeFoil(absAwa, mainAngle, 1.0);

		// Jib Sail (Area size factor 0.6) - Jib is smaller but efficient
		const jibPhysics = PhysicsEngine.computeFoil(absAwa, jibAngle, 0.6);
		...
		const totalLift =
			(mainPhysics.lift * 1.0 + jibPhysics.lift * 0.6) *
			apparentWindSpeed *
			apparentWindSpeed *
			0.1;
		const totalDrag =
			(mainPhysics.drag * 1.0 + jibPhysics.drag * 0.6) *
			apparentWindSpeed *
			apparentWindSpeed *
			0.1;
		...
		let driveForce =
			totalLift * Math.sin(absAwaRad) - totalDrag * Math.cos(absAwaRad);
		let heelForce =
			totalLift * Math.cos(absAwaRad) + totalDrag * Math.sin(absAwaRad);

		// Clamp Forces
		driveForce = PhysicsEngine.clamp(
			driveForce,
			-PhysicsEngine.MAX_FORCE,
			PhysicsEngine.MAX_FORCE,
		);
		heelForce = PhysicsEngine.clamp(heelForce, 0, PhysicsEngine.MAX_FORCE); // Heel is always positive (magnitude)

		// Efficiency metric (0-1)
		const combinedEfficiency =
			(mainPhysics.efficiency + jibPhysics.efficiency) / 2;
	...
	private static computeFoil(
		...
		let cd = PhysicsEngine.DRAG_COEFF_BASE; // Coefficient of Drag
		let isStalled = false;
		let isLuffing = false;
		...
		if (aoa < 0) {
		...
		} else if (aoa <= PhysicsEngine.MAX_LIFT_AOA) {
			// Linear Lift Region
			// Cl rises linearly with AoA
			// Max Cl approx 1.5
			const t = aoa / PhysicsEngine.MAX_LIFT_AOA;
			cl = t * 1.5;
			cd = PhysicsEngine.DRAG_COEFF_BASE + cl * cl * 0.05; // Induced drag
		} else if (aoa <= PhysicsEngine.STALL_AOA) {
			// Peak / Transition
			cl = 1.5;
			cd = 0.2;
		} else {
			...
			const excess = aoa - PhysicsEngine.STALL_AOA;
		...
		if (!isStalled && !isLuffing) {
			...
			const dist = Math.abs(aoa - (PhysicsEngine.MAX_LIFT_AOA - 5)); // Peak around 15
	...
	public static getOptimalSailAngle(
		...
		return PhysicsEngine.clamp(target, 0, 90);
	}
