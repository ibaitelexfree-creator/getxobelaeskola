...
export const useWindLabPhysics = () => {
    ...
    const [physics, setPhysics] = useState<DerivedPhysics>(PhysicsEngine.calculatePhysics(state));
    ...
    const animate = (time: number) => {
        if (previousTimeRef.current !== undefined) {
            ...
            // 1. Calculate Forces
            const currentPhysics = PhysicsEngine.calculatePhysics(physicsState.current);
            ...
            const hullDrag = physicsState.current.boatSpeed * physicsState.current.boatSpeed * 0.05;
            const netForwardForce = currentPhysics.driveForce - hullDrag;

            // Update Speed
            // Accel = Force / Mass. Let's say Mass = 10.
            let newBoatSpeed = physicsState.current.boatSpeed + (netForwardForce * deltaTime * 2.0);
            ...
            if (isNaN(newBoatSpeed) || isNaN(newHeading) || isNaN(newHeel) || !Number.isFinite(newBoatSpeed)) {
                console.warn("[WindLab] Physics Singularity Detected! Hard Resetting Simulation...");
    ...
    const setTrueWind = useCallback((speed: number, direction: number) => {
        if (physicsState.current.trueWindSpeed === speed && physicsState.current.trueWindDirection === direction) return;
        physicsState.current.trueWindSpeed = speed;
        physicsState.current.trueWindDirection = direction;
        setState(prev => ({ ...prev, trueWindSpeed: speed, trueWindDirection: direction }));
    }, []);
