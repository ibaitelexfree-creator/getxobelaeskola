...
export const useWindLabPhysics = () => {
	...
	const animate = (time: number) => {
		if (previousTimeRef.current !== undefined) {
			...
			if (Number.isNaN(newBoatSpeed)) newBoatSpeed = 0;
			...
			if (Number.isNaN(newHeading)) newHeading = 0;
			...
			if (Number.isNaN(newHeel)) newHeel = 0;

			// Catastrophic Failure Check (The "God Hand" Reset)
			if (
				Number.isNaN(newBoatSpeed) ||
				Number.isNaN(newHeading) ||
				Number.isNaN(newHeel) ||
