...
export const ParticleSystemCanvas: React.FC<ParticleSystemCanvasProps> = ({
	state,
	physics,
}) => {
	...
	useEffect(() => {
		...
		const update = () => {
			...
			const flowVx =
				Math.sin(globalWindRad + Math.PI) *
				(currentPhysics.apparentWindSpeed * 0.4 + 2);
			const flowVy =
				-Math.cos(globalWindRad + Math.PI) *
				(currentPhysics.apparentWindSpeed * 0.4 + 2);
			...
			for (let i = 0; i < spawnCount; i++) {
				if (particles.current.length < MAX_PARTICLES) {
					...
					particles.current.push(
						initParticle({ x: sx, y: sy, vx: flowVx, vy: flowVy }),
					);
				}
			}
		...
		const draw = () => {
			...
			particles.current.forEach((p) => {
				...
				if (currentPhysics.mainIsStalled) {
					hue = 0;
