...
export const GhostBoatOverlay: React.FC<GhostBoatOverlayProps> = ({
	...
	useEffect(() => {
		...
		const updateLerp = (time: number) => {
			...
			const optimalJib = PhysicsEngine.getOptimalSailAngle(
				apparentWindAngle,
				5,
			);
		...
		>
			...
			>
				...
				>
					...
					<div
						className="absolute inset-0 bg-cyan-400/5 rounded-tr-[100%]"
						style={{
							width: "150px",
				...
				>
					<div className="absolute inset-0 border-l-4 border-cyan-400/40 shadow-[0_0_10px_cyan]" />
					<div
						className="absolute inset-0 bg-cyan-400/5 rounded-br-[100%]"
						style={{
							width: "120px",
							transform: windSide > 0 ? "scaleX(-1)" : "none",
							transformOrigin: "left",
