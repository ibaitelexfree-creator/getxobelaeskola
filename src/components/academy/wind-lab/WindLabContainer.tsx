...
export const WindLabContainer = () => {
	...
	const [showVectors, _setShowVectors] = useState(false);
	...
	const [simPhysics, setSimPhysics] = useState<DerivedPhysics>(
		PhysicsEngine.calculatePhysics(simState),
	);
	...
	useEffect(() => {
		...
		try {
			...
			const particleOffscreen =
				particleCanvasRef.current.transferControlToOffscreen();
	...
	return (
		<div className="fixed inset-0 h-[100dvh] bg-slate-950 text-white p-2 md:p-4 flex flex-col overflow-hidden select-none">
			...
			<div
				className={`flex flex-1 gap-2 md:gap-4 transition-transform duration-75 min-h-0 ${simState.boatSpeed > 15 ? "animate-pulse" : ""}`}
			>
				...
				>
					...
					<canvas
						ref={seaCanvasRef}
						className="absolute inset-0 w-full h-full opacity-60 rounded-3xl"
					/>
					<canvas
					...
					>
						...
						>
							...
							<div
								className="absolute w-[400px] h-[400px] border border-cyan-500/10 rounded-full flex items-center justify-center"
							...
							>
								...
								<div
									className={`relative w-full h-full rounded-[50%_50%_18%_18%] border-2 ${simPhysics.efficiency > 0.8 ? "border-cyan-400 shadow-[0_0_40px_cyan,inset_0_0_20px_cyan]" : "border-white/30 shadow-2xl"} bg-slate-950/95 backdrop-blur-3xl transition-all duration-300`}
								...
								<div
									className="absolute -bottom-12 left-1/2 w-3 h-18 bg-red-600 border-2 border-red-400/50 origin-top transform -translate-x-1/2 rounded-full shadow-[0_10px_30px_red]"
									style={{
										transform: `translateX(-50%) translateZ(10px) rotateZ(${simState.rudderAngle}deg)`,
									}}
								/>
							</div>
						</div>
					...
					<div className="absolute top-10 right-10 w-36 h-36 flex flex-col items-center justify-center p-5 bg-slate-950/85 backdrop-blur-2xl rounded-full border border-white/15 shadow-2xl z-20">
						<div
							...
							style={{
								transform: `rotate(${simState.trueWindDirection + 180}deg)`,
							}}
						>
							<svg
								width="28"
							...
							>
								<path
									d="M12 60V10M12 10L4 22M12 10L20 22"
									stroke="#22d3ee"
									strokeWidth="6"
									strokeLinecap="round"
					...
					<div className="bg-slate-950/95 backdrop-blur-3xl rounded-3xl border border-cyan-500/30 p-5 shadow-2xl flex flex-col gap-6 shrink-0 relative overflow-visible">
						...
						<div className="flex flex-col gap-5 border-t border-white/10 pt-5">
							<TrimFeedback
								...
								optimalSailAngle={PhysicsEngine.getOptimalSailAngle(
									simPhysics.apparentWindAngle,
								)}
					...
					<div className="flex-1 min-h-0 grid grid-cols-2 gap-4">
						...
						<div className="flex flex-col gap-4 h-full">
							...
							<div className="h-36 bg-slate-950/95 backdrop-blur-2xl rounded-3xl border border-cyan-500/30 p-5 shadow-xl flex flex-col justify-center relative overflow-visible">
								<label className="text-[11px] font-black text-slate-400 tracking-widest uppercase mb-4 italic">
									{t("controls.rudder")}
								</label>
								<input
									type="range"
									...
									max="45"
									value={rudderInput}
									onChange={(e) => setRudderInput(Number(e.target.value))}
						...
						<div className="bg-slate-950/95 backdrop-blur-2xl rounded-3xl border border-cyan-500/30 p-5 shadow-xl flex justify-around items-center h-full relative overflow-visible">
							...
							<div className="flex flex-col h-full justify-around items-center w-full">
								...
								/>
								<div className="h-4" />{" "}
								{/* Spacer to prevent overlap of labels if they grow */}
								<HydraulicWinch
									label="GÉNOVA"
									min={0}
									max={90}
