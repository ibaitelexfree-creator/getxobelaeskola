...
function updatePhysics(dt: number) {
	...
	if (Number.isNaN(newBoatSpeed) || newBoatSpeed < 0) newBoatSpeed = 0;
	...
	newAngularVelocity *= baseDamping ** (deltaTime / FPS_60_DT);
...
function renderParticles() {
	...
	const flowVx =
		Math.sin(globalWindRad + Math.PI) * (physics.apparentWindSpeed * 0.4 + 2);
	const flowVy =
		-Math.cos(globalWindRad + Math.PI) * (physics.apparentWindSpeed * 0.4 + 2);
	...
	particles = particles.filter((p) => {
		...
		const localX =
			dx * Math.cos(-boatHeadingRad) - dy * Math.sin(-boatHeadingRad);
		const localY =
			dx * Math.sin(-boatHeadingRad) + dy * Math.cos(-boatHeadingRad);
		// Hull Collision
		if (
			(localX * localX) / (hullB * hullB) +
				(localY * localY) / (hullA * hullA) <
			1.1
		)
			return false;
		...
		// Main Sail Collision
		const sLocalX =
			dx * Math.cos(-totalSailAngleRad) - dy * Math.sin(-totalSailAngleRad);
		const sLocalY =
			dx * Math.sin(-totalSailAngleRad) + dy * Math.cos(-totalSailAngleRad);
	...
	particles.forEach((p) => {
		...
		} else if (physics.efficiency > 0.8) {
			hue = 150;
			saturation = 100;
		}
