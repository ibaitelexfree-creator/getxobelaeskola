...
import { Anchor, Info, MessageCircle, Send, X } from "lucide-react";
	...
	const [messages, setMessages] = useState<Message[]>([
		{
			role: "model",
			content:
				"¡Hola! Soy El Piloto, tu instructor virtual. ¿En qué puedo ayudarte hoy con tu navegación?",
		},
	]);
		...
		try {
			...
			setMessages((prev) => [
				...prev,
				{
					role: "model",
					content:
						"Lo siento, he tenido un problema técnico. Por favor, intenta de nuevo.",
				},
			]);
		} finally {
			setIsLoading(false);
											...
											<ReactMarkdown
												...
												components={{
													...
													blockquote: ({ node, ...props }) => (
														<blockquote
															{...props}
															className="border-l-2 border-blue-500 pl-3 italic text-slate-400 my-2"
														/>
													),
													code: ({ node, ...props }) => (
														<code
															{...props}
															className="bg-slate-800 px-1 py-0.5 rounded text-xs font-mono text-blue-200"
														/>
													),
												}}
											>
