...
export default function WrittenExerciseSubmission({
	...
	const [content, setContent] = useState(
		existingAttempt?.datos_json?.text || "",
	);
		...
		try {
			...
			if (res.error) {
				...
				setMessage({
					...
					text:
						locale === "eu"
							? "Bidalita! Zure ariketa berrikuspenaren zain dago."
							: "¡Enviado! Tu ejercicio está pendiente de revisión.",
				});
			}
		} catch (e) {
			...
			<div className="flex items-center gap-3 mb-6">
				...
				<div>
					...
					<p className="text-white/70 text-sm">
						{locale === "eu"
							? activity.descripcion_eu
							: activity.descripcion_es}
					</p>
				</div>
			</div>
			...
			<div className="space-y-4">
				...
				<div className="flex justify-between items-center">
					<div>
						...
						{message && (
							<span
								className={`ml-4 text-sm ${message.type === "success" ? "text-green-400" : "text-red-400"}`}
							>
								{message.text}
							</span>
						)}
