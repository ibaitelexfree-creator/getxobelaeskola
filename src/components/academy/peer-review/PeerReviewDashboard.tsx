...
import RubricForm, { RubricCriterion } from './RubricForm';
...
const UI_TEXT = {
    es: {
        ...
        noReviewsDesc: "¡Buen trabajo! Vuelve más tarde para ayudar a otros compañeros y ganar XP.",
    ...
    eu: {
        ...
        noReviewsDesc: "Lan bikaina! Itzuli geroago beste ikaskideei laguntzeko eta XP irabazteko.",
    ...
    fr: {
        ...
        noReviewsDesc: "Bon travail! Revenez plus tard pour aider les autres et gagner des XP.",
...
export default function PeerReviewDashboard({ moduleId, locale }: PeerReviewDashboardProps) {
    ...
    if (loading) return <div className="text-white/60 animate-pulse text-center py-8">{t.loading}</div>;

    if (tasks.length === 0) {
        return (
            <div className="bg-white/5 border border-white/10 p-8 rounded-lg text-center max-w-2xl mx-auto">
                <div className="text-4xl mb-4">🤝</div>
                <h3 className="text-xl font-display italic text-white mb-2">{t.noReviewsTitle}</h3>
        ...
        return (
            <div className="bg-nautical-black/50 p-6 rounded-xl border border-accent/20 animate-fade-in">
                ...
                <SubmissionViewer
                    ...
                    activityTitle={locale === 'eu' ? selectedTask.activityTitleEu : selectedTask.activityTitleEs}
                />

                <div className="bg-white/5 border border-white/10 rounded-lg p-6">
                    <RubricForm rubric={rubricData} onChange={(s, t) => { setScores(s); setTotalScore(t); }} />

                    <div className="mt-6">
                        <label className="block text-white font-bold mb-2">{t.feedbackLabel}</label>
                        <textarea
                            value={feedback}
                    ...
                    <div className="mt-8 flex justify-end items-center gap-4">
                        <div className="text-right">
                            <span className="text-sm text-white/60 block uppercase tracking-widest">{t.finalScore}</span>
                            <span className="text-3xl font-display italic text-accent">{totalScore} / 100</span>
                        </div>
                        <button
    ...
    return (
        <div className="space-y-6">
            <header className="flex justify-between items-end border-b border-white/10 pb-6 mb-10">
                <h2 className="text-3xl font-display italic text-white">{t.dashboardTitle}</h2>
                        ...
                        <div className="flex justify-between items-center relative z-10">
                            <div>
                                <h3 className="text-xl font-bold text-white group-hover:text-accent transition-colors">
                                    {locale === 'eu' ? task.activityTitleEu : task.activityTitleEs}
                                </h3>
                                <p className="text-sm text-white/60 mt-2 max-w-md">
                                    {t.cardDesc} <span className="text-accent font-bold">50 XP</span>.
                                </p>
                            </div>
                            <div className="text-right">
