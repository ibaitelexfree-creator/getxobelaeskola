...
export default function RubricForm({ rubric, onChange }: RubricFormProps) {
	...
	const handleScoreChange = (id: string, value: number) => {
		...
		const normalizedScore =
			maxPossible > 0 ? Math.round((currentPoints / maxPossible) * 100) : 0;
	...
	return (
		<div className="space-y-6">
			<h3 className="text-xl font-display italic text-white mb-4">
			...
			<div className="grid gap-6">
				{rubric.map((criterion) => (
					<div
						key={criterion.id}
						className="bg-white/5 p-4 rounded-lg border border-white/10 hover:border-accent/30 transition-colors"
					>
