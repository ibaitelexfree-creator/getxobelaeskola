...
import { Mic, Play, Save, Send, Square, Trash2 } from "lucide-react";
...
export default function FeedbackRecorder({
	...
	const handleSave = async () => {
		...
		try {
			...
			if (audioBlob) {
				...
				const {
					data: { publicUrl },
				} = supabase.storage.from("academy-assets").getPublicUrl(fileName);
			...
			if (studentId) {
				await supabase.from("notifications").insert({
					...
					message:
						entityType === "logbook"
							? "Tienes feedback en tu bitácora."
							: "Tienes feedback en tu evaluación.",
					type:
						entityType === "logbook"
							? "feedback_logbook"
							: "feedback_evaluation",
	...
	return (
		<div className="bg-nautical-black/80 border border-white/10 rounded-xl p-4 space-y-4">
			<h4 className="text-xs uppercase tracking-widest text-accent font-bold">
