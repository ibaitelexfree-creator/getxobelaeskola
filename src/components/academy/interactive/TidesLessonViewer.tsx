...
import type { TideLesson } from "@/lib/academy/tides-data";
...
export default function TidesLessonViewer({ lesson }: TidesLessonViewerProps) {
	...
	const renderContent = (content: string) => {
		return content.split("\n").map((line, i) => {
			...
			if (line.startsWith("# ")) {
				return (
					<h3 key={i} className="text-2xl font-bold text-white mt-6 mb-4">
						{line.substring(2)}
					</h3>
				);
			}
...
function RuleOfTwelfthsCalculator() {
	...
	const hours = [
		...
		{ label: "2ª Hora", factor: 2, height: startHeight + twelfth * (1 + 2) }, // 3/12
		{
			label: "3ª Hora",
			factor: 3,
			height: startHeight + twelfth * (1 + 2 + 3),
		}, // 6/12 (Half tide)
		{
			label: "4ª Hora",
			factor: 3,
			height: startHeight + twelfth * (1 + 2 + 3 + 3),
		}, // 9/12
		{
			label: "5ª Hora",
			factor: 2,
			height: startHeight + twelfth * (1 + 2 + 3 + 3 + 2),
		}, // 11/12
		...
		<div className="bg-nautical-black/50 border border-white/10 rounded-xl p-6">
			<div className="flex items-center gap-2 mb-6">
				...
				<h3 className="text-xl font-bold text-white">
					Calculadora: Regla de los Duodécimos
				</h3>
			</div>
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-8">
				...
				<div className="space-y-6">
					<div className="bg-white/5 p-4 rounded-lg">
						<label
							htmlFor="startHeight"
							className="block text-xs font-bold text-white/60 uppercase tracking-widest mb-2"
						>
							Nivel Inicial (m)
						</label>
						<input
					...
					<div className="bg-white/5 p-4 rounded-lg">
						...
						<p className="text-white/30 text-xs mt-2">
							Ej: Siguiente Pleamar o Bajamar
						</p>
					</div>
					...
					<div className="p-4 bg-blue-500/10 border border-blue-500/20 rounded-lg flex gap-3 items-start">
						...
						<div className="text-sm text-blue-200">
							<p className="font-bold mb-1">
								Amplitud: {Math.abs(range).toFixed(2)}m
							</p>
							<p>
								Un duodécimo (1/12) equivale a {Math.abs(twelfth).toFixed(2)}m
								de cambio.
							</p>
							<p className="mt-2 text-xs opacity-70">
								{range > 0
									? "Marea Entrante (Flujo)"
									: "Marea Saliente (Reflujo)"}
							</p>
						</div>
					</div>
				</div>
						...
						<tbody className="divide-y divide-white/5">
							{hours.map((row, idx) => (
								<tr
									...
									className={
										idx === 0 || idx === 6
											? "bg-white/5 font-bold text-accent"
								...
								>
									...
									<td className="px-4 py-3 flex gap-1 items-center h-full">
										...
										{idx > 0 &&
											idx < 6 &&
											Array.from({ length: row.factor }).map((_, i) => (
												<div
													key={i}
