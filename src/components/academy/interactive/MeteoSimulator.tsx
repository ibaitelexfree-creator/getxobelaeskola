...
import { AlertTriangle, CheckCircle, FileText, Ship, Wind } from "lucide-react";
import { useEffect, useState } from "react";
import type { MeteoScenario } from "@/lib/academy/meteo-scenarios";
...
export default function MeteoSimulator({ scenario }: MeteoSimulatorProps) {
	...
	const allCorrect = Object.keys(scenario.correctAnswers).every((qId) =>
		isCorrect(qId),
	);
			...
			<div className="flex flex-col gap-6">
				<div className="bg-white/5 border border-white/10 rounded-xl p-6 shadow-2xl relative overflow-hidden group hover:border-white/20 transition-all duration-300">
					...
					<div className="flex items-center gap-3 mb-6">
						...
						<h3 className="text-xl font-display italic text-white tracking-wide">
							Parte Meteorológico
						</h3>
					</div>
				...
				{submitted && (
					<div
						className={`p-6 rounded-xl border transition-all duration-500 animate-fade-in
                        ${
													allCorrect
					...
					>
						<div className="flex items-center gap-3 mb-4">
							...
							<h4
								className={`text-xl font-display italic ${allCorrect ? "text-emerald-400" : "text-red-400"}`}
							>
				...
				<div className="bg-white/5 border border-white/10 rounded-xl p-6 lg:p-8 flex-1 flex flex-col">
					...
					<div className="space-y-8 flex-1">
						{scenario.questions.map((q, idx) => {
							const _answered = !!userAnswers[q.id];
							const _correct = isCorrect(q.id);
							...
							return (
								...
								>
									<h4 className="text-sm font-bold text-white mb-3 flex items-start gap-2">
										<span className="bg-white/10 w-6 h-6 rounded flex items-center justify-center text-xs text-accent shrink-0 mt-0.5">
											{idx + 1}
										</span>
										{q.text}
									</h4>
									...
									<div className="space-y-2 pl-8">
										{q.options.map((opt) => {
											...
											const isCorrectOption =
												scenario.correctAnswers[q.id] === opt.id;
											...
											if (submitted) {
												if (isCorrectOption) {
													...
													styles =
														"bg-red-500/20 border-red-500/50 text-red-300 line-through opacity-70";
												} else {
												...
												if (isSelected) {
													styles =
														"bg-accent text-nautical-black border-accent font-bold shadow-[0_0_15px_rgba(255,191,0,0.2)]";
												}
											}
											...
											return (
												...
												>
													...
													{!submitted && isSelected && (
														<div className="w-2 h-2 bg-nautical-black rounded-full" />
													)}
												</button>
											);
										})}
