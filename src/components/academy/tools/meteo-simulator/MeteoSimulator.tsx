...
import DecisionForm, { UserDecision } from './DecisionForm';
import FeedbackResult from './FeedbackResult';
import { CloudRain, Anchor, Trophy } from 'lucide-react';
...
export default function MeteoSimulator() {
    ...
    const handleSubmit = (decision: UserDecision) => {
        ...
        let points = 0;
        if (decision.navigable === currentScenario.correctAnswer.navigable) points++;
        if (decision.route_idx === currentScenario.correctAnswer.route_idx) points++;
            ...
            <div className="min-h-screen bg-nautical-black text-white p-6 flex flex-col items-center justify-center">
                <div className="max-w-md w-full text-center space-y-8 animate-fade-in">
                    ...
                    <p className="text-white/60 text-lg">
                        Puntuación Final: <strong className="text-accent">{score}</strong> / {METEO_SCENARIOS.length * 3}
                    </p>

                    <div className="p-6 bg-white/5 border border-white/10 rounded-xl">
                        <p className="italic text-white/80">
                            "La meteorología es la ciencia que estudia el tiempo que hace, para que sepamos el tiempo que nos queda."
                        </p>
                    </div>
                ...
                <div className="container mx-auto px-6 h-16 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <h1 className="font-display italic text-lg hidden sm:block">
                            Simulador Meteo <span className="text-white/40 mx-2">|</span> Escenario {currentIndex + 1}/{METEO_SCENARIOS.length}
                        </h1>
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                    ...
                    <div className="space-y-6">
                        <div className="flex items-center gap-2 mb-4">
                            <span className="text-accent text-xs font-black uppercase tracking-widest">BOLETÍN OFICIAL</span>
