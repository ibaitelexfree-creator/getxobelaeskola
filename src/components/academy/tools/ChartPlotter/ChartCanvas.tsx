...
const _SCALE_MARK = 1852; // 1 Nautical Mile in our fake coordinates (px)
...
export default function ChartCanvas() {
	...
	return (
		<div
			className="relative w-full h-[calc(100vh-64px)] overflow-hidden bg-[#e0e0e0] cursor-crosshair select-none"
		...
		>
			...
			<div className="absolute top-4 left-4 z-50 flex flex-col gap-2 bg-white/90 backdrop-blur border border-black/10 p-2 rounded-lg shadow-xl">
				...
				<select
					value={currentChart}
					onChange={(e) => setCurrentChart(parseInt(e.target.value, 10))}
			...
			<div className="absolute top-4 right-4 z-50 bg-white/90 backdrop-blur border border-black/10 p-4 rounded-lg shadow-xl text-2xs font-mono space-y-2 w-48">
				<div className="flex justify-between">
					...
					<span className="font-bold">
						{(Math.abs(viewPos.x) / 10).toFixed(0)}N,{" "}
						{(Math.abs(viewPos.y) / 10).toFixed(0)}W
					</span>
				</div>
				<div className="flex justify-between">
					<span>Escala:</span>
				...
				{activeTool === "divider" && (
					<div className="flex justify-between text-accent-dark">
						...
						<span className="font-bold">
							{(
								Math.sqrt(
									(dividerState.x2 - dividerState.x1) ** 2 +
										(dividerState.y2 - dividerState.y1) ** 2,
								) / 100
							).toFixed(2)}{" "}
							MN
						</span>
			...
			>
				...
				<div className="w-[3000px] h-[2000px] bg-[#f8f5f0] relative shadow-2xl border-[12px] border-[#3e2723] rounded-sm overflow-hidden">
					...
					<div
						...
						style={{
							backgroundImage:
								"linear-gradient(#94a3b833 1px, transparent 1px), linear-gradient(90deg, #94a3b833 1px, transparent 1px)",
				...
				{activeTool === "divider" && (
					<DividerTool
						...
						onUpdate={(p1, p2) =>
							setDividerState({ x1: p1.x, y1: p1.y, x2: p2.x, y2: p2.y })
						}
					/>
				)}
			</div>
