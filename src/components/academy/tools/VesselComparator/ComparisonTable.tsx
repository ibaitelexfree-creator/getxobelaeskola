...
import { vessels, Vessel } from '@/data/vessels';
...
export default function ComparisonTable({ selectedIds }: ComparisonTableProps) {
  ...
  const features: Array<{ label: string; key: keyof Vessel['specs']; unit: string }> = [
      ...
      <table className="w-full min-w-[600px] border-collapse text-sm">
        <thead>
          <tr>
            ...
            {sortedVessels.map(v => (
              <th key={v.id} className="text-left p-4 text-accent font-display italic text-lg border-b border-white/10 min-w-[180px]">
                {v.name}
              </th>
            ))}
          </tr>
        ...
        <tbody>
           ...
           {features.map((feature, idx) => (
             <tr key={feature.key} className={idx % 2 === 0 ? 'bg-white/[0.02]' : ''}>
               ...
               {sortedVessels.map(v => (
                 <td key={v.id} className="p-4 text-white/90 border-b border-white/5 font-mono">
                   {v.specs[feature.key]}
                   {feature.unit && <span className="text-white/40 ml-1 text-xs">{feature.unit}</span>}
                 </td>
               ))}
             </tr>
           ))}
           ...
           <tr>
              ...
              {sortedVessels.map(v => (
                <td key={v.id} className="p-4 text-white/60 text-xs leading-relaxed border-b border-white/5 align-top pt-6">
                  {v.description}
                </td>
              ))}
           </tr>
