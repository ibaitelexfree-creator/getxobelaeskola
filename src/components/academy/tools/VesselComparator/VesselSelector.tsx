		...
		<div className="w-full">
			<div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
              ...
              `}
							title={
								isDisabled
									? "Máximo 3 embarcaciones seleccionadas"
									: `Seleccionar ${vessel.name}`
							}
						>
							{isSelected && (
							...
							<div
								className={`p-3 rounded-full mb-3 transition-colors ${isSelected ? "bg-accent/20 text-accent" : "bg-white/5 text-white/40 group-hover:text-white/60"}`}
							>
								<Sailboat size={24} />
							</div>

							<h3
								className={`text-sm font-bold uppercase tracking-wide mb-1 ${isSelected ? "text-accent" : "text-white"}`}
							>
								{vessel.name}
							</h3>
