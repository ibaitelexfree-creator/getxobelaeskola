...
import { AlertTriangle, Battery, Compass, Wifi, Wind } from "lucide-react";
...
export default function WindStation() {
	...
	const [currentReading, setCurrentReading] = useState<WindReading | null>(
		null,
	);
		...
		<div className="w-full max-w-6xl mx-auto space-y-6 animate-fade-in">
			<header className="flex justify-between items-center bg-white/5 p-6 rounded-2xl border border-white/10 backdrop-blur-md">
				...
				<div className="flex flex-col items-end gap-1 text-xs font-mono">
					<div
						className={`flex items-center gap-2 px-3 py-1 rounded-full border ${connected ? "bg-green-500/10 border-green-500/30 text-green-400" : "bg-red-500/10 border-red-500/30 text-red-400"}`}
					>
						<div
						...
						<>
							<div className="relative w-64 h-64 mb-8 flex items-center justify-center">
								...
								<div
									...
									style={{
										transform: `rotate(${currentReading.direction_deg}deg)`,
									}}
								>
								...
								<div className="relative z-10 text-center bg-nautical-black/90 backdrop-blur-md rounded-full w-32 h-32 flex flex-col items-center justify-center border border-white/10 shadow-2xl">
									<span className="text-5xl font-bold text-white tabular-nums tracking-tighter drop-shadow-lg">
										{currentReading.speed_knots.toFixed(1)}
									</span>
									<span className="text-[10px] text-cyan-400 uppercase tracking-[0.2em] font-bold mt-1">
										KNOTS
									</span>
								</div>
							</div>
							...
							<div className="grid grid-cols-2 gap-4 w-full">
								<div className="bg-white/5 rounded-xl p-4 text-center border border-white/5">
									...
									<span className="text-2xl font-mono text-white tabular-nums">
										{currentReading.direction_deg.toFixed(0)}°
									</span>
								</div>
								<div className="bg-white/5 rounded-xl p-4 text-center border border-white/5">
									<div className="flex items-center justify-center gap-2 mb-2">
										<Wind size={14} className="text-pink-400/70" />
										<span className="text-[10px] text-pink-400/70 uppercase tracking-widest font-bold">
											Racha Máx
										</span>
									</div>
									<span className="text-2xl font-mono text-pink-400 tabular-nums">
										{currentReading.gust_knots?.toFixed(1) || "-"}
									</span>
								</div>
							</div>
						</>
