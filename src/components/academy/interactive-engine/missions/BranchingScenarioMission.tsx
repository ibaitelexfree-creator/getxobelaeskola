...
import type { GraphMissionData, MissionOption } from "../types";
...
export const BranchingScenarioMission: React.FC<Props> = ({
	data,
	onComplete,
}) => {
	const {
	...
	const handleOptionSelect = async (option: MissionOption) => {
		...
		const isCorrect =
			option.is_correct !== undefined ? option.is_correct : true;
		...
		if (option.next_step_id) {
			...
			if (data.steps?.[option.next_step_id]) {
