...
export const TacticalScenarioMission: React.FC<Props> = ({
	data,
	onComplete,
}) => {
	const {
	...
	const handleFinish = useCallback(() => {
		...
		if (percentage >= requiredScore - 0.01) {
			// Float tolerance
			completeMission(finalScore);
			if (onComplete) onComplete(finalScore);
		} else {
			failMission(
				`Puntuación: ${Math.round(finalScore)}%. Requerido: ${requiredScore * 100}%`,
			);
		}
	...
	const handleOptionSelect = async (optionIdx: number) => {
		...
		setFeedback(
			isCorrect
				? currentScenario.feedback_ok || "¡Correcto!"
				: currentScenario.feedback_err || "Incorrecto",
	...
	if (!currentScenario)
		return (
			<div className="text-white/50 text-center p-8">Cargando escenario...</div>
		);
				...
				>
					...
					<div className="aspect-video bg-nautical-black/50 rounded-lg overflow-hidden border border-white/10 mb-6 relative group shadow-2xl">
						{currentScenario.imagen_url ? (
							...
							<div className="w-full h-full flex items-center justify-center bg-white/5">
								<span className="text-white/20 font-display italic">
									Sin imagen táctica
								</span>
							</div>
						)}
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
						{currentScenario.opciones?.map((opt: any, idx: number) => {
							...
							const _isCorectVisual = idx === currentScenario.correcta; // Only reveal if we want to show correct answer on fail, but let's keep it mystery for now or show on selection
