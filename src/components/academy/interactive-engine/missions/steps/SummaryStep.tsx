...
export const SummaryStep: React.FC<StepProps> = ({ step, onOptionSelect }) => {
    ...
    const isSuccess = (step.content.status === 'success') || !title?.toLowerCase().includes('error'); // heuristic or explicit

    return (
        <div className={`p-8 rounded-lg border-2 ${isSuccess ? 'border-green-500/30 bg-green-900/10' : 'border-red-500/30 bg-red-900/10'} text-center space-y-6`}>
            <div className="text-6xl mb-4">
                {isSuccess ? '🎉' : '⚠️'}
            </div>

            <h2 className={`text-3xl font-display italic ${isSuccess ? 'text-green-400' : 'text-red-400'}`}>
                {title}
            </h2>
