...
import { calculateNextState } from "@/lib/academy/mission-logic";
import { getUserProgress, saveUserProgress } from "@/lib/academy/missions";
import { createClient } from "@/lib/supabase/client";
import type { BranchingMissionData, MissionProgress } from "../types";
...
export default function BranchingMission({
	data,
	onComplete,
}: BranchingMissionProps) {
	const [currentStepId, setCurrentStepId] = useState<string>(
		data.initial_step_id,
	);
		...
		const loadProgress = async () => {
			...
			if (user && data.id) {
				...
				if (savedProgress) {
					...
					setCurrentStepId(
						savedProgress.current_step_id || data.initial_step_id,
					);
	...
	const handleOptionSelect = async (optionIndex: number) => {
		...
		} = calculateNextState(currentStep, optionIndex);
		...
		return (
			<div className="p-8 text-center border border-red-500/30 rounded bg-red-900/10">
				...
				<p className="text-white/60">
					No se encontró el paso actual ({currentStepId})
				</p>
				<button
