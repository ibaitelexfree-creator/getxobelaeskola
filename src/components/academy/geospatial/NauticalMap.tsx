...
const NauticalMap: React.FC = () => {
	...
	useEffect(() => {
		...
		L.marker([NAUTICAL_CLUB.lat, NAUTICAL_CLUB.lng], { icon: clubIcon })
			.addTo(map)
			.bindPopup(
				`<b>${NAUTICAL_CLUB.name}</b><br>${NAUTICAL_CLUB.description}`,
			);
		...
		L.polygon(DANGER_ZONE, {
			...
			.addTo(map)
			.bindPopup(
				'<div style="color:red;font-weight:bold">ZONA DE PELIGRO</div>Atención: Bajos rocosos.',
			);
	...
	return (
		<div className="relative w-full h-full rounded-3xl overflow-hidden border border-white/10 shadow-2xl">
			...
			<div className="absolute bottom-4 right-4 bg-white/90 p-4 rounded-xl shadow-lg z-[1000] text-black text-xs space-y-2 pointer-events-none">
				<h4 className="font-bold mb-2 uppercase tracking-wider text-slate-500">
					Leyenda
				</h4>
