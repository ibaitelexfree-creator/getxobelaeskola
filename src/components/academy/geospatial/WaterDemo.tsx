...
import { Waves, MapPin, AlertTriangle, CheckCircle } from 'lucide-react';
...
export const WaterDemo: React.FC = () => {
    ...
    const testPositions = [
        ...
        { name: 'Santurtzi', lat: 43.3300, lng: -3.0300 },
        { name: 'Centro Abra', lat: 43.3550, lng: -3.0500 },
        { name: 'Zierbena Outer', lat: 43.3750, lng: -3.0900 },
        { name: 'Tierra (Algorta)', lat: 43.3550, lng: -3.0050 },
        { name: 'Tierra (Portugalete)', lat: 43.3200, lng: -3.0200 },
    ];
    ...
    const handleRandomize = () => {
        const rand = testPositions[Math.floor(Math.random() * testPositions.length)];
            ...
            <div className="space-y-4 mb-8">
                <div className="bg-slate-800/50 p-4 rounded-2xl border border-slate-700">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="text-[10px] text-slate-500 uppercase tracking-wider">Latitud</label>
                            <div className="font-mono text-blue-300">{coords.lat.toFixed(6)}</div>
                        </div>
                        <div>
                            <label className="text-[10px] text-slate-500 uppercase tracking-wider">Longitud</label>
                            <div className="font-mono text-blue-300">{coords.lng.toFixed(6)}</div>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex justify-center">
                    <motion.div
                        ...
                        className={`w-full py-8 rounded-3xl flex flex-col items-center gap-2 transition-colors duration-500 ${inWater === true
                    ...
                    >
                        {inWater === true ? (
                            <>
                                ...
                                <span className="text-2xl font-black text-blue-400 uppercase tracking-tighter">ESTÁS EN AGUA</span>
                                <span className="text-xs text-blue-300/70">Navegación segura permitida</span>
                            </>
                        ) : inWater === false ? (
                            <>
                                <AlertTriangle className="text-amber-500 w-12 h-12 mb-2" />
                                <span className="text-2xl font-black text-amber-500 uppercase tracking-tighter">ESTÁS EN TIERRA</span>
                                <span className="text-xs text-amber-500/70">¡Peligro! Encallamiento detectado</span>
                            </>
                        ) : (
                            <span className="text-slate-500 italic">Analizando superficie...</span>
                        )}
                    </motion.div>
                </div>
            </div>
