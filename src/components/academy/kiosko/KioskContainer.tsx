...
export default function KioskContainer({ locale }: KioskContainerProps) {
	...
	const slides = [
		...
		{
			id: "nomenclature",
			component: <KioskNomenclatureSlide locale={locale} />,
		},
	];
	...
	return (
		<div className="w-full h-full min-h-screen overflow-hidden bg-nautical-black relative">
			...
			<div
				className="absolute top-0 bottom-0 left-0 w-32 z-40 cursor-w-resize"
				onClick={handlePrev}
			/>
			<div
				className="absolute top-0 bottom-0 right-0 w-32 z-40 cursor-e-resize"
				onClick={handleNext}
			/>
		</div>
	);
