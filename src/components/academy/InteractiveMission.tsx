...
export default function InteractiveMission({ data, onComplete }: InteractiveMissionProps) {
...
function InventoryMission({ data, onComplete }: { data: any, onComplete?: () => void }) {
    ...
    return (
        <div className="bg-premium-mesh p-8 border-2 border-accent/40 rounded-sm">
            ...
            {found.length === items.length && (
                <div className="mt-6 text-accent animate-pulse">¡Inventario Completo!</div>
            )}
        </div>
    );
}

function TacticMission({ data, onComplete }: { data: any, onComplete?: () => void }) {
    ...
    return (
        <div className="bg-nautical-black p-8 border-2 border-red-500/40 rounded-sm relative overflow-hidden">
            ...
            <p className="text-red-400 text-3xs uppercase font-bold tracking-widest mb-8">ESCENARIO {current + 1} DE {scenarios.length}</p>

            <div className="glass-panel p-6 mb-8 border-white/10">
                <p className="text-xl text-white mb-6 font-display italic">{currentScenario.pregunta}</p>
                ...
                {selected !== null && (
                    <div className={`mt-6 p-4 text-sm ${selected === currentScenario.correcta ? 'text-accent' : 'text-red-400'}`}>
    ...
    return (
        <div className="bg-gradient-to-br from-blue-900/40 to-black p-8 border-2 border-blue-500/40 rounded-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {nudos.map((nudo: any) => (
                    <div key={nudo.id} className="glass-card p-6 border-white/5 flex flex-col justify-between">
                        <div>
                            <h4 className="text-lg font-display italic text-accent mb-2">{nudo.nombre}</h4>
