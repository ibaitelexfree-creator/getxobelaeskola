		...
		>
			<div className="flex justify-between items-start gap-4">
				...
				<div className="flex flex-col items-end gap-2 text-[10px] uppercase tracking-widest font-bold">
					<div
						className={`px-2 py-1 rounded border ${question.votos > 0 ? "text-accent border-accent/20 bg-accent/5" : "text-white/30 border-white/10"}`}
					>
						{question.votos} Votos
					</div>
					<div
						className={`px-2 py-1 rounded border ${question.respuestas_count > 0 ? "text-white/70 border-white/20" : "text-white/30 border-white/10"}`}
					>
