...
export default function ForumMain({ moduloId }: ForumMainProps) {
	...
	const fetchQuestions = async () => {
		...
		try {
			const res = await fetch(
				apiUrl(`/api/forum/questions?modulo_id=${moduloId}`),
			);
	...
	const handleAskSuccess = (_newQuestion: any) => {
		fetchQuestions();
		setView("list");
	};
	...
	const handleDeleteQuestion = async () => {
		...
		try {
			const res = await fetch(
				apiUrl(`/api/forum/questions/${selectedQuestion.id}`),
				{
					method: "DELETE",
				},
			);
		...
		<section className="container mx-auto px-6 py-20 border-t border-white/10">
			<div className="max-w-4xl mx-auto">
				<header className="flex justify-between items-end mb-10">
					<div>
						<span className="text-accent uppercase tracking-[0.4em] text-[10px] font-black block mb-4">
							Comunidad
						</span>
						<h2 className="text-4xl font-display italic text-white">
							Foro del Módulo
						</h2>
					</div>
					{view === "list" && (
				...
				{loading ? (
					<div className="text-center py-20 text-white/20 animate-pulse">
						Cargando foro...
					</div>
				) : (
					<>
