...
export default function QuestionDetail({
	...
	const handleAnswerSuccess = (_newAnswer: any) => {
	...
	const handleMarkCorrect = async (answerId: string) => {
		try {
			const res = await fetch(
				apiUrl(`/api/forum/answers/${answerId}/mark-correct`),
				{
					method: "POST",
				},
			);
	...
	return (
		<div className="animate-fade-in">
			...
			<div className="glass-card p-8 border-accent/20 mb-8 relative overflow-hidden">
				<div className="absolute top-0 right-0 p-4 opacity-10 text-9xl leading-none font-display italic text-accent pointer-events-none">
					?
				</div>
				...
				<div className="flex gap-6 relative z-10">
					...
					<div className="flex-1">
						<header className="mb-6">
							<h2 className="text-3xl font-display italic text-white mb-3 leading-tight">
								{question.titulo}
							</h2>
							<div className="flex items-center gap-3 text-xs text-white/50 font-medium">
								<div className="flex items-center gap-2">
									...
									<span className="text-white/80">
										{question.profiles?.nombre} {question.profiles?.apellidos}
									</span>
								</div>
								<span>•</span>
								<span>
