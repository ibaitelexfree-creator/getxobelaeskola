...
export class BoatPhysics {
	...
	public getPolarTargetSpeed(twsKnots: number, twaDeg: number): number {
		...
		const polarData = [
			...
			{ angle: 52, factor: 0.7 }, // Optimal Upwind
		...
		if (targetSpeed > maxDisplacementSpeed) {
			...
			if (isSurfing) {
				// Allow more speed
				targetSpeed =
					maxDisplacementSpeed + (targetSpeed - maxDisplacementSpeed) * 0.6;
			} else {
				// Harder cap
				targetSpeed =
					maxDisplacementSpeed + (targetSpeed - maxDisplacementSpeed) * 0.2;
			}
		}
	...
	public update(
		...
		const ironsFactor = MathUtils.smoothstep(
			awa,
			ironsThreshold - 0.1,
			ironsThreshold,
		);
		...
		const maxDriveForce =
			this.HULL_DRAG_COEFF * (targetSpeedMs * targetSpeedMs);
		...
		const hullDrag =
			currentSpeed * Math.abs(currentSpeed) * this.HULL_DRAG_COEFF;
