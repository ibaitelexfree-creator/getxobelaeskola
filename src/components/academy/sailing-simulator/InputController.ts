...
>>>>>>> pr-286
        ...
        this.rudderAngle = Math.max(-this.MAX_RUDDER, Math.min(this.MAX_RUDDER, this.rudderAngle));
        ...
        if (this.keys['KeyW'] || this.keys['ArrowUp'] || this.keys['w'] || this.keys['W']) {
            this.sailAngle -= this.SAIL_SPEED * dt;
        } else if (this.keys['KeyS'] || this.keys['ArrowDown'] || this.keys['s'] || this.keys['S']) {
            this.sailAngle += this.SAIL_SPEED * dt;
        }
        this.sailAngle = Math.max(-this.MAX_SAIL, Math.min(this.MAX_SAIL, this.sailAngle));
    }
