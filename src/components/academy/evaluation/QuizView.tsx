				...
				<div className="grid grid-cols-2 gap-4 pt-4">
					{options.map((opt) => {
						...
						return (
							...
							>
								<span
									className={`text-lg font-bold ${isSelected ? "text-primary" : theme.optionTextIdle}`}
								>
									{opt.texto}
								</span>
								{isSelected && (
					...
					<div className={`p-10 rounded-2xl border ${theme.card} space-y-8`}>
						<div className="space-y-4">
							<div className="flex items-center justify-between">
								...
								<span
									className={`text-3xs font-bold ${isNautical ? "text-white/50" : "text-muted-foreground"}`}
								>
									{question.puntos} {question.puntos === 1 ? "punto" : "puntos"}
								</span>
							</div>
							<h3
								className={`text-2xl font-semibold leading-tight tracking-tight ${theme.text}`}
							>
