	...
	React.useEffect(() => {
		const handleKeyDown = (e: KeyboardEvent) => {
			...
			if (["1", "2", "3", "4"].includes(e.key)) {
				...
				const options =
					question.tipo_pregunta === "verdadero_falso"
					...
					{options.map((opt) => {
						...
						return (
							<button
								...
								className={`group relative flex flex-col items-center justify-center p-8 border-2 rounded-xl transition-all focus:outline-none focus:ring-2 focus:ring-primary/20 disabled:opacity-50 ${
									isSelected
				...
				{question.opciones_json?.map((option, idx) => {
					...
					return (
						<button
							...
							disabled={isSubmitting}
							className={`group relative flex items-center w-full p-5 text-left border-2 rounded-xl transition-all focus:outline-none focus:ring-2 focus:ring-primary/20 disabled:opacity-50 ${
								isSelected
									? "border-primary bg-primary/10 ring-2 ring-primary/20"
									: "border-border hover:border-primary/50 hover:bg-primary/5"
							}`}
						>
							<span
								className={`flex-shrink-0 w-10 h-10 flex items-center justify-center rounded-lg text-base font-bold mr-5 transition-colors ${
									isSelected
							...
							<span
								className={`text-base font-medium transition-colors ${
									isSelected
										? "text-foreground"
										: "text-white/90 group-hover:text-foreground"
								}`}
							>
								{option.texto}
							</span>
			...
			<div className="w-full max-w-2xl mx-auto p-12 bg-white/5 backdrop-blur-xl rounded-3xl border border-white/10 shadow-2xl text-center space-y-8 animate-in zoom-in-95 duration-300">
				...
				<div className="space-y-4">
					...
					<p className="text-slate-400">
						Has respondido {Object.keys(answers).length} de {totalQuestions}{" "}
						preguntas. Una vez enviado no podrás cambiar tus respuestas.
					</p>
				</div>
	...
	return (
		<div className="w-full max-w-4xl mx-auto space-y-8 pb-12">
			...
			<div className="flex flex-col md:flex-row items-center gap-6 mb-4">
				...
				<div className="flex-1 w-full space-y-3">
					<div className="flex items-center justify-between text-2xs uppercase tracking-[0.2em] font-bold text-white/60">
						<div className="flex items-center gap-3">
							<span>
								Progreso <span className="text-white/30">|</span>{" "}
								{currentQuestionIndex + 1} de {totalQuestions}
							</span>
							{lastSaved && (
								<span className="text-3xs text-green-500/60 font-mono animate-pulse">
									✓ Guardado
								</span>
							)}
						</div>
				...
				{timeLeft !== undefined && (
					<div
						className={`flex items-center gap-4 px-6 py-3 rounded-2xl border backdrop-blur-md transition-all duration-500 ${
							isUrgent
					...
					>
						<div className="relative w-8 h-8">
							<svg className="w-full h-full -rotate-90" viewBox="0 0 36 36">
								...
								<circle
									...
									strokeDasharray="100"
									strokeDashoffset={
										100 - (timeLeft / (durationSecs || 1)) * 100
									}
									strokeLinecap="round"
								/>
							</svg>
						...
						<div className="flex flex-col">
							<span className="text-3xs uppercase tracking-widest text-white/60 font-bold">
								Tiempo
							</span>
							<span
								className={`text-xl font-mono font-bold tabular-nums ${isUrgent ? "text-red-400" : "text-white"}`}
							>
								{formatTime(timeLeft)}
							</span>
						</div>
			...
			<div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
				...
				<div className="hidden lg:block space-y-6">
					<div className="bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 p-6">
						<h4 className="text-3xs uppercase tracking-widest font-black text-white/60 mb-6">
							Mapa de Examen
						</h4>
							...
							{allQuestionIds.map((qId, idx) => {
								...
								return (
									<button
										...
										className={`w-10 h-10 rounded-lg text-2xs font-bold transition-all flex items-center justify-center border ${
											isCurrent
					...
					<div className="bg-card/40 backdrop-blur-md p-10 rounded-2xl border border-white/10 shadow-2xl space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
						<div className="space-y-4">
							<div className="flex items-center justify-between">
								...
								<span className="text-3xs font-bold text-white/50">
