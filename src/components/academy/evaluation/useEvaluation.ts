...
export function useEvaluation({
	...
	const submitEvaluation = useCallback(async () => {
		...
		try {
			...
			const details: DetailedResult[] = state.questions.map((q) => {
				const correctInfo = data.respuestas_correctas?.find(
					(rc: any) => rc.id === q.id,
				);
	...
	}, [state.status, submitEvaluation, state.timeLeft]);
		...
		const handleBeforeUnload = () => {
			if (
				...
				navigator.sendBeacon(
					apiUrl("/api/academy/evaluation/autosave"),
		...
		try {
			...
			if (data.allowed === true && data.preguntas) {
				...
				if (wasResumed && Object.keys(savedAnswers).length > 0) {
					...
					startIndex =
						firstUnanswered >= 0 ? firstUnanswered : questions.length - 1;
				}
				...
				setState((prev) => ({
					...
					blockInfo: undefined,
					timeLeft,
					durationSecs: data.evaluacion.tiempo_limite_min
						? data.evaluacion.tiempo_limite_min * 60
						: undefined,
				}));
