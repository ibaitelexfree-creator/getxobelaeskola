...
export default function SimpleEvaluation({
	...
	titulo,
	onComplete,
}: SimpleEvaluationProps) {
	const [realEvaluacionId, setRealEvaluacionId] = useState<string | null>(
		evaluacionId || null,
	);
		...
		async function buscarEvaluacion() {
			...
			try {
				const res = await fetch(
					apiUrl(
						`/api/academy/evaluaciones?entidad_tipo=${entidadTipo}&entidad_id=${entidadId}`,
					),
				);
		...
		return (
			<div className="max-w-2xl mx-auto p-8 bg-card rounded-lg border border-border shadow-lg">
				...
				<p className="text-muted-foreground mb-6">
					Estás a punto de comenzar la evaluación. Asegúrate de tener suficiente
				...
				{isResumed && (
					<div className="mb-4 p-3 bg-blue-500/10 border border-blue-500/30 rounded-lg flex items-center gap-3 text-sm animate-in slide-in-from-top duration-500">
						...
						<span className="text-blue-300">
							Se ha reanudado tu intento anterior. Tienes{" "}
							{Object.keys(state.answers).length} respuesta(s) guardada(s).
						</span>
					</div>
				)}
