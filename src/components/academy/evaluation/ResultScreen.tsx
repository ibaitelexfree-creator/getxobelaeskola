 ...
 */
const CountingNumber = ({ value, duration = 2000 }: { value: number, duration?: number }) => {
    ...
    return (
        <>
            ...
            <div className="flex flex-col items-center justify-center p-6 md:p-12 space-y-10 animate-in zoom-in-95 fade-in duration-700 max-w-4xl mx-auto text-center bg-white/5 backdrop-blur-xl rounded-3xl border border-white/10 shadow-2xl relative overflow-hidden">
                ...
                {/* Background Glow */}
                <div className={`absolute top-0 left-1/2 -translate-x-1/2 w-64 h-64 blur-[100px] pointer-events-none opacity-20 ${isPassing ? 'bg-green-500' : 'bg-red-500'}`} />

                <div className="relative z-10 w-full flex flex-col items-center space-y-10">
                    {/* Status Decoration */}
                    <div className={`relative w-32 h-32 rounded-full flex items-center justify-center shadow-[0_0_50px_-12px_rgba(0,0,0,0.5)] ${isPassing
                        ? 'bg-green-500/10 ring-1 ring-green-500/30 shadow-green-500/10'
                        : 'bg-red-500/10 ring-1 ring-red-500/30 shadow-red-500/10'
                        }`}>
                        <div className="relative w-20 h-20">
                            <Image
                                src={isPassing ? "/images/icon-result-success.svg" : "/images/icon-result-fail.svg"}
                                alt={isPassing ? "Aprobado" : "Suspenso"}
                                fill
                                className="object-contain"
                            />
                        </div>
                        <div className={`absolute inset-[-8px] border border-dashed rounded-full animate-[spin_15s_linear_infinite] opacity-20 ${isPassing ? 'border-green-500' : 'border-red-500'}`} />
                    </div>

                    {/* Score Display */}
                    <div className="space-y-4">
                        <h2 className={`text-3xl md:text-4xl font-black uppercase tracking-tighter ${isPassing ? 'text-green-400' : 'text-red-400'}`}>
                                    ...
                                    {result.details.map((detail, idx) => (
                                        <div key={idx} className={`p-5 rounded-2xl border backdrop-blur-sm ${detail.isCorrect ? 'bg-green-500/5 border-green-500/20' : 'bg-red-500/5 border-red-500/20'
                                            ...
                                            <div className="flex items-start gap-4">
                                                <div className={`mt-1 flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center text-3xs font-bold ${detail.isCorrect ? 'bg-green-500 text-green-950' : 'bg-red-500 text-red-950'
                                                    }`}>
                                                    {idx + 1}
                                                </div>
                                                <div className="flex-1 space-y-3">
                                                    ...
                                                    {detail.explicacion_es && (
                                                        <div className="mt-4 p-4 bg-white/5 rounded-xl border border-white/5 italic text-2xs text-slate-400 leading-relaxed">
                                                            <span className="block not-italic font-bold text-blue-400 mb-1 tracking-widest uppercase text-3xs">💡 Nota técnica</span>
                                                            "{detail.explicacion_es}"
                                                        </div>
                                                    )}
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 w-full pt-4">
                        {!isPassing && (
                            <button
                                ...
                                className={`flex flex-1 items-center justify-center gap-3 py-4 px-6 rounded-xl font-bold text-2xs uppercase tracking-widest transition-all ${maxAttemptsExceeded || cooldownActive
                                    ? 'bg-slate-800 text-slate-500 cursor-not-allowed border border-white/5'
                                    : 'bg-white text-black hover:bg-slate-200 shadow-[0_0_20px_rgba(255,255,255,0.1)] active:scale-95'
                                    }`}
                            >
                                <span>{maxAttemptsExceeded ? 'Intentos Agotados' : cooldownActive ? 'Cooldown Activo' : 'Reintentar Evaluación'}</span>
                                {cooldownActive && <span className="w-2 h-2 rounded-full bg-blue-500 animate-ping" />}
                            </button>
                        )}
