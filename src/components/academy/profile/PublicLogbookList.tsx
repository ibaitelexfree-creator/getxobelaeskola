        ...
        <section className="mb-12">
            ...
            <div className="space-y-4">
                {logbook.map((entry: any) => (
                    <div key={entry.id} className="bg-white/5 border border-white/10 rounded-2xl p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 hover:bg-white/[0.07] transition-colors">
                        <div className="flex items-center gap-6">
                            ...
                            <div>
                                <h3 className="font-bold text-white text-lg mb-1">{entry.tipo?.replace('_', ' ').toUpperCase() || 'NAVEGACIÓN'}</h3>
                                <div className="flex flex-wrap items-center gap-4 text-xs text-white/40">
                                    <span className="flex items-center gap-1.5"><Calendar size={12} /> {new Date(entry.fecha).toLocaleDateString()}</span>
                                    {entry.embarcacion && <span className="hidden md:inline">•</span>}
                                    {entry.embarcacion && <span className="flex items-center gap-1.5">{entry.embarcacion}</span>}
                                </div>
                            </div>
                        </div>
                        <div className="flex items-center gap-3 bg-black/20 px-5 py-3 rounded-xl border border-white/5 self-start md:self-auto">
                            ...
                            <span className="font-bold text-white text-sm">{entry.duracion_h} h</span>
                        </div>
                    </div>
                ))}
            </div>
