...
import { Play, Pause, SkipBack, SkipForward, Wind, RotateCcw, Anchor, Settings2 } from 'lucide-react';
...
export default function SailingManeuverPlayer({ className = '' }: SailingManeuverPlayerProps) {
  const [selectedManeuverId, setSelectedManeuverId] = useState<string>(MANEUVERS[0].id);
  const [currentStepIndex, setCurrentStepIndex] = useState(0);
  const [isPlaying, setIsPlaying] = useState(false);
  const [speed, setSpeed] = useState(1); // 0.5, 1, 2

  const maneuver = MANEUVERS.find((m) => m.id === selectedManeuverId) || MANEUVERS[0];
  ...
  return (
    <div className={`bg-white dark:bg-slate-900 rounded-xl shadow-lg overflow-hidden border border-slate-200 dark:border-slate-700 ${className}`}>
      {/* Header */}
        ...
        <div className="w-full md:w-2/3 bg-blue-50 dark:bg-slate-900 relative aspect-square md:aspect-[4/3] overflow-hidden">
          ...
          <svg viewBox="0 0 100 100" className="w-full h-full">
            ...
            <g transform="translate(90, 10)">
              <text x="0" y="8" textAnchor="middle" fontSize="3" className="fill-slate-500 font-bold select-none">WIND</text>
              <motion.g
                initial={false}
                animate={{ rotate: currentStep.wind?.angle || 0 }}
                transition={{ duration: 1 / speed }}
              >
                <circle cx="0" cy="0" r="4" fill="none" stroke="currentColor" className="text-slate-400" strokeWidth="0.5" />
                <path d="M0 -3 L0 3 M-2 1 L0 3 L2 1" fill="none" stroke="currentColor" className="text-blue-600" strokeWidth="1" />
              </motion.g>
            </g>
            ...
            {currentStep.extraElements?.map((el, i) => (
              ...
              >
                ...
                {el.type === 'person' && (
                  <circle r="1.5" fill={el.color || 'orange'} stroke="white" strokeWidth="0.5" />
                )}
                {el.type === 'buoy' && (
            ...
            >
              ...
              <ellipse
                cx="0" cy="0"
                rx={boatWidth / 2} ry={boatLength / 2}
                fill="white"
                stroke="#334155"
                strokeWidth="0.5"
              />
              ...
              >
                {/* Boom */}
                <line x1="0" y1="0" x2="0" y2={boatLength / 2 + 1} stroke="black" strokeWidth="0.5" strokeLinecap="round" />
                {/* Sail Fabric */}
                <path d={`M0 0 L0 ${boatLength/2} Q 2 ${boatLength/4} 0 0`} fill="#e2e8f0" fillOpacity="0.8" />
              </motion.g>
              ...
              >
                 {/* Jib Line */}
                <line x1="0" y1={-boatLength / 2} x2="0" y2="0" stroke="transparent" /> {/* Invisible guide */}
                <path d={`M0 ${-boatLength/2} L 0 0 Q 1 ${-boatLength/4} 0 ${-boatLength/2}`} fill="#e2e8f0" fillOpacity="0.8" stroke="black" strokeWidth="0.2" />
              </motion.g>
            </motion.g>
          </svg>
        ...
        <div className="w-full md:w-1/3 p-6 flex flex-col justify-between border-l border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800">
          <div className="space-y-4">
            <div className="flex items-center justify-between text-sm text-slate-500 dark:text-slate-400">
              ...
              <span className="font-mono">{Math.round(((currentStepIndex + 1) / totalSteps) * 100)}%</span>
            </div>
