...
export default function BadgeCard({ badge, locale }: BadgeCardProps) {
	...
	const description =
		locale === "eu" ? badge.descripcion_eu : badge.descripcion_es;
	...
	return (
		...
		>
			...
			<div className="text-center space-y-2">
				<h3
					className={`font-bold text-lg ${isUnlocked ? "text-white" : "text-white/40"}`}
				>
					{name}
				</h3>
			...
			<div className="mt-4 pt-4 border-t border-white/5 w-full flex justify-between items-center text-[10px] uppercase font-bold tracking-widest">
				<span
					className={`px-2 py-1 rounded-full border ${isUnlocked ? borderColor.split(" ")[0] : "border-white/10"}`}
				>
					{badge.rareza}
				</span>
				<span className="text-accent/80">{badge.puntos} PTS</span>
			</div>
