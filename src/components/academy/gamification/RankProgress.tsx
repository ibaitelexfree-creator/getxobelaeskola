...
import { getNextRank, type Rank } from "@/lib/gamification/ranks";
		...
		<div className="bg-white/5 border border-white/10 rounded-xl p-6 relative overflow-hidden group hover:border-accent/30 transition-all duration-300">
			...
			<div className="relative z-10 flex flex-col md:flex-row items-center gap-6">
				...
				<div className="flex-shrink-0">
					<div
						className={`w-20 h-20 md:w-24 md:h-24 rounded-full border border-white/10 flex items-center justify-center bg-nautical-black relative overflow-hidden shadow-lg shadow-black/50`}
					>
						{isImage ? (
							<Image
						...
						<div
							className={`absolute inset-0 bg-accent/10 blur-[20px] opacity-50`}
						/>
					</div>
				</div>
				...
				<div className="flex-1 w-full text-center md:text-left">
					<div className="flex flex-col md:flex-row justify-between items-end mb-2 gap-2">
						<div>
							<div
								className={`text-2xs font-black uppercase tracking-[0.2em] mb-1 ${currentRank.color}`}
							>
								Rango Actual
							</div>
						...
						<div className="text-right">
							...
							<div className="text-xl font-bold text-accent font-mono">
								...
								<span className="text-2xs text-white/40">XP</span>
							</div>
						</div>
					</div>
