...
import { Search, X, BookOpen, ExternalLink, Anchor } from 'lucide-react';
import Link from 'next/link';
import { useTranslations } from 'next-intl';
import { GlossaryTerm, GLOSSARY_TERMS, GlossaryCategory } from '@/data/glossary';

const CATEGORIES: GlossaryCategory[] = ['Maniobras', 'Partes del barco', 'Meteorología', 'Reglamento'];

const CATEGORY_COLORS: Record<GlossaryCategory, string> = {
  'Maniobras': 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20 hover:bg-emerald-500/20',
  'Partes del barco': 'bg-amber-500/10 text-amber-400 border-amber-500/20 hover:bg-amber-500/20',
  'Meteorología': 'bg-cyan-500/10 text-cyan-400 border-cyan-500/20 hover:bg-cyan-500/20',
  'Reglamento': 'bg-red-500/10 text-red-400 border-red-500/20 hover:bg-red-500/20',
};
    ...
    <div className="space-y-8 animate-fade-in">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-medium transition-all border ${
                selectedCategory === category
                  ? CATEGORY_COLORS[category].replace('bg-', 'bg-opacity-100 bg-').replace('text-', 'text-nautical-black text-').replace('border-', 'border-transparent border-')
                  : 'bg-white/5 text-white/60 border-white/10 hover:bg-white/10 hover:border-white/20'
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {results.length > 0 ? (
          ...
          <div className="col-span-full py-20 text-center">
            ...
            <h3 className="text-xl font-display text-white mb-2">{t('no_results')}</h3>
      ...
      <div className="text-center pt-8 border-t border-white/5 mt-12">
        <p className="text-white/30 text-xs">
          {t('showing_results', { count: results.length, total: GLOSSARY_TERMS.length })}
        </p>
      </div>
    </div>
  );
