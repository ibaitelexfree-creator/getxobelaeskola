...
import { X, Check, HelpCircle, BookOpen, RotateCcw } from 'lucide-react';
...
export default function InfoOverlay({ selectedId, hoveredId, onClose }: InfoOverlayProps) {
    ...
    const selectedPart = selectedId ? BOAT_PARTS.find(p => p.id === selectedId) : null;
    const hoveredPart = hoveredId ? BOAT_PARTS.find(p => p.id === hoveredId) : null;
        ...
        <>
            ...
            <AnimatePresence>
                {hoveredPart && !selectedPart && (
                    ...
                    >
                        <span className="font-bold text-sm tracking-widest uppercase">{hoveredPart.name}</span>
                    </motion.div>
                )}
            </AnimatePresence>
                    ...
                    >
                        ...
                        <div className="flex justify-between items-start mb-6">
                            <div>
                                ...
                                <h2 className="text-3xl font-display text-white">{selectedPart.name}</h2>
                            </div>
                            <button
                                onClick={onClose}
                            ...
                            {!quizMode ? (
                                    ...
                                    <div className="space-y-6">
                                        <div className="bg-blue-950/30 p-4 rounded-xl border border-blue-500/20">
                                            <div className="flex items-center gap-2 mb-2 text-blue-300">
                                                ...
                                                <h3 className="text-xs font-bold uppercase tracking-widest">Definición</h3>
                                            </div>
                                        ...
                                        <div className="bg-emerald-950/30 p-4 rounded-xl border border-emerald-500/20">
                                            <div className="flex items-center gap-2 mb-2 text-emerald-300">
                                                <RotateCcw className="w-4 h-4" /> {/* Icon representing function/action */}
                                                <h3 className="text-xs font-bold uppercase tracking-widest">Función</h3>
                                            </div>
                                            <p className="text-white/80 leading-relaxed font-light">
                                ...
                                >
                                    <div className="mb-6">
                                        ...
                                        <div className="space-y-3">
                                            {selectedPart.quiz.options.map((option, index) => {
                                                ...
                                                let btnClass = "w-full p-4 text-left rounded-xl border transition-all relative overflow-hidden ";

                                                if (!isSelected) {
                                                    btnClass += "bg-white/5 border-white/10 hover:bg-white/10 hover:border-white/20 text-white";
                                                } else {
                                                    if (isCorrect) {
                                                        btnClass += "bg-green-500/20 border-green-500 text-green-300";
                                                    } else if (quizResult === 'wrong' && !isCorrect) {
                                                        // Dim incorrect options
                                                        btnClass += "opacity-50 bg-white/5 border-transparent text-white/40";
                                                    }
                                                }
                                                ...
                                                return (
                                                    <button
                                                        key={index}
                                                        onClick={() => !isSelected && handleQuizOption(index)}
                                                        className={btnClass}
                                                        disabled={isSelected}
                                                    >
                                    ...
                                    {quizResult === 'correct' && (
                                        ...
                                        >
                                            <p className="text-green-300 font-bold mb-2">¡Correcto!</p>
                                            <button onClick={onClose} className="text-xs underline text-green-200/60 hover:text-green-200">
                                                Explorar otra parte
                                            </button>
                                        </motion.div>
                                    )}
