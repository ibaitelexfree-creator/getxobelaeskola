...
const Nomenclature3DView = dynamic(() => import("./Nomenclature3DView"), {
	ssr: false,
});
...
export default function NomenclatureLesson({
	locale,
}: NomenclatureLessonProps) {
	const [activeCategory, setActiveCategory] = useState<string>("all");
	const [_difficulty, _setDifficulty] = useState<"beginner" | "master">(
		"beginner",
	);
	const [learningMode, setLearningMode] = useState<"learn" | "practice">(
		"learn",
	);
			...
			{isAllMatched && learningMode === "practice" && (
				<div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
					<div className="bg-white p-8 rounded-lg shadow-2xl text-center max-w-sm">
						<h2 className="text-2xl font-bold text-nautical-black mb-2">
							¡Perfecto!
						</h2>
						<p className="text-gray-600 mb-6 font-display">
							Has identificado correctamente todas las partes.
						</p>
						<button
							onClick={() => {
								...
								let filtered = NAUTICAL_TERMS;
								if (activeCategory !== "all") {
									filtered = NAUTICAL_TERMS.filter(
										(c) => c.category === activeCategory,
									);
								}
								const shuffledDeck = [...filtered].sort(
									() => 0.5 - Math.random(),
								);
								const finalDeck = shuffledDeck.slice(0, 8);
								setDeck(finalDeck);
			...
			{learningMode === "learn" ? (
				<div className="w-full flex-grow flex flex-col items-center justify-center p-4">
					...
					<div className="mt-8 text-center text-white/50 text-sm max-w-2xl">
						<p>
							Interactúa con el modelo 3D para descubrir las partes de la
							embarcación.
						</p>
						<p>
							Haz click y arrastra para rotar. Usa la rueda del ratón para hacer
							zoom.
						</p>
					</div>
				</div>
										...
										>
											...
											{card.svgPath && (
												<path
													...
													stroke={
														matchedPairs.includes(card.id) ? "#166534" : "none"
													}
													strokeWidth="2"
												/>
											)}
