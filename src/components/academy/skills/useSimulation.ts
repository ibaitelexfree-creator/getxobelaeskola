...
import type { EvaluationResult, Question } from "../evaluation/types";
...
export function useSimulation(): UseSimulationReturn {
	const [status, setStatus] = useState<
		"idle" | "loading" | "active" | "submitting" | "complete" | "error"
	>("idle");
	...
	const submitSimulation = useCallback(async () => {
		...
		try {
			...
			setResult({
				...
				feedback: data.aprobado
					? "¡Enhorabuena! Has superado el examen simulado."
					: "No has alcanzado la puntuación necesaria. ¡Sigue practicando!",
				details: details,
			});
