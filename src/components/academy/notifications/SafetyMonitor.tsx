...
export default function SafetyMonitor() {
	...
	const { notificationsEnabled, soundEnabled, addAlertToHistory } =
	...
	const triggerSafetyAlert = useCallback(
		(title: string, message: string, color: "critical" | "warning") => {
		...
		const checkSafety = async () => {
			try {
				...
				if (alerts.length > 0) {
					...
					if (alertKey !== lastAlertId) {
						triggerSafetyAlert(
							...
							latestAlert.text ||
								"Evento meteorológico detectado por Euskalmet.",
							["Roja", "Red", "Naranja", "Orange"].includes(latestAlert.level)
								? "critical"
								: "warning",
						);
				...
				const isInWater =
					...
					statusMessage === "Navegación Detectada";
