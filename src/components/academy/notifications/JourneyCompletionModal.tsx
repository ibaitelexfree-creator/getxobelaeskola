...
import { Book, CheckCircle2, ChevronRight, X, Sparkles, AlertCircle, Smile } from 'lucide-react';
...
export default function JourneyCompletionModal({ isOpen, onClose, onSaveSuccess }: JourneyCompletionModalProps) {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (res.ok) {
                showMessage('¡Excelente!', 'Tu reflexión ha sido guardada en la bitácora.', 'success');
                onSaveSuccess();
                onClose();
            } else {
                showMessage('Error', 'No pudimos guardar tu diario. Inténtalo de nuevo.', 'error');
            }
        } catch (error) {
                ...
                <div className="fixed inset-0 z-[10000] flex items-center justify-center p-6 bg-black/80 backdrop-blur-md">
                    ...
                    >
                                ...
                                >
                                    <header>
                                        <div className="inline-flex items-center gap-2 px-3 py-1 bg-accent/10 rounded-full border border-accent/20 mb-4">
                                            ...
                                            <span className="text-[10px] font-black uppercase tracking-widest text-accent">Travesía Finalizada</span>
                                        </div>
                                        <h2 className="text-4xl font-display italic text-white mb-2">¡Bienvenido a puerto!</h2>
                                        <p className="text-white/40 text-sm">Tu track ha sido registrado con éxito. Antes de terminar, cuéntanos qué has <span className="text-white font-bold italic">aprendido</span> hoy.</p>
                                    </header>
                                ...
                                >
                                    <header>
                                        <div className="inline-flex items-center gap-2 px-3 py-1 bg-blue-500/10 rounded-full border border-blue-500/20 mb-4">
                                            ...
                                            <span className="text-[10px] font-black uppercase tracking-widest text-blue-400">Progreso Detectado</span>
                                        </div>
                                        <h2 className="text-4xl font-display italic text-white mb-2">¿En qué has mejorado?</h2>
                                        <p className="text-white/40 text-sm">Identificar tus avances es clave para subir de rango. ¿Qué maniobra te salió <span className="text-blue-400 font-bold italic">más fluida</span> hoy?</p>
                                    </header>
                                ...
                                >
                                    <header>
                                        <div className="inline-flex items-center gap-2 px-3 py-1 bg-red-500/10 rounded-full border border-red-500/20 mb-4">
                                            ...
                                            <span className="text-[10px] font-black uppercase tracking-widest text-red-400">Análisis Crítico</span>
                                        </div>
                                        <h2 className="text-4xl font-display italic text-white mb-2">¿Qué salió mal?</h2>
                                        <p className="text-white/40 text-sm">Incluso los capitanes fallan. ¿Dónde te has <span className="text-red-400 font-bold italic">equivocado</span> o qué reto te ha costado superar?</p>
                                    </header>
                                    ...
                                    <div className="flex gap-4">
                                        ...
                                        >
                                            {isSaving ? 'Guardando...' : <>Guardar en Bitácora <CheckCircle2 size={18} /></>}
                                        </button>
                                    </div>
                                </motion.div>
                            )}
                        ...
                        <div className="bg-black/40 p-6 flex items-center justify-center gap-3 border-t border-white/5">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-[0.3em] text-white/20">Getxo Bela Eskola Digital Logbook</span>
                        </div>
                    </motion.div>
                </div>
            )}
