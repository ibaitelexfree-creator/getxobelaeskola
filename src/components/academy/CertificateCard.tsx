...
export default function CertificateCard({
	...
	const handleDownload = async (e: React.MouseEvent) => {
		...
		try {
			const courseName =
				certificate.tipo === "curso"
					? locale === "eu"
						? certificate.curso?.nombre_eu
						: certificate.curso?.nombre_es
					: certificate.tipo === "nivel"
						? locale === "eu"
							? certificate.nivel?.nombre_eu
							: certificate.nivel?.nombre_es
						: "Capitán de Vela";

			// Map distinction to English types expected by the generator
			const distinctionVal = certificate.nivel_distincion || "estandar";
			const distinctionMap: Record<
				string,
				"standard" | "merit" | "excellence"
			> = {
				estandar: "standard",
				merito: "merit",
	...
	return (
		<div className="p-4 rounded-lg bg-white/5 border border-white/10 hover:border-accent/30 transition-all group relative overflow-hidden">
			...
			<h4 className="text-white font-bold mb-1 relative z-10">
				...
				{certificate.tipo === "curso"
					? locale === "eu"
						? certificate.curso?.nombre_eu || certificate.curso?.nombre_es
						: certificate.curso?.nombre_es
					: certificate.tipo === "nivel"
						? locale === "eu"
							? certificate.nivel?.nombre_eu || certificate.nivel?.nombre_es
							: certificate.nivel?.nombre_es
						: "Capitán de Vela"}
			</h4>
