...
export default function LeafletLogbookMap({
	...
	useEffect(() => {
		...
		if (isTracking && activePoints.length > 0) {
			const positions = activePoints.map(
	...
	useEffect(() => {
		...
		if (Array.isArray(sessions)) {
			sessions.forEach((session) => {
				...
				if (
					session.track_log &&
					Array.isArray(session.track_log) &&
					session.track_log.length >= 2
				) {
				...
				if (session.ubicacion) {
					const lat =
						typeof session.ubicacion.lat === "string"
							? parseFloat(session.ubicacion.lat)
							: session.ubicacion.lat;
					const lng =
						typeof session.ubicacion.lng === "string"
							? parseFloat(session.ubicacion.lng)
							: session.ubicacion.lng;
					...
					if (!isNaN(lat) && !isNaN(lng)) {
						...
						L.circleMarker([lat, lng], {
							...
							.bindPopup(
								`<div style="color: black; font-weight: bold; font-family: sans-serif; font-size: 10px; text-transform: uppercase;">${session.zona_nombre || session.tipo}</div>`,
							)
							.on("click", () => setSelectedPoint(session))
							.addTo(markersGroup);
					}
	...
	return (
		<div
			...
			style={{ minHeight: "400px", background: "#050b14" }}
		>
			<div
				ref={containerRef}
