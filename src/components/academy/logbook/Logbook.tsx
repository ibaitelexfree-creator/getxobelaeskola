...
export default function Logbook() {
	...
	const handleImportGPX = async (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		try {
			...
			if (res.ok) {
				...
				showMessage(
					"Ruta Importada",
					`Se han importado ${data.pointsCount} puntos y calculado estadísticas.`,
					"success",
				);
				loadData();
			} else {
	...
	const handleToggleTracking = async () => {
		if (isTracking) {
				...
				if (trackedPoints.length >= 2) {
					try {
						...
						if (res.ok) {
							showMessage(
								"Travesía Guardada",
								"Tu recorrido ha sido registrado en la bitácora",
								"success",
							);
							loadData();
						}
			...
			showMessage(
				...
				"Coge el timón, estamos grabando tu travesía",
				"info",
			);
		}
	};
	...
	if (loading) {
		return (
			<div className="flex flex-col items-center justify-center min-h-[400px]">
				...
				<p className="text-white/40 text-[10px] uppercase mt-6 tracking-[0.3em]">
	...
	return (
		<div
			className={`w-full ${activeTab === "map" ? "max-w-7xl" : "max-w-6xl"} mx-auto h-full flex flex-col font-display p-6 relative pb-20 transition-all duration-700`}
		>
			{/* Header */}
			<header className="flex flex-col md:flex-row items-center justify-between mb-12 gap-8">
				<div className="flex items-center gap-6">
					...
					<div>
						...
						<p className="text-white/40 text-sm mt-1">
							Historial del Navegante: {officialData?.user?.full_name}
						</p>
					</div>
				</div>
				...
				<div className="px-8 py-5 bg-white/5 border border-white/10 rounded-3xl text-center backdrop-blur-md">
					...
					<div className="text-4xl font-black text-white">
						{((officialData?.estadisticas?.horas_totales || 0) * 5.2).toFixed(
							1,
						)}
					</div>
				</div>
						...
						<div className="bg-blue-500/5 border border-blue-500/20 rounded-[2rem] p-8 flex flex-col md:flex-row items-center gap-8 backdrop-blur-sm">
							...
							<div className="flex-grow text-center md:text-left">
								<h3 className="text-xl font-bold text-white mb-2 italic">
									Registro de Millas Oficial
								</h3>
								<p className="text-white/50 text-sm leading-relaxed max-w-2xl">
									Este cuaderno contiene tus horas de navegación certificadas
									por Getxo Bela Eskola. Los registros aquí presentes son{" "}
									<span className="text-blue-400 font-bold">oficiales</span>,
									validados por instructores, y computan para la obtención de
									rangos y certificados. Cada milla te acerca más a tu próximo
									título.
								</p>
							</div>
							<button
								onClick={() =>
									showMessage(
										"Info",
										"Funcionalidad de solicitud en desarrollo",
										"info",
									)
								}
						...
						<div className="grid grid-cols-1 gap-6">
							{sessions.length > 0 ? (
								sessions.map((session: any) => (
									<div
										key={session.id}
										className="group bg-white/[0.03] hover:bg-white/[0.07] border border-white/10 rounded-3xl p-8 transition-all duration-500"
									>
										<div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
												...
												<div>
													...
													<div className="flex items-center gap-4 text-sm text-white/40 mt-2">
														<span className="flex items-center gap-1">
															<Ship size={14} /> {session.embarcacion}
														</span>
														<span className="flex items-center gap-1">
															<Wind size={14} /> {session.duracion_h}h
														</span>
													</div>
												</div>
											</div>
											<div className="text-right flex flex-col items-end gap-2">
												<div
													className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border ${session.verificado ? "bg-green-500/20 border-green-500/30 text-green-400" : "bg-amber-500/20 border-amber-500/30 text-amber-400"}`}
												>
								...
								<div className="py-32 text-center border-2 border-dashed border-white/5 rounded-[3rem] bg-white/[0.01]">
									...
									<p className="text-white/40 font-bold uppercase tracking-widest text-xs mb-2">
										Aún no hay registros oficiales
									</p>
									<p className="text-white/20 text-xs max-w-xs mx-auto">
										Tus sesiones certificadas por la escuela aparecerán aquí
										automáticamente tras ser validadas.
									</p>
								</div>
							)}
						</div>
					...
					<div className="relative w-full h-[650px] bg-[#050b14] rounded-[2.5rem] border border-white/10 overflow-hidden shadow-2xl flex flex-col">
						<div className="absolute top-8 left-8 z-[1000] pointer-events-none">
							<div className="bg-black/40 backdrop-blur-xl border border-white/10 p-4 rounded-3xl">
								<h3 className="text-white font-bold text-sm mb-1 italic">
									Mapa de Travesías
								</h3>
								...
								>
									...
									<span className="relative z-10 flex items-center gap-3">
										{isTracking ? (
											...
											<Waves size={18} className="animate-bounce" />
										)}
						...
						{selectedPoint && (
							<div className="absolute inset-y-6 right-6 w-80 bg-[#0a1628]/95 backdrop-blur-2xl border border-accent/30 rounded-3xl p-8 shadow-2xl z-[1000] overflow-hidden">
								...
								<div className="flex items-center gap-3 mb-4 border-b border-white/5 pb-4">
									...
									<div>
										<h4 className="text-accent font-black uppercase text-[10px] tracking-widest">
											Travesía
										</h4>
										<p className="text-white font-bold">
											{selectedPoint.zona_nombre || "Navegación"}
										</p>
									</div>
								</div>
								<div className="space-y-4">
									<div>
										...
										<p className="text-sm text-white">
											{new Date(selectedPoint.fecha).toLocaleDateString()}
										</p>
									</div>
									<div className="grid grid-cols-2 gap-4">
										<div>
											...
											<p className="text-lg font-black text-white">
												{selectedPoint.duracion_h}{" "}
												<span className="text-xs text-accent">h</span>
											</p>
										</div>
										{selectedPoint.ubicacion?.stats && (
											<>
												<div>
													...
													<p className="text-lg font-black text-white">
														{selectedPoint.ubicacion.stats.distance_nm}{" "}
														<span className="text-xs text-accent">nm</span>
													</p>
												</div>
												<div>
													<span className="text-[10px] uppercase tracking-widest text-white/40 font-black">
													...
													<p className="text-lg font-black text-white">
														{selectedPoint.ubicacion.stats.avg_speed_kn}{" "}
														<span className="text-xs text-accent">kn</span>
													</p>
												</div>
						...
						<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
							{(allSkills.length > 0
								...
								return (
									<div
										key={skill.id}
										className={`group relative bg-white/[0.03] p-8 rounded-[2.5rem] border transition-all duration-500 ${isObtained ? "border-accent/30" : "border-white/5 opacity-60"}`}
									>
										{!isObtained && (
										...
										<div
											className={`w-14 h-14 rounded-2xl flex items-center justify-center text-3xl mb-6 ${isObtained ? "bg-accent/10 border border-accent/20 shadow-[0_0_20px_rgba(var(--accent-rgb),0.1)]" : "bg-white/5 grayscale"}`}
										>
											{skill.icono || "⚡"}
										</div>
										<h3
											className={`text-xl font-bold mb-2 ${isObtained ? "text-white" : "text-white/40"}`}
										>
											{skill.nombre_es}
										</h3>
										<p className="text-xs text-white/30 leading-relaxed mb-6">
											{skill.descripcion_es}
										</p>
										...
										<div className="flex items-center justify-between mt-auto pt-6 border-t border-white/5">
											...
											{isObtained ? (
												...
												<span className="text-[10px] uppercase tracking-widest text-white/10 font-black italic">
													Bloqueado
												</span>
											)}
										</div>
				...
				{activeTab === "diary" && (
					<div className="flex flex-col gap-8 max-w-4xl mx-auto">
						...
						<div className="bg-white/[0.03] border border-white/10 rounded-[2.5rem] p-8 backdrop-blur-md">
							<div className="flex items-center gap-4 mb-6">
								...
								<div>
									...
									<p className="text-white/40 text-[10px] uppercase tracking-widest">
										Tus reflexiones personales de mar
									</p>
								</div>
							</div>
						...
						<div className="space-y-6">
							<h4 className="text-[10px] uppercase tracking-widest text-white/20 font-black ml-4">
								Entradas Recientes
							</h4>
							...
							{loadingDiary ? (
								...
								diaryEntries.map((entry: any) => (
									<div
										key={entry.id}
										className="group bg-white/[0.02] hover:bg-white/[0.04] border border-white/5 rounded-[2.2rem] p-8 transition-all relative overflow-hidden"
									>
											...
											{entry.tags && entry.tags.length > 0 && (
												<div className="flex gap-2 mt-4">
													{entry.tags.map((tag: string) => (
														<span
															key={tag}
															className="text-[8px] uppercase tracking-tighter bg-white/5 px-2 py-0.5 rounded text-white/20 font-black"
														>
															#{tag}
														</span>
													))}
								...
								<div className="py-20 text-center bg-white/[0.01] rounded-[2.5rem] border border-dashed border-white/5">
									...
									<p className="text-white/20 italic text-sm font-serif">
										Tu mar de reflexiones está esperando...
									</p>
								</div>
							)}
						</div>
