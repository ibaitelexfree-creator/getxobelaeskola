...
import { Award, Info, Ship, TrendingUp } from "lucide-react";
		...
		return (
			<div className="py-20 text-center bg-white/5 border-2 border-dashed border-white/10 rounded-3xl">
				...
				<p className="text-white/40 max-w-xs mx-auto text-sm">
					Registra tus horas en diferentes embarcaciones para ver tu nivel de
					maestría aquí.
				</p>
			</div>
		);
				...
				>
					...
					<div className="flex justify-between items-start mb-6">
						...
						<div className="text-right">
							<div className="text-[10px] uppercase tracking-widest text-white/40 border-b border-white/5 pb-1 mb-1">
								Horas Totales
							</div>
							<div className="text-2xl font-black text-white">
								{boat.hours} <span className="text-xs text-accent">h</span>
							</div>
						</div>
					</div>
					...
					<div className="mt-6 flex items-center gap-2 text-[8px] uppercase tracking-widest text-white/20">
						...
						{boat.level === "Maestro"
							...
							: `Próximo Rango: ${boat.level === "Iniciado" ? "Avanzado" : "Maestro"}`}
					</div>
				</motion.div>
			))}
		</div>
