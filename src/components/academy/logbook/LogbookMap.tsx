...
export default function LogbookMap({ sessions }: LogbookMapProps) {
	const [selectedPoint, setSelectedPoint] = useState<NavigationPoint | null>(
		null,
	);
	...
	if (!mounted)
		return (
			<div className="w-full aspect-[5/4] bg-[#050b14] rounded-3xl border border-white/10 flex items-center justify-center">
				<div className="animate-pulse text-blue-500/20">
					Cargando Bitácora...
				</div>
			</div>
		);
					...
					<div className="space-y-4">
						<div>
							...
							<p className="text-sm text-white">
								{new Date(selectedPoint.fecha).toLocaleDateString("es-ES", {
									day: "2-digit",
									month: "long",
									year: "numeric",
								})}
