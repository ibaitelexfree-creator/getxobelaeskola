...
import {
    ...
    Wind, Thermometer, Waves, Anchor,
    Map as MapIcon, Info, ChevronRight
} from 'lucide-react';
import { HISTORICAL_REGATTAS, HistoricalRegatta } from '@/lib/data/historical-regattas';

// Dynamic import for the Map component to avoid SSR issues
const RegattaMap = dynamic(() => import('./RegattaMap'), {
    ssr: false,
    loading: () => <div className="w-full h-full flex items-center justify-center bg-[#000510] text-white/50">Cargando carta náutica...</div>
});

export default function RegattaPlayer() {
    const [selectedRegatta, setSelectedRegatta] = useState<HistoricalRegatta>(HISTORICAL_REGATTAS[0]);
                ...
                {showSidebar && (
                    ...
                    >
                        <div className="p-6 border-b border-white/10">
                            <h2 className="text-xl font-display italic text-accent">Exploración</h2>
                            <p className="text-xs text-white/50 uppercase tracking-widest mt-1">Regatas Históricas</p>
                        </div>
                            ...
                            {HISTORICAL_REGATTAS.map(regatta => (
                                ...
                                >
                                    <div className="flex justify-between items-start mb-2">
                                        ...
                                        <span className="text-xs text-white/40 font-mono">{regatta.year}</span>
                                    </div>
                ...
                <div className="absolute top-0 left-0 right-0 p-6 pointer-events-none z-10 flex justify-center">
                    <div className="bg-black/60 backdrop-blur-md border border-white/10 rounded-full px-8 py-2 text-center pointer-events-auto">
                        <h1 className="text-lg font-display text-white tracking-wide">
                            {selectedRegatta.title} <span className="text-accent mx-2">•</span> {currentStep.date}
                        </h1>
                    </div>
                </div>
                    ...
                    <div className="max-w-6xl mx-auto w-full grid grid-cols-1 md:grid-cols-3 gap-6 items-end pointer-events-auto">
                        ...
                        <div className="bg-[#0a1628]/90 backdrop-blur border border-white/10 rounded-xl p-5 shadow-2xl">
                            <div className="flex items-center gap-2 mb-3 border-b border-white/10 pb-2">
                                ...
                                <h3 className="text-xs uppercase tracking-widest text-white/70 font-bold">Datos del Barco</h3>
                            </div>
                            <div className="grid grid-cols-2 gap-y-3 gap-x-4 text-sm">
                                <div>
                                    <p className="text-white/40 text-[10px] uppercase">Embarcación</p>
                                    <p className="text-white font-mono">{selectedRegatta.boatStats.name}</p>
                                </div>
                                <div>
                                    <p className="text-white/40 text-[10px] uppercase">Tipo</p>
                                    <p className="text-white font-mono truncate">{selectedRegatta.boatStats.type}</p>
                                </div>
                                <div>
                                    <p className="text-white/40 text-[10px] uppercase">Patrón</p>
                                    <p className="text-white font-mono truncate">{selectedRegatta.boatStats.skipper}</p>
                                </div>
                                <div>
                                    <p className="text-white/40 text-[10px] uppercase">Eslora</p>
                                    <p className="text-white font-mono">{selectedRegatta.boatStats.length}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-[#0a1628]/90 backdrop-blur border border-white/10 rounded-xl p-5 shadow-2xl">
                            ...
                            <div className="space-y-3">
                                <div className="flex items-center justify-between">
                                    ...
                                    <span className="text-white font-mono font-bold">{currentStep.weather.windSpeed} kn {currentStep.weather.windDirection}</span>
                                </div>
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2 text-white/60">
                                        <Waves size={14} />
                                    ...
                                    <span className="text-white font-mono text-sm">{currentStep.weather.seaState}</span>
                                </div>
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2 text-white/60">
                                        <Thermometer size={14} />
                                    ...
                                    <span className="text-white font-mono">{currentStep.weather.temperature}°C</span>
                                </div>
                                <div className="mt-2 pt-2 border-t border-white/5">
                                    <p className="text-xs text-white/50 italic text-center">"{currentStep.weather.description}"</p>
                                </div>
                            </div>
                        </div>
