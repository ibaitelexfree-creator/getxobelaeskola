...
import {
	HISTORICAL_REGATTAS,
	type Regatta,
} from "@/data/academy/historicalRegattas";

// Dynamically import map to avoid SSR issues with Leaflet
const RegattaMap = dynamic(() => import("./RegattaMap"), {
	ssr: false,
	loading: () => (
		<div className="w-full h-full bg-[#000510] flex items-center justify-center text-white/50">
			Cargando Mapa...
		</div>
	),
});
...
export default function HistoricalRegattas() {
	const [selectedRegatta, setSelectedRegatta] = useState<Regatta>(
	...
	const currentWeather =
		selectedRegatta.weatherConditions[
			...
			>
				...
				<div className="flex-1 overflow-y-auto custom-scrollbar">
					{HISTORICAL_REGATTAS.map((regatta) => (
						...
						>
							<h3
								className={`font-bold text-sm ${selectedRegatta.id === regatta.id ? "text-accent" : "text-white group-hover:text-accent"}`}
							>
								{regatta.name}
							</h3>
							<p className="text-xs text-white/50 truncate mt-1">
								{regatta.boatStats.type}
							</p>
						</button>
					))}
				</div>
			...
			<div className="flex-1 relative flex flex-col h-full overflow-hidden">
				...
				<div className="absolute top-16 md:top-6 left-4 right-4 md:left-8 md:right-auto md:w-96 bg-black/80 backdrop-blur-md border border-white/10 p-5 rounded-xl z-10 pointer-events-none md:pointer-events-auto shadow-2xl">
					...
					<div className="grid grid-cols-2 gap-3 text-xs text-white/60 bg-white/5 p-3 rounded-lg border border-white/5">
						<div>
							<span className="block uppercase tracking-wider text-[10px] text-white/40 mb-1">
								Barco
							</span>
						...
						<div>
							...
							<span className="text-white font-medium">
								{selectedRegatta.boatStats.length}m
							</span>
						</div>
						<div>
							<span className="block uppercase tracking-wider text-[10px] text-white/40 mb-1">
							...
							<span className="text-white font-medium">
								{selectedRegatta.boatStats.crew} pax
							</span>
						</div>
						<div>
							<span className="block uppercase tracking-wider text-[10px] text-white/40 mb-1">
							...
							<span className="text-white font-medium">
								{selectedRegatta.boatStats.speedRecord} kn
							</span>
						</div>
					</div>
				</div>
				...
				<div className="absolute bottom-0 left-0 w-full h-1 bg-white/10 z-20">
					<div
						...
						style={{
							width: `${(currentStep / (selectedRegatta.route.length - 1)) * 100}%`,
						}}
					/>
				</div>
			</div>
