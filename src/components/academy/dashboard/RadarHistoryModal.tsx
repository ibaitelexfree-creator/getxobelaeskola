...
import { ArrowDown, BarChart3, Thermometer, Wind, X } from "lucide-react";
			...
			<div className="bg-[#050b14] border border-blue-500/30 rounded-2xl w-full max-w-5xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-300">
				<div className="bg-blue-900/40 p-4 border-b border-blue-500/30 flex justify-between items-center shrink-0">
					<h3 className="text-white font-display italic uppercase tracking-widest text-sm flex items-center gap-2">
						...
						{t("history_title")}
						{historyLoading && (
							<span className="ml-2 text-[8px] animate-pulse text-accent uppercase">
								{t("syncing")}
							</span>
						)}
					</h3>
				...
				<div
					className={`flex-1 overflow-y-auto p-6 space-y-12 custom-scrollbar ${historyLoading ? "opacity-50 pointer-events-none" : ""}`}
				>
					...
					<div className="bg-black/40 rounded-xl border border-white/5 overflow-hidden shrink-0">
						<div className="overflow-x-auto scrollbar-thin scrollbar-thumb-blue-500/20">
							<div
								style={{
									minWidth:
										history.length > 12 ? `${history.length * 60}px` : "100%",
								}}
							>
								{/* Hours Row */}
								<div className="flex border-b border-white/5 bg-white/5">
									<div className="w-28 shrink-0 p-3 text-[10px] font-black uppercase text-white/40 flex items-center bg-black/20 sticky left-0 z-10 border-r border-white/5 backdrop-blur-md">
										{t("history_time")}
									</div>
									{history.map((h, i) => (
										<div
											key={i}
											className="flex-1 text-center p-3 text-[10px] font-mono text-white/60 border-l border-white/5"
										>
											{h?.time || "--:--"}
										</div>
									))}
								...
								<div className="flex border-b border-white/5">
									...
									{history.map((h, i) => {
										const wind = h
											? typeof h.wind === "number"
												? h.wind
												: parseFloat(h.wind as any) || 0
											: 0;
										return (
											<div
												key={i}
												...
												style={{
													backgroundColor: getWindColor(wind),
													color: getWindTextColor(wind),
												}}
											>
												{Math.round(wind)}
											</div>
								...
								<div className="flex border-b border-white/5">
									...
									{history.map((h, i) => (
										<div
											key={i}
											className="flex-1 text-center p-3 text-[10px] font-bold text-amber-500/80 border-l border-white/5"
										>
											{h
												? typeof h.gust === "number"
													? Math.round(h.gust)
													: Math.round(parseFloat(h.gust as any) || 0)
												: 0}
										</div>
									))}
								</div>
								...
								<div className="flex border-b border-white/5">
									...
									{history.map((h, i) => (
										<div
											key={i}
											className="flex-1 flex items-center justify-center p-3 border-l border-white/5 bg-blue-900/5"
										>
											<ArrowDown
												size={14}
						...
						<div className="bg-white/[0.02] border border-white/5 rounded-2xl p-6 shadow-inner relative overflow-hidden">
								...
								<div className="flex gap-4">
									<div className="flex items-center gap-2">
										...
										<span className="text-[9px] font-black text-white/40 uppercase tracking-widest">
											{t("wind_chart_wind_label")}
										</span>
									</div>
									<div className="flex items-center gap-2">
										<div className="w-2.5 h-2.5 rounded-full bg-red-500" />
										<span className="text-[9px] font-black text-white/40 uppercase tracking-widest">
											{t("wind_chart_gust_label")}
										</span>
									</div>
								</div>
							</div>
							...
							<div className="overflow-x-auto pb-4 scrollbar-thin scrollbar-thumb-blue-500/20">
								<div
									style={{
										width:
											history.length > 24 ? `${history.length * 30}px` : "100%",
										minWidth: "100%",
									}}
									className="h-72"
								>
									...
									<ResponsiveContainer width="100%" height="100%">
										...
										>
											...
											<Line
												...
												stroke="#fb923c"
												strokeWidth={3}
												dot={{ r: 4, fill: "#fb923c", stroke: "#050b14" }}
												name={t("history_temp")}
											/>
											<Line
												yAxisId="right"
												...
												stroke="#22d3ee"
												strokeWidth={3}
												dot={{ r: 4, fill: "#22d3ee", stroke: "#050b14" }}
												name={t("history_tide")}
											/>
										</LineChart>
									</ResponsiveContainer>
