				...
				<div className="flex items-center gap-4">
					...
					<div>
						<div className="flex items-center gap-2 mb-1">
							<span className="text-[10px] uppercase font-black tracking-[0.4em] text-accent">
								{t("title")}
							</span>
							<Sparkles size={10} className="text-accent animate-pulse" />
						</div>
						<h3 className="text-white font-display font-bold text-lg italic tracking-tight">
							{t("subtitle")}
						</h3>
					</div>
				</div>
				{!isCompleted && (
					<div className="px-3 py-1 bg-white/5 border border-white/10 rounded-full flex items-center gap-2">
						...
						<span className="text-[10px] font-black text-white/80">
							{t("reward", { xp: challenge.xp_recompensa })}
						</span>
					</div>
				)}
			</div>
				...
				{!isCompleted ? (
					...
					>
						...
						>
							{challenge.opciones.map((opcion: string, idx: number) => (
								<button
									...
									className={`w-full text-left p-4 rounded-xl border transition-all duration-300 flex items-center justify-between group/opt focus-visible:ring-2 focus-visible:ring-accent outline-none ${
										selectedOption === idx
								...
								>
									...
									{selectedOption === idx && (
										<div className="w-2 h-2 rounded-full bg-nautical-black" />
									)}
								</button>
							))}
						</div>
						...
						<button
							...
							className={`mt-8 w-full py-4 rounded-xl font-black uppercase tracking-[0.3em] text-xs transition-all flex items-center justify-center gap-3 focus-visible:ring-2 focus-visible:ring-accent outline-none ${
					...
					>
						<div
							className={`w-20 h-20 rounded-full mx-auto mb-6 flex items-center justify-center border-2 ${data.correct ? "bg-emerald-500/20 border-emerald-500 text-emerald-500 shadow-[0_0_30px_rgba(16,185,129,0.2)]" : "bg-red-500/20 border-red-500 text-red-500"}`}
						>
							{data.correct ? (
						...
						<h4
							className={`text-2xl font-black uppercase tracking-tighter mb-2 ${data.correct ? "text-emerald-400" : "text-red-400"}`}
						>
							{data.correct
								? t("correct", { xp: challenge.xp_recompensa })
								: t("wrong", {
										answer:
											challenge.opciones[data.result.respuesta_correcta] || "",
									})}
						</h4>

						<p className="text-white/60 text-[10px] uppercase tracking-[0.3em] mb-8 font-black">
							{data.correct
								? `Has ganado ${challenge.xp_recompensa} XP`
								: t("come_back")}
						</p>
						...
						<div className="bg-white/5 border border-white/10 rounded-2xl p-6 text-left relative overflow-hidden">
							...
							<span className="text-[9px] uppercase font-black text-accent tracking-widest block mb-1">
								{t("explanation")}
							</span>
							<p className="text-white/80 text-sm leading-relaxed italic">
								"
								{locale === "eu"
									? data.result.explicacion_eu
									: data.result.explicacion_es}
								"
							</p>
						</div>
					</motion.div>
