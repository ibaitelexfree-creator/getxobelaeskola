				...
				<AnimatePresence mode="wait">
					{isAnalyzing ? (
						...
						>
							...
							<h4 className="mt-8 text-xs font-black uppercase tracking-[0.5em] text-accent animate-pulse">
								Sincronizando Bitácora...
							</h4>
							<p className="mt-2 text-[10px] text-white/20 uppercase tracking-widest">
								Algoritmo de Carrera Náutica V2.1
							</p>
						</motion.div>
					) : (
						<motion.div
						...
						>
							...
							<div className="flex items-center justify-between px-10 pt-8 border-b border-white/5 pb-6">
								...
								<div className="text-[8px] uppercase tracking-[0.3em] font-black text-white/20">
									Career AI • Beta
								</div>
							</div>
							...
							<div className="p-10 flex flex-col lg:flex-row gap-12">
									...
									>
										<div className="w-32 h-32 bg-white/[0.02] border border-white/10 rounded-[2rem] flex items-center justify-center relative overflow-hidden group-hover/icon:border-accent transition-colors">
											...
											<div className="relative z-10 text-accent">
												{mainRec.type === "specialization" && (
													<Target size={40} />
												)}
												{mainRec.type === "path" && <Award size={40} />}
												{mainRec.type === "versatility" && (
													<BarChart3 size={40} />
												)}
												{mainRec.type === "ascension" && (
								...
								<div className="flex-1 space-y-6">
									...
									<div className="flex flex-wrap gap-x-8 gap-y-4 pt-4">
										<StatBar
											label="Consistencia"
											value={mainRec.stats.consistency}
											icon={Activity}
										/>
										<StatBar
											label="Versatilidad"
											value={mainRec.stats.variety}
											icon={BarChart3}
										/>
										<StatBar
											label="Experiencia"
											value={mainRec.stats.experience}
											icon={Compass}
										/>
									</div>
										...
										<div className="hidden sm:flex items-center gap-3">
											<div className="flex gap-1.5">
												{[1, 2, 3].map((i) => (
													<div
														key={i}
														className="w-8 h-8 rounded-full border border-accent/20 bg-accent/5 flex items-center justify-center"
													>
											...
											<span className="text-[9px] text-white/30 font-bold uppercase tracking-widest">
												3 expertos recomiendan esta ruta
											</span>
										</div>
									</div>
								</div>
