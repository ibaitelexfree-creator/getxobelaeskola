...
const LeafletMap = dynamic(() => import("./LeafletMap"), {
	ssr: false,
	loading: () => (
		<div className="w-full h-full bg-blue-950/20 animate-pulse flex items-center justify-center text-blue-400/30">
			Cargando Mapa...
		</div>
	),
});

const JourneyCompletionModal = dynamic(
	() => import("../notifications/JourneyCompletionModal"),
	{ ssr: false },
);

export default function NavigationExperienceMap({
	sessions,
}: {
	sessions: any[];
}) {
	...
	const mappedSessions = useMemo(
		() => sessions.filter((s) => s.ubicacion).slice(-10),
		[sessions],
	);
			...
			<div className="flex items-center justify-between mb-8">
				<div>
					<h2 className="text-2xl font-display italic text-white flex items-center gap-3">
						<span className="text-accent drop-shadow-[0_0_8px_rgba(255,191,0,0.4)]">
							🗺️
						</span>{" "}
						Aguas Navegadas
					</h2>
			...
			<div className="flex flex-col gap-8">
				...
				<div className="relative aspect-video lg:aspect-[21/9] bg-blue-950/20 rounded-2xl border border-blue-500/20 overflow-hidden shadow-2xl group min-h-[400px] w-full">
					...
					<div className="absolute top-4 left-4 flex flex-col gap-2 z-[1000]">
						<div
							className={`px-3 py-1.5 backdrop-blur-md border rounded-lg text-[10px] font-black uppercase tracking-widest shadow-xl flex items-center gap-2 transition-colors duration-300 ${
								statusMessage.includes("TIERRA")
					...
					<div className="absolute bottom-4 left-4 flex flex-col gap-2 z-[1000]">
						{isTracking && (
							...
							>
								<span
									className={`w-2 h-2 rounded-full ${isAutoTracking ? "bg-blue-400" : "bg-red-500 animate-ping"}`}
								/>
				...
				<div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-stretch">
					{/* Main Control Panel */}
					<div
						className={`md:col-span-5 p-8 rounded-2xl shadow-xl backdrop-blur-md transition-all duration-500 flex flex-col justify-center ${
							isTracking
					...
					>
						<div className="flex items-center justify-between mb-8">
							<div className="flex items-center gap-4 font-display italic text-xl text-white">
								<div
									className={`p-2 rounded-lg ${isTracking ? "bg-red-500/20 text-red-400" : "bg-accent/20 text-accent"}`}
								>
									<Compass
										className={isTracking ? "animate-spin-slow" : ""}
										size={28}
									/>
								</div>
								<div className="flex flex-col">
									...
									<span className="text-[10px] text-white/40 uppercase tracking-widest not-italic">
										Sistema de Posicionamiento Smart
									</span>
								</div>
							</div>
						</div>
						...
						{isTracking ? (
							<div className="grid grid-cols-2 h-full gap-4">
								<div className="p-6 bg-white/5 border border-white/10 rounded-2xl flex flex-col justify-center items-center text-center">
									...
									<div className="text-4xl font-black text-white">
										{currentPosition?.speed
											? (currentPosition.speed * 1.94384).toFixed(1)
											: "0.0"}
										<span className="text-xs text-accent ml-2 font-bold uppercase">
											kn
										</span>
									</div>
								</div>
								<div className="p-6 bg-white/5 border border-white/10 rounded-2xl flex flex-col justify-center items-center text-center">
									...
									<div className="text-4xl font-black text-white">
										{livePoints.length}
										<span className="text-xs text-blue-400 ml-2 font-bold uppercase">
											pts
										</span>
									</div>
								</div>
							...
							>
								<div className="flex items-start justify-between">
									<div className="flex flex-col gap-1">
										...
										<p className="text-white/40 text-[10px] uppercase tracking-widest">
											Lista para sincronizar con tu bitácora oficial
										</p>
									</div>
								...
								{saveError && (
									<p className="text-red-400 text-[10px] mt-2 font-bold uppercase tracking-widest text-center">
										{saveError}
									</p>
								)}
							</motion.div>
							...
							<div className="h-full p-8 bg-white/[0.02] border border-white/5 rounded-2xl flex flex-col items-center justify-center text-center">
								...
								<h4 className="text-white/40 font-bold text-xs uppercase tracking-[0.2em] mb-2">
