		...
		<div className="relative w-full h-full rounded-2xl overflow-hidden border border-white/10 shadow-2xl">
			...
			<div className="absolute top-4 right-4 bg-black/80 backdrop-blur-md p-4 rounded-xl border border-white/10 z-[1000] pointer-events-none">
				...
				<div className="text-xl font-black text-white flex items-center gap-2">
					{currentState.type === "SLACK" && (
						<span className="text-white/60">REPUNTE (SIN CORRIENTE)</span>
					)}
					{currentState.type === "FLOOD" && (
						<span className="text-blue-400">FLUJO (ENTRANTE)</span>
					)}
					{currentState.type === "EBB" && (
						<span className="text-orange-400">REFLUJO (SALIENTE)</span>
					)}
				</div>
