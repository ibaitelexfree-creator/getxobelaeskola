...
export default function MicroLeccionesWidget({
	locale,
	translations,
	preloadedLessons,
}: {
	locale: string;
	translations: any;
	preloadedLessons?: MicroLeccion[];
}) {
	// eslint-disable-line @typescript-eslint/no-explicit-any
	const [lessons, setLessons] = useState<MicroLeccion[]>(
		preloadedLessons || [],
	);
	...
	if (loading)
		return <div className="animate-pulse h-40 bg-white/5 rounded-sm mb-12" />;
					...
					>
						...
						<div className="absolute bottom-0 left-0 right-0 p-3">
							<div className="flex items-center gap-1 text-[9px] text-accent font-bold uppercase tracking-wider mb-1">
								<Clock size={10} />
								{Math.floor(lesson.duracion_segundos / 60)}:
								{(lesson.duracion_segundos % 60).toString().padStart(2, "0")}
							</div>
