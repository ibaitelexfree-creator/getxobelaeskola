		...
		<div className="fixed inset-0 z-[10001] flex items-center justify-center p-4 bg-nautical-black/90 backdrop-blur-md animate-fade-in">
			<div className="bg-card border border-white/10 p-8 rounded-sm w-full max-w-md shadow-2xl space-y-8 relative overflow-hidden">
				...
				<header className="relative z-10">
					...
					<p className="text-3xs uppercase tracking-[0.2em] text-white/40 font-bold">
				...
				<div className="space-y-6 relative z-10">
					...
					<div className="flex items-center justify-between p-4 bg-white/5 border border-white/5 rounded-sm hover:border-white/10 transition-colors">
						<div className="flex items-center gap-4">
							<div
								className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors ${notificationsEnabled ? "bg-accent/20 text-accent" : "bg-white/5 text-white/20"}`}
							>
								<Bell size={20} />
							</div>
							<div>
								...
								<p className="text-[10px] text-white/40">
									Recibe avisos de Euskalmet y viento
								</p>
							</div>
						</div>
						<button
						...
						>
							<div
								className={`absolute top-1 w-4 h-4 bg-nautical-black rounded-full transition-all ${notificationsEnabled ? "right-1" : "left-1"}`}
							/>
						</button>
					</div>
					...
					<div className="flex items-center justify-between p-4 bg-white/5 border border-white/5 rounded-sm hover:border-white/10 transition-colors">
						<div className="flex items-center gap-4">
							<div
								className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors ${soundEnabled ? "bg-red-500/20 text-red-500" : "bg-white/5 text-white/20"}`}
							>
								<Volume2 size={20} />
							</div>
							<div>
								<p className="text-sm font-bold text-white">
									Alerta Sonora (Sirena)
								</p>
								<p className="text-[10px] text-white/40">
									Sonido de emergencia persistente
								</p>
							</div>
						</div>
						<button
						...
						>
							<div
								className={`absolute top-1 w-4 h-4 bg-nautical-black rounded-full transition-all ${soundEnabled ? "right-1" : "left-1"}`}
							/>
						</button>
					</div>
					...
					<div className="p-4 bg-accent/5 border border-accent/10 rounded-sm">
						<p className="text-[9px] text-accent/60 leading-relaxed uppercase tracking-wider font-bold italic">
							* Las alertas sonoras críticas solo se activan en horario lectivo
							para administradores e instructores para evitar molestias
						...
						<div className="max-h-[160px] overflow-y-auto pr-2 space-y-2 scrollbar-thin scrollbar-thumb-white/10">
							{!alertHistory || alertHistory.length === 0 ? (
								<div className="py-8 text-center border border-dashed border-white/5 rounded-sm">
									<p className="text-[10px] uppercase tracking-widest text-white/20 font-bold italic">
