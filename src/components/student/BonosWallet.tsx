			...
			<div className="grid gap-4">
				{bonos.map((bono) => {
					const percentage = Math.min(
						100,
						(bono.horas_restantes / bono.horas_iniciales) * 100,
					);
					...
					return (
						<div
							key={bono.id}
							className="bg-gradient-to-br from-card to-card/50 border border-white/10 p-5 rounded-sm relative overflow-hidden group hover:border-accent/30 transition-all"
						>
							...
							<div className="relative z-10">
								<div className="flex justify-between items-start mb-4">
									<div>
										<h3 className="text-white font-display text-xl italic">
											{bono.tipos_bono?.nombre || "Bono General"}
										</h3>
										<p className="text-white/40 text-xs mt-1">
											Válido hasta{" "}
											{new Date(bono.fecha_expiracion).toLocaleDateString()}
										</p>
									</div>
									<div className="text-right">
										<div className="text-2xl font-black text-white leading-none">
											{bono.horas_restantes}h
										</div>
										<div className="text-[9px] uppercase tracking-widest text-accent/60 mt-1">
											Disponibles
										</div>
									</div>
								</div>
