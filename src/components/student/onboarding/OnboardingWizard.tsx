...
import { Anchor, Award, ChevronRight, Compass, Map } from "lucide-react";
		...
		<div className="fixed inset-0 z-[9999] flex items-center justify-center">
			...
			<div className="relative z-10 w-full max-w-2xl px-4">
				<AnimatePresence mode="wait">
					{step === 0 && <WelcomeStep key="welcome" t={t} onNext={nextStep} />}
					{step === 1 && (
						<LevelTestStep
							key="level"
							t={t}
							onNext={(ans) => {
								setAnswers({ ...answers, level: ans });
								nextStep();
							}}
						/>
					)}
					...
					{step === 4 && <MissionStep key="mission" t={t} onNext={nextStep} />}
					{step === 5 && (
...
function WelcomeStep({ t, onNext }: { t: any; onNext: () => void }) {
	return (
		...
		>
			<div className="absolute top-0 right-0 p-12 opacity-[0.05] text-9xl pointer-events-none">
				⚓
			</div>
...
function LevelTestStep({ t, onNext }: { t: any; onNext: (ans: any) => void }) {
	...
	const questions = [
		{
			id: "q1",
			text: t("level_test.q1"),
			options: [
				{ val: "yes", label: t("level_test.a1_yes") },
				{ val: "no", label: t("level_test.a1_no") },
			],
		},
		{
			id: "q2",
			text: t("level_test.q2"),
			options: [
				{ val: "yes", label: t("level_test.a2_yes") },
				{ val: "no", label: t("level_test.a2_no") },
			],
		},
		{
			id: "q3",
			text: t("level_test.q3"),
			options: [
				{ val: "yes", label: t("level_test.a3_yes") },
				{ val: "no", label: t("level_test.a3_no") },
			],
		},
	];
...
function GoalSettingStep({
	...
	return (
		...
		>
			...
			<div className="py-8">
				<label className="block text-xs uppercase tracking-widest text-accent mb-2 font-bold">
					{t("goals.label_date")}
				</label>
				<input
					type="date"
...
function TourStep({
	...
	const steps = [
		{
			id: "dashboard-profile-sidebar",
			title: t("tour.step_sidebar"),
			desc: t("tour.desc_sidebar"),
		},
		{
			id: "dashboard-academy-widget",
			title: t("tour.step_academy"),
			desc: t("tour.desc_academy"),
		},
		{
			id: "dashboard-courses-section",
			title: t("tour.step_courses"),
			desc: t("tour.desc_courses"),
		},
		{
			id: "dashboard-rentals-section",
			title: t("tour.step_rentals"),
			desc: t("tour.desc_rentals"),
		},
	];
...
function MissionStep({ t, onNext }: { t: any; onNext: () => void }) {
	return (
		...
		>
			...
			<p className="text-accent uppercase tracking-widest text-xs font-bold mb-6">
				{t("mission.subtitle")}
			</p>
			...
			<div className="bg-white/5 p-6 rounded-sm mb-8 italic text-white/80 font-serif leading-relaxed relative">
				...
				{t("mission.manifesto")}
				<span className="text-4xl absolute -bottom-6 -right-2 opacity-20">
					"
				</span>
			</div>
