...
import { format, startOfWeek, endOfWeek, eachDayOfInterval, addDays, subDays, isSameDay, getHours, getMinutes, differenceInMinutes, parseISO } from 'date-fns';
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar">
                <div className="grid grid-cols-8 min-w-[800px]">
                    ...
                    <div className="border-r border-white/5 bg-black/20 sticky left-0 z-10">
                        <div className="h-10 border-b border-white/5"></div> {/* Empty corner */}
                        {hours.map(hour => (
                            <div key={hour} className="h-[60px] border-b border-white/5 flex items-start justify-center pt-1 text-[10px] text-white/40">
                                {hour}:00
                            </div>
                        ))}
                    </div>
                    ...
                    {/* Days Columns */}
                    {weekDays.map(day => (
                        <div key={day.toISOString()} className="flex flex-col border-r border-white/5 last:border-0 min-w-[100px]">
                            {/* Day Header */}
                            <div className={`h-10 border-b border-white/5 flex flex-col items-center justify-center ${isSameDay(day, new Date()) ? 'bg-accent/5' : ''}`}>
                                <span className="text-[10px] uppercase font-bold text-white/40">
                                    {format(day, 'EEE', { locale: dateLocale })}
                                </span>
                                <span className={`text-sm font-bold ${isSameDay(day, new Date()) ? 'text-accent' : 'text-white'}`}>
                                    {format(day, 'd')}
                                </span>
                            </div>
