...
export default function StudySessionsHistory({
	locale = "es",
}: {
	locale?: string;
}) {
	...
	const downloadCSV = () => {
		...
		link.setAttribute(
			"download",
			`study_sessions_${new Date().toISOString().split("T")[0]}.csv`,
		);
			...
			<div className="flex flex-col xl:flex-row justify-between items-start xl:items-end mb-8 gap-4">
				...
				<div className="flex flex-wrap gap-3 items-center w-full xl:w-auto">
					...
					<div className="relative group min-w-[140px]">
						...
						<ChevronDown
							size={14}
							className="absolute right-3 top-1/2 -translate-y-1/2 text-white/30 group-hover:text-indigo-400 pointer-events-none transition-colors"
						/>
					</div>
								...
								>
									...
									<td className="p-4">
										<div className="flex items-center gap-2 text-white/50 group-hover:text-white/90 text-sm font-mono transition-colors duration-300">
											<Clock
												size={13}
												className="text-white/20 group-hover:text-cyan-400 transition-colors duration-300"
											/>
									...
									<td className="p-4 text-center">
										<div
											className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-sm text-[11px] font-medium transition-all duration-300 ${session.modules_count > 0 ? "bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 group-hover:bg-cyan-500/20" : "text-white/10 group-hover:text-white/20"}`}
										>
											<BookOpen
												size={12}
												className={
													session.modules_count > 0 ? "" : "opacity-30"
												}
											/>
									...
									<td className="p-4 text-center">
										<div
											className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-sm text-[11px] font-medium transition-all duration-300 ${session.questions_count > 0 ? "bg-amber-500/10 text-amber-400 border border-amber-500/20 group-hover:bg-amber-500/20" : "text-white/10 group-hover:text-white/20"}`}
										>
											<Brain
												size={12}
												className={
													session.questions_count > 0 ? "" : "opacity-30"
												}
											/>
									...
									<td className="p-4 pr-6 text-right">
										<div
											className={`inline-flex items-center gap-1.5 font-mono text-sm tracking-tight transition-transform duration-300 group-hover:-translate-x-1 ${session.xp_earned > 0 ? "text-indigo-400 font-semibold" : "text-white/20"}`}
										>
											{session.xp_earned > 0 && (
											...
											<Trophy
												size={13}
												className={`ml-1 ${session.xp_earned > 0 ? "text-cyan-400 group-hover:drop-shadow-[0_0_8px_rgba(34,211,238,0.5)] transition-all" : "opacity-30"}`}
											/>
										</div>
									</td>
								</tr>
				...
				{filteredSessions.length === 0 && (
					<div className="p-16 flex flex-col items-center justify-center text-center">
						...
						<p className="text-white/30 text-xs">
							Prueba a cambiar los filtros o el periodo seleccionado.
						</p>
					</div>
				)}
			</div>
