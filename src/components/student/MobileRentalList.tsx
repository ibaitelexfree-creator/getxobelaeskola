...
import { Anchor, Calendar, ChevronRight, Clock } from "lucide-react";
...
export default function MobileRentalList({
	...
	const [selectedService, setSelectedService] = useState<RentalService | null>(
		null,
	);
		...
		try {
			...
			await fetch(getApiUrl("/api/legal/consent"), {
				...
				body: JSON.stringify({
					...
					consentType:
						selectedService.categoria === "membresias"
							? "membership"
							: "rental",
	...
	return (
		<div className="pb-32">
					...
					<div className="bg-nautical-black border-t border-white/10 w-full max-w-md rounded-t-3xl p-6 pb-12 relative z-10 animate-fade-in-up max-h-[85vh] overflow-y-auto">
						...
						<div className="space-y-6">
							<div className="space-y-2">
								<label className="text-xs uppercase tracking-widest text-accent font-bold block">
							...
							<div className="space-y-2">
								...
								<div className="relative">
									...
									>
										<option value="">
											{t("booking.no_times_available")?.includes("hoy")
												? "Selecciona hora"
												: t("booking.date_label")}
										</option>
										{times.map((t) => (
									...
									<div className="grid gap-2">
										{selectedService.opciones.map((opt, idx) => (
											<button
												key={idx}
												onClick={() =>
													setSelectedOption(selectedOption === idx ? null : idx)
												}
												className={`p-3 rounded-xl border text-left transition-all flex justify-between ${
													selectedOption === idx
							...
							>
								{loading
									...
									: `${t("booking.confirm")} (${selectedService.precio_base + (selectedOption !== null ? selectedService.opciones[selectedOption].extra : 0)}€)`}
							</button>
						</div>
					</div>
				</div>
			...
			<LegalConsentModal
				...
				activityType={
					selectedService?.categoria === "membresias" ? "membership" : "rental"
				}
				initialData={
					user
						? {
