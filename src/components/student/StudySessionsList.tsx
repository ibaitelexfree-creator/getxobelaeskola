...
import { Download, Calendar, Clock, BookOpen, HelpCircle, Trophy } from 'lucide-react';
...
export default function StudySessionsList({ sessions, locale }: StudySessionsListProps) {
    const [selectedMonth, setSelectedMonth] = useState<string>('all');

    const months = useMemo(() => {
        const uniqueMonths = Array.from(new Set(sessions.map(s => s.date.substring(0, 7)))).sort().reverse();
        return uniqueMonths;
    }, [sessions]);
    ...
    const exportCSV = () => {
        const headers = ['Fecha', 'Duración (min)', 'Módulos Visitados', 'Preguntas Respondidas', 'XP Ganado'];
        ...
        link.setAttribute("download", `sesiones_estudio_${new Date().toISOString().split('T')[0]}.csv`);
    ...
    return (
        <section className="bg-card border border-card-border rounded-sm p-6 md:p-8">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
                <div>
                    <h2 className="text-xs uppercase tracking-widest text-accent font-bold mb-2">Historial de Estudio</h2>
                    <p className="text-foreground/60 text-sm">Resumen de tu actividad académica</p>
                </div>
                    ...
                    >
                        ...
                        {months.map(m => (
                            <option key={m} value={m}>
                                {new Date(m + '-01').toLocaleDateString(locale === 'es' ? 'es-ES' : 'en-GB', { month: 'long', year: 'numeric' })}
                            </option>
                        ))}
                    </select>
