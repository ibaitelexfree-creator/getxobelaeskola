...
export default function PlannerContainer({ locale }: { locale: string }) {
    ...
    const fetchPlannerData = useCallback(async () => {
        ...
        try {
            ...
            const res = await fetch(getApiUrl(`/api/student/study-planner?start_date=${start.toISOString()}&end_date=${end.toISOString()}`));
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row justify-between items-end gap-4 mb-8">
                <div>
                    <h2 className="text-xs uppercase tracking-widest text-accent font-bold mb-2">Planificador de Estudio</h2>
                    <h1 className="text-3xl font-display text-white italic">
                        Semana del {days[0].toLocaleDateString(locale, { day: 'numeric', month: 'long' })}
                    </h1>
                </div>

                <div className="flex gap-2">
                    <button onClick={() => changeWeek(-1)} className="p-2 bg-white/5 hover:bg-white/10 rounded-sm text-white transition-colors">
                        <ChevronLeft size={20} />
                    </button>
                    <button onClick={() => setCurrentDate(new Date())} className="px-4 py-2 bg-white/5 hover:bg-white/10 rounded-sm text-white text-xs uppercase tracking-widest font-bold transition-colors">
                        Hoy
                    </button>
                    <button onClick={() => changeWeek(1)} className="p-2 bg-white/5 hover:bg-white/10 rounded-sm text-white transition-colors">
