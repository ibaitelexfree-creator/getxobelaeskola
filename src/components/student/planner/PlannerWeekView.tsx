		...
		<div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-2">
			{days.map((day) => {
				...
				return (
					<div
						key={day.toISOString()}
						className={`bg-card border ${isToday ? "border-accent" : "border-card-border"} rounded-sm p-3 min-h-[150px] flex flex-col`}
					>
						<div
							className={`text-[10px] uppercase tracking-widest ${isToday ? "text-accent" : "text-white/40"} mb-3 text-center border-b border-white/5 pb-2 font-bold`}
						>
							{formatDate(day)}
						</div>
