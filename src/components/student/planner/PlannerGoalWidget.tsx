...
export default function PlannerGoalWidget({ actualMinutes, goalMinutes, onUpdateGoal }: PlannerGoalWidgetProps) {
    const percentage = goalMinutes > 0 ? Math.min((actualMinutes / goalMinutes) * 100, 100) : 0;
        ...
        <div className="bg-card border border-card-border p-6 rounded-sm flex flex-col justify-between h-full relative overflow-hidden">
            ...
            <div>
                <div className="flex justify-between items-start mb-6 relative z-10">
                    ...
                    <button
                        onClick={() => {
                            const newGoal = prompt('Establece tu meta semanal en minutos:', goalMinutes.toString());
                ...
                <div className="flex items-end gap-2 mb-2 relative z-10">
                    <span className="text-4xl font-display text-white italic">{actualMinutes}</span>
                    <span className="text-sm text-white/40 mb-1 font-medium">/ {goalMinutes} min</span>
                </div>
            </div>
