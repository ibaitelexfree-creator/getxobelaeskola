...
export default function MobileProfileClient({
	...
	useEffect(() => {
		async function loadProfile() {
			...
			if (!user) {
				router.replace(
					`/${locale}/auth/login?returnTo=/${locale}/student/profile`,
				);
				return;
			}
		...
		<div className="pb-24 bg-nautical-black min-h-screen">
			...
			<div className="pt-12 px-6 pb-8 bg-gradient-to-b from-white/5 to-transparent">
				<div className="flex flex-col items-center">
					...
					<h1 className="text-2xl font-display text-white">
						{currentProfile?.nombre || "Navegante"}{" "}
						{currentProfile?.apellidos || ""}
					</h1>
					...
					{isSocio && (
						<div className="mt-4 px-4 py-1.5 rounded-full bg-gradient-to-r from-amber-100 via-yellow-300 to-amber-500 border border-yellow-200 shadow-[0_0_15px_rgba(252,211,77,0.3)] flex items-center gap-2">
							...
							<span className="text-yellow-900 text-[10px] font-black uppercase tracking-widest">
								Socio Oficial
							</span>
						</div>
					)}
			...
			<div className="px-6 space-y-6 mt-4">
				...
				<section className="space-y-3">
					<h2 className="text-xs uppercase tracking-widest text-white/40 font-bold ml-1">
						Cuenta
					</h2>
				...
				<section className="space-y-3">
					...
					<div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden flex divide-x divide-white/10">
						...
						>
							<span
								className={`text-[8px] font-black ${locale === "es" ? "text-accent" : "text-white/40"}`}
							>
								ES
							</span>
							<span
								className={`text-xs ${locale === "es" ? "text-white" : "text-white/40"}`}
							>
								Español
							</span>
						</button>
						...
						>
							<span
								className={`text-[8px] font-black ${locale === "eu" ? "text-accent" : "text-white/40"}`}
							>
								EU
							</span>
							<span
								className={`text-xs ${locale === "eu" ? "text-white" : "text-white/40"}`}
							>
								Euskara
							</span>
						</button>
						...
						>
							<span
								className={`text-[8px] font-black ${locale === "en" ? "text-accent" : "text-white/40"}`}
							>
								EN
							</span>
							<span
								className={`text-xs ${locale === "en" ? "text-white" : "text-white/40"}`}
							>
								English
							</span>
						</button>
						...
						>
							<span
								className={`text-[8px] font-black ${locale === "fr" ? "text-accent" : "text-white/40"}`}
							>
								FR
							</span>
							<span
								className={`text-xs ${locale === "fr" ? "text-white" : "text-white/40"}`}
							>
								Français
							</span>
						</button>
