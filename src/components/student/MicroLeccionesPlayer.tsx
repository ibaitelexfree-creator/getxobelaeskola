...
import { X, ChevronDown, ChevronUp, Play, Volume2, VolumeX } from 'lucide-react';
import { MicroLeccion } from './MicroLeccionesWidget';
...
export default function MicroLeccionesPlayer({ lessons, initialIndex, onClose, locale }: MicroLeccionesPlayerProps) {
    ...
    return (
        <div className="fixed inset-0 z-[10000] bg-black flex items-center justify-center backdrop-blur-3xl">
                            ...
                            <div className="flex justify-between items-end">
                                <div className="flex-1 mr-4">
                                    ...
                                    <p className="text-sm text-white/90 line-clamp-2 drop-shadow-sm font-medium">
                                        {locale === 'es' ? lesson.descripcion_es : lesson.descripcion_eu}
                                    </p>
                                </div>
                                <div className="flex flex-col gap-4">
                                    <button onClick={toggleMute} className="p-2 bg-white/10 rounded-full backdrop-blur-md text-white hover:bg-white/20">
            ...
            <div className="hidden md:flex flex-col gap-4 absolute right-10 top-1/2 -translate-y-1/2 text-white/20">
                <ChevronUp size={32} />
                <span className="text-[10px] uppercase tracking-widest rotate-90 origin-center translate-y-2">Scroll</span>
                <ChevronDown size={32} />
            </div>
        </div>
    );
