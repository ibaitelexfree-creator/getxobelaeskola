...
export default function DailyChallengeWidget({
	...
	const [status, setStatus] = useState<
		"idle" | "answering" | "correct" | "wrong" | "completed"
	>("idle");
	...
	useEffect(() => {
		fetchDailyChallenge();
		fetchUserStats();
	}, [fetchDailyChallenge, fetchUserStats]);
		...
		try {
			...
			if (attempts && attempts.length > 0) {
				...
				const diffDays = Math.floor(
					(today.getTime() - checkDate.getTime()) / (1000 * 3600 * 24),
				);
					...
					{status === "completed" ? (
						...
						>
							...
							>
								<div className="flex items-center gap-3 mb-4">
									...
									<p className="text-sm font-bold tracking-tight uppercase">
										{status === "correct"
											? t("correct", { xp: xpAwarded })
											: t("wrong", {
													answer:
														challenge?.opciones[challenge.respuesta_correcta] ||
														"",
												})}
									</p>
								</div>
								...
								<div className="space-y-3">
									...
									<p className="text-xs text-white/70 italic leading-relaxed font-light">
										"
										{locale === "es"
											? challenge?.explicacion_es
											: challenge?.explicacion_eu}
										"
									</p>
								</div>
							</div>
							...
							<div className="bg-white/5 p-4 flex justify-between items-center rounded-sm">
								<div className="text-[9px] uppercase tracking-widest text-white/40 italic">
									Progreso de Racha
								</div>
						...
						>
							<h3 className="text-2xl font-display text-white leading-[1.2] italic underline decoration-accent/20 underline-offset-8">
								{locale === "es"
									? challenge?.pregunta_es
									: challenge?.pregunta_eu}
							</h3>
							...
							<div className="grid gap-3">
								{challenge?.opciones.map((opcion, index) => (
									<motion.button
										...
										className={`group/btn w-full text-left p-4 text-xs transition-all border relative overflow-hidden ${
									...
									>
										<div className="relative z-10 flex items-center justify-between">
											...
											{selectedOption === index && (
												<motion.span layoutId="active" className="text-accent">
													⚓
												</motion.span>
											)}
										</div>
							...
							>
								{status === "answering" ? (
									<span className="animate-pulse">SINCRONIZANDO RUMBO...</span>
								) : (
									t("submit")
								)}
							</motion.button>
