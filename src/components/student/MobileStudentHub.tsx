...
export default function MobileStudentHub({
	...
	const hasBookings =
		upcomingInscripciones.length > 0 || upcomingRentals.length > 0;
					...
					<div className="flex gap-4 overflow-x-auto pb-8 pr-6 snap-x snap-mandatory scrollbar-hide">
						{upcomingInscripciones.map((ins: any, i: number) => {
							...
							return (
								<div
									key={`course-${i}`}
									className="min-w-[85%] sm:min-w-[300px] snap-center bg-nautical-black border border-white/10 rounded-2xl p-5 relative overflow-hidden h-32 flex flex-col justify-between group"
								>
									...
									<div className="z-10">
										<h3 className="text-lg font-display text-white italic truncate">
											{name}
										</h3>
						...
						{upcomingRentals.map((rent: any, i: number) => {
							...
							return (
								<div
									key={`rental-${i}`}
									className="min-w-[85%] sm:min-w-[300px] snap-center bg-nautical-black border border-white/10 rounded-2xl p-5 relative overflow-hidden h-32 flex flex-col justify-between"
								>
				...
				>
					<div className="flex items-center gap-4 relative z-10">
						...
						<div>
							...
							<p className="text-accent text-xs uppercase tracking-wider font-bold">
								Gana +50 XP hoy
							</p>
						</div>
