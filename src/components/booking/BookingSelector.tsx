...
export default function BookingSelector({
	...
	useEffect(() => {
		...
		checkUser();
	}, [supabase.auth.getUser, supabase.from]);
	...
	const handleLegalConfirm = async (legalData: {
		...
		try {
			const selectedEditionData = editions.find(
			...
			const consentResponse = await fetch(apiUrl("/api/legal/consent"), {
				...
				body: JSON.stringify({
					...
					dni: legalData.dni,
					legalText:
						"He leído y acepto expresamente las condiciones legales detalladas anteriormente. Entiendo que esta aceptación equivale a una firma digital vinculante.",
			...
			if (!consentResponse.ok) {
				throw new Error(
					"No se pudo registrar la firma legal. Inténtalo de nuevo.",
				);
			}
			...
			const errorMessage =
				(error as Error).message || t("payment_gateway_error");
	...
	return (
		<div className="space-y-6">
						...
						{editions && editions.length > 0 ? (
							<div className="grid gap-3">
								{editions.map((edition) => {
									const seatsLeft =
										edition.plazas_totales - edition.plazas_ocupadas;
									...
									return (
										<button
											...
											className={`w-full p-4 border text-left transition-all duration-300 flex justify-between items-center focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-2 focus:ring-offset-nautical-black ${
												isSelected
										...
										>
											...
											<div className="text-right">
												<p
													className={`text-2xs uppercase tracking-widest font-bold ${isSelected ? "text-accent" : "text-foreground/40"}`}
												>
			...
			<button
				...
				aria-label={
					loading ? t("processing") : `${t("book_for")} ${coursePrice} euros`
				}
				aria-busy={loading}
