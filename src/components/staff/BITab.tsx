...
export default function BITab() {
	...
	const handleExportCSV = () => {
		...
		link.setAttribute(
			"download",
			`BI_Report_${dateRange.start}_${dateRange.end}.csv`,
		);
	...
	const handleExportPDF = () => {
		...
		autoTable(doc, {
			...
			body: [
				...
				[
					"Beneficio Neto",
					`${(data.kpis.totalRevenue - data.kpis.totalCost).toLocaleString()}€`,
				],
		...
		autoTable(doc, {
			...
			head: [
				[
					"Embarcacion / Servicio",
					"Ingresos (€)",
					"Gastos (€)",
					"Beneficio (€)",
				],
			],
			body: data.boatProfitability.map((p) => [
				p.name,
				p.revenue,
				p.cost,
				p.profit,
	...
	if (loading)
		return (
			<div className="flex flex-col items-center justify-center h-96 space-y-4">
				...
				<p className="text-white/40 font-display italic animate-pulse">
					Analizando bitácoras de inteligencia...
				</p>
			</div>
		);
	...
	if (error)
		return (
			<div className="p-10 text-red-500 bg-red-500/10 border border-red-500/20 rounded-sm">
				{error}
			</div>
		);
	...
	return (
		<div className="space-y-12 animate-premium-in pb-20">
			...
			<div className="flex flex-wrap items-center justify-between gap-6 p-6 bg-white/[0.02] border border-white/5 rounded-sm">
				<div className="flex items-center gap-4">
					...
					<div>
						...
						<div className="text-sm font-display text-white italic">
							Sistema Chronos Active
						</div>
					</div>
				</div>
			...
			<div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-12 border-b border-white/5 pb-12">
				<div className="space-y-3">
					...
					<p className="text-white/30 text-xs max-w-xl leading-relaxed">
						Datos del {dateRange.start} al {dateRange.end}. Evalúa la
						rentabilidad de tu flota, la demanda de cursos y el rendimiento
						financiero frente a objetivos.
					</p>
				</div>
					...
					{[
						{
							...
							comparison: data?.kpis.prevPeriodRevenue
								? {
										value: (
											((data.kpis.totalRevenue - data.kpis.prevPeriodRevenue) /
												data.kpis.prevPeriodRevenue) *
											100
										).toFixed(1),
										positive:
											data.kpis.totalRevenue >= data.kpis.prevPeriodRevenue,
						...
						{
							label: "COSTES MTTO",
							value: `${data?.kpis.totalCost.toLocaleString()}€`,
							icon: <Activity className="text-red-400" />,
							color: "border-red-500/20",
						},
						{
							label: "BENEFICIO NETO",
							value: `${((data?.kpis.totalRevenue || 0) - (data?.kpis.totalCost || 0)).toLocaleString()}€`,
						...
						<div
							key={i}
							className={`glass-card p-6 border ${kpi.color} group hover:bg-white/5 transition-all relative overflow-hidden`}
						>
							<div className="flex justify-between items-start mb-4">
								<div className="p-2 bg-white/5 rounded-sm">{kpi.icon}</div>
								{kpi.comparison && (
									<div
										className={`flex items-center gap-1 text-[9px] font-black tracking-tighter ${kpi.comparison.positive ? "text-green-400" : "text-red-400"}`}
									>
										...
										<span className="text-[7px] text-white/20 ml-0.5">YoY</span>
									</div>
								)}
							</div>
							<div className="text-2xl font-mono text-white mb-1 group-hover:scale-110 transition-transform origin-left">
								{kpi.value}
							</div>
							<div className="text-[9px] text-white/40 font-black tracking-widest">
								{kpi.label}
							</div>
						</div>
					))}
			...
			<div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
					...
					<header className="flex justify-between items-center">
						<div className="space-y-1">
							...
							<p className="text-[10px] text-white/20 uppercase tracking-[0.2em] font-black">
								Conversión de alumnos a clientes recurrentes
							</p>
						</div>
				...
				<div className="glass-panel p-10 space-y-8 min-h-[500px] flex flex-col">
					<header className="flex justify-between items-center">
						<div className="space-y-1">
							<h3 className="text-2xl font-display text-white italic">
								Análisis ROI de Flota
							</h3>
							<p className="text-[10px] text-white/20 uppercase tracking-[0.2em] font-black">
								Ingresos vs Gastos de Mantenimiento
							</p>
						</div>
				...
				<div className="glass-panel p-10 space-y-8 min-h-[500px] flex flex-col">
					<header className="flex justify-between items-center">
						<div className="space-y-1">
							...
							<p className="text-[10px] text-white/20 uppercase tracking-[0.2em] font-black">
								Progreso mensual vs Objetivo: {monthlyTarget.toLocaleString()}€
							</p>
						</div>
						<div className="flex items-center gap-3">
							{isAdjustingTarget ? (
								<div className="flex items-center gap-2 animate-premium-in">
									...
									/>
									<button
										onClick={() => setIsAdjustingTarget(false)}
										className="text-[10px] text-accent font-black"
									>
										OK
									</button>
								</div>
				...
				<div className="glass-panel p-10 col-span-full space-y-8 min-h-[500px] flex flex-col relative overflow-hidden">
					...
					<header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
						<div className="space-y-1">
							<h3 className="text-2xl font-display text-white italic">
								Densidad de Demanda Académica
							</h3>
							<p className="text-[10px] text-white/20 uppercase tracking-[0.2em] font-black">
								Inscripciones por categoría de curso y mes
							</p>
						</div>
						<div className="flex gap-2">
							<span className="px-3 py-1 bg-white/5 border border-white/10 rounded-full text-[9px] font-black text-white/40 uppercase tracking-widest">
								Vista Mensual
							</span>
						</div>
					</header>
						...
						<ResponsiveContainer width="100%" height="100%">
							<LineChart data={courseDemandData}>
								...
								<Legend
									...
									wrapperStyle={{ paddingLeft: 40, fontSize: 10 }}
								/>
			...
			<div className="glass-panel p-12 bg-gradient-to-br from-white/[0.03] to-transparent border border-white/10 rounded-sm relative group overflow-hidden">
				...
				<h4 className="text-3xl font-display text-white italic mb-8">
				...
				<div className="grid md:grid-cols-3 gap-8 relative z-10">
					...
					<div className="space-y-4">
						...
						<p className="text-sm text-white/60 leading-relaxed font-display italic">
							Los cursos de 'Licencia de Navegación' muestran un pico de demanda
							en{" "}
							{courseDemandData.length > 0
								? courseDemandData[courseDemandData.length - 1].month
								: "este mes"}
							. Refuerza el staff de instructores para evitar lista de espera.
						</p>
					</div>
					...
					<div className="space-y-4">
						...
						<p className="text-sm text-white/60 leading-relaxed font-display italic">
							{data &&
							data.revenueComparison.length > 0 &&
							data.revenueComparison[data.revenueComparison.length - 1]
								.cumulativeActual > monthlyTarget
