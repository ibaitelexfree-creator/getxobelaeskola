			...
			<div className="grid lg:grid-cols-2 gap-20">
				<div className="space-y-8">
					<div className="animate-premium-in">
						<h2 className="text-5xl font-display text-white italic">
							Académico
						</h2>
						<p className="text-technical mt-3">
							Gestión de expedientes y alumnos
						</p>
					</div>
					<div className="relative group/search">
						...
						/>
						<div className="absolute right-8 top-1/2 -translate-y-1/2 text-technical group-hover/search:text-accent transition-colors">
							Escaneo de Base 🔍
						</div>
					</div>
						...
						{students.map((s, idx) => (
							...
							>
								<div className="text-left">
									<span className="block font-display text-xl leading-none text-white group-hover:text-accent transition-colors italic">
								...
								<div className="flex flex-col items-end">
									<span className="text-3xs font-black italic text-white/20 group-hover:text-accent transition-all tracking-widest">
										EXPEDIENTE
									</span>
						...
						<div className="glass-panel p-10 rounded-sm space-y-12 sticky top-32 animate-premium-in shadow-2xl overflow-hidden">
							...
							<div className="border-b border-white/5 pb-10">
								<div className="flex justify-between items-start mb-6">
									<span className="text-technical text-accent bg-accent/5 px-4 py-1.5 border border-accent/20 rounded-full">
										{t("courses.edit_profile.title")}
									</span>
									<div className="flex items-center gap-6">
										{!isEditingStudent ? (
											<button
												onClick={() => setIsEditingStudent(true)}
												className="text-technical text-white/40 hover:text-accent transition-colors flex items-center gap-2 group/edit"
											>
											...
											<button
												...
												className="text-technical text-white/20 hover:text-white transition-colors"
											>
										...
										<button
											...
											className="text-technical text-white/20 hover:text-red-500 transition-colors"
										>
											{t("courses.edit_profile.close_btn")} ✕
										</button>
									</div>
								</div>
								...
								{!isEditingStudent ? (
									<div className="space-y-4">
										<h3 className="text-6xl font-display text-white italic leading-none tracking-tight">
											{selectedStudent?.nombre || "Alumno"}
											<br />
										...
										<p className="text-technical text-accent/60 flex items-center gap-4">
											<span>
												{t(
													`courses.edit_profile.role.${selectedStudent?.rol || "student"}`,
												)}
											</span>
											<span className="w-1 h-1 bg-white/20 rounded-full" />
											<span className="font-mono">
												{selectedStudent?.telefono || "Sin teléfono"}
											</span>
										</p>
									</div>
								) : (
									<div className="space-y-4">
										...
										<div className="grid grid-cols-2 gap-4">
											...
											>
												<option value="alumno">
													{t("courses.edit_profile.role.student")}
												</option>
												<option value="socio">
													{t("courses.edit_profile.role.partner")}
												</option>
												<option value="instructor">
													{t("courses.edit_profile.role.instructor")}
												</option>
												<option value="admin">
													{t("courses.edit_profile.role.admin")}
												</option>
											</select>
											<input
										...
										>
											{isSavingProfile
												? t("courses.edit_profile.saving")
												: t("courses.edit_profile.save_btn")}
										</button>
									</div>
								)}
							</div>

							<h4 className="text-technical mb-6 opacity-30">
								{t("courses.inscriptions_title")}
							</h4>
							{(studentInscriptions || []).length > 0 ? (
								<div className="space-y-4">
									{(studentInscriptions || []).map((ins, idx) => (
										<div
											key={`ins-${idx}`}
											className="p-8 glass-card flex justify-between items-center group/ins"
										>
											<div className="space-y-2">
												<p className="text-lg font-display text-white italic group-hover:text-accent transition-colors">
													{ins?.ediciones_curso?.cursos?.nombre_es ||
														ins?.cursos?.nombre_es ||
														t("courses.equipment_rental")}
												</p>
												<div className="flex items-center gap-4">
													<p className="text-technical">
														<ClientDate
															date={
																ins?.ediciones_curso?.fecha_inicio ||
																ins?.created_at
															}
														/>
													</p>
																...
																if (confirm(t("courses.delete_confirm"))) {
																	try {
																		const res = await fetch(
																			apiUrl("/api/admin/delete-inscription"),
																			{
																				method: "POST",
																		...
																		if (res.ok)
																			setStudentInscriptions((prev) =>
																				prev.filter((i) => i.id !== ins.id),
																			);
																	} catch {
																		alert(t("courses.delete_error"));
																	}
																}
															}}
