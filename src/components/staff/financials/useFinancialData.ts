...
import { FinancialTransaction, ChartDataPoint } from './types';
import { parseAmount } from '@/lib/utils/financial';

export function useFinancialData(initialData: FinancialTransaction[], initialView?: string | null) {
    const [transactions, setTransactions] = useState<FinancialTransaction[]>(initialData);

    // View & Date State
    const [activeFilter, setActiveFilter] = useState(initialView || 'year');
    ...
    const chartData = useMemo(() => {
        ...
        const agg: Record<string, ChartDataPoint> = {};
        ...
        filteredData.forEach(item => {
            ...
            if (isHourly) {
                ...
                if (targetBucket) {
                    ...
                    // Add to first bucket if earlier
