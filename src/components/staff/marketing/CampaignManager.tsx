...
export default function CampaignManager() {
	...
	const handleToggleStatus = async (id: string, currentStatus: boolean) => {
		try {
			...
			if (res.ok) {
				setCampaigns((prev) =>
					prev.map((c) => (c.id === id ? { ...c, activo: !currentStatus } : c)),
				);
			}
		} catch (error) {
	...
	if (isLoading) {
		return (
			<div className="flex items-center justify-center p-20">
				...
				<span className="ml-4 text-white/40 tracking-widest text-2xs uppercase font-bold">
		...
		<div className="space-y-8">
			<div className="flex justify-between items-center border-b border-white/5 pb-6">
				<div>
					...
					<p className="text-xs text-white/30 tracking-tight font-mono mt-1">
						Configura disparadores automáticos para el retorno de alumnos
					</p>
				</div>
				<button
			...
			<div className="grid gap-4">
				{campaigns.length === 0 ? (
					...
					campaigns.map((campaign) => (
						<div
							key={campaign.id}
							className={`p-6 border ${campaign.activo ? "border-white/10" : "border-white/5 opacity-50"} bg-white/[0.02] hover:bg-white/[0.05] transition-all group relative overflow-hidden`}
						>
							{campaign.activo && (
							...
							<div className="flex justify-between items-start gap-6">
								<div className="space-y-2 flex-grow">
									<div className="flex items-center gap-3">
										<h4 className="text-lg font-display text-white italic">
											{campaign.nombre}
										</h4>
										<span
											className={`px-2 py-0.5 text-[8px] font-black tracking-widest uppercase rounded-full ${campaign.activo ? "bg-accent/20 text-accent" : "bg-white/5 text-white/20"}`}
										>
											{campaign.activo ? "Activa" : "Inactiva"}
										</span>
									</div>
									<p className="text-xs text-white/40 max-w-2xl font-mono">
									...
									<div className="flex gap-8 mt-4">
										<div className="space-y-1">
											...
											<div className="text-xs text-white/80 font-display italic">
												{campaign.trigger_course?.nombre_es}
											</div>
											<div className="text-[10px] text-accent/60">
												+ {campaign.dias_espera} días de espera
											</div>
										</div>
										<div className="space-y-1">
											<span className="text-[9px] uppercase tracking-widest text-white/20 font-bold">
											...
											<div className="text-xs text-white/80 font-display italic">
												{campaign.target_course?.nombre_es}
											</div>
											<div className="text-[10px] text-accent/60">
												Cupón: {campaign.cupon_codigo} (-
												{campaign.descuento_porcentaje}%)
											</div>
								...
								<div className="flex gap-2 shrink-0">
									<button
										onClick={() =>
											handleToggleStatus(campaign.id, campaign.activo)
										}
			...
			>
				<form onSubmit={handleSave} className="space-y-6">
					<div className="grid gap-4">
						<div className="space-y-1">
							<label className="text-2xs uppercase tracking-widest text-white/40 font-bold">
							...
							<div className="space-y-1">
								...
								<select
									...
									onChange={(e) =>
										setFormData({
											...formData,
											curso_trigger_id: e.target.value,
										})
									}
								...
								>
									<option value="" disabled className="bg-nautical-black">
										Seleccionar curso...
									</option>
									{courses.map((course) => (
										<option
							...
							<div className="space-y-1">
								...
								<input
									...
									onChange={(e) =>
										setFormData({
											...formData,
											dias_espera: parseInt(e.target.value, 10),
										})
									}
							...
							<div className="space-y-1">
								...
								<select
									...
									onChange={(e) =>
										setFormData({
											...formData,
											curso_objetivo_id: e.target.value,
										})
									}
									className="w-full bg-white/5 border border-white/20 p-4 text-white outline-none focus:border-accent appearance-none custom-select"
								>
							...
							<div className="space-y-1">
								...
								<input
									...
									onChange={(e) =>
										setFormData({
											...formData,
											descuento_porcentaje: parseInt(e.target.value, 10),
										})
									}
									className="w-full bg-white/5 border border-white/10 p-4 text-white font-mono outline-none focus:border-accent"
								/>
							...
							<div className="space-y-1">
								...
								<input
									...
									onChange={(e) =>
										setFormData({
											...formData,
											cupon_codigo: e.target.value.toUpperCase(),
										})
									}
								...
								<p className="text-[9px] text-white/20 italic">
									Se usará si falla Stripe
								</p>
							</div>
							<div className="space-y-1">
								<label className="text-2xs uppercase tracking-widest text-white/40 font-bold text-yellow-400/60">
									ID Cupón Stripe (Dinámico)
								</label>
								<input
								...
								/>
								<p className="text-[9px] text-yellow-400/40 italic">
									Genera códigos únicos de un solo uso
								</p>
							</div>
						</div>
					</div>
