...
import { type CalendarDay, useCalendarGrid } from "@/hooks/useCalendarGrid";
		...
		<div className="glass-panel p-4 md:p-8 rounded-sm animate-premium-in overflow-hidden">
			<header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
				<div className="space-y-1">
					<h3 className="text-3xl font-display text-white italic capitalize">
						{monthName} <span className="text-accent">{year}</span>
					</h3>
					<p className="text-3xs text-white/40 uppercase tracking-widest font-black">
				...
				<div className="flex gap-4 w-full md:w-auto">
					<button
						onClick={prevMonth}
						className="flex-1 md:flex-none px-6 py-3 border border-white/10 hover:border-accent hover:text-accent transition-all text-sm"
					>
						←
					</button>
					<button
			...
			<div className="overflow-x-auto border border-white/10 rounded-sm">
				<div className="grid grid-cols-7 gap-px bg-white/5 min-w-[700px]">
					{weekDays.map((d) => (
						<div
							key={d}
							className="bg-nautical-black/50 p-4 text-3xs uppercase tracking-widest text-white/40 font-black text-center border-b border-white/10"
						>
							{d}
						</div>
					))}
						...
						return (
							...
							>
								<div className="flex justify-between items-start">
									<span
										className={`text-xl font-display italic transition-colors ${item.isToday ? "text-accent" : item.isCurrentMonth ? "text-white/20 group-hover:text-white/40" : "text-white/10"}`}
									>
										...
										return (
											...
											>
												<p className="text-[8px] font-black uppercase truncate mb-0.5">
													{new Date(s.fecha_inicio).toLocaleTimeString(locale, {
														hour: "2-digit",
														minute: "2-digit",
													})}
												</p>
												<p className="text-3xs font-display italic truncate opacity-80 group-hover/sess:opacity-100 text-white">
													{locale === "eu"
														? s.curso?.nombre_eu || s.curso?.nombre_es
														: s.curso?.nombre_es}
												</p>
											</button>
										);
