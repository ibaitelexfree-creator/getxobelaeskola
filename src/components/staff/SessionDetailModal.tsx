...
export default function SessionDetailModal({
	...
	const [activeTab, setActiveTab] = useState<"info" | "attendance" | "report">(
		"info",
	);
	...
	const [formData, setFormData] = useState<Session>({
		...
		instructor_id:
			session.instructor_id === "external" ? "" : session.instructor_id,
		...
		const loadData = async () => {
			...
			try {
				...
				const studentsRes = await fetch(
					apiUrl(`/api/admin/course-students?courseId=${session.curso_id}`),
				);
				if (studentsRes.ok) {
				...
				const attendanceRes = await fetch(
					apiUrl(`/api/admin/sessions/attendance?sessionId=${session.id}`),
				);
				if (attendanceRes.ok) {
					const attendanceData = await attendanceRes.json();
					if (attendanceData.attendance) {
						const attMap: Record<string, AttendanceRecord> = {};
						attendanceData.attendance.forEach(
							(rec: {
								usuario_id: string;
								asistencia: string;
								notas: string;
							}) => {
								attMap[rec.usuario_id] = {
									status: rec.asistencia,
									notes: rec.notas || "",
								};
							},
						);
		...
		const checkAvailability = async () => {
			try {
				const res = await fetch(
					apiUrl(
						`/api/admin/boats/availability?start=${encodeURIComponent(formData.fecha_inicio)}&end=${encodeURIComponent(formData.fecha_fin)}&excludeSessionId=${encodeURIComponent(session.id)}`,
					),
				);
			...
			<div className="-mx-8 -mt-8 mb-8">
				...
				<div className="p-8 border-b border-white/10 bg-white/5">
					...
					<p className="text-white/40 text-sm mt-1 font-light">
						...
						{formatTime(session.fecha_inicio)} - {formatTime(session.fecha_fin)}
					</p>
				</div>
				...
				<div className="flex border-b border-white/10 px-8 bg-card">
					{["info", "attendance", "report"].map((tab) => (
						<button
							key={tab}
							onClick={() =>
								setActiveTab(tab as "info" | "attendance" | "report")
							}
							className={`px-8 py-5 text-3xs uppercase tracking-widest font-black transition-all border-b-2 ${
								activeTab === tab
					...
					<div className="space-y-8 max-w-3xl mx-auto py-4">
						<div className="grid grid-cols-2 gap-8">
							<div className="space-y-3">
								<label className="text-3xs uppercase tracking-widest text-accent font-bold">
									Estado Actual
								</label>
								<select
								...
								<SearchableSelect
									...
									options={allBoats
										...
										.sort((a, b) => {
											...
											if (a.status === "occupied" && b.status === "free")
												return 1;
											if (a.status === "free" && b.status === "occupied")
												return -1;
									...
									options={[
										{
											...
											label: "Otros / Eventos Especiales",
											subLabel: "Actividades varias",
										},
										...allCourses.map((c) => ({
											id: c.id,
											label: c.nombre_es,
						...
						<div className="space-y-3">
							...
							<div className="grid grid-cols-2 gap-4">
								<div className="space-y-1">
									<span className="text-[8px] uppercase tracking-widest text-white/30 block">
										Inicio
									</span>
									<input
										type="datetime-local"
					...
					<div className="space-y-6">
						...
						{loading ? (
							<div className="flex flex-col items-center justify-center py-20 gap-4">
								...
								<span className="text-white/40 text-2xs uppercase tracking-widest">
									Cargando lista...
								</span>
							</div>
						) : students.length > 0 ? (
							<div className="grid gap-3">
								{students.map((student: Student) => {
									...
									return (
										<div
											key={student.id}
											className="p-5 bg-white/5 border border-white/5 rounded-sm flex flex-col md:flex-row justify-between items-center gap-4 group hover:bg-white/[0.07] transition-all"
										>
											<div className="flex items-center gap-5 w-full md:w-auto">
												<div
													className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-nautical-black text-lg transition-colors duration-300 ${
														status === "presente"
															? "bg-sea-foam text-nautical-black shadow-[0_0_15px_-3px_rgba(16,185,129,0.4)]"
															: status === "ausente"
																? "bg-red-500/20 text-red-500 border border-red-500/30"
																: status === "retraso"
																	? "bg-amber-400 text-nautical-black"
												...
												<div>
													<p className="text-lg text-white font-display italic group-hover:text-accent transition-colors">
														{student.nombre || "Desconocido"}{" "}
														{student.apellidos || ""}
													</p>
													<div className="flex items-center gap-3 mt-1">
														<p className="text-2xs text-white/40 font-light tracking-wide">
												...
												{[
													{
														...
														label: "Presente",
														activeClass:
															"bg-sea-foam border-sea-foam text-nautical-black font-bold",
													},
													{
														id: "retraso",
														label: "Retraso",
														activeClass:
															"bg-amber-400 border-amber-400 text-nautical-black font-bold",
													},
													{
														id: "ausente",
														label: "Ausente",
													...
													<button
														key={option.id}
														onClick={() =>
															handleAttendanceChange(student.id, option.id)
														}
														className={`flex-1 md:flex-none px-5 py-2.5 text-3xs uppercase tracking-widest transition-all border rounded-sm ${
															status === option.id
																? option.activeClass
				...
				{activeTab === "report" && (
					<div className="space-y-6 h-full flex flex-col max-w-4xl mx-auto">
						<div className="bg-amber-500/5 border border-amber-500/20 p-6 rounded-sm mb-4">
							<h4 className="text-amber-500 text-3xs uppercase tracking-widest font-bold mb-2">
								Instrucciones
							</h4>
							<p className="text-amber-200/60 text-2xs font-light leading-relaxed">
								Utiliza este espacio para registrar cualquier incidencia,
								cambios en las condiciones meteorológicas, daños en el material
								o notas sobre el progreso del grupo. Esta información será
								visible por la administración.
							</p>
						</div>
