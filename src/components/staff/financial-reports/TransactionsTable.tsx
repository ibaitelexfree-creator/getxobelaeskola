...
interface TransactionsTableProps {
    ...
    totalRevenue: number;
    mounted: boolean;
    onEdit: (item: FinancialTransaction, field: string) => void;
    initialDataLength: number;
    totalRecords: number;
    startDate: string;
    endDate: string;
    setForceShowAll: (v: boolean) => void;
}
                            ...
                            <tr key={item.id} className="group hover:bg-white/[0.01] transition-colors relative">
                                <td className="py-6 text-sm font-mono text-white/60">
                                    <div className="flex items-center gap-2 group/cell">
                                        <ReportDate date={item.fecha_pago || item.created_at || item.fecha_reserva} />
                                        ...
                                        <button
                                            onClick={() => onEdit(item, 'fecha_pago')}
                                ...
                                <td className="py-6">
                                    <div className="flex items-center gap-2 group/cell">
                                        ...
                                        <button
                                            onClick={() => onEdit(item, 'estado_pago')}
                                            className="opacity-0 group-hover/cell:opacity-100 p-1 hover:text-accent transition-all"
                                        >
                                            <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                                        </button>
                                ...
                                <td className="py-6 text-right">
                                    <div className="flex items-center justify-end gap-2 group/cell">
                                        ...
                                        <button
                                            onClick={() => onEdit(item, 'monto_total')}
                                            className="opacity-0 group-hover/cell:opacity-100 p-1 hover:text-accent transition-all"
                                        >
                                            <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                                        </button>
