			...
			<header className="flex justify-between items-end">
				<div className="space-y-2">
					<div className="flex items-center gap-3">
						...
						<span className="text-[10px] uppercase tracking-[0.4em] font-black text-brass-gold/60">
							Cloud Documentation
						</span>
					</div>
					<h2 className="text-4xl font-display text-white italic tracking-tight">
						Google Drive{" "}
						<span className="text-brass-gold underline underline-offset-8 decoration-brass-gold/20">
							Explorer
						</span>
					</h2>
				</div>
			...
			{error && (
				<div className="p-6 bg-red-500/10 border border-red-500/20 rounded-sm">
					...
					<p className="text-white/40 text-[10px] mt-2 italic">
						Asegúrate de que la carpeta de Drive esté compartida con la cuenta
						de servicio.
					</p>
				</div>
			)}
				...
				{isLoading ? (
					Array(6)
						...
						.map((_, i) => (
							<div
								key={i}
								className="h-32 bg-white/5 border border-white/5 rounded-sm animate-pulse"
							/>
						))
				) : files.length > 0 ? (
					files.map((file) => (
						...
						>
							...
							<div className="relative z-10 space-y-4">
								<div className="space-y-1">
									...
									<h4 className="text-lg font-display text-white group-hover:text-brass-gold transition-colors line-clamp-1">
										{file.name}
									</h4>
								</div>
								...
								<div className="flex justify-between items-end">
									...
									<span className="text-xs text-brass-gold opacity-0 group-hover:opacity-100 translate-x-4 group-hover:translate-x-0 transition-all">
										Abrir 🡒
									</span>
								</div>
							</div>
					...
					<div className="col-span-full py-20 text-center border border-dashed border-white/10 rounded-sm">
						<p className="text-white/20 italic font-display text-xl underline underline-offset-8 decoration-white/5">
							No se encontraron archivos en la carpeta compartida.
						</p>
					</div>
				)}
			</div>
