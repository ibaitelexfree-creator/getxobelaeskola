	...
	const fetchAcademyData = async (studentId: string) => {
		...
		try {
			const res = await fetch(
				`/api/admin/academy/student-progress?student_id=${studentId}`,
			);
			if (res.ok) {
		...
		<div className="space-y-12 animate-premium-in">
			<header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-white/10 pb-8 md:pb-12">
				<div className="space-y-2">
					...
					<p className="text-technical text-white/40 tracking-[0.2em] uppercase text-2xs">
						{t("academia.subtitle")}
					</p>
				</div>
							...
							students.map((s, idx) => (
								...
								>
									<div className="text-left">
										<span
											className={`block font-display text-xl leading-none italic ${selectedStudent?.id === s.id ? "text-accent" : "text-white group-hover:text-accent"} transition-colors`}
										>
											{s.nombre} {s.apellidos || ""}
										</span>
										<span className="text-technical mt-1 block group-hover:text-white transition-colors">
											{s.email}
										</span>
									</div>
									<div className="text-right">
										<span className="text-2xs font-black italic text-white/20 group-hover:text-accent transition-all tracking-widest uppercase">
											{t("courses.record")}
										</span>
									</div>
								</button>
				...
				<div className="min-h-[600px]">
					{selectedStudent ? (
						<div className="glass-panel p-10 rounded-sm space-y-12 sticky top-32 animate-premium-in shadow-2xl relative overflow-hidden">
							...
							<div className="border-b border-white/5 pb-10 flex justify-between items-start">
								<div className="space-y-4">
									<h3 className="text-5xl font-display text-white italic leading-none">
									...
									<div className="flex gap-4">
										<span className="text-technical text-accent bg-accent/5 px-4 py-1 border border-accent/20 rounded-full text-2xs uppercase font-bold">
											{t("academia.skills_count", {
												count: academyData?.skills?.length || 0,
											})}
										</span>
										<span className="text-technical text-white/40 px-4 py-1 border border-white/10 rounded-full text-2xs uppercase font-bold">
											{t("academia.diplomas_count", {
												count: academyData?.certificates?.length || 0,
											})}
										</span>
									</div>
								</div>
							...
							{isLoading ? (
								<div className="py-20 flex flex-col items-center justify-center space-y-4 animate-pulse">
									...
									<span className="text-technical text-white/20 uppercase tracking-[0.3em] text-3xs">
										{t("academia.loading_record")}
									</span>
								</div>
							) : academyData ? (
								<div className="space-y-12">
									...
									<div className="grid grid-cols-2 gap-6">
										<div className="p-6 bg-white/5 border border-white/5 rounded-sm">
											<span className="text-2xs text-white/40 uppercase font-black block mb-2">
												{t("academia.unidades_leidas")}
											</span>
											<span className="text-4xl font-display text-white italic">
												{
													academyData.progress.filter(
										...
										<div className="p-6 bg-white/5 border border-white/5 rounded-sm">
											<span className="text-2xs text-white/40 uppercase font-black block mb-2">
												{t("academia.quizzes_aprobados")}
											</span>
											<span className="text-4xl font-display text-accent italic">
												{
													academyData.progress.filter(
														(p) => p.nota_quiz !== null,
													).length
											...
											{academyData.skills.length > 0 ? (
												academyData.skills.map((s, idx) => (
													<div key={idx} className="group relative">
														<div
															className="w-14 h-14 bg-accent/10 border border-accent/20 rounded-full flex items-center justify-center text-2xl hover:bg-accent/20 transition-all cursor-default"
												...
												<p className="text-3xs text-white/20 italic">
													{t("academia.no_records")}
												</p>
											)}
										</div>
									</section>
												...
												academyData.certificates.map((c) => {
													const handleDownload = async (cert: any) => {
														...
														await generateCertificatePDF({
															studentName:
																`${selectedStudent?.nombre || ""} ${selectedStudent?.apellidos || ""}`.trim() ||
																"Estudiante",
															...
															distinction:
																(cert.nivel_distincion as any) || "standard",
														});
													};
													...
													return (
														<div
															key={c.id}
															className="p-6 glass-card flex justify-between items-center group/cert"
														>
															<div>
																<span className="text-3xs text-accent uppercase font-black tracking-widest mb-1 block">
																	{t("academia.official_cert")}
																</span>
																<p className="text-lg font-display text-white italic">
																	{c.curso?.nombre_es ||
																		c.nivel?.nombre_es ||
																		t("academia.nautical_formation")}
																</p>
																<p className="text-technical mt-1 opacity-40 text-2xs">
																	<ClientDate
																		date={c.created_at}
																		format="short"
																	/>
									...
									<section>
										...
										<div className="space-y-2 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
											{academyData.progress
												.slice()
												.reverse()
												.map((p, idx) => (
													<div
														key={idx}
														className="p-4 bg-white/5 border border-white/5 rounded-sm flex justify-between items-center"
													>
														<div className="flex items-center gap-4">
															<span
																className={`w-2 h-2 rounded-full ${p.estado === "completado" ? "bg-accent shadow-[0_0_10px_rgba(var(--color-accent),0.5)]" : "bg-white/20"}`}
															/>
														...
														<div className="text-right">
															<span className="text-3xs text-white/40">
																<ClientDate date={p.updated_at} format="date" />
															</span>
															{p.nota_quiz && (
											...
											{academyData.progress.length === 0 && (
												<p className="p-10 text-center text-3xs text-white/10 uppercase font-black">
													{t("academia.no_records")}
												</p>
											)}
										</div>
						...
						<div className="h-full border border-dashed border-white/5 rounded-sm flex flex-col items-center justify-center p-20 text-center space-y-4">
							<span className="text-6xl text-white/5 font-display italic">
								📚
							</span>
