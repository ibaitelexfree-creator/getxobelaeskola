...
export default function RentalsTab({
    filteredRentals, rentalSearch, setRentalSearch, rentalStatusFilter, setRentalStatusFilter,
    rentalSort, setRentalSort,
    setViewingHistory, setUpdatingStatus, totalPages, currentPage, onPageChange, loading
}: RentalsTabProps) {
        ...
        <div className="space-y-12 animate-premium-in">
            <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-white/10 pb-8 md:pb-12">
                <div className="space-y-2">
                    <span className="text-accent uppercase tracking-[0.4em] text-2xs font-bold block">{t('rentals.fleet_scan')}</span>
                    <h2 className="text-4xl md:text-6xl font-display text-white italic">{t('rentals.title')}</h2>
                </div>
                <div className="flex flex-col md:flex-row gap-4 w-full md:w-auto overflow-x-auto pb-2 md:pb-0">
                    ...
                    >
                        ...
                        <option value="pendiente">{t('rentals.status_filter.pending')}</option>
                        <option value="entregado">{t('rentals.status_filter.in_water')}</option>
                        <option value="devuelto">{t('rentals.status_filter.finished')}</option>
                    </select>
                    <select
                        value={rentalSort}
            ...
            {loading ? (
                <div className="py-20 text-center animate-pulse">
                    <div className="text-3xl font-display text-white/20 italic">Cargando flota...</div>
                </div>
            ) : (
                <div className="grid gap-4">
                    {filteredRentals.length > 0 ? filteredRentals.map(r => (
                        <div key={r.id} className="glass-card p-6 md:p-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 md:gap-8 rounded-sm animate-premium-in">
                            <div className="text-center min-w-[110px] md:min-w-[150px] border-r border-white/5 pr-6 md:pr-10 shrink-0">
                                <div className="flex items-center justify-center gap-1 -mb-1">
                                    <span className="text-2xl md:text-4xl font-display text-accent italic tracking-tighter">
                                        {typeof r.fecha_reserva === 'string' ? r.fecha_reserva.split('-')[2] : '--'}
                                    </span>
                                    <span className="text-lg md:text-xl font-display text-accent/30 italic">/</span>
                                    <span className="text-2xl md:text-4xl font-display text-accent italic tracking-tighter">
                                        {typeof r.fecha_reserva === 'string' ? r.fecha_reserva.split('-')[1] : '--'}
                                    </span>
                                    <span className="text-lg md:text-xl font-display text-accent/30 italic">/</span>
                                    <span className="text-2xl md:text-4xl font-display text-accent italic tracking-tighter">
                                        {typeof r.fecha_reserva === 'string' ? r.fecha_reserva.split('-')[0].slice(-2) : '--'}
                                    </span>
                                </div>
                                <span className="block text-technical opacity-40 text-3xs md:text-2xs tracking-[0.3em] mt-1 font-bold">
                                    {r.hora_inicio ? r.hora_inicio.split(':').slice(0, 2).join(':') : '00:00'}
                                </span>
                            </div>
                            <div className="flex-1 min-w-0">
                                <h4 className="text-2xl md:text-3xl font-display text-white italic mb-1 truncate">{r.servicios_alquiler?.nombre_es}</h4>
                                <p className="text-technical text-white/30 text-2xs md:text-base truncate">{r.profiles?.nombre} {r.profiles?.apellidos}</p>
                            </div>
                            <div className="flex flex-wrap md:flex-nowrap items-center gap-4 md:gap-8 w-full md:w-auto">
                                <button onClick={() => setViewingHistory(r)} className="p-2 text-technical text-white/20 hover:text-accent transition-all text-2xs md:text-base">HISTORIAL ✍︎</button>
                    ...
                    )) : <div className="p-20 text-center border border-dashed border-white/5 rounded-sm text-white/20 italic">No hay salidas que coincidan con los filtros.</div>}
                </div>
            )}
