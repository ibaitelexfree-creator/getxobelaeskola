...
interface RentalStatusModalProps {
...
export default function RentalStatusModal({
    updatingStatus,
    statusNote,
    setStatusNote,
    onClose,
    onConfirm
}: RentalStatusModalProps) {
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-nautical-black/80 backdrop-blur-xl">
            <div className="w-full max-w-md glass-panel p-10 rounded-sm space-y-8 animate-premium-in">
                ...
                <div className="flex gap-4">
                    <button onClick={onClose} className="flex-1 py-4 border border-white/10 text-3xs uppercase tracking-widest text-white/40">{t('audit_editor.cancel')}</button>
                    <button onClick={onConfirm} className="flex-1 py-4 bg-accent text-nautical-black text-3xs uppercase tracking-widest font-bold">{t('rentals.status_modal.confirm')}</button>
                </div>
            </div>
        </div>
    );
