...
export default function FinancialReportsClient({ initialData, initialView, totalRecords = 0, error = null }: FinancialReportsClientProps) {
    ...
    const chartData = useMemo<ChartDataPoint[]>(() => {
        ...
        let mode: 'hour' | 'day' | 'month' | 'year' = 'day';
        ...
        if (mode === 'hour') {
            ...
            while (curr <= end) {
                ...
                agg[key] = { label, amount: 0, sortKey: key, dateObj: new Date(curr) };
            }
        }
        ...
        filteredData.forEach(item => {
            ...
            else if (mode === 'day') key = `${y}-${String(m).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
