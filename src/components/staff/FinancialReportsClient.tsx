	...
	try {
		...
		return (
			<span suppressHydrationWarning className="flex flex-col">
				...
				<span className="text-3xs text-accent/60">
...
export default function FinancialReportsClient({
	...
	const [transactions, setTransactions] =
		useState<FinancialTransaction[]>(initialData);
	...
	const filteredData = useMemo(() => {
		...
		if (!forceShowAll && startDate && endDate) {
			base = base.filter((item) => {
				const dateStr =
					item.fecha_pago || item.created_at || item.fecha_reserva;
		...
		if (searchTerm) {
			...
			base = base.filter((item) => {
				const clientName =
					`${item.profiles?.nombre || ""} ${item.profiles?.apellidos || ""}`.toLowerCase();
				const serviceName = (
					item.servicios_alquiler?.nombre_es || ""
				).toLowerCase();
		...
		if (serviceFilter !== "all") {
			base = base.filter(
				(item) => item.servicios_alquiler?.nombre_es === serviceFilter,
			);
		}
		...
		return [...base].sort((a, b) => {
			if (sortBy === "date_desc" || sortBy === "date_asc") {
				const dateA = new Date(
					a.fecha_pago || a.created_at || a.fecha_reserva || 0,
				).getTime();
				const dateB = new Date(
					b.fecha_pago || b.created_at || b.fecha_reserva || 0,
				).getTime();
	...
	const totalRevenue = useMemo(() => {
		return filteredData.reduce(
			(acc, curr) => acc + parseAmount(curr.monto_total),
			0,
		);
	}, [filteredData]);
	...
	const exportToCSV = () => {
		...
		link.setAttribute(
			"download",
			`informe_financiero_${startDate}_${endDate}.csv`,
		);
	...
	const handleSaveEdit = async (
		...
		try {
			const response = await fetch(
				apiUrl("/api/admin/rentals/financials/update"),
				{
					method: "POST",
	...
	const chartData = useMemo(() => {
		...
		if (forceShowAll && filteredData.length > 0) {
			const dates = filteredData
				.map((d) =>
					new Date(
						d.fecha_pago || d.created_at || d.fecha_reserva || "",
					).getTime(),
				)
				.filter((t) => !isNaN(t));
			...
			if (dates.length > 0) {
				...
				effectiveStart = new Intl.DateTimeFormat("en-CA", {
					timeZone: "Europe/Madrid",
					year: "numeric",
					month: "2-digit",
					day: "2-digit",
				}).format(new Date(min));
				effectiveEnd = new Intl.DateTimeFormat("en-CA", {
					timeZone: "Europe/Madrid",
					year: "numeric",
					month: "2-digit",
					day: "2-digit",
		...
		const agg: Record<
			string,
			{ label: string; amount: number; sortKey: string; dateObj: Date }
		> = {};
		...
		if (isHourly) {
			...
			while (
				curr <= end ||
				(curr.getUTCMonth() === end.getUTCMonth() &&
					curr.getUTCFullYear() === end.getUTCFullYear())
			) {
		...
		filteredData.forEach((item) => {
			...
			if (isHourly) {
				...
				const aggValues = Object.values(agg).sort(
		...
		<div className="space-y-12 pb-32">
			...
			<div className="flex flex-wrap items-end justify-between gap-8 p-10 glass-panel rounded-sm relative overflow-hidden">
				...
				<div className="flex flex-wrap items-center gap-6">
					<div className="space-y-2">
						<label className="text-[8px] uppercase tracking-widest text-white/20 block font-black">
							{t("from")}
						</label>
						<input
							type="date"
					...
					<div className="pt-6">
						<div className="text-right">
							<span className="text-technical text-accent block">
								{t("total_revenue")}
							</span>
							<span
								className="text-4xl font-display text-white italic"
								suppressHydrationWarning
							>
								{mounted ? totalRevenue.toLocaleString("es-ES") : "--"}€
							</span>
							<div
								className="text-[8px] text-white/20 font-mono mt-1"
								onClick={() => setShowDebug(!showDebug)}
							>
								Showing {filteredData.length} records (Loaded:{" "}
								{initialData.length}, DB: {totalRecords})
							</div>
						</div>
					</div>
				</div>
			...
			{showDebug && (
				<div className="p-4 bg-black/50 font-mono text-3xs text-green-400 overflow-auto max-h-40 border border-green-500/20">
					<div>
						DB Status: {totalRecords > 0 ? "HAS DATA" : "EMPTY or UNKNOWN"}
					</div>
					...
					<div>
						First Raw Reserva Date: {initialData[0]?.fecha_reserva || "N/A"}
					</div>
					<div>
						First Raw Hora Inicio: {initialData[0]?.hora_inicio || "N/A"}
					</div>
				</div>
			)}
			...
			<div className="glass-panel p-12 rounded-sm space-y-12">
				<header className="flex flex-wrap items-center justify-between gap-4 border-b border-white/5 pb-8">
					<h3 className="text-2xl font-display text-white italic underline underline-offset-[12px] decoration-accent/30 decoration-1">
						{t("chart_title")}
					</h3>
				...
				<div className="relative h-[300px] w-full flex items-end gap-2 group/chart border-b border-white/5 pb-2">
					{!mounted ? (
						...
						chartData.map((d, i) => {
							const barHeight = Math.max(
								(d.amount / maxChartValue) * CHART_H,
								4,
							);
							return (
								<div
					...
					<div className="absolute inset-0 flex flex-col justify-between pointer-events-none opacity-5">
						{[0, 1, 2, 3, 4].map((l) => (
							<div key={l} className="w-full h-px bg-white" />
						))}
					</div>
				</div>
			</div>
				...
				<div className="flex flex-wrap items-center justify-between gap-6">
					...
					<div className="flex flex-wrap items-center gap-4 bg-white/5 p-2 rounded-sm border border-white/10">
						...
						<div className="flex items-center gap-2">
							<label className="text-3xs uppercase tracking-widest text-white/40 font-black">
								{t("filter_status")}
							</label>
							<select
								value={statusFilter}
					...
					<table className="w-full text-left border-collapse">
						<thead>
							<tr className="border-b border-white/5">
								<th className="py-6 text-3xs uppercase tracking-widest text-white/40 font-black">
									{t("date")}
								</th>
								<th className="py-6 text-3xs uppercase tracking-widest text-white/40 font-black">
									{t("client")}
								...
								<th className="py-6 text-right text-3xs uppercase tracking-widest text-white/40 font-black">
									{t("amount")}
								</th>
							</tr>
						</thead>
									...
									>
											...
											<div className="flex items-center gap-2 group/cell">
												...
												{hasHistory(item, "fecha_pago") && (
													<div className="relative group/hist">
														<span className="text-orange-500 font-bold cursor-help animate-pulse">
															!
														</span>
														<div className="absolute left-0 bottom-full mb-2 hidden group-hover/hist:block z-50 w-64 p-3 bg-nautical-deep border border-white/10 rounded-lg shadow-2xl glass-panel">
															<p className="text-3xs uppercase tracking-tighter text-white/40 mb-2 border-b border-white/5 pb-1">
																Historial de fecha
															</p>
															<div className="space-y-2 max-h-40 overflow-y-auto custom-scrollbar">
																{getHistoryForField(item, "fecha_pago").map(
																	(h: any) => (
																		<div
																			key={h.id}
																			className="text-3xs border-l border-accent/20 pl-2"
																		>
												...
												<button
													onClick={() =>
														setEditingTx({ ...item, _field: "fecha_pago" })
													}
										...
										<td className="py-6">
											<p className="text-white font-display italic text-lg">
												{item.profiles?.nombre} {item.profiles?.apellidos}
											</p>
										</td>
												...
												{hasHistory(item, "servicio_id") && (
													<div className="relative group/hist">
														...
														<div className="absolute left-0 bottom-full mb-2 hidden group-hover/hist:block z-50 w-64 p-3 bg-nautical-deep border border-white/10 rounded-lg shadow-2xl glass-panel">
															...
															<div className="space-y-2 max-h-40 overflow-y-auto custom-scrollbar">
																{getHistoryForField(item, "servicio_id").map(
																	(h: any) => (
																		<div
																			key={h.id}
																			className="text-3xs border-l border-accent/20 pl-2"
										...
										<td className="py-6">
											<div className="flex items-center gap-2 group/cell">
												<span
													className={`text-3xs uppercase tracking-tighter px-3 py-1 border rounded-full ${item.estado_pago === "pagado" ? "border-green-500/30 text-green-500" : "border-accent/30 text-accent"}`}
												>
													{item.estado_pago}
												</span>
												{hasHistory(item, "estado_pago") && (
													<div className="relative group/hist">
														...
														<div className="absolute left-0 bottom-full mb-2 hidden group-hover/hist:block z-50 w-64 p-3 bg-nautical-deep border border-white/10 rounded-lg shadow-2xl glass-panel">
															...
															<div className="space-y-2 max-h-40 overflow-y-auto custom-scrollbar">
																{getHistoryForField(item, "estado_pago").map(
																	(h: any) => (
																		<div
																			key={h.id}
																			className="text-3xs border-l border-accent/20 pl-2"
												...
												<button
													onClick={() =>
														setEditingTx({ ...item, _field: "estado_pago" })
													}
													className="opacity-0 group-hover/cell:opacity-100 p-1 hover:text-accent transition-all"
												>
													<svg
										...
										<td className="py-6 text-right">
											<div className="flex items-center justify-end gap-2 group/cell">
												{hasHistory(item, "monto_total") && (
													<div className="relative group/hist">
														...
														<div className="absolute right-0 bottom-full mb-2 hidden group-hover/hist:block z-50 w-64 p-3 bg-nautical-deep border border-white/10 rounded-lg shadow-2xl glass-panel">
															...
															<div className="space-y-2 max-h-40 overflow-y-auto custom-scrollbar">
																{getHistoryForField(item, "monto_total").map(
																	(h: any) => (
																		<div
																			key={h.id}
																			className="text-3xs border-l border-accent/20 pl-2 text-right"
																		>
												...
												<span className="text-lg font-display text-white italic">
													{mounted
												...
												<button
													onClick={() =>
														setEditingTx({ ...item, _field: "monto_total" })
													}
													className="opacity-0 group-hover/cell:opacity-100 p-1 hover:text-accent transition-all"
												>
													<svg
									...
									>
										<div className="flex flex-col items-center gap-4">
											...
											<div className="text-2xs text-left bg-white/5 p-4 rounded border border-white/10 space-y-2 max-w-md">
												<div
													className={`font-bold ${totalRecords > 0 ? "text-green-400" : "text-red-400"}`}
												>
													Database Status: {totalRecords} total records found.
												</div>
												<div
													className={`${initialData.length > 0 ? "text-green-400" : "text-orange-400"}`}
												>
												...
												{initialData.length > 0 &&
													filteredData.length === 0 && (
														<div className="text-orange-400">
															Warning: {initialData.length} records are hidden
															by the current date filter ({startDate} to{" "}
															{endDate}).
															<br />
															<button
												...
												{totalRecords > 0 && initialData.length === 0 && (
													<div className="text-red-400">
														CRITICAL: Database has records, but API returned 0.
														This suggests an API limit or Permission issue.
													</div>
												)}
											</div>
										</div>
						...
						<tfoot>
							<tr className="border-t border-white/10 bg-white/[0.01]">
								<td colSpan={5} className="py-8 text-right pr-6">
									<span className="text-3xs uppercase tracking-[0.3em] font-black text-white/20">
										{t("total_sum")}
									</span>
								</td>
				...
				{editingTx && (
					<div className="space-y-8 py-2">
						<div className="p-4 bg-white/5 border border-white/10 rounded-xl mb-4">
							...
							<p className="text-accent font-mono text-2xs truncate">
								{editingTx.id}
							</p>
						</div>
						...
						<div className="space-y-6">
							{editingTx._field === "fecha_pago" && (
								<div className="space-y-3">
									<label className="text-3xs uppercase tracking-widest text-white/40 font-black block">
										Nueva Fecha de Pago
									</label>
									<input
										type="datetime-local"
										...
										id="edit-fecha"
									/>
									<p className="text-3xs text-white/30 italic">
										La fecha se guardará en formato UTC.
									</p>
								</div>
							)}
								...
								<div className="space-y-3">
									...
									<div className="relative">
										<input
											...
											autoFocus
										/>
										<span className="absolute right-4 top-1/2 -translate-y-1/2 text-white/20 text-xl font-display italic">
											€
										</span>
									</div>
								</div>
