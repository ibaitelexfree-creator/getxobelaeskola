	...
	return (
		<section className="relative h-screen w-full overflow-hidden bg-nautical-black">
			...
			{slides.map((slide, index) => (
				<div
					key={slide.id}
					className={`absolute inset-0 transition-opacity duration-1000 ease-[cubic-bezier(0.165,0.84,0.44,1)] ${
				...
				>
					<Image
						...
						quality={90}
						sizes="100vw"
						className={`object-cover transition-transform duration-[8000ms] ease-linear will-change-transform ${
							index === current
					...
					<div className="absolute inset-0 flex items-center">
						<div className="container mx-auto px-6 md:px-12">
							<div
								className={`max-w-4xl transition-all duration-1000 delay-500 transform ${
									index === current
							...
							>
								...
								<h2 className="text-5xl sm:text-7xl md:text-[10rem] lg:text-[12rem] font-display text-white mb-10 leading-[0.85] tracking-tighter">
									{slide.title.split(" ").map((word, i) => (
										<span
											key={i}
											className={`block ${i % 2 !== 0 ? "italic font-light text-accent/90 ml-12 sm:ml-24" : ""}`}
										>
											{word}
										</span>
									))}
			...
			>
				{slides.map((slide, index) => (
					...
					>
						<span
							className={`text-[10px] font-black tracking-widest transition-premium ${index === current ? "text-accent opacity-100" : "text-white/20 opacity-0 group-hover:opacity-100"}`}
						>
							0{index + 1}
						</span>
						<div
							className={`relative w-8 h-[2px] transition-premium overflow-hidden ${index === current ? "w-16 bg-accent/40" : "bg-white/10 group-hover:bg-white/30"}`}
						>
							{index === current && (
			...
			<div className="absolute bottom-12 left-1/2 -translate-x-1/2 flex flex-col items-center gap-6 z-30 opacity-40 hover:opacity-100 transition-premium group shadow-2xl">
				<span className="text-[9px] uppercase tracking-[0.5em] font-black text-white vertical-text">
					Scroll
				</span>
