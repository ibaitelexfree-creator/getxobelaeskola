...
export default function WeatherPremium({
		...
		return (
			<div className="bg-[#050c18] border border-red-500/20 rounded-sm p-8 text-center space-y-4">
				...
				<p className="text-white/60 text-xs uppercase tracking-widest">
					Error de Sincronización Meteorológica
				</p>
				<button
					onClick={fetchData}
					className="px-4 py-2 bg-white/5 text-[10px] uppercase font-black tracking-widest hover:bg-white/10 transition-all rounded-full"
				>
					Reintentar
				</button>
			</div>
					...
					>
						<div className="flex items-center gap-4 flex-1">
							...
							<div className="flex-1">
								<h5 className="text-red-500 text-[10px] uppercase font-black tracking-widest leading-none mb-1">
									Avisos Oficiales Euskalmet
								</h5>
								<div className="flex flex-wrap gap-x-4">
									{alerts.map((alert, i) => (
										<p key={i} className="text-white text-xs font-medium">
											...
											{alert.phenomenon || "Condiciones adversas"}
										</p>
									))}
								</div>
							</div>
			...
			<div className="grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-6">
				...
				>
					...
					<header className="flex justify-between items-center mb-6">
						<div>
							<span className="text-accent uppercase tracking-[0.3em] text-[10px] font-black block mb-1">
								Viento Hoy
							</span>
						...
						<button onClick={fetchData} disabled={isRefreshing}>
							<RefreshCw
								size={14}
								className={`text-white/20 hover:text-accent transition-colors ${isRefreshing ? "animate-spin" : ""}`}
							/>
						</button>
					</header>
						...
						<div className="relative w-40 h-40">
							<svg className="w-full h-full transform -rotate-90">
								...
								<motion.circle
									...
									animate={{
										strokeDashoffset:
											465 - (465 * Math.min(weather.knots, 40)) / 40,
									}}
									className="text-accent drop-shadow-[0_0_8px_rgba(var(--accent-rgb),0.5)] transition-all duration-1000"
								/>
							</svg>
							<div className="absolute inset-0 flex flex-col items-center justify-center">
								<span className="text-6xl font-display text-white italic tracking-tighter transition-all group-hover:scale-110">
									{weather.knots}
								</span>
								<span className="text-xs uppercase tracking-[0.3em] text-accent font-black">
									NUDOS
								</span>
							</div>
						</div>
						<div className="mt-6 text-center space-y-1">
							<p className="text-lg text-white font-display italic">
								{weather.kmh}{" "}
				...
				>
					...
					<header className="mb-8">
						<span className="text-accent uppercase tracking-[0.3em] text-[10px] font-black block mb-4">
							Monitor Flota
						</span>
						<p className="text-white/40 text-xs leading-relaxed max-w-[180px]">
							Estado operacional de embarcaciones en tiempo real.
						</p>
					</header>
					...
					<div className="grid grid-cols-1 gap-4">
						<div className="flex items-center justify-between p-4 bg-white/5 border border-white/5 rounded-sm hover:translate-x-1 transition-transform group">
							<div className="flex items-center gap-4">
								...
								<span className="text-xs uppercase tracking-widest font-bold text-white/80 group-hover:text-white">
									En Agua
								</span>
							</div>
						...
						<div className="flex items-center justify-between p-4 bg-white/5 border border-accent/30 rounded-sm hover:translate-x-1 transition-transform group shadow-[0_0_15px_rgba(var(--accent-rgb),0.1)]">
							<div className="flex items-center gap-4">
								...
								<span className="text-xs uppercase tracking-widest font-black text-accent drop-shadow-sm">
									Pendientes
								</span>
							</div>
							<span className="text-2xl font-display text-accent italic shadow-glow">
								{fleet.pendiente}
							</span>
						</div>
					</div>
					...
					<div className="mt-8 flex gap-2">
						<Link
							href="/staff/activity"
							className="text-[9px] uppercase tracking-widest text-accent font-black hover:underline underline-offset-4"
						>
				...
				>
					...
					<div className="space-y-8 h-full flex flex-col">
						<section>
							<h3 className="text-white/40 uppercase tracking-[0.3em] text-[10px] font-black mb-6">
								Nota Meteorológica
							</h3>
								...
								<ul className="space-y-2">
									{alerts && alerts.length > 0 ? (
										alerts.map((alert: any, i: number) => (
											<li
												key={i}
												className="text-xs text-red-400 font-mono tracking-tight flex items-center gap-2"
											>
												<div className="w-1 h-1 bg-red-400 rounded-full" />
												{alert.level?.toUpperCase()}:{" "}
												{alert.phenomenon || "Meteorología adversa"}
											</li>
										))
									) : (
										<>
						...
						<div className="mt-auto pt-6 border-t border-white/5">
							<button className="w-full flex items-center justify-between p-4 bg-sea-foam/10 border border-sea-foam/30 rounded-sm group hover:bg-sea-foam/20 transition-all">
								<div className="flex items-center gap-4">
									<Map
										className="text-sea-foam group-hover:scale-110 transition-transform"
										size={18}
									/>
									<span className="text-xs uppercase tracking-widest font-black text-sea-foam">
										Radar Marítimo
									</span>
								</div>
								<Navigation
									size={14}
									className="text-sea-foam rotate-45 group-hover:translate-x-1 group-hover:-translate-y-1 transition-all"
								/>
							</button>
						</div>
					</div>
