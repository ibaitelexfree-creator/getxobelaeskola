...
export default function RentalClient({
	...
	const [bookingOption, _setBookingOption] = useState<number | null>(null);
	...
	const availableTimes = useMemo(() => {
		...
		if (Number.isNaN(d) || Number.isNaN(m) || Number.isNaN(y)) return ALL_TIMES;
		...
		return ALL_TIMES;
	}, [day, month, year, spainNow, ALL_TIMES]);
	...
	const _datePickerRef = useRef<HTMLInputElement>(null);
	...
	const _handlePickerChange = (val: string) => {
		if (!val) return;
	...
	const handleWheel = (
		...
		if (type === "day") {
			...
			const currentYearNum = parseInt(year, 10) || currentYear;
		...
		try {
			const consentResponse = await fetch(apiUrl("/api/legal/consent"), {
				...
				body: JSON.stringify({
					...
					consentType:
						services.find((s) => s.id === serviceId)?.categoria === "membresias"
							? "membership"
							: "rental",
		...
		<div className="space-y-16 pb-48">
			...
			{bookingService && (
				<div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 overflow-y-auto bg-nautical-black/20">
					<div
						className="absolute inset-0 bg-nautical-black/90 backdrop-blur-md transition-opacity duration-500"
					...
					<div className="relative bg-nautical-black border border-white/10 max-w-lg w-full p-8 md:p-12 animate-fade-in shadow-2xl rounded-sm">
						<button
							onClick={() => setBookingService(null)}
							className="absolute top-4 right-4 text-white/40 hover:text-white p-2 z-50"
						>
							<X className="w-6 h-6" />
						</button>
						<h3 className="text-3xl font-display italic text-white mb-8 pr-8">
			...
			<LegalConsentModal
				...
				activityType={
					services.find((s) => s.id === pendingBooking?.serviceId)
