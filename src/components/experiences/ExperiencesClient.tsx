	...
	if (experiences.length === 0) {
		return (
			<div className="flex flex-col items-center justify-center py-32 text-center">
				...
				<p className="text-white/30 max-w-md">{t("footer_note")}</p>
			</div>
		);
	}
			...
			<div className="flex flex-col md:flex-row items-center justify-between gap-8 pb-12 border-b border-white/5">
				...
				<div className="flex items-center gap-4 overflow-x-auto pb-2 md:pb-0 no-scrollbar w-full md:w-auto justify-start md:justify-end">
					<div className="flex items-center gap-2 px-4 py-1.5 bg-white/5 backdrop-blur-xl border border-white/10 rounded-full shrink-0">
						...
						<span className="text-[10px] uppercase tracking-widest font-bold text-white/30 mr-2 border-r border-white/10 pr-4 hidden sm:inline">
							Filtrar
						</span>
						<div className="flex gap-1">
							{CATEGORIES.map((cat) => (
								<button
									...
									className={`px-4 py-1.5 rounded-full text-[9px] uppercase tracking-[0.15em] font-black transition-all duration-500 whitespace-nowrap ${
				...
				<span className="text-[10px] uppercase tracking-[0.4em] text-white/20 font-medium">
					...
					{locale === "eu"
						...
						: locale === "en"
							? "experiences"
							: "experiencias"}{" "}
					{locale === "eu" ? "aurkitu dira" : "encontradas"}
				</span>
				<div className="h-px flex-1 bg-gradient-to-r from-transparent via-white/5 to-transparent" />
			</div>
