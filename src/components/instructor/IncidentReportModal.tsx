		...
		<div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-nautical-black/90 backdrop-blur-md animate-in fade-in duration-300">
			<div className="w-full max-w-lg bg-nautical-black border border-red-500/30 p-0 shadow-2xl flex flex-col rounded-sm">
				...
				<div className="p-8 border-b border-red-500/20 bg-red-500/5 flex justify-between items-start">
					<div>
						<span className="text-red-500/80 uppercase tracking-[0.4em] text-3xs font-bold block mb-2">
							Sistema de Alertas
						</span>
					...
					<button
						onClick={onClose}
						className="text-white/20 hover:text-white transition-colors text-xl w-8 h-8 flex items-center justify-center"
					>
						✕
					</button>
				</div>
				...
				<div className="p-8 bg-card">
					<form onSubmit={handleSubmit} className="space-y-6">
						<div className="space-y-3">
							...
							{loading ? (
								<div className="p-4 bg-white/5 border border-white/10 text-2xs text-white/40 italic">
									Cargando flota...
								</div>
							) : (
								<select
									...
									onChange={(e) =>
										setFormData({ ...formData, embarcacion_id: e.target.value })
									}
									required
								>
