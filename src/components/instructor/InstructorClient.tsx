...
export default function InstructorClient({
	...
	const [activeTab, setActiveTab] = useState<
		"dashboard" | "sessions" | "students" | "fleet"
	>("dashboard");
		...
		<main className="min-h-screen pt-24 pb-12 px-6 flex flex-col items-center relative overflow-hidden">
			...
			<div className="container max-w-7xl relative z-10 w-full">
				...
				<header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-12">
					...
					<div className="flex items-center gap-4">
						<div className="text-right hidden md:block">
							<p className="text-2xs font-bold text-white/60">
								{profile.email}
							</p>
							<p className="text-3xs uppercase tracking-widest text-accent font-black">
								Instructor Activo
							</p>
						</div>
				...
				<nav className="flex flex-wrap gap-2 mb-12 border-b border-white/5">
					{tabs.map((tab) => (
						<button
							...
							className={`px-6 py-4 text-3xs uppercase tracking-widest font-black transition-all border-b-2 flex items-center gap-3 ${
								activeTab === tab.id
				...
				<div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
						...
						<div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
										...
										<div className="space-y-4">
											{sessions.slice(0, 5).map((session) => (
												...
												>
													...
													<span
														className={`text-3xs font-black tracking-widest px-3 py-1 rounded-sm uppercase ${
															session.estado === "completada"
																? "bg-sea-foam/10 text-sea-foam"
							...
							<div className="space-y-8">
								<div className="bg-card border border-card-border p-8 rounded-sm">
									<h3 className="text-3xs uppercase tracking-widest text-accent mb-6 font-black">
										Bitácora Rápida
									</h3>
									<p className="text-2xs text-white/40 leading-relaxed font-light mb-6">
										Registra incidencias o notas importantes del día
					...
					{activeTab === "sessions" && (
						<section className="bg-card border border-card-border p-8 rounded-sm">
							<h2 className="text-2xs uppercase tracking-widest text-accent mb-8 font-black">
								Historial y Programación de Sesiones
							</h2>
							<div className="space-y-4">
								{sessions.map((session) => (
									...
									>
										...
										<div className="flex items-center gap-4">
											<span
												className={`text-3xs font-black tracking-widest px-4 py-2 rounded-sm uppercase ${
													session.estado === "completada"
														? "bg-sea-foam/10 text-sea-foam"
						...
						<section className="bg-card border border-card-border p-8 rounded-sm">
							...
							<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
								{initialInscriptions.map((inscription) => (
									<div
										key={inscription.id}
										className="p-6 bg-white/5 border border-white/5 rounded-sm hover:border-white/20 transition-all"
									>
										...
										<h3 className="text-lg font-display text-white mb-1">
											{inscription.profiles.nombre}{" "}
											{inscription.profiles.apellidos}
										</h3>
										<p className="text-2xs text-white/40 mb-4">
											{inscription.profiles.email}
										</p>
										<div className="pt-4 border-t border-white/5">
											<p className="text-3xs uppercase tracking-widest text-white/40 mb-1">
												Curso Actual
											</p>
											<p className="text-2xs text-accent">
												{locale === "es"
													? inscription.ediciones_curso.cursos.nombre_es
													: inscription.ediciones_curso.cursos.nombre_eu}
											</p>
										</div>
									</div>
								))}
								{initialInscriptions.length === 0 && (
									<p className="col-span-full text-white/40 italic text-center py-12">
										No hay alumnos activos en tus cursos.
									</p>
								)}
							</div>
						</section>
					...
					{activeTab === "fleet" && (
						<section className="bg-card border border-card-border p-8 rounded-sm">
							...
							<p className="text-white/40 italic">
								Consulta la disponibilidad y mantenimiento de las
								embarcaciones...
							</p>
						</section>
					)}
