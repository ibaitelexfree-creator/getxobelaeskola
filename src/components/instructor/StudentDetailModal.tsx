...
import { Book, FileText, Loader2, MessageSquare, X } from "lucide-react";
...
export default function StudentDetailModal({
	...
	const [activeTab, setActiveTab] = useState<"logbook" | "evaluations">(
		"logbook",
	);
	...
	const loadData = async () => {
		...
		try {
			const res = await fetch(
				`/api/admin/academy/student-logbook?student_id=${student.id}`,
			);
			if (res.ok) {
								...
								(data.diary.length === 0 ? (
									...
									data.diary.map((entry: any) => {
										...
										return (
											<div
												key={entry.id}
												className="bg-white/[0.03] border border-white/5 rounded-2xl p-6 transition-all hover:border-white/10"
											>
												<div className="flex items-start justify-between gap-4">
													<div>
														<div className="flex items-center gap-3 mb-2">
															...
															{entry.tags?.map((tag: string) => (
																<span
																	key={tag}
																	className="px-2 py-0.5 rounded bg-white/5 text-[10px] uppercase tracking-wider text-white/40 font-bold"
																>
																	#{tag}
																</span>
															))}
													...
													<button
														onClick={() =>
															setExpandedItem(isExpanded ? null : entry.id)
														}
												...
												{isExpanded && (
													<div className="mt-6 pt-6 border-t border-white/5 animate-in slide-in-from-top-2">
														...
														<div className="mt-6">
															<h4 className="text-[10px] uppercase tracking-widest text-white/40 font-black mb-3">
																Añadir Feedback
															</h4>
															<FeedbackRecorder
							...
							{activeTab === "evaluations" &&
								(data.evaluations.length === 0 ? (
									...
									data.evaluations.map((attempt: any) => {
										...
										const passed =
											attempt.nota >= (attempt.evaluacion?.nota_aprobado || 5);
										...
										return (
											...
											>
												<div className="flex items-center justify-between gap-4">
													...
													<button
														onClick={() =>
															setExpandedItem(isExpanded ? null : attempt.id)
														}
														className={`shrink-0 w-8 h-8 rounded-full flex items-center justify-center transition-colors ${feedback.length > 0 ? "bg-accent text-nautical-black" : "bg-white/5 text-white/40 hover:bg-white/10"}`}
													>
														<MessageSquare size={14} />
