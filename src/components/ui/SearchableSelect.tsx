...
export default function SearchableSelect({
	...
	return (
		<div
			className={`space-y-2 relative ${isOpen ? "z-[400]" : "z-10"}`}
			ref={wrapperRef}
		>
			{label && (
			...
			>
				<div className="truncate flex items-center gap-2">
					{selectedOption ? (
						<>
							{selectedOption.status === "free" && (
								<span className="w-2 h-2 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.6)]" />
							)}
							{selectedOption.status === "occupied" && (
								...
								>
									<div className="flex justify-between items-start gap-2">
										<div className="flex flex-col">
											<div
												className={`text-sm font-medium ${value === opt.id ? "text-accent" : "text-white/90 group-hover:text-white"}`}
											>
												{opt.label}
											</div>
											{opt.subLabel && (
												<div
													className={`text-3xs tracking-wide mt-1 font-light ${opt.status === "occupied" ? "text-red-400/80" : "text-white/40"}`}
												>
													{opt.subLabel}
												</div>
											)}
										...
										{opt.status && (
											<div
												className={`px-2 py-0.5 rounded-[2px] text-[9px] uppercase font-bold tracking-tighter ${
													opt.status === "free"
