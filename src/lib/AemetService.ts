...
export class AemetService {
	...
	public static async getGetxoContextString(): Promise<string> {
		try {
			const apiKey = AemetService.getApiKey();
			if (!apiKey) {
			...
			const coastalData = await AemetService.fetchCoastalForecast("48"); // 48 = Bizkaia
			if (coastalData) {
				return AemetService.formatForLLM(coastalData);
			}

			// Fallback: Try Port Forecast (Bilbao) if coastal fails
			const portData = await AemetService.fetchPortForecast("4801"); // 4801 = Bilbao
			if (portData) {
				return AemetService.formatForLLM(portData);
			}
	...
	private static async fetchAemetData(endpoint: string): Promise<any> {
		...
		const url = `${AemetService.BASE_URL}${endpoint}?api_key=${apiKey}`;
		...
		if (!res1.ok)
			throw new Error(
				`AEMET API Request Failed: ${res1.status} ${res1.statusText}`,
			);

		const json1 = (await res1.json()) as AemetResponse;
		if (json1.estado !== 200) {
			throw new Error(
				`AEMET API Error: ${json1.estado} - ${JSON.stringify(json1)}`,
			);
		}
		...
		if (!res2.ok)
			throw new Error(
				`AEMET Data Download Failed: ${res2.status} ${res2.statusText}`,
			);

		return res2.json();
	}

	private static async fetchCoastalForecast(
		coastId: string,
	): Promise<AemetForecast | null> {
		try {
			const data = await AemetService.fetchAemetData(
				`/prediccion/maritima/costera/costa/${coastId}`,
			);
			...
			const zone = prediction.zona?.find(
				(z: any) =>
					z.nombre &&
					(z.nombre.includes("BIZKAIA") || z.nombre.includes("VIZCAYA")),
			);
	...
	private static async fetchPortForecast(
		portId: string,
	): Promise<AemetForecast | null> {
		try {
			const data = await AemetService.fetchAemetData(
				`/prediccion/maritima/puerto/${portId}`,
			);

			// Expected structure: Array of objects.
			if (!Array.isArray(data) || data.length === 0) return null;

			const _report = data[0];
