...
async function fetchAemetData<T>(endpoint: string, apiKey: string): Promise<T> {
	...
	if (!initialResponse.ok) {
		throw new Error(
			`AEMET API Error (Step 1): ${initialResponse.status} ${initialResponse.statusText}`,
		);
	}
	...
	if (initialData.estado !== 200) {
		throw new Error(
			`AEMET API Error: ${initialData.descripcion} (Code: ${initialData.estado})`,
		);
	}
	...
	if (!dataResponse.ok) {
		throw new Error(
			`AEMET API Error (Step 2): ${dataResponse.status} ${dataResponse.statusText}`,
		);
	}
...
export function narrateForecast(forecast: MunicipalityForecast): string {
	if (
		!forecast ||
		!forecast.prediccion ||
		!forecast.prediccion.dia ||
		forecast.prediccion.dia.length === 0
	) {
	...
	const getMainValue = (arr: PeriodValue[]) => {
		...
		return arr.reduce((prev, curr) => {
			const pVal = parseInt(String(prev.value || 0), 10);
			const cVal = parseInt(String(curr.value || 0), 10);
	...
	const sky =
		skyValues.find((x) => x.periodo === "00-24") ||
		skyValues[0] ||
		today.estadoCielo[0];
	...
	const wind = today.viento.reduce(
		(prev, curr) => (curr.velocidad > prev.velocidad ? curr : prev),
		today.viento[0],
	);
	...
	return (
		`Informe meteorológico para ${forecast.nombre} (${forecast.provincia}), para el ${dateStr}. ` +
