	...
	for (const campaign of campaigns) {
		...
		for (const ins of triggerInscriptions) {
			...
			if (userError || !user?.email) {
				console.warn(
					`Could not get email for profile ${profileId}:`,
					userError,
				);
				continue;
			}
			...
			if (stripe && campaign.stripe_coupon_id) {
				try {
					...
					console.log(
						`Generated dynamic Stripe code: ${finalCode} for ${user.email}`,
					);
				} catch (stripeErr) {
			...
			if (emailSent) {
				...
				totalSent++;
				console.log(
					`Successfully sent marketing email to ${user.email} for campaign ${campaign.nombre}`,
				);
			}
		}
	}
