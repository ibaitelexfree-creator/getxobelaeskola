...
export async function processMarketingAutomations() {
	...
	for (const campaign of campaigns as any[]) {
		...
		if (insError || !triggerInscriptions) {
			console.error(
				`Error fetching inscriptions for campaign ${campaign.id}:`,
				insError,
			);
			continue;
		}
		...
		console.log(
			`Found ${triggerInscriptions.length} possible candidates who took the trigger course.`,
		);

		const candidateProfileIds = triggerInscriptions.map(
			(ins: any) => ins.perfil_id,
		);

		if (candidateProfileIds.length === 0) continue;

		// 2. Batch check if already bought target course
		const { data: boughtTargetInscriptions, error: boughtError } =
			await supabase
				.from("inscripciones")
				.select("perfil_id")
		...
		if (boughtError) {
			console.error(
				`Error checking target course inscriptions for campaign ${campaign.id}:`,
				boughtError,
			);
			continue;
		}
		const boughtTargetSet = new Set(
			boughtTargetInscriptions?.map((ins: any) => ins.perfil_id) || [],
		);
		...
		if (historyError) {
			console.error(
				`Error checking marketing history for campaign ${campaign.id}:`,
				historyError,
			);
			continue;
		}
		const historySet = new Set(
			historyExistsList?.map((h: any) => h.perfil_id) || [],
		);
		...
		for (const ins of triggerInscriptions as any[]) {
			...
			if (stripe && campaign.stripe_coupon_id) {
				try {
					...
					console.log(
						`Generated dynamic Stripe code: ${finalCode} for ${userEmail}`,
					);
				} catch (stripeErr) {
			...
			if (emailSent) {
				...
				totalSent++;
				console.log(
					`Successfully sent marketing email to ${userEmail} for campaign ${campaign.nombre}`,
				);
			}
		}
	}
...
async function sendAutomationEmail(
	...
	if (!resend) {
		console.log(
			`[SIMULATION] Sending email to ${email} for campaign ${campaign.nombre} with code ${dynamicCode}`,
		);
		return true;
	}

	const targetCourseName =
		campaign.target_course?.nombre_es || "Siguiente nivel";
	const triggerCourseName =
		campaign.trigger_course?.nombre_es || "Tu último curso";
