...
describe('processMarketingAutomations', () => {
    ...
    it('should fallback to static code on Stripe error', async () => {
        const campaignWithStripe = { ...mockCampaign, stripe_coupon_id: 'coupon_1', cupon_codigo: 'STATIC10' };
        mockSupabase.from.mockImplementation((table: string) => {
            if (table === 'marketing_campaigns') return createQueryBuilder({ data: [campaignWithStripe] });
            if (table === 'inscripciones') {
                const builder = createQueryBuilder();
                builder.then = (resolve: any) => {
                    if (builder.lte.mock.calls.length > 0) resolve({ data: [mockInscription], error: null });
            ...
            if (table === 'marketing_history') {
                const builder = createQueryBuilder();
                builder.then = (resolve: any) => {
                    if (builder.in.mock.calls.length > 0) resolve({ data: [], error: null });
