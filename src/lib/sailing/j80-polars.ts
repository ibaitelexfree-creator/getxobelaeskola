...
export function calculateJ80Speed(twa: number, tws: number): number {
	...
	const clampedTws = Math.max(
		J80_POLAR_DATA.tws[0],
		Math.min(tws, J80_POLAR_DATA.tws[J80_POLAR_DATA.tws.length - 1]),
	);
	...
	const clampedTwa = Math.max(
		J80_POLAR_DATA.twa[0],
		Math.min(absTwa, J80_POLAR_DATA.twa[J80_POLAR_DATA.twa.length - 1]),
	);
	...
	let j = 0;
	while (
		j < J80_POLAR_DATA.twa.length - 2 &&
		absTwa > J80_POLAR_DATA.twa[j + 1]
	) {
		j++;
	}
