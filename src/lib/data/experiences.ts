...
export interface Experiencia {
    ...
    tipo: 'cumpleanos' | 'taller' | 'evento' | 'visita' | 'consultoria' | 'bono' | 'atraque';
    precio?: number;
...
const TIPO_LABELS: Record<ExperienciaTipo, { es: string; eu: string; en: string }> = {
...
export function getExperienciaDescription(exp: Experiencia, locale: string): string {
...
export function formatExperienciaPrice(exp: Experiencia, locale: string): string {
...
export async function fetchExperiencias(tipo?: ExperienciaTipo): Promise<Experiencia[]> {
...
export async function fetchExperienciaBySlug(slug: string): Promise<Experiencia | null> {
    const supabase = createClient();
...
export function groupExperienciasByTipo(experiencias: Experiencia[]): Record<ExperienciaTipo, Experiencia[]> {
    return experiencias.reduce((acc, exp) => {
