...
const translations = {
  es: {
    ...
    payment_failed_title: 'Error en el Pago ⚠️',
    payment_failed_p1: 'Hola {name},',
    payment_failed_p2: 'No hemos podido procesar tu pago de <strong>{amount} {currency}</strong>.',
    payment_failed_p3: 'Esto puede deberse a fondos insuficientes, tarjeta caducada o bloqueo del banco.',
    payment_failed_button: 'REVISAR MI CUENTA',
    payment_failed_footer: 'Si crees que es un error, contacta con tu banco o con nosotros.'
  },
  en: {
    ...
    payment_failed_p1: 'Hello {name},',
    payment_failed_p2: 'We were unable to process your payment of <strong>{amount} {currency}</strong>.',
    payment_failed_p3: 'This may be due to insufficient funds, expired card, or a bank block.',
    payment_failed_button: 'CHECK MY ACCOUNT',
    payment_failed_footer: 'If you think this is a mistake, please contact your bank or us.'
  },
  fr: {
    ...
    payment_failed_title: 'Échec du paiement ⚠️',
    payment_failed_p1: 'Bonjour {name},',
    payment_failed_p2: 'Nous n\'avons pas pu traiter votre paiement de <strong>{amount} {currency}</strong>.',
    payment_failed_p3: 'Cela peut être dû à des fonds insuffisants, une carte expirée ou un blocage bancaire.',
    payment_failed_button: 'VÉRIFIER MON COMPTE',
    payment_failed_footer: 'Si vous pensez qu\'il s\'agit d\'une erreur, contactez votre banque ou nous-mêmes.'
  },
  eu: {
    ...
    payment_failed_title: 'Ordainketa Errorea ⚠️',
    payment_failed_p1: 'Kaixo {name},',
    payment_failed_p2: 'Ezin izan dugu zure <strong>{amount} {currency}</strong>-ko ordainketa prozesatu.',
    payment_failed_p3: 'Hau funts nahikoa ez izateagatik, txartela iraungita egoteagatik edo bankuaren blokeoagatik izan daiteke.',
    payment_failed_button: 'NIRE KONTUA IKUSKATU',
    payment_failed_footer: 'Akats bat dela uste baduzu, jarri harremanetan zure bankuarekin edo gurekin.'
  }
};
 ...
 */
export const paymentFailedTemplate = (name: string, amount: string, currency: string, locale: EmailLocale = 'es') => {
  const t = translations[locale] || translations.es;
  return baseLayout(`
  <h1>${t.payment_failed_title}</h1>
  <p>${t.payment_failed_p1.replace('{name}', `<span class="accent">${name}</span>`)}</p>
  <div class="card" style="border-left: 6px solid #ef4444;">
    <p>${t.payment_failed_p2.replace('{amount}', amount).replace('{currency}', currency.toUpperCase())}</p>
    <p>${t.payment_failed_p3}</p>
  </div>
  <div style="text-align: center;">
    <a href="${BASE_URL}/dashboard" class="button" style="background-color: #ef4444;">${t.payment_failed_button}</a>
  </div>
  <p>${t.payment_failed_footer}</p>
`, locale);
};
