...
const translations = {
  es: {
    ...
    payment_failed_title: 'Problema con tu pago ⚠️',
    payment_failed_p1: 'Hola {name},',
    payment_failed_p2: 'Hemos detectado un problema al procesar el pago de tu suscripción o compra reciente. Para garantizar que sigas disfrutando de todos los servicios, por favor actualiza tu método de pago.',
    payment_failed_button: 'ACTUALIZAR MÉTODO DE PAGO',
    payment_failed_footer: 'Si crees que esto es un error, por favor contacta con nosotros.'
  },
  en: {
    ...
    payment_failed_title: 'Payment Issue Detected ⚠️',
    payment_failed_p1: 'Hello {name},',
    payment_failed_p2: 'We encountered an issue processing your subscription or recent purchase payment. To ensure continued access to all services, please update your payment method.',
    payment_failed_button: 'UPDATE PAYMENT METHOD',
    payment_failed_footer: 'If you believe this is a mistake, please contact us.'
  },
  fr: {
    ...
    payment_failed_title: 'Problème de paiement détecté ⚠️',
    payment_failed_p1: 'Bonjour {name},',
    payment_failed_p2: 'Nous avons rencontré un problème lors du traitement de votre abonnement ou de votre achat récent. Pour garantir un accès continu à tous les services, veuillez mettre à jour votre mode de paiement.',
    payment_failed_button: 'METTRE À JOUR LE PAIEMENT',
    payment_failed_footer: 'Si vous pensez qu\'il s\'agit d\'une erreur, veuillez nous contacter.'
  },
  eu: {
    ...
    payment_failed_title: 'Zure ordainketarekin arazoa ⚠️',
    payment_failed_p1: 'Kaixo {name},',
    payment_failed_p2: 'Arazo bat antzeman dugu zure harpidetza edo azken erosketaren ordainketa prozesatzean. Zerbitzu guztiez gozatzen jarraitzeko, eguneratu ordainketa-metodoa.',
    payment_failed_button: 'EGUNERATU ORDAINKETA',
    payment_failed_footer: 'Hau akats bat dela uste baduzu, jarri gurekin harremanetan.'
  }
};
 ...
 */
export const paymentFailedTemplate = (name: string, locale: EmailLocale = 'es') => {
  const t = translations[locale] || translations.es;
  return baseLayout(`
  <h1>${t.payment_failed_title}</h1>
  <p>${t.payment_failed_p1.replace('{name}', `<span class="accent">${name}</span>`)}</p>
  <p>${t.payment_failed_p2}</p>
  <div style="text-align: center;">
    <a href="${BASE_URL}/student/membership/card" class="button">${t.payment_failed_button}</a>
  </div>
