...
describe('offlineSyncManager', () => {
    ...
    it('processes items concurrently for different resource groups', async () => {
        ...
        // Should be concurrent (~100ms), not sequential (~300ms)
    ...
    it('handles 409 conflict as success', async () => {
        offlineSyncManager.addToQueue('/api/409', 'POST', { data: 'conflict' });

        (fetch as any).mockResolvedValue({ ok: false, status: 409 });
