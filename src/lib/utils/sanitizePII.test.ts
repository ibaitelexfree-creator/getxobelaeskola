...
describe("sanitizePII", () => {
	...
	it("should sanitize credit card numbers (digits only)", () => {
		...
		expect(result).toBe(
			"Payment processed with card [CENSURADO_PII] successfully.",
		);
	});
	...
	it("should handle mixed content", () => {
		const text =
			"User 12345678Z paid with 1234 5678 1234 5678 using email foo@bar.com.";
		const result = sanitizePII(text);
		expect(result).toBe(
			"User [CENSURADO_PII] paid with [CENSURADO_PII] using email [CENSURADO_PII].",
		);
	});

	it("should not modify text without PII", () => {
		const text =
			"Hello world, this is a safe string with numbers 123 and 4567.";
		const result = sanitizePII(text);
