...
export function validateIdentityDocument(
	document: string,
	type?: DocumentType,
): { isValid: boolean; message?: string; type: DocumentType } {
	...
	if (type === "DNI") {
		...
		return {
			isValid: false,
			message: "Formato de DNI inválido (ej: 12345678Z).",
			type: "DNI",
		};
	}
	...
	if (type === "NIE") {
		if (/^[XYZ][0-9]{7}[A-Z]$/.test(value)) {
			return isValidNIE(value)
				...
				: {
						isValid: false,
						message: "La letra del NIE no es válida.",
						type: "NIE",
					};
		}
		return {
			isValid: false,
			message: "Formato de NIE inválido (ej: X1234567L).",
			type: "NIE",
		};
	}
	...
	if (type === "PASPORT") {
		...
		return {
			isValid: false,
			message: "Pasaporte inválido (6-20 caracteres alfanuméricos).",
			type: "PASPORT",
		};
	}
	...
	if (/^[0-9]{7,8}[A-Z]$/.test(value)) {
		return isValidDNI(value)
			? { isValid: true, type: "DNI" }
			: { isValid: false, message: "DNI inválido.", type: "DNI" };
	}
	if (/^[XYZ][0-9]{7}[A-Z]$/.test(value)) {
		return isValidNIE(value)
			? { isValid: true, type: "NIE" }
			: { isValid: false, message: "NIE inválido.", type: "NIE" };
	}
	if (/^[A-Z0-9]{6,20}$/.test(value)) {
		return { isValid: true, type: "PASPORT" };
	}
