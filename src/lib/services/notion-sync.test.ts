...
vi.mock("fs", () => ({
	...
	readFileSync: vi.fn((path: string) => {
		if (path.includes("map"))
			return JSON.stringify({ test_table: "test_db_id" });
...
describe("NotionSyncService", () => {
	...
	describe("syncTable", () => {
		it("should sync rows from Supabase to Notion (pull)", async () => {
			...
			expect(createCall.properties.Supabase_ID.rich_text[0].text.content).toBe(
				"1",
			);
			// 'name' is identified as the title column, so it is mapped to 'Title'
			expect(createCall.properties.Title.title[0].text.content).toBe(
				"Test Row",
			);
		...
		it("should update existing Notion pages (pull)", async () => {
			...
			expect(updateCall.properties.Title.title[0].text.content).toBe(
				"Updated Row",
			);
		});
		...
		it("should handle errors gracefully", async () => {
			...
			await expect(service.syncTable("test_table", "pull")).rejects.toThrow(
