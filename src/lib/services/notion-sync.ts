...
export class NotionSyncService {
	...
	async syncTable(sbTable: string, direction: "pull" | "push" = "pull") {
		...
		// Fetch Supabase data
		const { data: sbRows, error: sbError } = await this.supabase
			.from(sbTable)
			.select("*");
		if (sbError) {
			throw new Error(
				`Supabase fetch failed for ${sbTable}: ${sbError.message}`,
			);
		}
		...
		notionPages.forEach((p) => {
			if ("properties" in p) {
				const sid = (p.properties.Supabase_ID as any)?.rich_text?.[0]
					?.plain_text;
				if (sid) index[sid] = p;
			}
		});
		...
		const propDefinitions =
			this.schema?.definitions?.[sbTable]?.properties || {};

		if (direction === "pull") {
			for (const row of (sbRows as SupabaseRow[]) || []) {
				const props: any = {
					Supabase_ID: {
						rich_text: [{ text: { content: row.id.toString() } }],
					},
				};

				const titleVal = row[titleCol] || row.id;
				props.Title = {
					title: [{ text: { content: titleVal.toString() } }],
				};

				if (
					["cursos", "embarcaciones", "instructores", "habilidades"].includes(
						sbTable,
					)
				) {
					props.Nombre = {
				...
				for (const colName of Object.keys(propDefinitions)) {
					...
					if (def.type === "integer" || def.type === "number") {
						...
						def.format === "timestamp with time zone" ||
						def.format === "date"
					) {
						...
						props[colName] = {
							rich_text: [
								{ text: { content: val.toString().substring(0, 2000) } },
							],
						};
					}
				}
				...
				try {
					...
					console.error(
						`Failed to sync row ${row.id} to Notion:`,
			...
			for (const page of notionPages) {
				...
				const sbId = (page.properties.Supabase_ID as any)?.rich_text?.[0]
					?.plain_text;
				if (!sbId) continue;
				...
				for (const [propName, propValue] of Object.entries(page.properties) as [
					string,
					any,
				][]) {
					if (
						["Supabase_ID", "Title", "Nombre", "Edicion", "Asunto"].includes(
							propName,
						)
					)
						continue;
					...
					if (propValue.type === "rich_text")
						val = propValue.rich_text?.[0]?.plain_text;
					else if (propValue.type === "title")
						val = propValue.title?.[0]?.plain_text;
						...
						val = propValue.multi_select?.map((x: any) => x.name);
				...
				if (hasChanges) {
					...
					if (error)
						console.error(
							`Failed to update Supabase row ${sbId}:`,
							error.message,
						);
				}
			}
	...
	private async fetchDashboardStats(): Promise<DashboardStats> {
		...
		const [
			...
			this.supabase
				.from("reservas_alquiler")
				.select("monto_total")
				.gte("fecha_pago", firstOfMonth),
			this.supabase
				.from("reservas_alquiler")
				.select("monto_total")
				.gte("fecha_pago", firstOfYear),
		...
		const { data: rawRentals } = await this.supabase
			.from("reservas_alquiler")
			.select(
				"id,monto_total,perfil_id,servicio_id,estado_entrega,fecha_reserva,hora_inicio",
			)
			.order("created_at", { ascending: false })
			.limit(10);
		...
		const profileIds = [
			...new Set(
				((rawRentals as any[]) || []).map((r) => r.perfil_id).filter(Boolean),
			),
		];
		const serviceIds = [
			...new Set(
				((rawRentals as any[]) || []).map((r) => r.servicio_id).filter(Boolean),
			),
		];
		...
		return {
			revenue: {
				today: sumMonto(revToday),
				month: sumMonto(revMonth),
				year: sumMonto(revYear),
			},
			counts: {
				...
				rentals_pending: ((rawRentals as any[]) || []).filter(
					(r) => r.estado_entrega === "pendiente",
				).length,
			},
			auditLogs,
			recentRentals,
	...
	private async buildDashboardBlocks(pageId: string, stats: DashboardStats) {
		...
		const shortcuts = (label: string, dbId: string | undefined) => {
			const url = dbId
				? `https://www.notion.so/${dbId.replace(/-/g, "")}`
				: null;
			return rt(`${label} ➔\n`, true, "blue", url);
		};
		...
		const getStatusInfo = (status: string) => {
			switch (status) {
				case "entregado":
					return { emoji: "🌊", text: "EN AGUA", color: "blue" };
				case "devuelto":
					return { emoji: "✅", text: "FINALIZADO", color: "green" };
				default:
					return { emoji: "⏳", text: "PENDIENTE", color: "orange" };
			}
		};
		...
		const blocks: NotionBlock[] = [
			...
			createColumnList([
				...
				[
					createCallout(
						[
							rt("Alertas de Salida:\n", true),
							rt(
								`${stats.counts.rentals_pending} pendientes de entrega.`,
								false,
								stats.counts.rentals_pending > 0 ? "orange" : "default",
							),
						],
						"🚨",
					),
					createCallout(
						[
							rt("Estado de Suscriptores: ", true),
							rt(`${stats.counts.subs} 📬`),
						],
						"📧",
					),
				],
			...
			createColumnList([
				...
				[
					createCallout(
						[
							rt("Academia Activa: ", true),
							rt(`${stats.counts.students} alumn.`),
						],
						"📜",
						"gray_background",
					),
					createCallout(
						[
							rt("Flota Lista: ", true),
							rt(`${stats.boats.filter((b) => b.estado === "listo").length}`),
						],
						"⛵",
						"blue_background",
					),
			...
			createColumnList([
				[
					createCallout(
						[
							rt("🏢 CRM & VENTAS\n", true),
							shortcuts("Inscripciones", this.tableMap.inscripciones),
							shortcuts("Reservas Alquiler", this.tableMap.reservas_alquiler),
							shortcuts("Newsletter", this.tableMap.newsletter_subscriptions),
						],
						"🏬",
						"orange_background",
					),
				...
				[
					createCallout(
						[
							rt("⚓ OPERACIONES\n", true),
							shortcuts("Flota", this.tableMap.embarcaciones),
							shortcuts("Escuela Sesiones", this.tableMap.sesiones),
							shortcuts("Logs Auditoría", this.tableMap.audit_logs),
						],
						"⚙️",
						"blue_background",
					),
				...
				[
					createCallout(
						[
							rt("🎓 ACADEMIA\n", true),
							shortcuts("Cursos", this.tableMap.cursos),
							shortcuts("Ediciones", this.tableMap.ediciones_curso),
							shortcuts("Perfiles Master", this.tableMap.profiles),
						],
						"📚",
						"green_background",
					),
