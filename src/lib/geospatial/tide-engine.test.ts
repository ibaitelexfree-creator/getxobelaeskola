...
import { getCurrents, getTideLevel } from "./tide-engine";
...
describe("Tide Engine", () => {
	...
	describe("getTideLevel", () => {
		...
		it("should calculate mid-tide falling correctly", () => {
			const midTideFalling = new Date(
				REF_HIGH_TIDE.getTime() + M2_PERIOD_MS / 4,
			);
		...
		it("should calculate mid-tide rising correctly", () => {
			const midTideRising = new Date(
				REF_HIGH_TIDE.getTime() + (3 * M2_PERIOD_MS) / 4,
			);
