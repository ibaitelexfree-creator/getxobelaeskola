...
export const generateLogbookPDF = async (data: LogbookData): Promise<void> => {
	...
	const addFooter = (pageNumber: number) => {
		...
		doc.text(`Página ${pageNumber}`, width - margin, footerY, {
			align: "right",
		});
	};
	...
	doc.text(data.user.full_name || "Navegante", width / 2, 155, {
		align: "center",
	});
	...
	const stats = [
		{
			label: "Total Millas (est.)",
			value: `${(data.estadisticas.horas_totales * 5.2).toFixed(1)} nm`,
		},
		{
			label: "Horas de Mar",
			value: `${data.estadisticas.horas_totales.toFixed(1)} h`,
		},
	...
	if (data.diaryEntries.length > 0) {
		...
		data.diaryEntries.forEach((entry, _index) => {
