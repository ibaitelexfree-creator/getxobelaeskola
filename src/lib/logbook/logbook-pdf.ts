 ...
 */
export const generateLogbookPDF = (entries: LogbookEntry[], studentName: string): void => {
    const doc = new jsPDF({
    ...
    entries.forEach((entry, index) => {
        ...
        if (entry.puerto_salida) {
            ...
            doc.text(`Salida: ${entry.puerto_salida}`, width - margin - 5, currentY + 10, { align: 'right' });
        }
        ...
        if (entry.maniobras) {
            ...
            doc.setFont('helvetica', 'normal');
            doc.setTextColor(0, 0, 0);
            doc.text(entry.maniobras, margin + 30, detailsY, { maxWidth: width - margin * 2 - 35 });
        }
        ...
        if (obs) {
            ...
            doc.text(splitObs[0] + (splitObs.length > 1 ? '...' : ''), margin + 5, detailsY);
        }
    ...
    doc.text('Cuaderno de Bitácora Digital - Getxo Bela Eskola', width - margin, footerY, { align: 'right' });
