...
vi.mock('@/lib/supabase/client', () => {
    const mockSingle = vi.fn().mockResolvedValue({ data: { id: 'lobby-1', code: 'TEST12', host_id: 'user-1' }, error: null });
    const mockQuery = {
...
describe('useMultiplayerStore', () => {
    ...
    it('should create lobby correctly', async () => {
        const code = await useMultiplayerStore.getState().createLobby('user-1', 'TestUser');
