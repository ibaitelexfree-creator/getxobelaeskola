...
import { DEFAULT_CONFIG, GamificationEngine } from "./GamificationEngine";
import type { GamificationEvent, StreakState } from "./types";
...
describe("GamificationEngine", () => {
	...
	it("calculates streak bonus for lesson completion", () => {
		...
		expect(result.xpEarned).toBe(
			DEFAULT_CONFIG.pointsPerLesson + expectedBonus,
		);
	...
	it("awards perfect score bonus for test (100 score)", () => {
		...
		expect(result.xpEarned).toBe(
			DEFAULT_CONFIG.pointsPerTestBase + DEFAULT_CONFIG.pointsPerPerfectTest,
		);
		expect(result.breakdown.perfectBonus).toBe(
			DEFAULT_CONFIG.pointsPerPerfectTest,
		);
	});
	...
	it("combines streak and perfect bonus", () => {
		...
		const expectedTotal =
			DEFAULT_CONFIG.pointsPerTestBase +
			streakBonus +
			DEFAULT_CONFIG.pointsPerPerfectTest;
