...
import { DEFAULT_FROM_EMAIL, resend } from "../resend";
...
export class StripeHandlers {
	...
	async handleCheckoutCompleted(session: Stripe.Checkout.Session) {
		...
		if (!metadata || !metadata.user_id)
			throw new Error("Missing metadata.user_id");
		...
		if (mode === "subscription") {
			...
			mode === "bono_horas" ||
			metadata.purchase_type === "bono_horas"
		) {
	...
	private async processRental(
		...
		const { service_id, option_index, reserved_date, reserved_time } =
			session.metadata!;

		const discounts = (session as any).total_details?.breakdown?.discounts;
		const usedCoupon =
			discounts?.[0]?.discount?.promotion_code?.code ||
		...
		const { data: service } = await this.supabase
			...
			.single();
		const optionLabel =
			option_index && service?.opciones[parseInt(option_index, 10)]
				? service.opciones[parseInt(option_index, 10)].label
				: "Estándar";
		...
		if (
			error &&
			(error.message.includes("not find") || error.code === "PGRST202")
		) {
			console.warn(
				"⚠️ RPC confirm_rental_booking not found, falling back to raw insert",
			);
		...
		await this.sendRentalNotifications(
			userId,
			service,
			reserved_date,
			reserved_time,
	...
	private async processCourse(
		...
		const { error } = await this.supabase.rpc("confirm_course_enrollment", {
			...
			p_edition_id:
				edition_id &&
				edition_id !== "" &&
				!edition_id.startsWith("test-") &&
				!edition_id.startsWith("ext_")
					? edition_id
					: null,
			p_amount: session.amount_total ? session.amount_total / 100 : 0,
			p_session_id: session.id,
		...
		if (
			...
			console.warn(
				"⚠️ RPC confirm_course_enrollment not found, falling back to manual update",
			);
			const { error: insError } = await this.supabase
				.from("inscripciones")
				.insert({
			...
			const cleanEditionId =
				...
				!edition_id.startsWith("ext_")
					...
					: null;
			if (cleanEditionId) {
				const { data: edData } = await this.supabase
					.from("ediciones_curso")
					.select("plazas_ocupadas")
					.eq("id", cleanEditionId)
					.single();
				if (edData) {
					await this.supabase
						.from("ediciones_curso")
	...
	private async sendMembershipEmail(
		...
		try {
			...
			const subjects: any = {
				es: "¡Ya eres Socio!",
				eu: "Dagoeneko Bazkide zara!",
				en: "You are now a Member!",
				fr: "Vous êtes maintenant membre !",
			};
	...
	private async sendRentalNotifications(
		userId: string,
		service: any,
		date: string,
		time: string,
	...
	private async sendCourseNotifications(
		userId: string,
		courseId: string,
		session: any,
		locale: string,
		...
		try {
			...
			await resend.emails.send({
				...
				html: inscriptionTemplate(
					course?.nombre_es || "Curso",
					profile.nombre || "Alumno",
					locale as any,
				),
			});
	...
	private async processBonoPurchase(
		session: Stripe.Checkout.Session,
		userId: string,
		_locale: string,
	) {
