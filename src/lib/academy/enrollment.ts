...
export async function getUserEnrollments(userId: string): Promise<string[]> {
	...
	if (
		profile &&
		((profile as any).rol === "admin" || (profile as any).rol === "instructor")
	) {
 ...
 */
async function isEntityUnlocked(
	userId: string,
	type: "niveles" | "cursos" | "modulos" | "unidades",
	entityId: string,
): Promise<boolean> {
	console.log(
		`[isEntityUnlocked] Checking ${type} ${entityId} for user ${userId}`,
	);
	const supabaseAdmin = createAdminClient();
	const { data: unlockStatus, error } = await supabaseAdmin.rpc(
		"obtener_estado_desbloqueo_recursivo",
		{
			p_alumno_id: userId,
		} as any,
	);
 ...
 * @returns boolean - true if access is granted.
 */
export async function verifyUnitAccess(
	userId: string,
	unitId: string,
): Promise<boolean> {
	...
	if (
		error ||
		!unitData ||
		!(unitData as any).modulo ||
		!(unitData as any).modulo.curso_id
	) {
		return false;
	}
