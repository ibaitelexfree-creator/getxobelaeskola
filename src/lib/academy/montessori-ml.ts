import { MontessoriTopic, Difficulty, InteractionResult } from '@/components/academy/montessori/types';
 ...
 */
export function predictProbability(ability: number, difficulty: Difficulty): number {
 ...
 */
export function updateAbility(currentAbility: number, topicDifficulty: Difficulty, outcome: InteractionResult): number {
    const numericOutcome = outcome === 'success' ? 1 : 0;
    const probability = predictProbability(currentAbility, topicDifficulty);

    const newAbility = currentAbility + LEARNING_RATE * (numericOutcome - probability);
...
export function getRecommendation(
    ...
    if (candidates.length === 0) {
        ...
        return topics.reduce((prev, curr) => (curr.difficulty > prev.difficulty ? curr : prev), topics[0]);
    }
