...
describe("getUserEnrollments", () => {
	...
	it("returns all courses for admin user", async () => {
		...
		mockCursosChain.then.mockImplementation((resolve: any) =>
	...
	it("returns only paid enrollments for regular user", async () => {
		...
		mockInscripcionesChain.then.mockImplementation((resolve: any) =>
		...
		expect(mockInscripcionesChain.eq).toHaveBeenCalledWith(
			"estado_pago",
			"pagado",
		);
	...
	it("handles database error gracefully for regular user", async () => {
		...
		expect(result).toEqual([]);
		expect(consoleSpy).toHaveBeenCalledWith("Error fetching enrollments:", {
			message: "DB Error",
		});
