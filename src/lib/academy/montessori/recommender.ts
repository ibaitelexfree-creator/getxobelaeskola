import { MontessoriNode, UserProgress, RecommendationScore } from './types';
...
export class LearningPathRecommender {
    ...
    public recommend(
        ...
        return nodes.map(node => {
            ...
            const isPrereqsMet = node.prerequisites.every(pid =>
                progressMap[pid]?.status === 'completed'
            );
            ...
            if (!isPrereqsMet) {
                return {
                    nodeId: node.id,
                    score: 0,
                    factors: { mastery: 0, prerequisites: 0, difficulty: 0, timeDecay: 0 }
                };
            }
            ...
            const rawScore = (mastery * this.weights.mastery) +
