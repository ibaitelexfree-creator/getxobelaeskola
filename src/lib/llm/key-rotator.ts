import type { KeyRotator } from "./types";
...
export class RoundRobinRotator implements KeyRotator {
		...
		if (keys && keys.length > 0) {
			...
			if (envKeys) {
				...
				if (singleKey) {
					...
					console.warn(
						"No OpenAI API keys found in environment variables (OPENAI_API_KEYS or OPENAI_API_KEY).",
					);
					this.keys = [];
				}
			}
	...
	public reportFailure(key: string): void {
		...
		console.warn(
			`[KeyRotator] Key ending in ...${key.slice(-4)} reported failure.`,
		);
	}
