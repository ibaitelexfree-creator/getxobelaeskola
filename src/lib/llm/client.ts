		...
		for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
			...
			try {
				...
				if (llmError.status === 401) {
					console.warn(
						`[LLMClient] Key ...${currentKey.slice(-4)} unauthorized. Retrying with next key.`,
					);
				...
				console.warn(
					`[LLMClient] Attempt ${attempt + 1}/${this.maxRetries + 1} failed with key ...${currentKey.slice(-4)}. Error: ${llmError.message}`,
				);
