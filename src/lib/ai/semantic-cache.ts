...
export class SemanticCache {
	...
	private getOpenAI(): OpenAI {
		if (!this.openai) {
			if (!this.apiKey) {
				throw new Error(
					"OpenAI API Key is missing. Please set OPENAI_API_KEY environment variable or pass it to the constructor.",
				);
			}
	...
	): Promise<{ response: string; similarity: number; metadata: any } | null> {
		try {
			...
			if (
				...
				error.message.includes("OpenAI API Key is missing")
			) {
				throw error;
			}
			return null;
