	...
	const jsonLd = {
		...
		itemListElement: experiences.map((exp, index) => ({
			...
			item: {
				...
				name:
					locale === "eu"
						? exp.nombre_eu || exp.nombre
						: locale === "en"
							? exp.nombre_en || exp.nombre
							: locale === "fr"
								? exp.nombre_fr || exp.nombre
								: exp.nombre,
				description:
					locale === "eu"
						? exp.descripcion_eu || exp.descripcion
						: locale === "en"
							? exp.descripcion_en || exp.descripcion
							: locale === "fr"
								? exp.descripcion_fr || exp.descripcion
								: exp.descripcion,
				...
				<div className="container mx-auto px-6 relative z-10">
					<header className="max-w-4xl">
						...
						<h1 className="text-5xl md:text-7xl lg:text-9xl font-display leading-[0.9] text-white mb-12 animate-reveal relative z-20">
							{t("title_prefix")} <br />
							<span className="italic font-light text-brass-gold/90 drop-shadow-sm">
								{t("title_highlight")}
							</span>
						</h1>
						<p
							className="max-w-2xl text-foreground/40 font-light text-xl leading-relaxed border-l border-white/10 pl-12 mt-12 animate-fade-in"
