...
const BookingSelector = dynamic(
	() => import("@/components/booking/BookingSelector"),
	{ ssr: false },
);
...
export async function generateMetadata({
	...
	const fallbacks: Record<string, any> = {
		"iniciacion-j80": {
			...
			descripcion_es:
				"Iníciate en el mundo de la navegación a vela en veleros J80. Aprende maniobras básicas en Getxo.",
			descripcion_eu:
				"Hasi nabigazio munduan J80 belaontzietan. Ikasi oinarrizko maniobrak Getxon.",
		},
		"perfeccionamiento-vela": {
			nombre_es: "Perfeccionamiento Vela",
			nombre_eu: "Bela Hobetzea",
			descripcion_es:
				"Mejora tu técnica, táctica y seguridad a bordo. Navegación competitiva y autónoma.",
			descripcion_eu:
				"Hobetu zure teknika, taktika eta segurtasuna ontzian. Nabigazio lehiakorra.",
		},
		"licencia-navegacion": {
			nombre_es: "Licencia de Navegación",
			nombre_eu: "Nabigazio Lizentzia",
			descripcion_es:
				"Obtén tu titulación oficial en un solo día, sin examen. Válida para barcos de hasta 6m.",
			descripcion_eu:
				"Lortu zure titulu ofiziala egun bakar batean, azterketarik gabe. 6 metrorainoko ontziak.",
		},
		"vela-ligera": {
			nombre_es: "Curso de Vela Ligera",
			nombre_eu: "Bela Arina Ikastaroa",
			descripcion_es:
				"Entrenamientos en Optimist, Laser y 420. Ideal para formación continua escolar.",
			descripcion_eu:
				"Optimist, Laser eta 420 ontzietan entrenamenduak. Eskola urtean zehar.",
		},
	};
	...
	const name =
		locale === "es" ? displayCourse.nombre_es : displayCourse.nombre_eu;
	const description =
		locale === "es"
			? displayCourse.descripcion_es
			: displayCourse.descripcion_eu;
	...
	return {
		...
		openGraph: {
			...
			description: description,
			images: [
				displayCourse.imagen_url || "/images/home-hero-sailing-action.webp",
			],
		},
		twitter: {
...
export default async function CourseDetailPage({
	...
	const fallbacks: Record<string, CourseFallback> = {
		"campus-verano-getxo": {
			...
			descripcion_es:
				"Campus de verano para niños y jóvenes de 5 a 21 años empadronados en Getxo.",
			descripcion_eu:
				"5 eta 21 urte bitarteko haur eta gazteentzako udako campusa, Getxon erroldatuta daudenentzat.",
			precio: 130,
			duracion_h: 20,
			nivel: "iniciacion",
			...
			detalles: {
				...
				eu: [
					"Taldeko nabigazioa",
					"Segurtasuna itsasoan",
					"Jolasak eta jarduerak",
					"Astean 20 ordu",
				],
			},
		},
		"iniciacion-adultos": {
			...
			descripcion_es:
				"Curso básico para adultos que quieren empezar en el mundo de la vela.",
		...
		"licencia-navegacion": {
			...
			nombre_eu: "Nabigazio Lizentzia",
			descripcion_es:
				"Obtén tu titulación oficial en un solo día, sin examen. Válida para barcos de hasta 6 metros y motos náuticas de hasta 55 CV.",
			descripcion_eu:
				"Lortu zure titulu ofiziala egun bakar batean, azterketarik gabe. 6 metrorainoko ontzi eta 55 CV-rainoko motorrentzat balio du.",
			precio: 149,
			duracion_h: 6,
			nivel: "iniciacion",
			...
			detalles: {
				es: [
					"Título oficial sin examen",
					"Gobierno de barcos hasta 6m",
					"Motos náuticas hasta 55CV",
					"Navegación diurna (2 millas)",
				],
				eu: [
					"Azterketarik gabeko titulu ofiziala",
					"6 metrorainoko ontzien gobernua",
					"55CV-rainoko motorrak",
					"Eguneko nabigazioa (2 milia)",
				],
			},
		},
		"vela-ligera": {
			...
			nombre_eu: "Bela Arina Ikastaroa",
			descripcion_es:
				"Entrenamientos en Optimist, Laser y 420. Ideal para formación continua durante todo el año escolar, con entrenamientos los fines de semana.",
			descripcion_eu:
				"Optimist, Laser eta 420 ontzietan entrenamenduak. Eskola urtean zehar jarraitzeko ezin hobea, asteburuetako entrenamenduekin.",
			precio: 100,
			duracion_h: 12,
			nivel: "iniciacion",
			...
			detalles: {
				es: [
					"Optimist, Laser y 420",
					"Octubre a Junio",
					"3 domingos al mes",
					"Incluye monitor y equipo",
				],
				eu: [
					"Optimist, Laser eta 420",
					"Urritik Ekainera",
					"Hilean 3 igande",
					"Monitorea eta ekipoa barne",
				],
			},
		},
	};
	...
	const name =
		currentLocale === "eu" && displayCourse.nombre_eu
			? displayCourse.nombre_eu
			: currentLocale === "en" && displayCourse.nombre_en
				? displayCourse.nombre_en
				: currentLocale === "fr" && displayCourse.nombre_fr
					? displayCourse.nombre_fr
					: displayCourse.nombre_es ||
						displayCourse.nombre_en ||
						displayCourse.nombre_eu ||
						"Course";

	const description =
		currentLocale === "eu" && displayCourse.descripcion_eu
			? displayCourse.descripcion_eu
			: currentLocale === "en" && displayCourse.descripcion_en
				? displayCourse.descripcion_en
				: currentLocale === "fr" && displayCourse.descripcion_fr
					? displayCourse.descripcion_fr
	...
	const jsonLd = {
		...
		image:
			...
			"https://getxobelaeskola.cloud/images/home-hero-sailing-action.webp",
		offers: {
			"@type": "Offer",
		...
		<main className="min-h-screen bg-nautical-deep" suppressHydrationWarning>
			...
			<div className="relative z-10 pt-32 pb-24 px-6">
				<div className="container mx-auto">
					<Link
						href={`/${locale}/courses`}
						className="text-sm uppercase tracking-[0.3em] text-accent mb-12 inline-block hover:pl-2 transition-all"
					>
						← {t("back_to_catalog")}
					</Link>
							...
							<div className="flex flex-wrap gap-10 text-xs uppercase tracking-widest font-bold text-sea-foam/50 border-t border-white/5 pt-10">
								...
								<div>
									...
									<p className="text-foreground">
										{t(`levels.${displayCourse.nivel}`)}
									</p>
								</div>
								<div>
									<p className="text-accent mb-1">Inversión</p>
									<p className="text-brass-gold text-lg">
										{displayCourse.precio}€
									</p>
								</div>
							</div>
							...
							{displayCourse.detalles && (
								<div className="space-y-4 pt-12">
									...
									<ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
										{(locale === "es"
											? displayCourse.detalles.es
											: displayCourse.detalles.eu
						...
						<div className="space-y-12">
							<div className="relative h-[450px] rounded-sm overflow-hidden border border-white/10 shadow-2xl">
								<Image
									src={
										displayCourse.imagen_url ||
										"https://images.unsplash.com/photo-1544620347-c4fd4a3d5997?auto=format&fit=crop&q=80&w=2074"
									}
									alt={name}
									fill
									priority
							...
							<div className="card-luxury p-10 bg-white/5 backdrop-blur-xl border border-white/10">
								<h3 className="font-display text-4xl mb-6 text-sea-foam">
									Reserva tu plaza
								</h3>
								<BookingSelector
