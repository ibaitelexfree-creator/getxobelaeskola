...
export default async function StaffPage({
	...
	try {
		const results = await Promise.allSettled([
			...
			supabaseAdmin
				.from("reservas_alquiler")
				.select("monto_total")
				.gte("fecha_pago", firstOfMonth + "T00:00:00Z"),
			supabaseAdmin
				.from("reservas_alquiler")
				.select("monto_total")
				.gte("fecha_pago", firstOfYear + "T00:00:00Z"),
			supabaseAdmin
				.from("profiles")
				.select("*", { count: "exact", head: true })
				.eq("rol", "alumno"),
		...
		if (results[0].status === "fulfilled")
			rawRentalsData = results[0].value.data || [];
		if (results[1].status === "fulfilled")
			chartRentals = results[1].value.data || [];
		if (results[2].status === "fulfilled")
			socioCount = results[2].value.count || 0;
		if (results[3].status === "fulfilled")
			todayRevData = results[3].value.data || [];
		if (results[4].status === "fulfilled")
			monthRevData = results[4].value.data || [];
		if (results[5].status === "fulfilled")
			yearRevData = results[5].value.data || [];
		if (results[6].status === "fulfilled")
			totalStudentCount = results[6].value.count || 0;
	} catch (err) {
	...
	const profileIds = Array.from(
		new Set(rawRentalsData.map((r) => r.perfil_id).filter(Boolean)),
	);
	let enrichData: any[] = [];
	...
	const sumMonto = (data: { monto_total: number }[] | null) =>
		(data || []).reduce((acc, r) => acc + (Number(r.monto_total) || 0), 0);
			...
			<div className="container mx-auto px-6 pt-24 pb-20 relative z-10">
				<header className="mb-8 border-b border-white/5 pb-8 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
					<div>
						...
						<h1 className="text-4xl md:text-6xl font-display italic">
					...
					<div className="flex gap-4">
						<Link
							...
							className="text-[10px] uppercase tracking-widest text-accent hover:text-white font-bold px-4 py-2 border border-accent/20 hover:bg-accent/10 transition-all"
						>
							Vista Instructor
						</Link>
						<Link
							href={`/${locale}/academy`}
							className="text-[10px] uppercase tracking-widest text-white/40 hover:text-sea-foam font-bold px-4 py-2 border border-white/10 hover:bg-white/5 transition-all"
						>
							Ir a Academia →
						</Link>
					</div>
