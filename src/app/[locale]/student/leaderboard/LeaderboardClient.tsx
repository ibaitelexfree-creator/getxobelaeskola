...
import { ArrowLeft, BookOpen, Flame, Globe, Lock, Shield } from "lucide-react";
	...
	useEffect(() => {
		const fetchLeaderboard = async () => {
			try {
				...
				const currentUser = json.leaderboard.find(
					(u: LeaderboardUser) => u.id === json.currentUserId,
				);
				if (currentUser) {
				...
				<div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
									...
									<tbody className="divide-y divide-white/5">
										{leaderboard.map((user, index) => {
											...
											return (
												<tr
													key={user.id}
													className={`group hover:bg-white/5 transition-colors ${isMe ? "bg-accent/5" : ""}`}
												>
													<td className="p-4 text-center font-mono text-white/50">
														{index < 3 ? (
															<span
																className={`text-xl ${index === 0 ? "text-yellow-400" : index === 1 ? "text-gray-300" : "text-amber-600"}`}
															>
																{index === 0 ? "🥇" : index === 1 ? "🥈" : "🥉"}
															</span>
														) : (
													...
													<td className="p-4">
														<div className="flex items-center gap-3">
															...
															<div>
																<div
																	className={`font-medium ${isMe ? "text-accent" : "text-white"}`}
																>
																	{isMe
																		? `${user.nombre} (${t.my_position})`
																		: isAnonymous
																			? t.anonymous_user
																...
																{!isAnonymous && (
																	<div className="text-[10px] text-white/30 uppercase tracking-wider">
																		{t.level} {Math.floor(user.xp / 1000) + 1}
																	</div>
																)}
															</div>
														</div>
					...
					<div className="lg:col-span-1">
						<div className="bg-card p-6 rounded-sm border border-white/5 sticky top-24">
							<h3 className="text-lg font-display italic mb-4">
								{t.privacy_title}
							</h3>
							...
							>
								...
								<div
									className={`w-10 h-5 rounded-full p-1 transition-colors ${isPublic ? "bg-accent" : "bg-white/10"}`}
								>
									<div
										className={`w-3 h-3 bg-nautical-black rounded-full shadow-sm transition-transform ${isPublic ? "translate-x-5" : "translate-x-0"}`}
									/>
								</div>
							</button>
