...
import { ChevronRight, Flame, Menu, Sparkles, X } from "lucide-react";
...
const StudentProfileSidebar = dynamic(
	() => import("@/components/student/StudentProfileSidebar"),
);
const MembershipWidget = dynamic(
	() => import("@/components/student/MembershipWidget"),
);
const DailyChallengeWidget = dynamic(
	() => import("@/components/student/DailyChallengeWidget"),
);
const WeatherPremium = dynamic(
	() => import("@/components/shared/WeatherPremium"),
	{ ssr: false },
);
const MobileStudentHub = dynamic(
	() => import("@/components/student/MobileStudentHub"),
);
const NotificationPermissionBanner = dynamic(
	() => import("@/components/dashboard/NotificationPermissionBanner"),
	{ ssr: false },
);
const QuickContact = dynamic(() => import("@/components/student/QuickContact"));
const DashboardRefresh = dynamic(
	() => import("@/components/student/DashboardRefresh"),
);
const EmptyState = dynamic(() => import("@/components/ui/EmptyState"));
const BonosWallet = dynamic(() => import("@/components/student/BonosWallet"));
const BonoPurchaseModal = dynamic(
	() => import("@/components/student/BonoPurchaseModal"),
);
const DailyNauticalQuote = dynamic(
	() => import("@/components/student/DailyNauticalQuote"),
);
const MicroLessonTray = dynamic(
	() => import("@/components/academy/micro-lessons/MicroLessonTray"),
);
...
export default function StudentDashboardClient({
	...
	useEffect(() => {
		...
		supabase.auth.getSession().then(({ data: { session } }: any) => {
			if (!session) {
				router.replace(
					`/${locale}/auth/login?returnTo=${encodeURIComponent(`/${locale}/student/dashboard`)}`,
				);
			}
		});
	}, [locale, router]);
	...
	const getStatusInfo = (item: DashboardItem) => {
		...
		const dateStr =
			item.fecha_reserva ||
			item.ediciones_curso?.fecha_inicio ||
		...
		if (itemDate < today)
			return {
				label: "COMPLETADO",
				color: "text-white/40 border-white/10",
				paid: true,
			};
		return {
			label: "PENDIENTE",
	...
	const formatDateTime = useCallback(
		(date: string | Date | undefined, time?: string) => {
			if (!date) return "--/--/----";
	...
	const {
		...
		return {
			upcomingInscripciones: inscripciones.filter((ins: any) => {
				const dateStr =
					ins.ediciones_curso?.fecha_inicio || ins.metadata?.start_date;
					...
					<header>
						...
						<h1 className="text-4xl md:text-6xl lg:text-7xl font-display mb-4 text-white leading-[1.1]">
							{(() => {
								...
								const parts = welcome.includes("{{NAME}}")
									? welcome.split("{{NAME}}")
									: welcome.split("{name}");
								return (
									<>
						...
						<p className="text-foreground/60 font-medium tracking-wide">
							{t.subtitle}
						</p>
					</header>
						...
						<div className="lg:col-span-2 space-y-16">
							...
							<section>
								...
								<div className="bg-gradient-to-br from-[#0a1628] to-[#010816] p-1 lg:p-1 border border-accent/20 rounded-sm relative overflow-hidden group">
									<div className="absolute top-0 right-0 p-8 opacity-[0.03] text-9xl pointer-events-none group-hover:scale-110 group-hover:opacity-[0.07] transition-all duration-1000">
										🎓
									</div>
									...
									<div className="bg-[#050c18] p-8 rounded-[1px] relative z-10">
										...
										<div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-10">
											<div className="flex-1">
												...
												<div className="grid grid-cols-3 gap-4 lg:gap-8 mb-8">
													<div className="space-y-1">
														...
														<div className="text-[8px] uppercase tracking-[0.2em] text-accent/60 font-medium">
															Millas
														</div>
													</div>
													<div className="space-y-1">
											...
											<div className="flex flex-col items-center gap-6 w-full md:w-auto">
												...
												<div className="relative w-32 h-32 flex items-center justify-center">
													<svg className="w-full h-full transform -rotate-90">
														...
														<circle
															...
															strokeDasharray={364}
															strokeDashoffset={
																364 - (Math.min(totalMiles, 100) / 100) * 364
															}
												...
												<Link
													href={`/${locale}/academy/dashboard`}
													className="w-full md:w-auto px-10 py-4 bg-accent text-nautical-black font-black uppercase tracking-[0.2em] text-[10px] rounded hover:bg-white hover:scale-105 active:scale-95 transition-all shadow-[0_0_20px_rgba(var(--accent-rgb),0.3)] text-center"
												>
													{hasAcademyActivity
														? t.academy_widget.btn_continue
														: t.academy_widget.btn_start}
												</Link>
											</div>
										</div>
									</div>
							...
							<section>
								<h2 className="text-xs uppercase tracking-widest text-accent mb-8 font-bold">
									...
									<div className="space-y-6">
										{upcomingInscripciones.map((ins: any) => {
											...
											return (
												<div
													key={ins.id}
													className="bg-card p-6 border border-card-border flex justify-between items-center group hover:border-accent/30 transition-all rounded-sm"
												>
														...
														<div>
															...
															<p className="text-xs text-foreground/40 font-light">
																{edition?.fecha_inicio
																	? `Del ${formatDate(edition.fecha_inicio)}`
													...
													<div className="flex flex-col items-end gap-3 text-right">
														<span
															className={`text-[9px] uppercase tracking-widest px-3 py-1 border font-bold ${status.color}`}
														>
															{status.label}
														</span>
													</div>
								...
								{upcomingRentals.length > 0 ? (
									<div className="space-y-6">
										{upcomingRentals.map((rent: any) => {
											...
											const itemName = rent.servicios_alquiler
												? locale === "es"
													? rent.servicios_alquiler.nombre_es
													: rent.servicios_alquiler.nombre_eu
							...
							{(pastInscripciones.length > 0 || pastRentals.length > 0) && (
								<section className="pt-16 border-t border-white/5">
									<h2 className="text-xs uppercase tracking-widest text-foreground/20 mb-8 font-bold">
										{t.history_section.title}
									</h2>
									<div className="space-y-4 opacity-50 grayscale hover:grayscale-0 transition-all duration-500">
										{[...pastInscripciones, ...pastRentals]
											.sort(
												(a, b) =>
													new Date(
														b.fecha_reserva ||
															b.ediciones_curso?.fecha_inicio ||
															0,
													).getTime() -
													new Date(
														a.fecha_reserva ||
															a.ediciones_curso?.fecha_inicio ||
															0,
													).getTime(),
											)
											.slice(0, 3)
											.map((item: any) => (
												<div
													key={item.id}
													className="flex justify-between items-center py-4 border-b border-white/5 last:border-0 text-sm"
												>
													<div className="flex items-center gap-4">
														<div className="w-8 h-8 bg-white/5 flex items-center justify-center text-xs rounded-sm">
															✓
														</div>
														<div>
															<div className="text-white/60 font-medium">
																{item.cursos
																	? locale === "es"
																		? item.cursos.nombre_es
																		: item.cursos.nombre_eu
																	: item.servicios_alquiler
																		? locale === "es"
																			? item.servicios_alquiler.nombre_es
																			: item.servicios_alquiler.nombre_eu
																		: "Actividad pasada"}
															</div>
