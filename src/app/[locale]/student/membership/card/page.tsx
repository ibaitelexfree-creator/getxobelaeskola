		...
		async function fetchProfile() {
			try {
				...
				if (user) {
					...
					const guestQr = await QRCode.toDataURL(
						"https://getxobelaeskola.com/student/membership",
						{
							margin: 1,
							width: 200,
	...
	if (loading)
		return (
			<div className="min-h-screen flex flex-col items-center justify-center bg-nautical-black">
				...
				<div className="mt-8 text-brass-gold animate-pulse text-[10px] uppercase tracking-[0.8em] font-black pl-[0.8em]">
					Autenticando...
				</div>
			</div>
		);
			...
			<div className="max-w-md mx-auto relative z-10">
				...
				>
					...
					>
						...
						<div
							className={`absolute inset-0 bg-gradient-to-br ${isSocio ? "from-brass-gold/15 via-transparent to-accent/5" : "from-white/5 to-transparent"} pointer-events-none`}
						/>
						...
						>
							<div>
								<div
									className={`text-[10px] font-black uppercase tracking-[0.4em] ${isSocio ? "text-brass-gold" : "text-white/30"} mb-1 drop-shadow-sm`}
								>
									{isSocio ? "GOLD MEMBER" : "GUEST PASS"}
								</div>
								<div className="text-2xl font-display italic text-white leading-none">
									Getxo Bela{" "}
									<span
										className={isSocio ? "text-brass-gold" : "text-white/40"}
									>
										Eskola
									</span>
								</div>
							...
							<div className="w-12 h-12 relative opacity-80 flex justify-end">
								<span
									className={`text-4xl ${isSocio ? "text-brass-gold/20" : "text-white/10"} group-hover:rotate-12 transition-transform duration-700`}
								>
									⚓
								</span>
							</div>
						...
						>
							<div className="text-[8px] uppercase tracking-widest text-white/30 mb-2">
								Member Reference ID
							</div>
							<div className="text-lg font-mono tracking-[0.2em] text-white/90 mb-6 flex gap-2">
								<span className={isSocio ? "text-brass-gold" : "text-white/30"}>
									GB
								</span>
							...
							<div
								className={`text-[8px] uppercase tracking-[0.4em] ${isSocio ? "text-brass-gold/60" : "text-white/20"} mb-2 font-black`}
							>
								Licensed Skipper
							</div>
						...
						{/* Card Footer */}
						<div
							className="relative z-10 flex justify-between items-end border-t border-white/10 pt-6"
							style={{ transform: "translateZ(30px)" }}
						>
							<div className="flex gap-8">
								<div>
									<div className="text-[7px] uppercase tracking-[0.2em] text-white/30 mb-1">
										Status
									</div>
									<div
										className={`text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5 ${isSocio ? "text-green-500" : "text-orange-500"}`}
									>
										<span
											className={`w-1.5 h-1.5 rounded-full animate-pulse ${isSocio ? "bg-green-500" : "bg-orange-500"}`}
										/>
								...
								<div>
									...
									<div className="text-[10px] font-black text-white/80 uppercase">
										{profile?.fecha_fin_periodo
											? new Date(profile.fecha_fin_periodo)
													.toLocaleDateString(
							...
							<div className="bg-white p-1 rounded-sm shadow-xl hover:scale-110 transition-transform duration-500 cursor-zoom-in">
								{qrCode && (
									<Image
										...
										width={48}
										height={48}
										className={isSocio ? "grayscale-0" : "grayscale opacity-50"}
									/>
								)}
							</div>
						</div>
									...
									{isLoggedIn ? (
										<>
											...
											<p className="text-[9px] text-white/40 uppercase tracking-widest leading-relaxed">
												Necesitas activar tu suscripción de socio para usar este
												carnet.
											</p>
										</>
									) : (
										<>
											...
											<p className="text-[9px] text-white/40 uppercase tracking-widest leading-relaxed">
												Inicia sesión para ver tu carnet oficial de Getxo Bela.
											</p>
										</>
									)}
								</motion.div>
					...
					{!isLoggedIn ? (
						...
						<div className="grid grid-cols-2 gap-4">
							...
							>
								...
								{locale === "es" ? "Imprimir Físico" : "Print Card"}
							</button>
							<Link
						...
						>
							...
							<ul className="space-y-4 text-xs font-light">
								<li className="flex items-start gap-4 group">
									<span className="text-brass-gold group-hover:scale-125 transition-transform mt-0.5">
										⭐
									</span>
									<div>
										<p className="text-white font-medium mb-0.5">
											Tarifa Plana de Alquiler
										</p>
										<p className="text-white/40 text-[11px]">
											15% de descuento en toda la flota de Getxo.
										</p>
									</div>
								</li>
								<li className="flex items-start gap-4 group">
									...
									<div>
										<p className="text-white font-medium mb-0.5">
											Reserva Anticipada VIP
										</p>
										<p className="text-white/40 text-[11px]">
											Prioridad en travesías nocturnas y eventos del club.
										</p>
									</div>
								</li>
							</ul>
