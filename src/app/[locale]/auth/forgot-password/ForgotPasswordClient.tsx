		...
		<main className="min-h-screen grid grid-cols-1 lg:grid-cols-2">
			...
			<div className="hidden lg:block relative overflow-hidden bg-nautical-black">
				...
				<div className="absolute bottom-20 left-20 z-10">
					<h2 className="text-6xl font-display mb-4 italic">
						{tAuth("hero_text")}
					</h2>
					<p className="text-accent uppercase tracking-widest text-[10px] font-bold">
						Getxo Bela Eskola · Est. 1992
					</p>
				</div>
			</div>
				...
				<div className="w-full max-w-md">
					<header className="mb-12">
						<Link
							...
							className="text-accent hover:text-sea-foam transition-colors text-[10px] uppercase tracking-widest font-bold mb-4 block"
						>
							← {tAuth("login_here")}
						</Link>
					...
					{success ? (
						...
						>
							<div className="space-y-1">
								<label className="text-[10px] uppercase tracking-[0.2em] text-accent font-bold ml-1">
									{tForm("email")}
								</label>
								<input
									{...register("email")}
							...
							{error && (
								<p className="text-2xs text-red-500 text-center">{error}</p>
							)}
