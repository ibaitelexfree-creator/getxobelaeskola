...
import { Anchor, ArrowUpRight, Gauge, Navigation, Wind } from "lucide-react";
import React, { useMemo, useState } from "react";
import WindRose from "@/components/wind-lab/WindRose";
import { calculateWindStats, type WindStats } from "@/lib/wind-lab/utils";
			...
			<div className="bg-[#0a1628] text-white py-8 px-4 sm:px-8 shadow-lg">
				<div className="max-w-6xl mx-auto">
					...
					<p className="text-slate-300 max-w-2xl">
						Laboratorio interactivo de táctica. Ajusta el viento para analizar
						la estrategia de navegación, VMG y trimado óptimo.
					</p>
				</div>
			</div>
						...
						<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
							...
							<div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 col-span-1 sm:col-span-2 border-l-4 border-l-[#ca8a04]">
								...
								<h3 className="text-2xl font-bold text-[#0a1628] mt-1">
									{stats.pointOfSailLabel}
								</h3>
								<p className="text-slate-600 mt-2 text-sm">
									{stats.strategyAdvice}
								</p>
							</div>
							...
							<div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex items-start justify-between">
								<div>
									...
									<div className="flex items-baseline gap-1 mt-1">
										<h3
											className={`text-3xl font-bold ${stats.vmg >= 0 ? "text-emerald-600" : "text-rose-600"}`}
										>
											{stats.vmg.toFixed(1)}
										</h3>
										<span className="text-sm text-slate-500">kn</span>
								...
								<div
									className={`p-3 rounded-full ${stats.vmg >= 0 ? "bg-emerald-50" : "bg-rose-50"}`}
								>
									<ArrowUpRight
										className={`w-6 h-6 ${stats.vmg >= 0 ? "text-emerald-600" : "text-rose-600 rotate-180"}`}
									/>
								</div>
							</div>
