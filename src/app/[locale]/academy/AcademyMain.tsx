...
const MontessoriMode = dynamic(
	() =>
		import("@/components/academy/montessori/MontessoriMode").then(
			(mod) => mod.MontessoriMode,
		),
	{
		ssr: false,
...
export default function AcademyMain({
	...
	const { mode, toggleMode } = useAcademyMode();
	const {
		niveles,
		progreso,
		cursosPorNivel,
		...
		<div className="min-h-screen bg-premium-mesh text-white pb-32 relative">
			...
			<div className="fixed top-24 right-6 z-50 animate-fade-in">
				...
				>
					{mode === "structured" ? (
						<>
							<LayoutGrid className="w-5 h-5 text-accent group-hover:text-nautical-black" />
					...
					<header className="relative overflow-hidden border-b border-white/10 bg-white/5 backdrop-blur-md pt-24 pb-16">
						...
						<div className="container mx-auto px-6 relative">
							<div className="max-w-4xl animate-premium-in">
								<span
									className="text-accent uppercase tracking-[1em] text-[10px] font-black block mb-6 drop-shadow-sm"
									aria-hidden="true"
								>
									{t("map.eyebrow")}
								</span>
								<h1 className="text-4xl md:text-8xl font-display italic text-white mb-8 leading-tight">
									{t("map.title_prefix")}{" "}
									<span className="text-accent underline decoration-accent/20 underline-offset-[16px]">
										{t("map.title_highlight")}
									</span>
								</h1>
										...
										<div className="text-center py-20 animate-fade-in">
											<div className="inline-block p-8 border border-white/10 rounded-2xl bg-white/5">
												...
												<p className="text-white/70 max-w-md mx-auto">
													No hay niveles de formación disponibles en este
													momento. Vuelve pronto para comenzar tu travesía.
												</p>
											</div>
										</div>
									)}
										...
										return (
											<div key={nivel.id} className="relative group">
												{index < niveles.length - 1 && (
													<>
														<div
															className="absolute left-[60px] top-[140px] w-px h-16 bg-gradient-to-b from-accent/20 to-transparent hidden lg:block"
															aria-hidden="true"
														/>
														{/* Mobile Connector */}
														<div
															className="absolute left-1/2 -ml-px top-[100%] h-32 w-0.5 bg-gradient-to-b from-accent/20 to-transparent block lg:hidden"
															aria-hidden="true"
														/>
													</>
												)}
							...
							>
								...
								>
									{ACADEMY_TOOLS.map((tool) => (
										...
										>
											<div
												className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-transform group-hover:scale-110 shadow-lg ${tool.color}`}
												aria-hidden="true"
											>
												{tool.icon}
											</div>
											<div>
												<h3 className="text-white font-bold mb-1 tracking-tight group-hover:text-accent transition-colors">
													{tool.label}
												</h3>
