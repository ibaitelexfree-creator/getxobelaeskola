...
import { Anchor, Book, Globe, Shield, Ship, Wind, X } from "lucide-react";
...
const CertificateCard = dynamic(
	() => import("@/components/academy/CertificateCard"),
	{ ssr: false },
);
const RankProgress = dynamic(
	() => import("@/components/academy/gamification/RankProgress"),
	{ ssr: false },
);
const ActivityHeatmap = dynamic(
	() => import("@/components/academy/dashboard/ActivityHeatmap"),
	{ ssr: false },
);
const SkillRadar = dynamic(
	() => import("@/components/academy/dashboard/SkillRadar"),
	{ ssr: false },
);
const BoatMastery = dynamic(
	() => import("@/components/academy/dashboard/BoatMastery"),
	{ ssr: false },
);
const NauticalRadar = dynamic(
	() => import("@/components/academy/dashboard/NauticalRadar"),
	{ ssr: false },
);
const CareerAdvisor = dynamic(
	() => import("@/components/academy/dashboard/CareerAdvisor"),
	{ ssr: false },
);
const NavigationExperienceMap = dynamic(
	() => import("@/components/academy/dashboard/NavigationExperienceMap"),
	{ ssr: false },
);
const DailyChallengeWidget = dynamic(
	() => import("@/components/academy/dashboard/DailyChallengeWidget"),
	{ ssr: false },
);
const AchievementsWidget = dynamic(
	() => import("@/components/academy/dashboard/AchievementsWidget"),
	{ ssr: false },
);
...
import { calculateEstimatedXP, getRank } from "@/lib/gamification/ranks";
...
export default function DashboardPage({
	...
	useEffect(() => {
		...
		async function fetchData() {
			try {
				const resProgreso = await fetch(
					apiUrl(`/api/academy/progress?locale=${params.locale}`),
				);
		...
		MOTIVATIONAL_QUOTES,
		addNotification, // Clear any previous notifications to ensure only one quote is shown
		clearNotifications,
		params.locale,
	]);
	...
	useEffect(() => {
		...
		const lastSeenLogros = JSON.parse(
			localStorage.getItem("seen_logros") || "[]",
		);
		const currentLogros = data.logros.map((l) => l.logro.id);
		const newLogros = data.logros.filter(
			(l) => !lastSeenLogros.includes(l.logro.id),
		);
		...
		const lastSeenSkills = JSON.parse(
			localStorage.getItem("seen_skills") || "[]",
		);
		const currentSkills = data.habilidades.map((s) => s.habilidad.id);
		const newSkills = data.habilidades.filter(
			(s) => !lastSeenSkills.includes(s.habilidad.id),
		);
	...
	if (!data) {
		return (
			<div className="min-h-screen bg-nautical-black flex flex-col items-center justify-center p-6 text-center">
				...
				<h2 className="text-2xl font-display text-white mb-2 italic">
					No se pudo cargar tu bitácora
				</h2>
				<p className="text-white/40 mb-8 max-w-xs">
					Parece que hay un problema de conexión o tu sesión ha expirado.
				</p>
				<button
				...
				<Link
					...
					className="mt-4 text-white/20 text-[10px] uppercase tracking-widest hover:text-white transition-colors"
				>
	...
	let cursoMostrado: Curso | undefined;
	let progresoCursoMostrado: any;
	...
	if (cursoActivoProgreso) {
		...
		progresoCursoMostrado = data.progreso.find(
			(p) => p.entidad_id === cursoMostrado?.id,
		) || { porcentaje: 0, estado: "disponible" };
	...
	const todasUnidades = data.progreso.filter(
		(p) => p.tipo_entidad === "unidad",
	);
	const _proximaUnidadProgreso = todasUnidades.find(
		(p) => p.estado === "disponible" || p.estado === "en_progreso",
	);

	const skillsCount = data.estadisticas.habilidades_desbloqueadas;
	const skillsPercent = (skillsCount / 12) * 100;
	const _totalMiles = (data.estadisticas.horas_totales * 5.2).toFixed(1);
						...
						<div className="flex-1 text-center md:text-left">
							<h1 className="text-4xl font-display italic text-white mb-2 leading-none">
								...
								<span className="text-accent">
									{data.user?.full_name?.split(" ")[0] || "Navegante"}
								</span>
							</h1>
							...
							<div className="flex items-center gap-4 mb-6 justify-center md:justify-start">
								...
								>
									<span
										className={
											data.user.is_public ? "text-emerald-400" : "text-white/40"
										}
									>
										●
									</span>
							...
							<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
								...
								>
									<div className="text-2xl font-bold text-white mb-1">
										{data.estadisticas.progreso_global}%
									</div>
								...
								<div
									className="p-4 rounded-lg bg-white/5 border border-amber-500/20 animate-fade-in shadow-[0_0_15px_rgba(245,158,11,0.05)]"
								...
								>
									<div className="text-2xl font-bold text-white mb-1">
										{(data.estadisticas.horas_totales * 5.2).toFixed(0)}{" "}
								...
								>
									<div className="text-2xl font-bold text-white mb-1">
										#{data.estadisticas.posicion_ranking}
									</div>
									<div className="text-xs uppercase tracking-wider text-white/40">
										Ranking
									</div>
								...
								<div className="p-8">
									<div className="flex justify-between items-start mb-6">
										<div>
											...
											<h3 className="text-3xl font-display text-white mb-2">
												{params.locale === "eu"
													? cursoMostrado.nombre_eu
													: cursoMostrado.nombre_es}
											</h3>
											<p className="text-white/60 line-clamp-2">
												{params.locale === "eu"
													? cursoMostrado.descripcion_eu
													: cursoMostrado.descripcion_es}
											</p>
										</div>
									</div>
									<div className="mb-6">
										<div className="flex justify-between text-xs mb-2">
											...
											<span className="text-white font-bold">
												{Math.round(progresoCursoMostrado?.porcentaje || 0)}%
											</span>
										</div>
										<div className="h-2 bg-white/10 rounded-full overflow-hidden">
											<div
												...
												style={{
													width: `${progresoCursoMostrado?.porcentaje || 0}%`,
												}}
											/>
										</div>
									</div>
									<div className="flex gap-4">
										<Link
											...
											className="px-6 py-3 bg-accent text-nautical-black font-bold uppercase tracking-wider text-sm rounded"
										>
											Continuar →
										</Link>
									</div>
