	...
	try {
		...
		if (data.error || !data.unidad) {
			return {
				title:
					params.locale === "eu"
						? "Unitatea ez da aurkitu"
						: "Unidad no encontrada",
			};
		}

		const title =
			params.locale === "eu" ? data.unidad.nombre_eu : data.unidad.nombre_es;
		const objectives =
			params.locale === "eu"
				? data.unidad.objetivos_eu
				: data.unidad.objetivos_es;
		const description =
			objectives && objectives.length > 0
				? objectives[0]
				: params.locale === "eu"
					? "Ikasi nabigazioa urratsez urrats."
					: "Aprende navegación paso a paso.";
