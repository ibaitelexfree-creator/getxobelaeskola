...
import {
	type UnlockStatus,
	UnlockStatusBadge,
} from "@/components/academy/UnlockStatusBadge";

const SimpleEvaluation = dynamic(
	() =>
		import("@/components/academy/evaluation").then(
			(mod) => mod.SimpleEvaluation,
		),
	{ ssr: false },
);
const WrittenExerciseSubmission = dynamic(
	() => import("@/components/academy/peer-review/WrittenExerciseSubmission"),
	{ ssr: false },
);
const InteractiveMission = dynamic(
	() =>
		import("@/components/academy/interactive-engine").then(
			(mod) => mod.InteractiveMission,
		),
	{ ssr: false },
);
...
export default function UnitReaderMain({
	...
	const [activeTab, setActiveTab] = useState<"teoria" | "practica" | "errores">(
		"teoria",
	);
	...
	const {
		...
		erroresComunes:
			params.locale === "eu"
				? unidad?.errores_comunes_eu
				: unidad?.errores_comunes_es,
	});
		...
		try {
			...
			if (res.ok) {
				...
				if (navegacion?.siguiente) {
					setTimeout(() => {
						router.push(
							`/${params.locale}/academy/unit/${navegacion.siguiente?.id}`,
						);
					}, 1500);
				}
			} else {
		...
		return (
			<div className="min-h-screen bg-nautical-black flex items-center justify-center">
				<div className="text-center p-8 bg-white/5 rounded-lg border border-white/10 max-w-md">
					...
					<h1 className="text-2xl font-display italic text-white mb-2">
						Contenido Bloqueado
					</h1>
					<p className="text-white/80 mb-6">
						Debes completar las unidades anteriores.
					</p>
					<Link
	...
	return (
		<div className="min-h-screen bg-nautical-black text-white">
						...
						<div className="flex items-center justify-between">
							<Breadcrumbs
								items={[
									...
									{
										label: `Módulo ${unidad.modulo.orden}`,
										href: `/${params.locale}/academy/module/${unidad.modulo.id}`,
									},
							...
							>
								<span
									...
									aria-label={`Página ${navegacion?.posicion} de ${navegacion?.total}`}
								>
			...
			<button
				...
				title={
					zenMode ? "Salir de Modo Zen" : "Modo Zen (Lectura sin distracciones)"
				}
					...
					<div className="max-w-4xl mx-auto">
						<div className="mb-12">
							<div className="flex items-center gap-4 mb-6">
								...
								<div className="flex-1">
									<span className="text-accent uppercase tracking-[0.4em] text-[10px] font-black block mb-1">
										Unidad Didáctica {unidad.orden}
									</span>
							...
							{unidad.objetivos_es && unidad.objetivos_es.length > 0 && (
								...
								>
									<h2
										id="objetivos-title"
										className="text-[10px] uppercase tracking-widest text-accent font-black mb-4"
									>
						...
						<div className="mb-8">
							...
							>
								<button
									...
									className={`px-6 py-3 rounded-full text-xs uppercase tracking-widest transition-all duration-300 focus-ring ${
										activeTab === "teoria"
								...
								{unidad.errores_comunes_es &&
									unidad.errores_comunes_es.length > 0 && (
										<button
											role="tab"
											id="tab-errores"
										...
										<div className="bg-white/5 border border-white/10 rounded-sm p-8">
											<div
												...
												dangerouslySetInnerHTML={{
													__html:
														params.locale === "eu"
															? unidad.contenido_teorico_eu || ""
															: unidad.contenido_teorico_es || "",
												}}
											/>
										</div>
									</div>
									...
									<div className="bg-accent/5 border border-accent/20 rounded-sm p-8">
										<div
											...
											dangerouslySetInnerHTML={{
												__html:
													params.locale === "eu"
														? unidad.contenido_practico_eu || ""
														: unidad.contenido_practico_es || "",
											}}
										/>
									</div>
								)}
									...
									<div className="bg-red-500/5 border border-red-500/20 rounded-sm p-8 text-white/90">
										<ul className="space-y-4">
											{(params.locale === "eu"
												...
												<li key={i} className="flex items-start gap-3">
													<span
														className="text-red-400 text-xl mt-0.5"
														aria-hidden="true"
													>
														⚠️
													</span>
						...
						{actividades?.map((activity) => (
							<WrittenExerciseSubmission
								key={activity.id}
								activity={activity}
								existingAttempt={intentos.find(
									(i) => i.actividad_id === activity.id,
								)}
								unitId={unidad.id}
						...
						<div className="mt-12 border-t border-white/10 pt-12">
							{isExplorationMode && !isCompletada ? (
								<div className="bg-blue-500/10 border border-blue-500/20 rounded-sm p-8 text-center">
									<h2 className="text-xl font-display italic text-blue-200 mb-2">
										Modo Exploración
									</h2>
									<p className="text-white/80 text-sm">
										Estás explorando libremente. Para realizar el quiz y guardar
										tu progreso, debes desbloquear esta unidad en el modo
										estructurado.
									</p>
								</div>
							) : !mostrandoQuiz ? (
								<div className="bg-accent/5 border border-accent/20 rounded-sm p-8 text-center">
									<h2 className="text-2xl font-display italic text-white mb-3">
										Quiz de Evaluación
									</h2>
									<p className="text-white/80 mb-6">
			...
			<nav
				className={`fixed bottom-0 left-0 right-0 bg-nautical-black/95 backdrop-blur-md border-t border-white/10 transition-transform duration-300 ${zenMode ? "translate-y-full" : "translate-y-0"}`}
				aria-label="Navegación de unidades"
			>
				<div className="container mx-auto px-6 py-6 flex items-center justify-between max-w-4xl mx-auto">
					...
					{!isCompletada && !isExplorationMode && (
						<button
							...
							title={
								unidad.recursos_json?.tipo_contenido && !isMissionComplete
									? "Completa la misión primero"
									: ""
							}
							aria-label={
								unidad.recursos_json?.tipo_contenido && !isMissionComplete
									? "Completar unidad (bloqueado, misión pendiente)"
