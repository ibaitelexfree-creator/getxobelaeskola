...
export default function LeaderboardPage({
	...
	const handleVisibilityChange = async (newVisibility: string) => {
		...
		try {
			...
			if (res.ok) {
				setData((prev) =>
					prev ? { ...prev, visibility: newVisibility as any } : null,
				);
				fetchData();
			}
		} catch (error) {
			...
			<div className="min-h-screen bg-nautical-black flex items-center justify-center">
				<div className="text-white/40 text-center">
					...
					<Link
						...
						className="text-accent hover:underline mt-4 block text-sm"
					>
						Volver al Dashboard
					</Link>
				</div>
		...
		<div className="min-h-screen bg-nautical-black text-white pb-20">
			...
			<div className="relative overflow-hidden border-b border-white/10 bg-white/5 backdrop-blur-sm">
				...
				<div className="container mx-auto px-6 py-8 relative">
					<Link
						href={`/${params.locale}/academy/dashboard`}
						className="inline-flex items-center gap-2 text-xs uppercase tracking-widest text-white/40 hover:text-white transition-colors mb-6"
					>
					...
					<div className="flex flex-col md:flex-row justify-between items-end gap-6">
						<div>
							...
							<p className="text-white/60 max-w-xl">
								Compara tu progreso con otros navegantes de la escuela. Gana XP
								completando cursos y desafíos diarios.
							</p>
						</div>
						...
						<div className="bg-white/5 border border-white/10 rounded-lg p-4 flex flex-col gap-2 min-w-[250px]">
							...
							<p className="text-[9px] text-white/30 text-center mt-1">
								{visibility === "private"
									? "No apareces en la tabla."
									: visibility === "anonymous"
										? 'Apareces como "Navegante Anónimo".'
								...
								<div>
									...
									<div className="text-xs text-white/60 flex items-center gap-2">
										...
										<span className="flex items-center gap-1">
											...
											{currentUser.modules_completed} Módulos
										</span>
									</div>
								</div>
							</div>
						...
						<table className="w-full text-left border-collapse">
							<thead>
								<tr className="border-b border-white/10 bg-white/5">
									<th className="p-4 text-[10px] uppercase tracking-widest text-white/40 font-bold w-16 text-center">
										Pos
									</th>
									<th className="p-4 text-[10px] uppercase tracking-widest text-white/40 font-bold">
										Navegante
									</th>
									<th className="p-4 text-[10px] uppercase tracking-widest text-white/40 font-bold text-right">
										XP Total
									</th>
									<th className="p-4 text-[10px] uppercase tracking-widest text-white/40 font-bold text-right">
										Módulos
									...
									>
										<td className="p-4 text-center">
											{user.rank === 1 ? (
												...
												<Medal size={20} className="text-gray-300 mx-auto" />
											) : user.rank === 3 ? (
										...
										<td className="p-4">
											<div className="flex items-center gap-3">
												<div
													className={`relative w-10 h-10 rounded-full overflow-hidden border ${user.is_current_user ? "border-accent" : "border-white/10"} bg-white/5`}
												>
													{user.avatar_url ? (
														<Image
															src={user.avatar_url}
												...
												<div>
													<div
														className={`font-bold ${user.is_current_user ? "text-accent" : "text-white"}`}
													>
														{user.nombre} {user.apellidos}
													</div>
													{user.is_current_user && (
														<div className="text-[9px] uppercase tracking-wider text-accent/60">
															Tú
														</div>
													)}
												</div>
