		...
		async function fetchData() {
			try {
				...
				const modulesCompleted =
					progress?.filter(
						(p: any) =>
							p.tipo_entidad === "modulo" && p.estado === "completado",
					).length || 0;
				const coursesCompleted =
					progress?.filter(
						(p: any) => p.tipo_entidad === "curso" && p.estado === "completado",
					).length || 0;
				...
				const totalHours =
					logbook?.reduce(
						(acc: number, entry: any) => acc + (Number(entry.duracion_h) || 0),
						0,
					) || 0;
	...
	if (!data) {
		return (
			<div className="min-h-screen bg-[#050b14] flex flex-col items-center justify-center p-6 text-center">
				<h1 className="text-2xl font-display text-white mb-2 italic">
		...
		<div className="min-h-screen bg-[#050b14] text-white pb-20 font-sans">
			...
			<div className="container mx-auto px-6 py-12 max-w-5xl">
				...
				<div className="space-y-16">
					...
					<PublicCertificatesGrid
						certificates={(certificates as any) || []}
						studentName={`${profile?.nombre || ""} ${profile?.apellidos || ""}`}
					/>
				</div>

				<div className="mt-20 pt-10 border-t border-white/5 text-center">
					<p className="text-white/20 text-xs uppercase tracking-widest mb-2">
						Getxo Bela Eskola
					</p>
					<p className="text-white/10 text-[10px]">
						Perfil Oficial de Navegante
					</p>
				</div>
			</div>
		</div>
