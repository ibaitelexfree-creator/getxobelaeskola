...
export default function CourseDetailMain({
	...
	const _currentCourseStatus = unlockStatus?.cursos?.[curso.id] || "bloqueado";
		...
		<div className="min-h-screen bg-premium-mesh text-white pb-32">
			...
			<nav className="container mx-auto px-6 py-8" aria-label="Breadcrumb">
				<div className="flex items-center gap-4 text-[10px] uppercase tracking-[0.3em] font-bold text-white/40">
					...
					<Link
						href={`/${params.locale}/academy/level/${curso.nivel_formacion.slug}`}
						className="hover:text-accent transition-colors"
					>
						{params.locale === "eu"
							? curso.nivel_formacion.nombre_eu
							: curso.nivel_formacion.nombre_es}
					</Link>
					{curso.nombre_es.toLowerCase() !==
						curso.nivel_formacion.nombre_es.toLowerCase() && (
						<>
							<span aria-hidden="true">/</span>
					...
					<div className="max-w-5xl mx-auto animate-premium-in">
						{curso.nombre_es.toLowerCase() !==
							...
							<span className="text-accent uppercase tracking-[0.5em] text-[10px] font-black block mb-6 px-4 py-1 border border-accent/20 w-fit bg-accent/5 backdrop-blur-sm">
								NIVEL {curso.nivel_formacion.orden} ·{" "}
								{params.locale === "eu"
									? curso.nivel_formacion.nombre_eu
									: curso.nivel_formacion.nombre_es}
							</span>
						...
						<div className="grid grid-cols-2 md:grid-cols-4 gap-8 max-w-3xl">
							<div className="glass-panel p-6 border-white/10 hover:border-accent/40 transition-colors group">
								<div className="text-4xl font-display italic text-accent mb-2">
									{modulos.length}
								</div>
							...
							{curso.instructor && (
								<div className="glass-panel p-6 border-white/10 flex items-center gap-4">
									<div className="relative w-12 h-12 rounded-full overflow-hidden border border-white/20">
										<Image
											src={
												curso.instructor.foto_url ||
												"/images/instructor-avatar-default.jpg"
											}
									...
									<div>
										...
										<div className="text-[8px] uppercase text-white/40 tracking-widest">
											Instructor
										</div>
									</div>
								</div>
			...
			<main className="container mx-auto px-6">
				<div className="max-w-5xl mx-auto space-y-12">
					<header className="mb-16">
						<h2 className="text-4xl lg:text-5xl font-display italic text-white">
							Programa Académico
						</h2>
							...
							return (
								<Link
									...
									href={
										isLocked
											? "#"
											: `/${params.locale}/academy/module/${modulo.id}`
									}
									className={`
									...
									<div className="flex flex-col md:flex-row md:items-center gap-10">
										...
										<div className="flex-1">
											...
											<p className="text-lg text-white/50 font-light leading-relaxed line-clamp-2">
												{params.locale === "eu"
													? modulo.descripcion_eu
													: modulo.descripcion_es}
											</p>
										</div>
					...
					{!mostrandoExamenFinal && progreso?.porcentaje === 100 && (
						<section className="mt-24 p-12 lg:p-20 relative overflow-hidden border border-accent/40 rounded-sm group bg-accent/[0.03]">
							<div
								className="absolute inset-0 bg-waves opacity-20 pointer-events-none"
								aria-hidden="true"
							/>
							<div className="relative z-10 text-center max-w-2xl mx-auto">
								<span className="text-accent uppercase tracking-[0.6em] text-[10px] font-black block mb-6 animate-pulse">
									Reto Final Desbloqueado
								</span>
								...
								<p className="text-white/60 font-light text-lg leading-relaxed mb-12">
									Has demostrado dominio en todos los módulos. Es el momento de
									poner a prueba tus conocimientos para obtener el certificado
