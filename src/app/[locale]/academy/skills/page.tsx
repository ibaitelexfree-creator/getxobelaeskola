...
export default function SkillsPage({ params }: { params: { locale: string } }) {
	...
	const getRankInfo = (count: number) => {
		...
		if (count >= 7)
			return { name: "Patrón", icon: "⚓", color: "bg-indigo-500", next: 10 };
		if (count >= 4)
			return { name: "Timonel", icon: "舵", color: "bg-blue-500", next: 7 };
		if (count >= 1)
			return { name: "Marinero", icon: "⛵", color: "bg-green-500", next: 4 };
				...
				<div className="container mx-auto px-6 py-16 relative">
					<div className="flex flex-col md:flex-row items-center gap-12">
						...
						<div className="relative group">
							<div
								className={`w-40 h-40 rounded-full ${rank.color} flex items-center justify-center text-7xl shadow-[0_0_50px_rgba(0,0,0,0.5)] border-4 border-white/20 relative z-10 transition-transform duration-500 group-hover:scale-105`}
							>
								{rank.icon}
							</div>
							<div
								className={`absolute inset-0 rounded-full ${rank.color} blur-3xl opacity-20 animate-pulse`}
							/>
						</div>
						...
						<div className="flex-1 text-center md:text-left">
							...
							<h1 className="text-5xl font-display italic text-white mb-2">
								Tu Rango:{" "}
								<span className="text-white font-black">{rank.name}</span>
							</h1>
							<p className="text-white/60 mb-8 max-w-xl">
								Has desbloqueado {earnedCount} de {allSkills.length} habilidades
								técnicas. Sigue navegando para alcanzar el rango de Capitán.
							</p>
							...
							<div className="max-w-md">
								<div className="flex justify-between text-[10px] uppercase tracking-widest font-black mb-3">
									...
									<span className="text-accent">
										{earnedCount} / {rank.next || allSkills.length} SKILLS
									</span>
								</div>
				...
				<div className="flex flex-col md:flex-row items-center justify-between gap-8 bg-gradient-to-r from-blue-900/20 to-indigo-900/20 p-8 rounded-3xl border border-blue-500/30 backdrop-blur-sm relative overflow-hidden group">
					...
					<div className="relative z-10">
						...
						<h2 className="text-3xl font-display italic text-white mb-2">
							Examen Final Simulado (PER)
						</h2>
						<p className="text-white/60 max-w-xl text-sm leading-relaxed">
							Pon a prueba tus conocimientos con un examen completo de 60
							preguntas y 90 minutos, simulando las condiciones reales del
							examen oficial. Incluye revisión detallada.
						</p>
					</div>
							...
							>
								<div className="flex items-start gap-6">
									...
									<div className="flex-1">
										...
										<h3
											className={`text-xl font-bold mb-2 ${unlocked ? "text-white" : "text-white/40"}`}
										>
											{skill.name}
										</h3>
											...
											<div className="text-[10px] text-white/30 italic flex items-center gap-2">
												...
												<span>
													...
													{skill.rules.description ||
														"Cumple los requisitos del curso"}
												</span>
											</div>
										)}
