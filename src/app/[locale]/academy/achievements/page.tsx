...
export default function AchievementsPage({
	...
	const categories = [
		"todos",
		...Array.from(new Set(allLogros.map((l) => l.categoria))),
	];

	const filteredLogros =
		filter === "todos"
			? allLogros
	...
	const totalPuntos = Object.values(userLogros).reduce(
		(sum, la) => sum + la.logro.puntos,
		0,
	);
						...
						return (
							...
							>
								...
								<div className="flex flex-col items-center text-center space-y-4 relative z-10">
									<div
										className={`w-20 h-20 rounded-full flex items-center justify-center text-4xl shadow-xl transition-transform duration-500 group-hover:scale-110 ${
											obtenido
									...
									<div>
										<h3
											className={`text-lg font-bold mb-1 ${obtenido ? "text-white" : "text-white/40"}`}
										>
											{params.locale === "eu"
												? logro.nombre_eu
												: logro.nombre_es}
										...
										<p className="text-xs text-white/40 leading-relaxed line-clamp-2">
											{params.locale === "eu"
												? logro.descripcion_eu
												: logro.descripcion_es}
										</p>
									</div>
								...
								<div
									className={`absolute top-4 right-4 text-[10px] font-black ${obtenido ? "text-accent" : "text-white/10"}`}
								>
									+{logro.puntos} PTS
								</div>
							</div>
