...
import type { RegattaParticipant } from "@/types/regatta";

export default function ResultsClient() {
	const { code } = useParams();
	const router = useRouter();
	const [participants, setParticipants] = useState<RegattaParticipant[]>([]);
	...
	useEffect(() => {
		const fetchResults = async () => {
			...
			if (parts) setParticipants(parts as RegattaParticipant[]);
			setLoading(false);
		};
