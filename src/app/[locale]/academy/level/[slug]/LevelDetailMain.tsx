...
export default function LevelDetailMain({
	...
	const [unlockStatus, setUnlockStatus] = useState<UnlockStatusResponse | null>(
		null,
	);
	...
	useEffect(() => {
		async function fetchData() {
			try {
				...
				const nivelActual = dataNiveles.niveles?.find(
					(n: Nivel) => n.slug === params.slug,
				);

				if (nivelActual) {
					setNivel(nivelActual);
					const resCursos = await fetch(
						apiUrl(`/api/academy/courses?level_id=${nivelActual.id}`),
					);
	...
	if (loading) {
		return (
			<div
				className="min-h-screen bg-nautical-black flex items-center justify-center"
				aria-live="polite"
			>
				<div className="text-center">
					...
					<p className="text-white/60 text-lg font-display italic">
						Explorando el nivel...
					</p>
				</div>
			</div>
	...
	if (!nivel) {
		return (
			<div className="min-h-screen bg-nautical-black flex items-center justify-center text-white">
				<div className="text-center">
					<p className="text-xl font-display italic text-white/60 mb-8">
						Nivel no encontrado
					</p>
	...
	const isLevelLocked =
		currentLevelStatus === "locked" || currentLevelStatus === "bloqueado";
			...
			<header className="container mx-auto px-6 mb-24">
				<div className="flex flex-col lg:flex-row items-center lg:items-end gap-12 animate-premium-in">
					<div className="relative">
						<div
							className="w-32 h-32 lg:w-40 lg:h-40 rounded-full bg-accent/10 border-2 border-accent flex items-center justify-center text-7xl shadow-[0_0_50px_rgba(255,77,0,0.15)]"
							aria-hidden="true"
						>
							{nivel.icono}
						</div>
					...
					<div className="text-center lg:text-left flex-1">
						...
						<p className="text-xl lg:text-2xl text-white/70 font-light leading-relaxed max-w-3xl">
							{(params.locale === "eu"
								...
								: nivel.descripcion_es) || nivel.descripcion_es}
						</p>
					</div>

					<div className="hidden xl:flex gap-12 text-right">
						<div>
							<span className="block text-[10px] font-black text-accent tracking-[0.4em] mb-2 uppercase">
								Teoría
							</span>
							<span className="text-4xl font-display text-white">
								{nivel.duracion_teorica_h}h
							</span>
						</div>
				...
				<div className="max-w-5xl mx-auto space-y-8">
					<header className="mb-12 border-b border-white/10 pb-6 flex items-end justify-between">
						<h2 className="text-3xl font-display italic text-white group flex items-center gap-4">
							Cursos Disponibles
							<span className="text-sm font-sans not-italic text-white/40 tracking-widest">
								({cursos.length})
							</span>
						</h2>
					</header>
						...
						return (
							<Link
								...
								href={
									isLocked
										? "#"
										: `/${params.locale}/academy/course/${curso.slug}`
								}
								className={`
								...
								<div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
									<div className="flex-1">
										<div className="flex items-center gap-4 mb-4">
											<span
												className="text-[10px] font-black text-accent tracking-[0.5em] uppercase"
												aria-hidden="true"
											>
												0{index + 1}
											</span>
											<h3 className="text-3xl lg:text-4xl font-display italic text-white group-hover:text-accent transition-colors leading-none">
												{(params.locale === "eu"
													? curso.nombre_eu
													: curso.nombre_es) || curso.nombre_es}
											</h3>
										</div>
										<p className="text-white/60 font-light leading-relaxed max-w-2xl text-lg">
											{(params.locale === "eu"
												? curso.descripcion_eu
												: curso.descripcion_es) || curso.descripcion_es}
										</p>

										<div className="flex flex-wrap gap-8 mt-8 text-[9px] font-bold text-white/30 uppercase tracking-[0.3em]">
											<div className="flex items-center gap-2">
												⏱️ {curso.duracion_h}h Totales
											</div>
											<div className="flex items-center gap-2">
												📚 {curso.horas_teoricas}h Teoría
											</div>
											<div className="flex items-center gap-2">
												⛵ {curso.horas_practicas}h Práctica
											</div>
										</div>
									</div>
									...
									<div className="flex-shrink-0 flex items-center gap-6">
										...
										{!isLocked && (
											<div
												className="w-12 h-12 rounded-full border border-white/10 flex items-center justify-center text-accent group-hover:bg-accent group-hover:text-nautical-black transition-all duration-500"
												aria-hidden="true"
											>
												→
											</div>
