...
export default async function MochilaPage({ params }: { params: { locale: string } }) {
        ...
        <div className="min-h-screen bg-nautical-black text-white p-6 pb-20 safe-area-top">
             <div className="max-w-2xl mx-auto pt-8">
                <header className="mb-12 flex items-center gap-6">
                    <Link href={`/${params.locale}/academy/dashboard`} className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-white/60 hover:text-white hover:bg-white/10 transition-colors">
                        <ArrowLeft size={20} />
                    </Link>
                    <div>
                        <h1 className="text-3xl font-display italic flex items-center gap-3">
                            <span className="text-accent"><Backpack size={32} /></span>
                ...
                <main>
                    <MistakeReviewDeck initialMistakes={mistakes || []} locale={params.locale} />
                </main>
             </div>
        </div>
    );
