	...
	return (
		<div className="w-full h-screen relative bg-[#000510] flex flex-col items-center justify-center overflow-hidden">
			...
			<div className="w-full h-full relative">
				...
				<div
					className={`absolute inset-0 transition-opacity duration-500 ${mode === "map" ? "opacity-100 z-0" : "opacity-0 pointer-events-none"}`}
				>
					<ConstellationMap />
				</div>
			...
			<div
				className={`absolute inset-0 bg-radial-gradient from-transparent to-nautical-black/80 pointer-events-none z-[1] transition-opacity duration-500 ${mode === "map" ? "opacity-100" : "opacity-0"}`}
			/>
		</div>
	);
}
