...
export default function ModuleDetailMain({
	...
	const [_progreso, setProgreso] = useState<any>(null);
		...
		return (
			<div className="min-h-screen bg-nautical-black flex items-center justify-center">
				<div className="text-center" role="status" aria-live="polite">
					...
					<p className="text-white/80 text-lg font-display italic">
						Navegando el módulo...
					</p>
				</div>
			</div>
		...
		return (
			<div className="min-h-screen bg-nautical-black flex items-center justify-center text-white">
				<div className="text-center">
					<p className="text-xl font-display italic text-white/80 mb-2">
						Módulo no encontrado
					</p>
					{error && (
						<p className="text-red-400 mb-6 font-mono text-sm">
							Error: {error}
						</p>
					)}
	...
	const isModuleLocked =
		currentModuleStatus === "locked" || currentModuleStatus === "bloqueado";
	...
	const unidadesCompletadas = progresoUnidades.filter(
		...
		<div className="min-h-screen bg-premium-mesh text-white pb-32">
			...
			<nav className="container mx-auto px-6 py-8" aria-label="Breadcrumb">
				<div className="flex flex-wrap items-center gap-4 text-[10px] uppercase tracking-[0.3em] font-bold text-white/70">
					...
					<Link
						href={`/${params.locale}/academy/level/${modulo.curso.nivel_formacion.slug}`}
						className="hover:text-accent transition-colors"
					>
						{params.locale === "eu"
							? modulo.curso.nivel_formacion.nombre_eu
							: modulo.curso.nivel_formacion.nombre_es}
					</Link>
					<span aria-hidden="true">/</span>
					<Link
						href={`/${params.locale}/academy/course/${modulo.curso.slug}`}
						className="hover:text-accent transition-colors"
					>
						{params.locale === "eu"
							? modulo.curso.nombre_eu
							: modulo.curso.nombre_es}
					</Link>
					<span aria-hidden="true">/</span>
			...
			<main className="container mx-auto px-6">
					...
					{unidades.map((unidad, _index) => {
						...
						return (
							<Link
								key={unidad.id}
								href={
									isLocked ? "#" : `/${params.locale}/academy/unit/${unidad.id}`
								}
								className={`
							...
							>
								<div className="flex items-center justify-between gap-6">
									<div className="flex items-center gap-6">
										...
										<div>
											...
											<div className="flex gap-4 mt-1 text-[8px] uppercase tracking-widest text-white/60 font-bold">
												<span>
													{unidad.duracion_estimada_min} mins de lectura
												</span>
											</div>
										</div>
									</div>
									...
									<div className="flex items-center gap-6">
										{isCompleted && (
											<span className="text-accent text-sm" title="Completado">
												✓
											</span>
										)}
				...
				{/* Exam Section */}
				{!mostrandoExamen &&
					unidadesCompletadas === unidades.length &&
					unidades.length > 0 && (
						<section className="mt-20 max-w-4xl mx-auto p-12 lg:p-16 border-2 border-accent/40 rounded-sm bg-accent/[0.03] text-center relative overflow-hidden group">
							<div
								className="absolute inset-0 bg-waves opacity-10 pointer-events-none"
								aria-hidden="true"
							/>
							<div className="relative z-10">
								<span className="text-accent uppercase tracking-[0.5em] text-[9px] font-black block mb-6">
									Contenido Finalizado
								</span>
								<h2 className="text-4xl lg:text-5xl font-display italic text-white mb-8">
									Examen de Conocimientos
								</h2>
								<p className="text-white/80 font-light text-lg mb-10 max-w-xl mx-auto">
									Has navegado por todas las unidades de este módulo. Realiza el
									examen para validar tu aprendizaje y continuar tu formación.
								</p>
								<button
