...
import { cormorantGaramond, jetbrainsMono, outfit } from "@/app/fonts";
import "@/app/globals.css";
import dynamic from "next/dynamic";
import Footer from "@/components/layout/Footer";
import Navbar from "@/components/layout/Navbar";

const AcademyFeedbackProvider = dynamic(
	() => import("@/components/academy/AcademyFeedbackProvider"),
	{ ssr: false },
);
const PushNotificationInitializer = dynamic(
	() =>
		import("@/components/academy/notifications/PushNotificationInitializer"),
	{ ssr: false },
);
...
const OfflineSyncProvider = dynamic(
	() => import("@/components/offline/OfflineSyncProvider"),
	{ ssr: false },
);
	...
	return (
		...
		>
			<head>
				<link
					rel="dns-prefetch"
					href="https://xbledhifomblirxurtyv.supabase.co"
				/>
			</head>
...
export async function generateMetadata({
	...
	let description =
		"Aprende a navegar en el Cantábrico. Cursos de vela ligera, cruceros J80, academia náutica online y alquiler de material en el Puerto Deportivo de Getxo.";

	if (isEu) {
		title = "Getxo Bela Eskola | Nabigazio Akademia Getxon";
		description =
			"Ikasi nabigatzen Kantauri itsasoan. Bela ikastaroak, J80 ontziak, online akademia eta material alokairua Getxoko Portu Kiroleruan.";
	} else if (isEn) {
		title = "Getxo Getxo Bela Eskola | Sailing Lessons in Getxo";
		description =
			"Learn to sail in the Bay of Biscay. Pro sailing courses, J80 yacht training, online academy, and boat rentals in Getxo Marina.";
	} else if (isFr) {
		title = "Getxo Getxo Bela Eskola | École de Voile à Getxo";
		description =
			"Apprenez la navigation en mer Cantabrique. Cours de voile, formation J80, académie en ligne et location de bateaux à Getxo.";
	}
