...
describe("sitemap", () => {
	...
	it("should generate sitemap entries for static paths, courses, and modules", async () => {
		...
		expect(entries.some((e) => e.url.includes("/es/courses/course-1"))).toBe(
			true,
		);
		expect(entries.some((e) => e.url.includes("/eu/courses/course-2"))).toBe(
			true,
		);

		// Verify dynamic modules exist
		expect(
			entries.some((e) => e.url.includes("/es/academy/module/module-1")),
		).toBe(true);
		expect(
			entries.some((e) => e.url.includes("/eu/academy/module/module-2")),
		).toBe(true);
