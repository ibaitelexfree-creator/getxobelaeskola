...
export async function POST(request: Request) {
	...
	if (!isCompleted) {
		return NextResponse.json(
			{ error: "No autorizado: Debes completar el módulo primero." },
			{ status: 403 },
		);
	}
	...
	if (insertError) {
		if (insertError.code === "23505") {
			return NextResponse.json(
				{ error: "Ya has revisado este ejercicio." },
				{ status: 409 },
			);
		}
		return NextResponse.json(
			{ error: "Error al guardar la revisión" },
			{ status: 500 },
		);
	}
	...
	if (updateError) {
		return NextResponse.json(
			{ error: "Error al actualizar el estado del ejercicio" },
			{ status: 500 },
		);
	}
