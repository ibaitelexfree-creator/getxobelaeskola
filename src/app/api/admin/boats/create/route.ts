	...
	try {
		...
		if (!nombre || !tipo || capacidad === undefined || capacidad === null) {
			return NextResponse.json(
				{ error: "Faltan campos obligatorios (Nombre, Tipo, Capacidad)" },
				{ status: 400 },
			);
		}

		const capacityInt = parseInt(capacidad, 10);
		...
		const { data, error } = await supabaseAdmin
			...
			.insert({
				...
				notion_threshold:
					body.notion_threshold !== undefined ? body.notion_threshold : 0.2,
			})
			.select()
			.single();
