...
export async function GET(req: Request) {
	...
	const searchTable = async (tableName: string) => {
		...
		const relationCountsByRow = new Map<
			string,
			{ label: string; count: number; table: string }[]
		>();
		...
		for (const rel of rels) {
			try {
				...
				if (relError) {
					console.error(
						`Error fetching relation ${rel.table} for ${tableName}:`,
						relError,
					);
		...
		console.log(
			`[Explorer] Relation batching for ${tableName} (${rows.length} rows) took ${(end - start).toFixed(2)}ms`,
		);
