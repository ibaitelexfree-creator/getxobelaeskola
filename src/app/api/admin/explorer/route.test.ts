...
describe("Admin Explorer API", () => {
	...
	it("should use efficient batching and avoid N+1 queries for profiles", async () => {
		...
		const chainReservas = createChain({
			data: [{ perfil_id: "user1" }, { perfil_id: "user1" }],
			error: null,
		});
		const chainInscripciones = createChain({
			data: [{ perfil_id: "user1" }],
			error: null,
		});
		const chainMensajes = createChain({ data: [], error: null });
		const chainSubscriptions = createChain({
		...
		const req = new Request(
			"http://localhost/api/admin/explorer?q=john&table=profiles",
		);
	...
	it("should correctly handle tables without relations", async () => {
		...
		const req = new Request(
			"http://localhost/api/admin/explorer?q=res1&table=reservas_alquiler",
		);
		const response = await GET(req);
		const json = await response.json();
