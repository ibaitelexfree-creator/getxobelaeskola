...
export async function GET() {
	try {
		...
		const fleetMetrics = data.results.map((page: any) => ({
			id: page.id,
			supabase_id:
				page.properties.Supabase_ID?.rich_text[0]?.plain_text || null,
			...
			alerta:
				page.properties.Alerta_Rentabilidad?.formula?.string || "✅ Saludable",
			ingresos_reservas:
				page.properties.Ingresos_por_Reservas?.rollup?.number || 0,
