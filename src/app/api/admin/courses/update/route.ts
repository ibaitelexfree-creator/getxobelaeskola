...
export async function POST(request: Request) {
	try {
		...
		if (descripcion_es !== undefined)
			updateData.descripcion_es = descripcion_es;
		if (descripcion_eu !== undefined)
			updateData.descripcion_eu = descripcion_eu;
		if (precio !== undefined) updateData.precio = parseFloat(precio);
		if (duracion_h !== undefined)
			updateData.duracion_h = parseInt(duracion_h, 10);
