...
export async function POST(request: Request) {
	try {
		...
		if (isInstructor && !isAdmin && currentSession.instructor_id !== user.id) {
			return NextResponse.json(
				{ error: "No puedes modificar sesiones que no tienes asignadas" },
				{ status: 403 },
			);
		}
		...
		if (embarcacion_id !== undefined) {
			updateData.embarcacion_id =
				embarcacion_id === "" || embarcacion_id === "null" || !embarcacion_id
					? null
					: embarcacion_id;
		}
