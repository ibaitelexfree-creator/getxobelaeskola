	...
	try {
		...
		const funnel = [
			{ step: "Base de Usuarios", value: totalStudents, fill: "#8884d8" },
			{
				...
				value: Object.values(studentActivity).filter((s) => s.sessions > 0)
					.length,
				fill: "#83a6ed",
			},
			{
		...
		return NextResponse.json({
			...
			kpis: {
				...
				totalCost: (maintenance || []).reduce(
					(acc, curr) => acc + (Number(curr.coste) || 0),
					0,
				),
				totalRentals: rentals?.length || 0,
				totalInscriptions: inscriptions?.length || 0,
				activeBoats:
					boats?.filter((b) => b.estado === "disponible").length || 0,
				prevPeriodRevenue,
				retentionRate:
					totalStudents > 0
