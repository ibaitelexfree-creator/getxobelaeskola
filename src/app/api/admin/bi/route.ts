...
export async function GET(request: Request) {
	try {
		...
		if (compare === "yoy" && startDate && endDate) {
			...
			const [
				...
				supabaseAdmin
					...
					.select("monto_total")
					.eq("estado_pago", "pagado")
					.gte("fecha_pago", prevStart)
					.lte("fecha_pago", prevEnd),
				supabaseAdmin
					.from("inscripciones")
					.select("monto_total")
					.eq("estado_pago", "pagado")
				...
				supabaseAdmin
					.from("mantenimiento_logs")
					.select("coste")
					.gte("fecha_inicio", prevStart)
					.lte("fecha_inicio", prevEnd),
			]);

			const sumIncome = (arr: any[]) =>
				(arr || []).reduce(
					(acc, curr) => acc + (Number(curr.monto_total) || 0),
					0,
				);
			prevPeriodRevenue =
				sumIncome(pRentals || []) + sumIncome(pInscriptions || []);
		}
		...
		maintenance?.forEach((m: any) => {
			...
			if (bType === "crucero" || bName.includes("J80"))
				category = "Veleros J80";
		...
		inscriptions?.forEach((ins: any) => {
			...
			courseCategoryDemand[cat][monthKey] =
				(courseCategoryDemand[cat][monthKey] || 0) + 1;
		});
		...
		const allIncome = [
			...(rentals || []).map((r: any) => ({
				date: r.fecha_pago || r.created_at,
				amount: r.monto_total,
			})),
			...(inscriptions || []).map((i: any) => ({
				date: i.created_at,
				amount: i.monto_total || 0,
			})),
		];
		...
		allIncome.forEach((item: any) => {
			...
			if (!monthlyRevenue[key])
				monthlyRevenue[key] = { actual: 0, forecast: 0 };
		...
		const chartRevenue = sortedMonths.map((m: any, _i: any) => {
		...
		const studentActivity: Record<
			string,
			{ sessions: number; rentals: number; firstDate: Date }
		> = {};
		...
		const [{ data: historyInscriptions }, { data: historyRentals }] =
			await Promise.all([
				supabaseAdmin
					.from("inscripciones")
					.select("perfil_id, created_at")
					.in("perfil_id", allStudentIds),
				supabaseAdmin
					.from("reservas_alquiler")
					.select("perfil_id, fecha_pago")
					.in("perfil_id", allStudentIds),
			]);

		historyInscriptions?.forEach((i: any) => {
			if (!studentActivity[i.perfil_id]) {
				studentActivity[i.perfil_id] = {
					sessions: 0,
					rentals: 0,
					firstDate: new Date(i.created_at),
				};
			}
			studentActivity[i.perfil_id].sessions++;
			const d = new Date(i.created_at);
			if (d < studentActivity[i.perfil_id].firstDate)
				studentActivity[i.perfil_id].firstDate = d;
		});

		historyRentals?.forEach((r: any) => {
			if (!studentActivity[r.perfil_id]) {
				studentActivity[r.perfil_id] = {
					sessions: 0,
					rentals: 0,
					firstDate: new Date(r.fecha_pago),
				};
			}
			studentActivity[r.perfil_id].rentals++;
			const d = new Date(r.fecha_pago);
			if (d < studentActivity[r.perfil_id].firstDate)
				studentActivity[r.perfil_id].firstDate = d;
		});

		const totalStudents = Object.keys(studentActivity).length;
		const recurringStudents = Object.values(studentActivity).filter(
			(s: any) => s.sessions > 1,
		).length;
		const studentRenters = Object.values(studentActivity).filter(
			(s: any) => s.sessions > 0 && s.rentals > 0,
		).length;
		const loyalCustomerCount = Object.values(studentActivity).filter(
			(s: any) => s.sessions + s.rentals > 3,
		).length;

		const funnel = [
			{ step: "Base de Usuarios", value: totalStudents, fill: "#8884d8" },
			{
				step: "Alumnos Activos",
				value: Object.values(studentActivity).filter((s: any) => s.sessions > 0)
					.length,
				fill: "#83a6ed",
			},
			{
				step: "Alumnos Recurrentes",
				value: recurringStudents,
				fill: "#8dd1e1",
			},
		...
		return NextResponse.json({
			...
			kpis: {
				...
				totalCouponRevenue: [
					...
					(acc: any, curr: any) => acc + (Number(curr.monto_total) || 0),
					0,
				),
				totalCost: (maintenance || []).reduce(
					(acc: any, curr: any) => acc + (Number(curr.coste) || 0),
					0,
				),
				totalRentals: rentals?.length || 0,
				totalInscriptions: inscriptions?.length || 0,
				activeBoats:
					boats?.filter((b: any) => b.estado === "disponible").length || 0,
				prevPeriodRevenue,
				retentionRate:
					totalStudents > 0
						? ((recurringStudents / totalStudents) * 100).toFixed(1)
						: 0,
			},
		});
	} catch (error: any) {
