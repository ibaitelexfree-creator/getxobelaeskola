...
export async function POST(request: Request) {
	try {
		...
		const { data: inviteData, error: inviteError } =
			await supabaseAdmin.auth.admin.inviteUserByEmail(requestedEmail, {
		...
		if (inviteError) {
			...
			if (
				...
				const existing = users.find(
					(u: any) => u.email?.toLowerCase() === requestedEmail,
				);
				if (!existing) {
					throw new Error(
						`El usuario ${requestedEmail} parece existir pero no pudimos encontrar su registro. Contacte con soporte.`,
					);
				}
				userId = existing.id;
			} else {
