...
export async function POST(request: Request) {
	try {
		...
		try {
			...
			const { data: poolIds, error: poolError } = await supabase.rpc(
				...
				if (!srsError) {
					...
					if (srsData) {
						...
						srsData.forEach((s: any) =>
							srsMap.set(s.question_id, new Date(s.next_review).getTime()),
						);
					}
					...
					// 3. Future Questions
					if (remaining > 0) {
						const futureToTake = futureQuestions
							.sort(() => Math.random() - 0.5)
							.slice(0, remaining);
		...
		if (preguntasIds.length < evaluacion.num_preguntas) {
			const { data: standardIds, error: standardError } = await supabase.rpc(
				"seleccionar_preguntas_evaluacion",
				{
					p_entidad_tipo: evaluacion.entidad_tipo,
					p_entidad_id: evaluacion.entidad_id,
