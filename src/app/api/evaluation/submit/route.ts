...
export async function POST(request: Request) {
	try {
		...
		// Calcular puntuación
		const { data: resultado, error: calcError } = await supabase.rpc(
			"calcular_puntuacion_intento",
		...
		try {
			if (puntuacion.aprobado) {
				await supabase.rpc("incrementar_racha_quizzes", {
					p_alumno_id: user.id,
				});
			} else {
		...
		try {
			...
			if (allQuestions && allQuestions.length > 0) {
				...
				const { data: currentSrs } = await supabase
					...
					.in(
						"question_id",
						(allQuestions as any[]).map((q: any) => q.id),
					);
