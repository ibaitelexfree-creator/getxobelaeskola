...
export async function GET(
	...
	try {
		...
		if (!isPublic && !isOwner) {
			...
			if (
				!authError &&
				(authUser as any)?.user?.user_metadata?.is_public_profile === true
			) {
				isPublic = true;
			}
		}
		...
		if (!isPublic && !isOwner) {
			return withCors(
				NextResponse.json({ error: "Profile is private" }, { status: 403 }),
				request,
			);
		}
		...
		const responseData = {
			...
			badges: (logros || []).map((l: any) => ({
				...
				description:
					locale === "eu" ? l.logro.descripcion_eu : l.logro.descripcion_es,
				icon: l.logro.icono,
			...
			certificates: (certificados || []).map((c: any) => ({
				...
				name:
					locale === "eu"
						? c.nivel?.nombre_eu || c.curso?.nombre_eu
						: c.nivel?.nombre_es || c.curso?.nombre_es,
