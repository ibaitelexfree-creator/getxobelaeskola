...
export async function GET() {
	try {
		...
		const { data: habilidades, error: errorHabilidades } = await supabase.rpc(
		...
		const { data: rangoData, error: errorRango } = await supabase.rpc(
		...
		const habilidadesObtenidas_ =
			habilidades?.filter((h: any) => h.obtenida) || [];
		const habilidadesBloqueadas =
			habilidades?.filter((h: any) => !h.obtenida) || [];
		...
		return NextResponse.json({
			...
			estadisticas: {
				...
				categorias: {
					tecnica: habilidadesObtenidas_.filter(
						(h: any) => h.categoria === "tecnica",
					).length,
					tactica: habilidadesObtenidas_.filter(
						(h: any) => h.categoria === "tactica",
					).length,
					seguridad: habilidadesObtenidas_.filter(
						(h: any) => h.categoria === "seguridad",
					).length,
					meteorologia: habilidadesObtenidas_.filter(
						(h: any) => h.categoria === "meteorologia",
					).length,
					excelencia: habilidadesObtenidas_.filter(
						(h: any) => h.categoria === "excelencia",
					).length,
				},
			},
		});
...
export async function POST() {
	try {
		...
		const { data: habilidadesNuevas, error } = await supabase.rpc(
			"evaluar_habilidades",
			{
				p_alumno_id: alumnoId,
			},
