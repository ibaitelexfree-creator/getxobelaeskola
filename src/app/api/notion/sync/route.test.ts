...
describe("Notion Sync Route", () => {
	...
	it("rejects unauthorized requests", async () => {
		const req = new NextRequest(
			"http://localhost/api/notion/sync?secret=wrong",
		);
	...
	it("calls execFile with the correct arguments to prevent command injection", async () => {
		...
		const req = new NextRequest(
			`http://localhost/api/notion/sync?secret=getxo_notion_sync_2026_pro&mode=pull&table=${encodeURIComponent(maliciousTable)}`,
		);
		const res = await POST(req);
