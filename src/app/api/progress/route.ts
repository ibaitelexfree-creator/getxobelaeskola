...
export async function GET(request: Request) {
	try {
		...
		const _is_staff = profile?.rol === "admin" || profile?.rol === "instructor";
		...
		const [resProgress, resCourses, resLevels] = await Promise.all([
		...
		if (resProgress.error) {
			...
			return withCors(
				NextResponse.json({ error: "Error loading progress" }, { status: 500 }),
				request,
			);
		}
		...
		const levelMap = (allLevels || []).reduce(
		...
		const filteredProgress = (rawProgress || []).map((p: any) => {
			if (p.tipo_entidad === "curso" && courseMap[p.entidad_id]) {
				return {
					...p,
					slug: courseMap[p.entidad_id].slug,
					nombre:
						locale === "eu"
							? courseMap[p.entidad_id].nombre_eu
							: courseMap[p.entidad_id].nombre_es,
				};
			}
			if (p.tipo_entidad === "nivel" && levelMap[p.entidad_id]) {
				return {
					...p,
					slug: levelMap[p.entidad_id].slug,
					nombre:
						locale === "eu"
							? levelMap[p.entidad_id].nombre_eu
							: levelMap[p.entidad_id].nombre_es,
				};
			}
			return p;
		});
		...
		const { data: certificados } = await supabase
			.from("certificados")
			.select(
				"*, curso:curso_id(nombre_es, nombre_eu), nivel:nivel_id(nombre_es, nombre_eu)",
			)
			.eq("alumno_id", user.id);

		// 5. CALCULATE STATISTICS
		const horasTotales =
			horas?.reduce((acc: number, h: any) => acc + Number(h.duracion_h), 0) ||
			0;
		const puntosTotales =
		...
		const nivelesCompletados = filteredProgress.filter(
			(p: any) => p.tipo_entidad === "nivel" && p.estado === "completado",
		).length;

		const cursosProgreso = filteredProgress.filter(
			(p: any) => p.tipo_entidad === "curso",
		);
		const progresoGlobal =
			cursosProgreso.length > 0
				? Math.round(
						cursosProgreso.reduce(
							(acc: number, c: any) => acc + (c.porcentaje || 0),
							0,
						) / cursosProgreso.length,
					)
				: 0;
		...
		const skillRadar = Object.entries(categoriesMap).map(([dbCat, uiLabel]) => {
			const userSkillsInCategory = (skills || []).filter(
				(s: any) => s.habilidad?.categoria === dbCat,
			).length;
			const maxEstimate =
				dbCat === "tecnica" ? 6 : dbCat === "excelencia" ? 1 : 2;
			const score = Math.min(
				100,
				Math.round((userSkillsInCategory / maxEstimate) * 100),
			);
		...
		const boatStats: Record<
			string,
			{ hours: number; sessions: number; lastUsed: string }
		> = {};
		(horas || []).forEach((h: any) => {
		...
		const distinctBoats = new Set(
			(horas || []).map((h: any) => h.embarcacion).filter(Boolean),
		).size;
		const totalHours = horasTotales;

		const thirtyDaysAgo = new Date();
		thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
		const recentSessions = (horas || []).filter(
			(h: any) => new Date(h.fecha) > thirtyDaysAgo,
		).length;
		...
		const highWindSessions = (horas || []).filter((h: any) => {
			...
			return windMatch && parseInt(windMatch[1], 10) > 15;
		});
		...
		if (nivelesCompletados === 0 && totalSessions === 0) {
			recommendations.push({
				...
				message:
					"Tu perfil está listo para la transformación. El análisis sugiere comenzar con la base teórica de Vela Ligera para desbloquear tu primer rango oficial.",
			...
			recommendations.push({
				...
				message:
					"Tu técnica es sólida en vientos medios, pero el análisis de bitácora muestra una brecha en condiciones >15kt. El siguiente paso lógico es el perfeccionamiento de trimado.",
				analysis:
					"Base técnica completada. Falta experiencia en viento fuerte.",
			...
			recommendations.push({
				...
				message:
					"Has dominado tu embarcación habitual. Para alcanzar el rango de Timonel, el sistema recomienda probar un monotipo diferente o participar en una travesía de crucero.",
			...
			recommendations.push({
				...
				message:
					"Mantienes un rumbo excelente. Tu consistencia es superior al 80% de los alumnos de tu nivel. Sigue acumulando millas para desbloquear el siguiente certificado.",
