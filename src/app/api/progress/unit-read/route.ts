...
export async function POST(request: Request) {
	try {
		...
		if (!unidad_id || !seccion) {
			return withCors(
				NextResponse.json(
					{ error: "Faltan datos requeridos (unidad_id, seccion)" },
					{ status: 400 },
				),
				request,
			);
		...
		if (accessError || !tieneAcceso) {
			console.warn(
				`Intento de acceso no autorizado a unidad ${unidad_id} por usuario ${user.id}`,
			);
			return withCors(
				NextResponse.json(
					{ error: "Acceso denegado: La unidad está bloqueada." },
					{ status: 403 },
				),
				request,
			);
