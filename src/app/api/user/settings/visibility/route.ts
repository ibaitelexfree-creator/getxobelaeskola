...
export async function POST(request: Request) {
	try {
		...
		if (dbError) {
			console.error(
				"Error updating profiles table (schema might be missing column):",
				dbError,
			);
			...
			return withCors(
				NextResponse.json({
					success: true,
					warning:
						"Saved to preferences, but public profile might not be active due to database schema limitations.",
				}),
				request,
			);
		}
