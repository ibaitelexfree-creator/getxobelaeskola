...
import type { Course, CourseEdition, Inscription } from "@/types/courses";
...
export async function POST(request: Request) {
	...
	try {
		...
		if (editionId && !editionId.startsWith("ext_")) {
			...
			return NextResponse.json(
				{ error: "Faltan datos (editionId o courseId)" },
				{ status: 400 },
			);
		}
		...
		if (alreadyEnrolled) {
			return NextResponse.json(
				{ error: "Ya estás inscrito en este curso o edición" },
				{ status: 400 },
			);
		}
		...
		if (imageUrl?.startsWith("/")) imageUrl = `${origin}${imageUrl}`;
		...
		if (course.precio === 0) {
			...
			if (
				editionId &&
				editionId !== "" &&
				!editionId.startsWith("test-") &&
				!editionId.startsWith("ext_")
			) {
			...
			if (insError) {
				...
				return NextResponse.json(
					{ error: `Error al inscribirse: ${insError.message}` },
					{ status: 500 },
				);
			}
			...
			return NextResponse.json({
				url: `${origin}/${locale}/student/dashboard?success=true`,
			});
		}
		// -------------------------
