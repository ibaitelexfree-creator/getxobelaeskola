	...
	try {
		...
		if (!serviceId || !reservedDate || !reservedTime) {
			return NextResponse.json(
				{ error: "Faltan datos obligatorios (servicio, fecha o hora)" },
				{ status: 400 },
			);
		}
		...
		if (userBooking) {
			return NextResponse.json(
				{
					error:
						"Ya tienes una reserva para este horario. Por favor, revisa tu panel de control.",
				},
				{ status: 400 },
			);
		}
		...
		const appUrl =
			process.env.NEXT_PUBLIC_APP_URL || origin || `${protocol}://${host}`;
