...
export async function GET() {
	try {
		...
		console.log(
			`[DashboardStats] Inscriptions fetch took: ${(endInscriptions - startInscriptions).toFixed(2)}ms`,
		);
		...
		const [
			...
			supabase
				...
				.select("id, tipo_entidad, estado, updated_at, created_at")
		...
		const totalHours =
			horas?.reduce((acc, curr) => acc + Number(curr.duracion_h), 0) || 0;
		const totalMiles = totalHours * 5.2;
		const academyLevels =
			progress?.filter(
				(p) => p.tipo_entidad === "nivel" && p.estado === "completado",
			).length || 0;
		...
		const globalProgress = Math.min(
			100,
			Math.round((completedModules / safeTotalModules) * 100),
		);
