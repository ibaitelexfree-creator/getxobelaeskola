...
export async function GET() {
	try {
		...
		const [{ data: editions }, { data: allCourses }] = await Promise.all([
			supabase
				.from("ediciones_curso")
				.select("id, curso_id, fecha_inicio, fecha_fin"),
		...
		const enrichedInscriptions = (rawInscriptions || []).map((ins) => {
			...
			const courseDirect = (allCourses || []).find(
				(c) => c.id === ins.curso_id,
			);
			const courseViaEd = ed
				? (allCourses || []).find((c) => c.id === ed.curso_id)
				: null;
