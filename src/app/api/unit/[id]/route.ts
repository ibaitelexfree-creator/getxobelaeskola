...
export async function GET(
	...
	try {
		...
		const { data: actividades } = await supabase
			.from("actividades")
			.select(
				"id, tipo, titulo_es, titulo_eu, descripcion_es, descripcion_eu, rubrica",
			)
		...
		const currentIndex =
			unidadesHermanas?.findIndex((u: any) => u.id === params.id) ?? -1;
		const unidadAnterior =
			currentIndex > 0 ? unidadesHermanas?.[currentIndex - 1] : null;
		const unidadSiguiente =
			currentIndex >= 0 && currentIndex < (unidadesHermanas?.length ?? 0) - 1
