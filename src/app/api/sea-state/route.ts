	...
	try {
		...
		try {
			...
			const response = await fetch(
				"https://portus.puertos.es/Portus_RT/point/3136/data",
				{
			...
			if (response.ok) {
				...
				if (currentData) {
					const hm0 =
						currentData.wave_height ??
						currentData.hm0 ??
						currentData.waveHeight ??
						currentData.wave_height_hm0;
					const tp =
						currentData.wave_period ??
						currentData.tp ??
						currentData.period ??
						currentData.wave_period_tp;
					const temp =
					...
					const wspd =
						...
						currentData.vel_viento ??
						currentData.windSpeed;
					const wdir =
						currentData.wind_direction ??
						currentData.dir_viento ??
						currentData.windDirection;

					if (hm0 !== undefined && hm0 !== null)
						waveHeight = parseFloat(hm0).toFixed(1);
					if (tp !== undefined && tp !== null)
						wavePeriod = Math.round(parseFloat(tp));
					if (temp !== undefined && temp !== null)
						waterTemp = parseFloat(temp).toFixed(1);
				...
				console.warn(
					`Puertos del Estado API returned status ${response.status}`,
				);
			}
		} catch (apiError) {
			console.warn(
				"Failed to fetch from Puertos del Estado API, falling back to mock/weather data",
				apiError,
			);
		}
