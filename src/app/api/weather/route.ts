	...
	try {
		...
		try {
			...
			if (weather && weather.knots !== undefined) {
				...
				const shouldSave =
					!lastEntry ||
					now.getTime() - new Date(lastEntry.timestamp).getTime() >
						15 * 60 * 1000;
			...
			console.warn(
				"Weather history persistence skipped (table might not exist)",
			);
		}
		...
		try {
			...
			if (rentals) {
				fleetSub = {
					...
					retorno: rentals.filter((r) => r.estado_entrega === "devuelto")
						.length,
					pendiente: rentals.filter((r) => r.estado_entrega === "pendiente")
						.length,
				};
				...
				history = Array.from({ length: 24 }).map((_, i) => {
					...
					return {
						...
						direction: Math.round(
							(280 + Math.sin(i / 5) * 40 + Math.random() * 20) % 360,
						),
