...
export async function GET() {
	try {
		...
		const coursesEnrolled =
			progress?.filter((p) => p.tipo_entidad === "curso").length || 0;
		const coursesCompleted =
			progress?.filter(
				(p) => p.tipo_entidad === "curso" && p.estado === "completado",
			).length || 0;
		const unitsCompleted =
			progress?.filter(
				(p) => p.tipo_entidad === "unidad" && p.estado === "completado",
			).length || 0;
		...
		const completedModuleIds = new Set(
			progress
				?.filter(
					(p) => p.tipo_entidad === "modulo" && p.estado === "completado",
				)
				.map((p) => p.entidad_id) || [],
		);

		const academyProgress = calculateAcademyProgress(
			completedModuleIds,
			esquemaCursos.modules,
		);
		...
		const totalPoints =
			progress?.reduce((acc, p) => acc + (p.puntos_obtenidos || 0), 0) || 0;
		...
		const totalHours =
			hoursData?.reduce((acc, curr) => acc + Number(curr.duracion_h), 0) || 0;
		...
		const activeCourse = progress?.find(
			(p) => p.tipo_entidad === "curso" && p.estado === "en_progreso",
		);
