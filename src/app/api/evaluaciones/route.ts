...
export async function GET(request: Request) {
	try {
		...
		if (!entidadTipo || !entidadId) {
			return withCors(
				NextResponse.json({ error: "Faltan parámetros" }, { status: 400 }),
				request,
			);
		}
		...
		const { data, error } = await supabase
			.from("evaluaciones")
			.select(
				"id, tipo, titulo_es, titulo_eu, num_preguntas, tiempo_limite_min",
			)
		...
		return withCors(
			NextResponse.json({ error: "Error del servidor" }, { status: 500 }),
			request,
		);
	}
}
