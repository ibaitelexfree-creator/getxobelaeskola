...
export async function POST(request: Request) {
    try {
        ...
        if (!['public', 'anonymous', 'private'].includes(visibility)) {
            return withCors(NextResponse.json({ error: 'Invalid visibility value' }, { status: 400 }), request);
        }
        ...
        if (dbError) {
            ...
            return withCors(NextResponse.json({ error: 'Error updating visibility' }, { status: 500 }), request);
        }
