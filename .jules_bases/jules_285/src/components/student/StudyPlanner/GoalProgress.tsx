...
export default function GoalProgress({
    ...
    const [goalInput, setGoalInput] = useState(goal?.weekly_goal_minutes ? String(goal.weekly_goal_minutes / 60) : '0');
    ...
    const percentage = goalMinutes > 0 ? Math.min(100, (totalMinutes / goalMinutes) * 100) : 0;
        ...
        <div className="bg-card/30 border border-white/10 rounded-lg p-6 flex flex-col justify-between h-full">
            <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="text-sm font-bold text-white uppercase tracking-wider">Objetivo Semanal</h3>
            ...
            <div className="space-y-2 mt-auto">
                <div className="flex justify-between items-end">
                    <span className="text-2xl font-display font-bold text-white italic">
                        {totalHours} <span className="text-sm not-italic font-normal text-white/40">/ {goalHours} h</span>
                    </span>
                    <span className="text-accent font-bold text-sm">{Math.round(percentage)}%</span>
                </div>
