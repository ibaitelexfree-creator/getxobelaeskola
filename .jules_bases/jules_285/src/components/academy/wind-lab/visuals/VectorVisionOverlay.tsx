...
import { DerivedPhysics } from '../physics/PhysicsEngine';
...
export const VectorVisionOverlay: React.FC<VectorVisionOverlayProps> = ({
    ...
    const animate = (time: number) => {
        if (previousTimeRef.current !== undefined) {
            ...
            smoothed.current.awSpeed += (currentPhysics.apparentWindSpeed - smoothed.current.awSpeed) * alpha;
            smoothed.current.driveForce += (Math.abs(currentPhysics.driveForce) - smoothed.current.driveForce) * alpha;
            smoothed.current.heelForce += (currentPhysics.heelForce - smoothed.current.heelForce) * alpha;
    ...
    const liftAngleRad = (physics.apparentWindAngle > 0) ? (awaRad - Math.PI / 2) : (awaRad + Math.PI / 2);
    const dragAngleRad = awaRad + Math.PI;

    const renderArrow = (angleRad: number, magnitude: number, color: string, label: string) => {
    ...
    return (
        <svg className="absolute inset-0 w-full h-full pointer-events-none z-20">
            <g transform={`translate(${50}%, ${50}%)`}>
                {renderArrow(awaRad, smoothed.current.awSpeed * windScale, '#3B82F6', t('forces.aw'))}
                {renderArrow(liftAngleRad, smoothed.current.driveForce * forceScale + 20, '#10B981', t('forces.lift'))}
                {renderArrow(dragAngleRad, smoothed.current.heelForce * forceScale + 20, '#EF4444', t('forces.drag'))}
            </g>
        </svg>
    );
};
