...
import { MontessoriTopic, InteractionResult } from './types';
import AccessibleModal from '@/components/shared/AccessibleModal';
import { Check, X, HelpCircle, Image as ImageIcon, BookOpen } from 'lucide-react';
...
const TopicStudyModal: React.FC<TopicStudyModalProps> = ({ isOpen, onClose, topic, onRecordResult }) => {
                ...
                <div className="grid md:grid-cols-2 gap-8 items-start">
                    ...
                    <div className="w-full aspect-video bg-white/5 rounded-xl flex items-center justify-center border border-white/10 overflow-hidden relative group">
                        {topic.imageUrl ? (
                             ...
                             topic.imageUrl.startsWith('http') || topic.imageUrl.startsWith('/') ? (
                                <Image
                                    src={topic.imageUrl}
                                    alt={topic.title}
                                    fill
                            ...
                            <div className="flex flex-col items-center gap-4 text-white/20">
                                <ImageIcon size={48} />
                                <span className="text-xs uppercase tracking-widest font-bold">Sin imagen visual</span>
                            </div>
                        )}
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="p-6 bg-white/5 rounded-xl border border-white/10">
                            ...
                            <div className="grid grid-cols-2 gap-4 text-sm">
                                <div>
                                    <span className="block text-white/40 text-[10px] uppercase tracking-wider mb-1">Dificultad</span>
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="font-mono text-accent">{Math.round(topic.difficulty * 10)}/10</span>
                                    </div>
                                </div>
                                <div>
                                    <span className="block text-white/40 text-[10px] uppercase tracking-wider mb-1">Tipo</span>
