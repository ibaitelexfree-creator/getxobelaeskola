...
export default function LeafletMap({ center, mappedSessions, livePoints, currentPosition }: LeafletMapProps) {
    ...
    useEffect(() => {
        ...
        (mappedSessions || []).forEach(s => {
            if (s.ubicacion) {
                L.circleMarker([s.ubicacion.lat, s.ubicacion.lng], {
                    ...
                    weight: 1,
                    fillOpacity: 0.8
                }).addTo(map).bindPopup(s.zona_nombre || 'Sesión');

                if (s.track_log && Array.isArray(s.track_log)) {
                    const positions = s.track_log.map((p: any) => [p.lat, p.lng]);
                    L.polyline(positions, { color: '#0ea5e9', weight: 2, opacity: 0.4 }).addTo(map);
                }
            }
        });
        ...
        if (livePoints && livePoints.length > 1) {
            ...
            L.polyline(positions, { color: '#fbbf24', weight: 3, dashArray: '5, 10', opacity: 0.8 }).addTo(map);
        }
