...
import { MissionProgress, MissionStep, BranchingMissionData } from '@/components/academy/interactive-engine/types';

/**
 * Fetch a mission by its slug, including all steps.
 * Note: This assumes the mission_steps table has a FK to missions.
 */
export async function getMissionBySlug(slug: string): Promise<BranchingMissionData | null> {
 ...
 */
export async function getUserProgress(missionId: string, userId: string): Promise<MissionProgress | null> {
    const supabase = createClient();
 ...
 */
export async function saveUserProgress(progress: Partial<MissionProgress> & { mission_id: string, user_id: string }): Promise<MissionProgress | null> {
    const supabase = createClient();
