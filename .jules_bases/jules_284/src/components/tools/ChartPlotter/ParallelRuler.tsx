...
export default function ParallelRuler({ x, y, angle, scale, onUpdate, worldToScreen, screenToWorld }: ParallelRulerProps) {
    ...
    const handleMouseDown = (e: React.MouseEvent) => {
        ...
        setDragOffset({ x: screenMouse.x - screenPos.x, y: screenMouse.y - screenPos.y });
        setIsDragging(true);
    };
    ...
    }, [isDragging, isRotating, dragOffset, startAngle, onUpdate, x, y, angle, screenToWorld, screenPos.x, screenPos.y]);
    ...
    return (
        <g transform={`translate(${screenPos.x}, ${screenPos.y}) rotate(${angle})`}>
            ...
            />
            {/* Linkages (Visual) */}
            <line x1={-width / 3} y1={-height / 2 + 5} x2={-width / 4} y2={height / 2 - 5} stroke="#666" strokeWidth="2" />
            <line x1={width / 4} y1={-height / 2 + 5} x2={width / 3} y2={height / 2 - 5} stroke="#666" strokeWidth="2" />
            ...
            {/* Angle Text */}
            <text x="0" y="5" textAnchor="middle" fontSize={12 * scale} fill="#333" pointerEvents="none" style={{ userSelect: 'none' }}>
                {Math.round(angle)}°
            </text>
        </g>
    );
