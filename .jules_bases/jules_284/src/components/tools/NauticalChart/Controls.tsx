...
import { Navigation, MousePointerClick, MapPin, Trash2 } from 'lucide-react';
...
export default function Controls({ activeTool, onToolChange, onClear }: ControlsProps) {
    ...
    return (
        <div className="absolute top-4 left-4 z-[1000] flex flex-col gap-2 bg-white/95 p-2 rounded-xl shadow-lg border border-slate-200/60 backdrop-blur-md">
            <button
                ...
                title={t('navigate')}
            >
                <Navigation size={20} strokeWidth={activeTool === 'navigate' ? 2.5 : 2} />
                <span className="text-[10px] uppercase tracking-[0.1em] font-black hidden md:inline-block">{t('navigate')}</span>
            </button>
            <button
                ...
                title={t('route')}
            >
                <MousePointerClick size={20} strokeWidth={activeTool === 'route' ? 2.5 : 2} />
                <span className="text-[10px] uppercase tracking-[0.1em] font-black hidden md:inline-block">{t('route')}</span>
            </button>
            <button
            ...
            >
                ...
                <span className="text-[10px] uppercase tracking-[0.1em] font-black hidden md:inline-block">{t('waypoint')}</span>
            </button>
            ...
            >
                ...
                <span className="text-[10px] uppercase tracking-[0.1em] font-black hidden md:inline-block">{t('clear')}</span>
            </button>
        </div>
    );
}
