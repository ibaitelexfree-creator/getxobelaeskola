...
export default function StaffActivityContainer({ userId, locale }: StaffActivityContainerProps) {
    ...
    useEffect(() => {
        async function loadData() {
            try {
                ...
                if (!requesterProfile || (requesterProfile.rol !== 'admin' && requesterProfile.rol !== 'instructor')) {
    ...
    if (!staff && userId !== 'placeholder') {
        return (
            <div className="text-center py-20">
                ...
                <Link href={`/${locale}/staff`} className="text-accent underline mt-4 block">Volver</Link>
            </div>
        );
    }
    ...
    if (userId === 'placeholder') {
        return (
            <div className="text-center py-20">
                ...
                <Link href={`/${locale}/staff`} className="text-accent underline mt-4 block">Volver al panel</Link>
            </div>
        );
    }
