import { WindPhysicsState } from '../types';
import { PHYSICS_CONSTANTS } from './constants';
import { deg2rad, rad2deg, normalizeAngle180 } from './angleUtils';
import { polarToCartesian, subtractVectors, cartesianToPolar } from './vectorUtils';
import { getLiftCoefficient, getDragCoefficient, detectLuffing, detectStall } from './aeroModel';
 ...
 */
export function stepPhysics(state: WindPhysicsState, dt: number = PHYSICS_CONSTANTS.DT): WindPhysicsState {
    ...
    const scaleFactor = 0.5 * PHYSICS_CONSTANTS.AIR_DENSITY * PHYSICS_CONSTANTS.SAIL_AREA;
    ...
    const hullDrag = (state.boatSpeed * state.boatSpeed) * PHYSICS_CONSTANTS.HULL_DRAG_COEFF;
    ...
    if (newSpeed > PHYSICS_CONSTANTS.MAX_SPEED) newSpeed = PHYSICS_CONSTANTS.MAX_SPEED;

    // 6. Efficiency Metric (0..1)
    // ------------------------------------------------------------------
    // Based on how close AoA is to OPTIMAL (15 deg)
    // 1.0 = Perfect. 0.0 = Terrible.
    const distToOptimal = Math.abs(Math.abs(signedAoa) - PHYSICS_CONSTANTS.OPTIMAL_AOA);
