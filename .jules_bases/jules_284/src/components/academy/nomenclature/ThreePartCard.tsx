...
import { NomenclatureCard } from '@/data/academy/nautical-nomenclature';
...
export default function ThreePartCard({ card, mode, locale, isMatched, onDragStart }: ThreePartCardProps) {
    ...
    const SvgContent = card.svgPath ? (
        ...
        <g stroke="#94a3b8" strokeWidth="2" fill="none">
            ...
            <text x="100" y="150" textAnchor="middle" fontSize="40" fill="#cbd5e1" opacity="0.4">?</text>
        </g>
    );
    ...
    if (mode === 'definition') {
        return (
            <div className={`w-full p-4 bg-white rounded-lg border-2 ${isMatched ? 'border-blue-500' : 'border-dashed border-slate-200'} transition-all`}>
