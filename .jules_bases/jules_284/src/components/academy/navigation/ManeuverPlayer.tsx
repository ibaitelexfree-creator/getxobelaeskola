...
import {
    ...
    Wind, Anchor, LifeBuoy, Zap, Clock
} from 'lucide-react';
import { MANEUVERS, Maneuver } from './maneuverData';
...
export default function ManeuverPlayer({ initialManeuverId, className = '' }: ManeuverPlayerProps) {
    ...
    return (
        <div className={`flex flex-col gap-6 w-full max-w-4xl mx-auto p-4 bg-slate-900 rounded-xl shadow-2xl border border-slate-700 ${className}`}>
                ...
                <svg viewBox="0 0 100 100" className="w-full h-full p-8">
                    ...
                    <AnimatePresence>
                        {currentStep.extraElement && (
                            <motion.g
                                initial={{ opacity: 0 }}
                                animate={{ opacity: 1, x: currentStep.extraElement.x, y: currentStep.extraElement.y }}
                                exit={{ opacity: 0 }}
                                transition={{ duration: 0.5 }}
                            >
                                {currentStep.extraElement.type === 'dock' && (
                                    <rect x="-10" y="-5" width="20" height="10" fill="#78350f" rx="1" />
                                )}
                                {currentStep.extraElement.type === 'mob' && (
                                    <g>
                                        <circle r="3" fill="#ef4444" />
                                        <circle r="5" stroke="#ef4444" strokeWidth="0.5" fill="none" className="animate-ping" />
                                    </g>
                                )}
                            </motion.g>
                        )}
                    ...
                    >
                        {/* Hull */}
                        <path d="M0,-8 C3,-5 4,2 3,8 L-3,8 C-4,2 -3,-5 0,-8 Z" fill="#f8fafc" stroke="#334155" strokeWidth="0.5" />
                        ...
                        <motion.line
                            ...
                            transition={{ type: "spring", stiffness: 60, duration: 1 / speed }}
