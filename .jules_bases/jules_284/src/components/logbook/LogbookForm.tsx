...
import { LogbookFormData } from '@/types/logbook';
...
export default function LogbookForm({ onSuccess }: LogbookFormProps) {
    ...
    const inputClass = "w-full bg-nautical-deep border-b border-white/10 focus:border-accent outline-none p-3 text-sea-foam font-light transition-colors placeholder-white/20";
    const labelClass = "text-xs uppercase tracking-widest text-accent font-semibold ml-1 mb-1 block";

    return (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 bg-navy-800/50 p-6 rounded-lg border border-white/5">
            <h3 className="text-xl font-display text-white mb-4">Nueva Salida al Mar</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    <label htmlFor="puerto_salida" className={labelClass}>Puerto de Salida</label>
                    <input
                        id="puerto_salida"
                        type="text"
                        placeholder="Ej: Getxo Kaia"
                        {...register('puerto_salida', { required: 'El puerto es obligatorio' })}
                        className={inputClass}
                    />
                    {errors.puerto_salida && <p className="text-xs text-red-500 mt-1">{errors.puerto_salida.message}</p>}
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label htmlFor="viento_nudos" className={labelClass}>Viento (Nudos)</label>
                    <input
                        id="viento_nudos"
                        type="number"
                        step="0.1"
                ...
                <div>
                    <label htmlFor="viento_direccion" className={labelClass}>Dirección Viento</label>
                    <select
                        id="viento_direccion"
            ...
            {/* Tripulación */}
            <div>
                <label htmlFor="tripulacion" className={labelClass}>Tripulación</label>
                <input
                    id="tripulacion"
                    type="text"
            ...
            {/* Maniobras */}
            <div>
                <label htmlFor="maniobras" className={labelClass}>Maniobras Practicadas</label>
                <textarea
                    id="maniobras"
                    rows={2}
            ...
            {/* Observaciones */}
            <div>
                <label htmlFor="observaciones" className={labelClass}>Observaciones / Notas</label>
                <textarea
                    id="observaciones"
                    rows={3}
