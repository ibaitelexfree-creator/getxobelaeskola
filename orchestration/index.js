...
import { getIssue, getIssuesByLabel, formatIssueForPrompt, listReleases, downloadAsset, triggerWorkflowDispatch, listWorkflowRuns, getWorkflowRun, listWorkflowJobs, getWorkflowJobLogs } from './lib/github.js';
...
import { runLighthouseAudit, getBundleSizes } from './lib/performance.js';
...
app.get('/api/builds/jobs/:jobId/logs', async (req, res) => {
  const { jobId } = req.params;
  try {
    const logs = await getWorkflowJobLogs(GITHUB_OWNER, GITHUB_REPO, jobId, GITHUB_TOKEN);
    res.json({ logs });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
// Analyze a failed build and trigger auto-healing
app.post('/api/builds/analyze/:runId', async (req, res) => {
  const { runId } = req.params;
  try {
    const [run, jobs] = await Promise.all([
      getWorkflowRun(GITHUB_OWNER, GITHUB_REPO, runId, GITHUB_TOKEN),
    ...
    if (!failedJob) {
      return res.status(400).json({ error: 'No se encontró ningún job fallido en este run.' });
    }

    const logs = await getWorkflowJobLogs(GITHUB_OWNER, GITHUB_REPO, failedJob.id, GITHUB_TOKEN);

    // Trigger Jules analysis
    const apiKey = getJulesApiKey();
    const repoPath = `ibaitelexfree-creator/getxobelaeskola`;

    const task = `Analyze the following build failure from GitHub Actions Run ${runId} (${run.name}).
Job: ${failedJob.name}
Logs Summary:
${logs.substring(logs.length - 4000)}

Please identify the root cause, fix the code, and submit a PR with the label "auto-fix".`;

    const julesResponse = await axios.post('https://jules.googleapis.com/v1alpha/sessions', {
      prompt: task,
      sourceContext: {
        source: `sources/github/${repoPath}`,
        githubRepoContext: { startingBranch: run.head_branch || 'main' }
      },
    ...
    structuredLog('info', 'Auto-healing triggered via dashboard', { runId, sessionId: julesResponse.data.name });

    res.json({
      success: true,
      message: `Análisis de auto-healing iniciado para el job ${failedJob.name}.`,
      sessionId: julesResponse.data.name
    });
  } catch (error) {
    structuredLog('error', 'Auto-healing analysis failed', { runId, error: error.message });
    res.status(500).json({ error: error.message });
  }
});
...
app.post('/api/performance/audit', async (req, res) => {
  const url = req.body.url || 'https://getxobelaeskola.cloud';
  try {
...
// Get bundle sizes
app.get('/api/performance/bundle-sizes', async (req, res) => {
  try {
...
app.get('/mcp/tools', cacheMiddleware, (req, res) => {
  res.json({
    tools: [
      ...
      { name: 'telegram_send_message', description: 'Send a notification via Telegram', parameters: { text: { type: 'string', required: true, description: 'Message to send' }, parseMode: { type: 'string', required: false, description: 'Markdown or HTML' } } },
      // v2.8.0: Project Memory (Multi-Agent Shared State)
...
function initializeToolRegistry() {
  ...
  // v2.8.0: Project Memory (Multi-Agent Shared State)
  toolRegistry.set('project_memory_read', (p) => readProjectMemory(p.file));
