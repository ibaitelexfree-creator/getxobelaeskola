	...
	const modes = [
		{
			...
			icon: <Globe size={14} />,
			desc: "Entorno aislado en la nube",
		},
		{
			id: "local",
			label: "Local / VPN",
			icon: <HardDrive size={14} />,
			desc: "Túnel directo a desarrollo",
		},
	] as const;
	...
	return (
		<div className="flex flex-col gap-3">
			<div className="flex items-center justify-between">
				...
				<span
					className={`text-[10px] px-2 py-0.5 rounded-full font-mono ${previewMode === "render" ? "bg-buoy-orange/20 text-buoy-orange" : "bg-status-green/20 text-status-green"}`}
				>
				...
				{modes.map((mode) => (
					...
					>
						<div className="flex items-center gap-2">
							<span
								className={
									previewMode === mode.id ? "text-buoy-orange" : "text-white"
								}
							>
								{mode.icon}
							</span>
				...
				<div className="mt-2 flex flex-col gap-2">
					...
					{latestPreviews.slice(0, 3).map((preview, i) => (
						<motion.a
							key={i}
							href={
								previewMode === "render" ? preview.url : "http://localhost:3000"
							} // Local fallback
							target="_blank"
