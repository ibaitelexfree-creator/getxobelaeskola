...
export default function ControlPanel() {
	...
	const watchdogActive =
		...
		services.watchdog.state === "RUNNING";
	...
	const handlePoolToggle = async () => {
		setLoadingPool(true);
		try {
			if (poolOn) await pausePool();
			else await resumePool();
			setPoolOn(!poolOn);
		} catch (e) {
	...
	const handleWatchdogToggle = async () => {
		...
		try {
			if (watchdogOn) await pauseWatchdog();
			else await resumeWatchdog();
					...
					<div>
						<p
							className={`text-2xl font-display ${
								services.thermal.level > 2
									? "text-status-red"
									: services.thermal.level > 1
										? "text-status-amber"
										: "text-status-green"
							}`}
						>
						...
						<p className="text-2xs text-white/30">
							Level {services.thermal.level}/5
						</p>
					</div>
						...
						{[0, 1, 2, 3, 4].map((l) => (
							<div
								...
								className={`w-2 rounded-full transition-all ${
									l < services.thermal.level ? "h-8" : "h-3"
								} ${
									l < 2
										? "bg-status-green/50"
										: l < 4
				...
				<div className="mb-3">
					<div className="flex justify-between mb-1">
						<span className="text-2xs text-white/40">
							{services.jules.used} / {services.jules.total}
						</span>
						<span className="text-2xs text-white/40">
							{services.jules.active} active
						</span>
					</div>
					<div className="h-2 bg-white/5 rounded-full overflow-hidden">
						<motion.div
							...
							initial={{ width: 0 }}
							animate={{
								width: `${(services.jules.used / services.jules.total) * 100}%`,
							}}
			...
			>
				...
				<div className="grid grid-cols-4 gap-2 mb-3">
					{[
						{
							label: "Loops",
							value: services.watchdog.loops,
							color: "text-status-amber",
						},
						{
							label: "Stalls",
							value: services.watchdog.stalls,
							color: "text-status-amber",
						},
						{
							label: "Crashes",
							value: services.watchdog.crashes,
							color: "text-status-red",
						},
						{
							label: "State",
							value: services.watchdog.state.slice(0, 3),
							color: "text-white/60",
						},
					].map((s) => (
...
function TrustTunnelCard() {
	...
	const checkStatus = async () => {
		try {
			const status = await import("@/lib/maestro-client").then((m) =>
				m.getTrustStatus(),
			);
		...
		>
			<div className="flex items-center justify-between mb-3">
				<div className="flex items-center gap-2">
					<Shield
						size={16}
						className={active ? "text-status-green" : "text-white/40"}
					/>
					<span className="text-xs font-mono uppercase tracking-widest text-white/40">
						Trust Tunnel
					</span>
				...
				{active && (
					<span className="px-2 py-0.5 rounded-full bg-status-green/20 text-status-green text-[8px] font-bold uppercase tracking-tight">
						Active
					</span>
				)}
			</div>
				...
				{loading ? (
					...
					<>
						...
						<span className="text-sm font-medium">
							{showInput ? "Authorize Now" : "Activate Trust Tunnel"}
						</span>
					</>
				)}
			</button>
