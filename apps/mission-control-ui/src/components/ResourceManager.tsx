...
const EXTERNAL_VPN_HINTS: Record<
	string,
	{ label: string; password?: string; note?: string }
> = {
...
const getActions = (t: any): Record<string, ServiceAction> => ({
	start: {
		id: "start",
		label: t("actions.start.label"),
		description: t("actions.start.desc"),
		icon: Play,
		color: "text-status-green",
	},
	pause: {
		id: "pause",
		label: t("actions.pause.label"),
		description: t("actions.pause.desc"),
		icon: Pause,
	...
	stop: {
		...
		description: t("actions.stop.desc"),
		icon: Square,
		color: "text-status-red",
	},
	reset: {
		id: "reset",
		label: t("actions.reset.label"),
		description: t("actions.reset.desc"),
		icon: RotateCcw,
		color: "text-status-blue",
	},
	logs: {
		id: "logs",
		label: t("actions.logs.label"),
		description: t("actions.logs.desc"),
		icon: FileText,
		color: "text-status-blue",
	},
	link: {
		id: "link",
		label: t("actions.link.label"),
		description: t("actions.link.desc"),
		icon: ExternalLink,
		color: "text-status-blue",
	},
});
		...
		>
			...
			>
				...
				<div className="flex items-center gap-3 mb-6">
					...
					<div>
						<h3 className="font-mono font-black text-sm text-white uppercase tracking-widest">
							{hint.label}
						</h3>
						<p className="text-[9px] font-mono text-white/30 uppercase tracking-tight mt-0.5">
							ENLACE EXTERNO
						</p>
					</div>
				</div>
				...
				{hint.note && (
					<div className="flex items-start gap-2 p-3 rounded-xl bg-status-amber/10 border border-status-amber/20 mb-6">
						...
						/>
						<p className="text-[10px] font-mono text-status-amber/80 leading-relaxed">
							{hint.note}
						</p>
					</div>
				)}
		...
		>
			<div className="bg-[#0a0f1a]/95 border border-white/10 backdrop-blur-xl rounded-2xl p-4 shadow-[0_20px_50px_rgba(0,0,0,0.5)] max-w-[200px]">
				<div className="flex items-center gap-2 mb-2">
					...
					<span
						className={`text-[10px] font-mono font-black uppercase tracking-widest ${action.color}`}
					>
						{action.label}
					</span>
				</div>
									...
									<div className="flex flex-col gap-0.5">
										<span
											className={`font-black tracking-tighter ${
												log.event === "LOG_ERR" || log.event === "LOG_STDERR"
													? "text-status-red"
													: log.event === "START"
														? "text-status-green"
														: "text-status-blue"
	...
	return (
		<div className="flex flex-col gap-4 mx-1">
			<div className="flex items-center justify-between mb-1">
				<div className="flex flex-col gap-0.5">
					<h2 className="text-[10px] font-mono font-bold text-buoy-orange uppercase tracking-[0.3em]">
						{t("reactor.core")}
					</h2>
					<p className="text-[8px] font-mono text-white/20">
						{t("reactor.management")}
					</p>
				</div>
				...
				{Object.entries(power.services).map(([key, svc], i) => (
					...
					>
						...
						<div className="flex items-center gap-3 relative z-10 overflow-hidden">
							<div
								className={`p-2 rounded-lg shrink-0 ${svc.running ? "bg-status-green/10 text-status-green" : "bg-white/5 text-white/20"}`}
							>
								{key === "OLLAMA" ? (
									<Cpu size={14} />
								) : key === "CHROMA" ? (
							...
							<div className="flex flex-col overflow-hidden">
								<span
									className={`text-[10px] font-mono font-bold uppercase tracking-widest truncate ${svc.running ? "text-white" : "text-white/20"}`}
								>
									{svc.name}
								</span>
								<div className="flex flex-col gap-0.5">
									...
									{!svc.running ? (
										...
										<>
											...
											<ControlButton
												...
												actions={actions}
												onClick={() =>
													handleAction("reset", key, ImpactStyle.Medium)
												}
												className="p-1.5 bg-white/5 hover:bg-white/10 rounded-lg text-white/40 hover:text-status-blue transition-all border border-white/5"
											>
												<RotateCcw size={14} />
