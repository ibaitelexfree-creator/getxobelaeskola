...
import { type ServiceHealth, useMissionStore } from "@/store/useMissionStore";
	...
	return (
		...
		>
			...
			<div className="flex-1">
				...
				<p className="text-white/80 text-xs font-bold leading-none mt-1">
					{service.detail}
				</p>
			</div>
			<div className="text-right">
				...
				<div
					className={`text-[10px] font-bold uppercase py-0.5 px-2 rounded-full inline-block mt-1 ${
						service.health === "online"
...
export default function Dashboard() {
	...
	const generateReport = async () => {
		...
		try {
			...
			if (result.success) {
				console.log(
					"Report generation workflow triggered successfully:",
					result.message,
				);
			} else {
				throw new Error(
					result.message || "Error desconocido al generar informe",
				);
			}
		} catch (e: any) {
	...
	const watchdogH: ServiceHealth =
		watchdogS === "ACTIVE"
			? "online"
			: ["STALLED", "LOOPING", "RECOVERING"].includes(watchdogS)
				? "degraded"
				: ["CRASHED", "OFFLINE"].includes(watchdogS)
					? "offline"
					: "unknown";
	...
	const groups = [
		{
			...
			items: [
				...
				},
				{
					...
					detail: julesWaiting
						? t("dashboard.details.input_required")
						: `${t("dashboard.details.session")} ${services.jules.used}/${services.jules.total}`,
		...
		},
		{
			...
			items: [
				...
				},
				{
					...
					metric:
						services.browserless.health === "online"
							? `${services.browserless.used}/${services.browserless.total}`
							: "OFFLINE",
		...
		<div suppressHydrationWarning className="flex flex-col gap-6 pb-28">
			...
			<div
				className={`p-5 rounded-3xl border-2 flex items-center justify-between ${
					connected
						? "bg-status-green/20 border-status-green/40"
						: "bg-status-red/20 border-status-red/40 animate-pulse"
				}`}
			>
				<div className="flex items-center gap-4">
					<div
						className={`w-4 h-4 rounded-full ${connected ? "bg-status-green" : "bg-status-red"}`}
					/>
					<div>
						<h1
						...
						>
							{connected
								? t("dashboard.system_ready")
								: t("dashboard.system_offline")}
						</h1>
						<p className="text-white text-sm font-mono font-black mt-1 uppercase tracking-tighter">
			...
			<div className="grid grid-cols-3 gap-3">
				{[
					...
					},
					{
						...
						val: stats.completed,
						color: "#00FF95",
						bg: "rgba(0,255,149,0.15)",
						tab: "queue",
					},
					{
						label: t("dashboard.stats.fail"),
						val: stats.failed,
						color: "#FF3333",
					...
					>
						...
						<p className="text-white/60 text-[12px] font-black uppercase tracking-widest mt-1">
							{s.label}
						</p>
					</motion.div>
				))}
			...
			<div className="glass-panel p-6 rounded-[2.5rem] border-4 border-white/20">
				<div className="flex items-center justify-between mb-6">
					...
					<span className="bg-white/20 text-white text-[12px] font-black px-4 py-1.5 rounded-full">
			...
			<div className="glass-panel p-6 rounded-[2.5rem] border-4 border-status-blue/20 bg-status-blue/5 relative overflow-hidden">
				...
				<p className="text-white/70 mb-6 font-bold text-lg max-w-2xl">
					Sintetiza la actividad técnica, genera un reporte en Notebook LM, o
					re-indexa la memoria a largo plazo (Qdrant) apuntando a todos los
					repositorios activos vinculados al tenant.
				</p>
				...
				<div className="flex flex-col lg:flex-row gap-4">
					<motion.button
						...
						className={`flex-1 px-8 py-5 text-xl font-black rounded-3xl uppercase tracking-tighter border-2 flex items-center justify-center gap-3 transition-all ${
							isGeneratingReport
				...
				<div className="mt-6 flex flex-wrap gap-4 items-center">
					...
					<span
						className={`text-xs font-bold uppercase py-1 px-3 rounded-full ${cerebro.status === "idle" ? "bg-status-green/20 text-status-green border border-status-green/50" : "bg-status-amber/20 text-status-amber border border-status-amber/50"}`}
					>
						{cerebro.status === "idle"
							? "ONLINE (Sincronizado)"
							: "INDEXANDO (Operativo)"}
			...
			{pendingApproval && (
				...
				>
					<div className="flex items-center gap-4 mb-6">
						...
						<h2 className="text-black text-3xl font-black uppercase leading-tight">
							{t("dashboard.action_required")}
						</h2>
					</div>
					<p className="text-black font-black text-xl leading-snug mb-8">
						{pendingApproval.task}
					</p>
					<div className="flex gap-4">
						<button className="flex-1 py-5 bg-black text-white text-2xl font-black rounded-3xl uppercase tracking-tighter active:scale-95 transition-transform">
							{t("common.confirm")}
						</button>
						<button className="flex-1 py-5 bg-black/10 text-black text-2xl font-black rounded-3xl uppercase tracking-tighter border-2 border-black/20">
							{t("common.skip")}
						</button>
					</div>
				</motion.div>
				...
				{alertService && (
					...
					>
						...
						>
							...
							<h2 className="text-4xl font-black uppercase mb-2 leading-none">
								{alertService.name}
							</h2>
							<p className="text-white/80 font-black uppercase mb-6 tracking-widest">
								{alertService.health}
							</p>

							<div className="bg-black/20 p-5 rounded-3xl mb-8 border-2 border-white/20">
								<p className="text-lg font-bold leading-relaxed">
									{alertService.errorMsg}
								</p>
							</div>
