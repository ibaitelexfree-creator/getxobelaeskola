...
import { AlertCircle, Crosshair, X } from "lucide-react";
		...
		>
			...
			>
				...
				<div className="bg-black/40 border border-white/5 rounded-xl p-3 font-mono text-xs text-white/70 leading-relaxed">
					{contact.errorMsg ||
						"No hay respuesta del servicio. Puede que esté caído o inaccesible."}
				</div>
...
export default function TacticalRadar() {
	...
	const [selectedContact, setSelectedContact] = useState<RadarContact | null>(
		null,
	);
	...
	const radarContacts: RadarContact[] = useMemo(() => {
		const watchdogH =
			services.watchdog.state === "ACTIVE"
				? "online"
				: services.watchdog.state === "STALLED"
					? "degraded"
					: "offline";
		...
		const julesUsage = Math.min(
			1,
			services.jules.used / (services.jules.total || 1),
		);
		const clawdUsage = Math.min(1, services.clawdebot.delegations / 30);
		const infraUsage = Math.min(
			1,
			(services.flash.tasksToday / 50 +
				(services.browserless?.used || 0) /
					(services.browserless?.total || 100)) /
				2,
		);
		...
		const baseContacts: RadarContact[] = [
			// ── GROUP: ClawdeBot (top-left zone) ──
			{
				...
				group: "clawdebot",
				errorMsg:
					services.clawdebot.health !== "online"
		...
		<>
			<div className="relative w-full aspect-square max-w-[280px] mx-auto">
				...
				<motion.div
					...
					style={{
						background:
							"conic-gradient(from 0deg at 50% 50%, rgba(0, 255, 148, 0.4) 0deg, rgba(0, 255, 148, 0.1) 20deg, transparent 45deg)",
						borderRadius: "50%",
					}}
				/>
				...
				{radarContacts.map((c) => {
					...
					const x =
						50 + Math.cos((c.angle - 90) * (Math.PI / 180)) * (c.distance / 2);
					const y =
						50 + Math.sin((c.angle - 90) * (Math.PI / 180)) * (c.distance / 2);
					...
					return (
						...
						>
							...
							<button
								...
								className={`w-2.5 h-2.5 rounded-full transition-all duration-300 flex-shrink-0 ${healthBg(c.health)} ${
									clickable
							...
							>
								...
								>
									...
									{isOffline && (
										<span className="text-[10px] text-status-red font-black animate-pulse ml-1">
											!
										</span>
									)}
								</motion.div>
				...
				<div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-40">
					<Crosshair
						size={14}
						className="text-buoy-orange drop-shadow-[0_0_5px_rgba(255,77,0,0.6)]"
					/>
				</div>

				{/* Group labels */}
				<div className="absolute top-[12%] right-[8%] text-[7px] font-mono text-status-green/50 uppercase tracking-wider">
					JULES
				</div>
				<div className="absolute bottom-[22%] left-[5%] text-[7px] font-mono text-status-blue/50 uppercase tracking-wider">
					CLAWD
				</div>
				<div className="absolute bottom-[18%] right-[10%] text-[7px] font-mono text-status-amber/50 uppercase tracking-wider">
					INFRA
				</div>
				...
				<div className="absolute top-2 right-2 flex items-center gap-1">
					...
					<span className="text-[7px] font-mono text-status-green/50 uppercase">
						{radarContacts.filter((c) => c.health === "online").length}/
