...
export default function QueueHistory() {
	const { queue, history, approveTask, deleteTask, updateTask, serverUrl } =
		useMissionStore();

	const pendingApproval = queue.filter(
		(t) => t.status === "pending_approval" || t.requiresApproval,
	);
	const activeAndQueued = queue.filter(
		(t) => t.status !== "pending_approval" && !t.requiresApproval,
	);

	const handleClearLegacy = async () => {
		try {
			await fetch(`${serverUrl}/api/tasks/clear-deprecated`, {
				method: "POST",
			});
	...
	const renderTaskItem = (item: any, i: number, isPending: boolean = false) => (
		...
		>
			<span className="text-2xs font-mono text-white/20 w-6">
				#{item.position || i + 1}
			</span>
			...
			{pendingApproval.length > 0 && (
				<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
					<div className="flex items-center justify-between mb-3 px-2">
						<h3 className="text-xs font-black uppercase tracking-[0.2em] text-status-amber">
							Awaiting Approval
						</h3>
						<span className="text-2xs font-mono text-status-amber/50">
							{pendingApproval.length} blocked
						</span>
					</div>
			...
			<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
				<div className="flex items-center justify-between mb-3 px-2">
					...
					<span className="text-2xs font-mono text-white/30">
						{activeAndQueued.length} scheduled
					</span>
				</div>
