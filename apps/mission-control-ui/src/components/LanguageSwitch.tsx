...
export default function LanguageSwitch() {
	...
	return (
		...
		>
			<div
				className={`w-8 h-8 rounded-xl flex items-center justify-center transition-all ${currentLang === "es" ? "bg-brass-gold/20 text-brass-gold" : "bg-status-blue/20 text-status-blue"}`}
			>
				<Globe
					size={18}
			...
			<div className="flex flex-col items-start">
				<div className="flex items-center gap-2">
					<span
						className={`text-[10px] font-bold font-mono uppercase tracking-widest ${currentLang === "en" ? "text-white" : "text-white/40"}`}
					>
						EN
					</span>
					<div className="w-4 h-[1px] bg-white/10" />
					<span
						className={`text-[10px] font-bold font-mono uppercase tracking-widest ${currentLang === "es" ? "text-white" : "text-white/40"}`}
					>
						ES
					</span>
				</div>
