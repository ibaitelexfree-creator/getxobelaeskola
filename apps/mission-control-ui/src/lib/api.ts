...
async function request<T>(
	...
	const tenantId =
		typeof window !== "undefined"
			? localStorage.getItem("mc_active_tenant") || "getxo"
			: "getxo";
	...
	try {
		...
		if (!res.ok) {
			if (res.status >= 500 && retries > 0) {
				...
				await new Promise((r) =>
					setTimeout(r, 1000 * (MAX_RETRIES - retries + 1)),
				);
...
export const getActiveSessions = () =>
	request<{ activeSessions: any[]; activeCount: number }>(
		"GET",
		"/api/sessions/active",
	);
...
export const getWatchdogStatus = () =>
	request<WatchdogStatus>("GET", "/watchdog/status");
export const watchdogAction = (action: string, message?: string) =>
	request<{ success: boolean }>("POST", "/watchdog/action", {
		action,
		message,
	});
...
export const registerDevice = (
	...
	request<{ success: boolean }>("POST", "/watchdog/register-device", {
		token,
		platform,
		deviceId,
	});
...
export const getResourceStatus = () =>
	request<ResourceStatus>("GET", "/api/resources/status");
export const setPowerMode = (mode: "eco" | "performance") =>
	request<{ success: boolean; mode: string }>("POST", "/api/resources/mode", {
		mode,
	});
export const startService = (service: string) =>
	request<{ success: boolean }>("POST", `/api/resources/start/${service}`);
export const stopService = (service: string) =>
	request<{ success: boolean }>("POST", `/api/resources/stop/${service}`);
export const resetService = (service: string) =>
	request<{ success: boolean }>("POST", `/api/resources/reset/${service}`);
export const pauseService = (service: string) =>
	request<{ success: boolean }>("POST", `/api/resources/pause/${service}`);
export const getServiceLogs = (service: string) =>
	request<{ logs: any[] }>("GET", `/api/resources/logs/${service}`);
...
export const executeTool = (
	...
	request<MCPResult>("POST", "/mcp/execute", {
		tool: name,
		parameters: params,
	});
...
export const getJulesBlockedStatus = () =>
	request<{ blocked: boolean; sessions: any[] }>("GET", "/api/jules/blocked");
...
export const getVisualHistory = () =>
	request<{ screenshots: Screenshot[] }>("GET", "/api/visual/history");
export const getSyncHistory = (days = 7) =>
	request<any[]>("GET", `/api/analytics/sync-history?days=${days}`);

export const triggerBuild = (workflow_id = "android-build.yml") =>
	request<{ success: boolean; message: string }>(
		"POST",
		"/api/v1/github/trigger-build",
		{ workflow_id },
	);

export const triggerNotebookLMReport = () =>
	request<{ success: boolean; message: string; details?: string }>(
		"POST",
		"/api/v1/notebooklm/report",
	);
