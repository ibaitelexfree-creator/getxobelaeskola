...
export async function sendTask(
	description: string,
	mode: "cascade" | "flash" | "clawdebot" = "cascade",
) {
	const toolMap = {
...
export async function approve() {
	return maestroPost("/watchdog/action", {
		action: "feed",
		message: "/approve",
	});
}
...
export async function getStatus(): Promise<MaestroStatus> {
	...
	return {
		...
		flash: {
			enabled: health.services?.geminiFlash !== "error",
			tasksToday: 0,
			tokensUsed: 0,
		},
		clawdebot: {
			healthy: health.services?.clawdebot !== "error",
			delegations: 0,
		},
...
export async function doctor(): Promise<DoctorResult> {
	const health = await maestroGet<any>("/health");
	const checks = Object.entries(health.services || {}).map(
		([name, status]) => ({
			name,
...
export const pausePool = () =>
	maestroPost("/watchdog/action", { action: "pause" });

/** /resume — Resume Jules pool */
export const resumePool = () =>
	maestroPost("/watchdog/action", { action: "resume" });

/** /watchdog — Watchdog status */
export const getWatchdog = () => maestroGet("/watchdog/status");

/** /watchdog pause */
export const pauseWatchdog = () =>
	maestroPost("/watchdog/action", { action: "pause" });

/** /watchdog resume */
export const resumeWatchdog = () =>
	maestroPost("/watchdog/action", { action: "resume" });
...
/** Get list of releases from Maestro Proxy */
export async function getReleases(): Promise<{
	success: boolean;
	releases: Release[];
}> {
	return maestroGet("/api/releases");
}
...
export async function getTrustStatus(): Promise<{ active: boolean }> {
	return maestroGet("/api/trust/status");
}

/** Toggle trust tunnel state */
export async function toggleTrustTunnel(
	password: string,
