		...
		try {
			...
			if (health) {
				...
				updateServices({
					...
					browserless: {
						health:
							svc.browserless !== "error"
								? resources?.services?.BROWSERLESS?.running
									? "online"
									: "offline"
								: "offline",
			...
			if (
				...
				if (liveConfig.password)
					useMissionStore
						.getState()
						.setLivePreviewPassword(liveConfig.password);
			}
			...
			if (queueData?.success && queueData.result) {
				...
				if (history) {
					...
					const assigned =
						(queue || []).length +
						(history || []).filter(
							(h: any) => h.status === "running" || h.status === "queued",
						).length;
			...
			if (sessions?.activeSessions) {
				const threads = sessions.activeSessions.map((s: any) => {
					const executor =
						s.executor ||
						(s.title?.toLowerCase().includes("flash") ? "flash" : "jules");
					const layer = (
						executor === "jules" ? 1 : executor === "flash" ? 2 : 3
					) as 1 | 2 | 3;
					return {
						id: s.id,
