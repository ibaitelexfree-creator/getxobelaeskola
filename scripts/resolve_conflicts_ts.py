...
# Resolution Plan:
# 1. Rental page: Keep OURS (better types, metadata)
resolve_conflict('src/app/[locale]/rental/page.tsx', 'ours')

# 2. Explorer route: Keep THEIRS (SearchResult type)
resolve_conflict('src/app/api/admin/explorer/route.ts', 'theirs')

# 3. Weather route: Keep OURS (any[] | undefined)
resolve_conflict('src/app/api/weather/route.ts', 'ours')

# 4. Network monitor: Keep THEIRS (clean any)
resolve_conflict('src/hooks/useNetworkMonitor.ts', 'theirs')

# 5. Tests: Keep THEIRS (fixing test expectations for TS)
resolve_conflict('src/lib/auth-guard.test.ts', 'theirs')
resolve_conflict('src/lib/geospatial/water-check.test.ts', 'theirs')
