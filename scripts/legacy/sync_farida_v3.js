...
async function checkFinalFix() {
    ...
    console.log(`Searching for Farida's sessions to sync them...`);
    ...
    for (const s of herSessions) {
        const m = s.metadata;
        const targetCourseId = m.course_id || (m.item_name && m.item_name.includes('Vela Ligera') ? cursoLigeraId : null);

        if (targetCourseId) {
            // Check if already enrolled with this session ID
            const { data: existing } = await supabase.from('inscripciones')
                .select('id')
                .eq('stripe_session_id', s.id)
                .maybeSingle();

            if (existing) {
                console.log(`Session ${s.id} already processed.`);
                continue;
            }

            console.log(`Processing session ${s.id} for course ${targetCourseId}...`);

            // Sanitise Edition ID (must be a UUID or null)
            let editionId = null;
            if (m.edition_id && m.edition_id.length === 36) { // Simple UUID check
            ...
            const { error: insError } = await supabase.from('inscripciones').insert({
                ...
                metadata: {
                    manual_sync: true,
                    original_edition_id: m.edition_id,
