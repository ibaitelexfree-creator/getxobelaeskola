...
async function run() {
    ...
    console.log('Testing if "mañana" matches "ana"...');
    const { data, error } = await supabase.from('reservas_alquiler')
        .select('opcion_seleccionada')
        .ilike('opcion_seleccionada', '%ana%')
        .limit(5);
    ...
    if (error) {
        ...
        data.forEach(d => console.log(`- ${d.opcion_seleccionada}`));
    }
}
run();
