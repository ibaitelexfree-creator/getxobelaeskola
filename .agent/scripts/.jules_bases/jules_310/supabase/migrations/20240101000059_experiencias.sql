-- Migración: Tabla de Experiencias y Actividades
-- 20240101000059_experiencias.sql
-- Fase 1 del plan de plataforma híbrida
...
-- 2. Índices
CREATE INDEX IF NOT EXISTS idx_experiencias_tipo ON public.experiencias(tipo);
CREATE INDEX IF NOT EXISTS idx_experiencias_activo ON public.experiencias(activo, visible);
CREATE INDEX IF NOT EXISTS idx_experiencias_slug ON public.experiencias(slug);
...
CREATE TABLE IF NOT EXISTS public.reservas_experiencia (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_reservas_exp_perfil ON public.reservas_experiencia(perfil_id);
CREATE INDEX IF NOT EXISTS idx_reservas_exp_fecha ON public.reservas_experiencia(fecha_reserva);
CREATE INDEX IF NOT EXISTS idx_reservas_exp_stripe ON public.reservas_experiencia(stripe_session_id);
