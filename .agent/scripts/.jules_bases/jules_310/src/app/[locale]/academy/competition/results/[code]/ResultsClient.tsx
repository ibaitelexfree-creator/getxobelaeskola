...
export default function ResultsClient() {
    ...
    if (loading) return <div className="min-h-screen bg-black text-white flex items-center justify-center font-mono">CALCULANDO RESULTADOS...</div>;

    return (
        <div className="min-h-screen bg-black text-white p-8 flex flex-col items-center justify-center font-sans">
            <h1 className="text-4xl md:text-6xl font-black mb-12 text-cyan-500 uppercase italic tracking-tighter">Resultados Finales</h1>
                ...
                <table className="w-full text-left">
                    ...
                    <tbody>
                        {participants.map((p, i) => (
                            <tr key={p.id} className="border-b border-gray-800 last:border-0 hover:bg-white/5 transition-colors group">
                                <td className="py-6 pl-4 font-black text-2xl w-20">
                                    {i === 0 && <span className="text-yellow-400 text-4xl drop-shadow-[0_0_10px_rgba(250,204,21,0.5)]">🥇</span>}
                                    {i === 1 && <span className="text-gray-300 text-3xl">🥈</span>}
                                    {i === 2 && <span className="text-amber-700 text-3xl">🥉</span>}
                                ...
                                <td className="py-6 text-xl font-mono text-gray-300 group-hover:text-white transition-colors">
                                    {p.username}
                                    {i === 0 && <span className="ml-2 text-xs bg-yellow-400/20 text-yellow-400 px-2 py-1 rounded border border-yellow-400/30 uppercase tracking-wide font-bold">Ganador</span>}
                                </td>
                                <td className="py-6 pr-4 text-right font-bold text-cyan-400 text-3xl font-mono tracking-tighter">{p.score}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
